...
namespace {
    ...
    void BM_Abel_Pi(benchmark::State &state) {
        const char *pi = "3.14159";
        const char *pi_end = pi + strlen(pi);
        for (auto s : state) {
            benchmark::DoNotOptimize(pi);
            double v;
            abel::from_chars(pi, pi_end, v);
            benchmark::DoNotOptimize(v);
        }
    }

    BENCHMARK(BM_Abel_Pi);
    ...
    void BM_Abel_Pi_float(benchmark::State &state) {
        const char *pi = "3.14159";
        const char *pi_end = pi + strlen(pi);
        for (auto s : state) {
            benchmark::DoNotOptimize(pi);
    ...
    void BM_Abel_HardSmall(benchmark::State &state) {
    ...
    void BM_Abel_HugeMantissa(benchmark::State &state) {
        std::string huge(200, '3');
        const char *num = huge.c_str();
        ...
        for (auto s : state) {
            benchmark::DoNotOptimize(num);
            double v;
            abel::from_chars(num, num_end, v);
            benchmark::DoNotOptimize(v);
        }
    }

    BENCHMARK(BM_Abel_HugeMantissa);
    ...
    BENCHMARK(BM_Strtod_Big_And_Difficult)
    ->Range(3, 5000);

    void BM_Abel_Big_And_Difficult(benchmark::State &state) {
        std::string testcase = MakeHardCase(state.range(0));
        const char *begin = testcase.c_str();
        ...
        for (auto s : state) {
            benchmark::DoNotOptimize(begin);
            double v;
            abel::from_chars(begin, end, v);
            benchmark::DoNotOptimize(v);
        }
    }

    BENCHMARK(BM_Abel_Big_And_Difficult)
    ->Range(3, 5000);
...
// BM_Abel_Pi                               35 ns         35 ns   20031996
// BM_Strtod_Pi_float                       91 ns         91 ns    7745851
// BM_Abel_Pi_float                         35 ns         35 ns   20430298
// BM_Strtod_HardLarge                     133 ns        133 ns    5288341
// BM_Abel_HardLarge                       181 ns        181 ns    3855615
// BM_Strtod_HardSmall                     279 ns        279 ns    2517243
// BM_Abel_HardSmall                       287 ns        287 ns    2458744
// BM_Strtod_HugeMantissa                  433 ns        433 ns    1604293
// BM_Abel_HugeMantissa                    160 ns        160 ns    4403671
...
// BM_Abel_Big_And_Difficult/3              39 ns         39 ns   17799583
// BM_Abel_Big_And_Difficult/8              43 ns         43 ns   16096859
// BM_Abel_Big_And_Difficult/64            550 ns        550 ns    1259717
// BM_Abel_Big_And_Difficult/512          4167 ns       4167 ns     171414
// BM_Abel_Big_And_Difficult/4096         9160 ns       9159 ns      76297
// BM_Abel_Big_And_Difficult/5000         9738 ns       9738 ns      70140
