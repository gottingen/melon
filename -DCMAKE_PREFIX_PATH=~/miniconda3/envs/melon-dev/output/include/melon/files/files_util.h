...
#ifndef MELON_FILES_FILES_UTIL_H_
#define MELON_FILES_FILES_UTIL_H_

#include "melon/base/type_traits.h"
#include "melon/files/filesystem.h"
#include "melon/files/internal/list_sirectory.h"
...
namespace melon::files_internal {
    ...
    struct collector_is_convertible_to
            : collector_is_convertible_to_impl<C,
                    !melon::is_initializer_list<typename std::remove_reference<C>::type>::value &&
                    melon::has_value_type<C>::value &&
    ...
    public:
        ...
        template<typename Container, typename = typename std::enable_if<
                melon::has_value_type<Container>::value &&
                ...
                (std::is_constructible<typename Container::value_type, std::string>::value ||
                 ...
                 std::is_constructible<typename Container::value_type, melon::file_path>::value)>::type>
        operator Container() {
            Container c;
...
namespace melon {
    ...
    struct only_directory {
        bool operator()(const file_path &path, std::error_code &ec) {
            return melon::is_directory(path, ec);
        }
    };

    struct only_file {
        bool operator()(const file_path &path, std::error_code &ec) {
            return melon::is_regular_file(path, ec);
        }
    };
    ...
    typename std::enable_if<melon::has_value_type<Container>::value &&
...
#endif  // MELON_FILES_FILES_UTIL_H_
