...
// -----------------------------------------------------------------------------
//
// This file configures the melon *failure signal handler* to capture and dump
// useful debugging information (such as a stacktrace) upon program failure.
//
// To use the failure signal handler, call `melon::debugging::install_failure_signal_handler()`
...
//
// Note that you should *not* install the melon failure signal handler more
// than once. You may, of course, have another (non-melon) failure signal
// handler installed (which would be triggered if melon's failure signal
// handler sets `call_previous_handler` to `true`).

#ifndef MELON_DEBUGGING_FAILURE_SIGNAL_HANDLER_H_
#define MELON_DEBUGGING_FAILURE_SIGNAL_HANDLER_H_
...
namespace melon::debugging {
    ...
    //
    // Struct for holding `melon::debugging::install_failure_signal_handler()` configuration
    // options.
    struct failure_signal_handler_options {
        ...
        // the melon library into a state from which they cannot recover.
...
#endif  // MELON_DEBUGGING_FAILURE_SIGNAL_HANDLER_H_
