...
//
// Note: stack traces in melon that do not utilize a symbolizer will result in
...
#ifndef MELON_DEBUGGING_STACKTRACE_H_
#define MELON_DEBUGGING_STACKTRACE_H_
...
namespace melon::debugging {
    ...
    // stored. (Note that the frame generated for the `melon::debugging::get_stack_frames()`
    ...
    //        void* result[10];
    //        int sizes[10];
    //        int depth = melon::debugging::get_stack_frames(result, sizes, 10, 1);
    //      }
    //
    ...
    // that the frame generated for the `melon::debugging::get_stack_frames_with_context()` routine
    ...
    // stored. Note that this function is similar to `melon::debugging::get_stack_frames()`
    ...
    //      bar() {
    //        void* result[10];
    //        int depth = melon::debugging::get_stack_trace(result, 10, 1);
    //      }
    //
    // This produces:
    //
    ...
    // the `melon::debugging::get_stack_frames_with_context()` routine itself is also skipped.)
    //
    // The `uc` parameter, if non-null, should be a pointer to a `ucontext_t` value
    // passed to a signal handler registered via the `sa_sigaction` field of a
    // `sigaction` struct. (See
    ...
    // arguments passed to `melon::debugging::get_stack_frames_with_context()` except that sizes
    ...
    // *****************************************************************************
    //
    // melon::debugging::set_stack_unwinder is not suitable for general purpose use.  It is
    // provided for custom runtimes.
    // Some things to watch out for when calling `melon::debugging::set_stack_unwinder()`:
    //
    ...
    // case where `melon::debugging::set_stack_unwinder()` was never called. Equivalents are:
    //
...
#endif  // MELON_DEBUGGING_STACKTRACE_H_
