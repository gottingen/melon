...
#include "melon/debugging/internal/demangle.h"
#include "melon/debugging/internal/vdso_support.h"
...
// memset() is not guaranteed to be async-signal-safe.
            ...
            private:
                char *CopyString(const char *s) {
                    ...
                    MELON_CHECK(dst != nullptr) << "out of memory";
                    memcpy(dst, s, len + 1);
                    return dst;
                }

                ObjFile *FindObjFile(const void *const start,
                                     size_t size) MELON_NO_INLINE;
        ...
        Symbolizer::Symbolizer() : ok_(true), addr_map_read_(false) {
            for (SymbolCacheLine &symbol_cache_line : symbol_cache_) {
                for (size_t j = 0; j < MELON_ARRAY_SIZE(symbol_cache_line.name); ++j) {
        ...
        static ssize_t ReadPersistent(int fd, void *buf, size_t count) {
            ...
            while (num_bytes < count) {
                ...
                if (len < 0) {  // There was an error other than EINTR.
                    MELON_LOG(WARNING) << "read failed: errno=" << errno;
                    return -1;
                }
...
// return the number of bytes read.  Otherwise, return -1.
        static ssize_t ReadFromOffset(const int fd, void *buf, const size_t count,
            ...
            if (off == (off_t) -1) {
                MELON_LOG(WARNING) << "lseek(" << fd << "), " << static_cast<uintmax_t>(offset)
...
// inlined.
        static MELON_NO_INLINE bool GetSectionHeaderByType(const int fd, ElfW(Half) sh_num, const off_t sh_offset,
                                                           ElfW(Word) type,
                                                           ElfW(Shdr)
...
bool GetSectionHeaderByName(int fd, const char *name, size_t name_len, ElfW(Shdr)* out) {
    ...
    if (sizeof(header_name) < name_len) {
        MELON_LOG(WARNING)<<"Section name "<<name<<" is too long ("<<name_len<<"); ""section will not be found (even if present).";
...
// To keep stack consumption low, we would like this function to not get
// inlined.
static MELON_NO_INLINE FindSymbolResult FindSymbol(
...
MELON_CHECK(n_read <= out_size)<< "ReadFromOffset read too much data.";
...
static MELON_NO_INLINE bool ReadAddrMap(
    ...
    if (wrapped_maps_fd.get() < 0) {
        MELON_LOG(WARNING) << maps_path << "error: " << errno;
        return false;
    }
    ...
    while (true) {
        ...
        if (cursor == eol || *cursor != '-') {
            MELON_LOG(WARNING) << "Corrupt /proc/self/maps line: " << line;
            return false;
        }
        ++cursor;  // Skip '-'.
        ...
        if (cursor == eol || cursor < flags_start + 4) {
            MELON_LOG(WARNING) << "Corrupt /proc/self/maps: " << line;
            return false;
        }
...
bool Symbolizer::RegisterObjFile(const char *filename,
    ...
    if (addr_map_size != 0) {
        ...
        if (old->end_addr > end_addr) {
            MELON_LOG(ERROR)<<"Unsorted addr map entry: 0x"<<reinterpret_cast<uintptr_t>(end_addr)<<": "<<filename
            ...
            if (old->start_addr != start_addr ||
                ...
                MELON_LOG(ERROR)<<"Duplicate addr 0x"<<reinterpret_cast<uintptr_t>(end_addr)<<": "<<filename
                          <<" <-> 0x"<< reinterpret_cast<uintptr_t>(old->end_addr)<<": "<<old->filename;
            }
            return true;
        }
...
// get inlined.
static MELON_NO_INLINE void DemangleInplace(char *out, int out_size,
                                            char *tmp_buf,
                                            int tmp_buf_size) {
...
const char *Symbolizer::FindSymbolInCache(const void *const pc) {
    ...
    for (size_t i = 0; i < MELON_ARRAY_SIZE(line->pc); ++i) {
        if (line->pc[i] == pc) {
            AgeSymbols(line);
            line->age[i] = 0;
            return line->name[i];
...
const char *Symbolizer::InsertSymbolInCache(const void *const pc,
    ...
    AgeSymbols(line);
    MELON_CHECK(oldest_index >= 0) << "Corrupt cache";
...
static bool MaybeInitializeObjFile(ObjFile *obj) {
    if (obj->fd < 0) {
        ...
        if (obj->fd < 0) {
            MELON_LOG(WARNING) << obj->filename << "open failed: errno=" << errno;
            return false;
        }
        obj->elf_type = FileGetElfType(obj->fd);
        if (obj->elf_type < 0) {
            MELON_LOG(WARNING) << obj->filename << ": wrong elf type: " << obj->elf_type;
            return false;
        }

        if (!ReadFromOffsetExact(obj->fd, &obj->elf_header, sizeof(obj->elf_header),
                                 0)) {
            MELON_LOG(WARNING) << obj->filename << ": failed to read elf header";
            return false;
        }
    }
    return true;
    ...
    if (obj != nullptr) {
        ...
        if (vdso.IsPresent()) {
          ...
          if (vdso.LookupSymbolByAddress(pc, &symbol_info)) {
            ...
            MELON_CHECK(len + 1 < sizeof(symbol_buf_))<<
