 ...
 *****************************************************************/
#ifndef MELON_TIMES_TIME_H_
#define MELON_TIMES_TIME_H_
...
// potential to break melon users.
//
...
#include "melon/times/duration.h"
#include "melon/times/internal/chrono_time_internal.h"
#include "melon/times/civil_time.h"
...
namespace melon {
    ...
    // time_now()
    //
    // Returns the current time, expressed as an `melon::time_point` absolute time value.
    time_point time_now();

    // get_current_time_nanos()
    //
    // Returns the current time, expressed as a count of nanoseconds since the Unix
    // Epoch (https://en.wikipedia.org/wiki/Unix_time). Prefer `melon::time_now()` instead
    ...
    // sleep_for()
    //
    // Sleeps for the specified duration, expressed as an `melon::duration`.
    //
    // Notes:
    // * signal interruptions will not reduce the sleep duration.
    // * Returns immediately when passed a nonpositive duration.
    void sleep_for(melon::duration duration);

    class time_zone;  // Defined below

    // time_point
    //
    // An `melon::time_point` represents a specific instant in time. Arithmetic operators
    // are provided for naturally expressing time calculations. Instances are
    // created using `melon::time_now()` and the `melon::From*()` factory functions that
    // accept the gamut of other time representations. Formatting and parsing
    // functions are provided for conversion to and from strings.  `melon::time_point`
    // should be passed by value rather than const reference.
    //
    // `melon::time_point` assumes there are 60 seconds in a minute, which means the
    // underlying time scales must be "smeared" to eliminate leap seconds.
    // See https://developers.google.com/time/smear.
    //
    // Even though `melon::time_point` supports a wide range of timestamps, exercise
    // caution when using values in the distant past. `melon::time_point` uses the
    ...
    // governments.
    //
    // The `melon::time_point` class represents an instant in time as a count of clock
    // ticks of some granularity (resolution) from some starting point (epoch).
    //
    // `melon::time_point` uses a resolution that is high enough to avoid loss in
    ...
    //
    //   melon::time_point t1 = ...;
    //   melon::time_point t2 = t1 + melon::minutes(2);
    //   melon::duration d = t2 - t1;  // == melon::minutes(2)
    //
    class time_point {
    public:
        // Value semantics.
        ...
        //
        // Example:
        //   melon::time_point t = melon::unix_epoch();
        //   melon::time_point t = melon::time_now();
        //   melon::time_point t = melon::time_from_timeval(tv);
        //   melon::time_point t = melon::infinite_past();
        ...
        // `melon::time_point` in a certain `melon::time_zone`. This struct is not
        // intended to represent an instant in time. So, rather than passing
        // a `time_point::breakdown` to a function, pass an `melon::time_point` and an
        // `melon::time_zone`.
        //
        // Deprecated. Use `melon::time_zone::chrono_info`.
        struct breakdown {
        ...
        //
        // Deprecated. Use `melon::time_zone::At(time_point)`.
    ...
    public:
        ...
        // to_universal()
        //
        // Converts an `melon::time_point` to a variety of other representations.  Note that
        ...
        // to_chrono_time()
        //
        // Converts an melon::time_point to a std::chrono::system_clock::time_point. If
        // overflow would occur, the returned value will saturate at the min/max time
        // point value instead.
        //
        // Example:
        //
        //   melon::time_point t = melon::from_time_t(123);
        //   auto tp = melon::to_chrono_time(t);
    ...
    public:
        ...
        // unix_epoch()
        //
        // Returns the `melon::time_point` representing "1970-01-01 00:00:00.0 +0000".
        static constexpr time_point unix_epoch();

        // universal_epoch()
        //
        // Returns the `melon::time_point` representing "0001-01-01 00:00:00.0 +0000", the
        // epoch of the ICU Universal time_point Scale.
        static constexpr time_point universal_epoch();

        // infinite_future()
        //
        // Returns an `melon::time_point` that is infinitely far in the future.
        static constexpr time_point infinite_future();

        // infinite_past()
        //
        // Returns an `melon::time_point` that is infinitely far in the past.
        ...
        // from_universal()
        //
        // Creates an `melon::time_point` from a variety of other representations.
        ...
        // from_chrono()
        //
        // Converts a std::chrono::system_clock::time_point to an melon::time_point.
        //
        // Example:
        //
        //   auto tp = std::chrono::system_clock::from_time_t(123);
        //   melon::time_point t = melon::from_chrono(tp);
        //   // t == melon::from_time_t(123)
    ...
    // Additive Operators
    MELON_FORCE_INLINE time_point operator+(time_point lhs, duration rhs) { return lhs += rhs; }

    MELON_FORCE_INLINE time_point operator+(duration lhs, time_point rhs) { return rhs += lhs; }

    MELON_FORCE_INLINE time_point operator-(time_point lhs, duration rhs) { return lhs -= rhs; }

    MELON_FORCE_INLINE duration operator-(time_point lhs, time_point rhs) { return lhs.rep_ - rhs.rep_; }
    ...
    MELON_FORCE_INLINE duration time_point::to_duration() const {
    ...
    // format that matches melon::RFC3339_full. For example:
    //

    // time_zone
    //
    // The `melon::time_zone` is an opaque, small, value-type class representing a
    // geo-political region within which particular rules are used for converting
    // between absolute and civil times (see https://git.io/v59Ly). `melon::time_zone`
    // values are named using the TZ identifiers from the IANA time_point Zone Database,
    // such as "America/Los_Angeles" or "Australia/Sydney". `melon::time_zone` values
    // are created from factory functions such as `melon::load_time_zone()`. Note:
    ...
    // Examples:
    //
    //   melon::time_zone utc = melon::utc_time_zone();
    //   melon::time_zone pst = melon::fixed_time_zone(-8 * 60 * 60);
    //   melon::time_zone loc = melon::local_time_zone();
    //   melon::time_zone lax;
    //   if (!melon::load_time_zone("America/Los_Angeles", &lax)) {
    //     // handle error case
    //   }
    //
    class time_zone {
    public:
        explicit time_zone(melon::times_internal::time_zone tz) : cz_(tz) {}
        ...
        explicit operator melon::times_internal::time_zone() const { return cz_; }
        ...
        // than passing a `time_zone::chrono_info` to a function, pass an `melon::time_point`
        // and an `melon::time_zone`.
        struct chrono_info {
            chrono_second cs;
            duration subsecond;
        ...
        //
        // Returns the civil time for this time_zone at a certain `melon::time_point`.
        ...
        // Example:
        //
        //   const auto epoch = lax.At(melon::unix_epoch());
        //   // epoch.cs == 1969-12-31 16:00:00
        //   // epoch.subsecond == melon::zero_duration()
        //   // epoch.offset == -28800
        //   // epoch.is_dst == false
        //   // epoch.abbr == "PST"
        ...
        // To account for these possibilities, `melon::time_zone::time_info` is
        // richer than just a single `melon::time_point`.
        struct time_info {
            enum chrono_kind {
        ...
        //
        // Returns an `melon::time_info` containing the absolute time(s) for this
        // time_zone at an `melon::chrono_second`. When the civil time is skipped or
        ...
        //
        //   // A unique civil time
        //   const auto jan01 = lax.At(melon::chrono_second(2011, 1, 1, 0, 0, 0));
        ...
        //   const auto mar13 = lax.At(melon::chrono_second(2011, 3, 13, 2, 15, 0));
        ...
        //   const auto nov06 = lax.At(melon::chrono_second(2011, 11, 6, 1, 15, 0));
        ...
        // Example:
        //   melon::time_zone nyc;
        //   if (!melon::load_time_zone("America/New_York", &nyc)) { ... }
        //   const auto now = melon::time_now();
        //   auto t = melon::infinite_past();
    ...
    private:
        ...
        melon::times_internal::time_zone cz_;
    };
    ...
    MELON_FORCE_INLINE bool load_time_zone(const std::string &name, time_zone *tz) {
        if (name == "localtime") {
            *tz = time_zone(melon::times_internal::local_time_zone());
            return true;
        }
        melon::times_internal::time_zone cz;
    ...
    MELON_FORCE_INLINE time_zone fixed_time_zone(int seconds) {
        return time_zone(
                melon::times_internal::fixed_time_zone(std::chrono::seconds(seconds)));
    }

    // utc_time_zone()
    //
    // Convenience method returning the UTC time zone.
    MELON_FORCE_INLINE time_zone utc_time_zone() {
        return time_zone(melon::times_internal::utc_time_zone());
    }
    ...
    MELON_FORCE_INLINE time_zone local_time_zone() {
        return time_zone(melon::times_internal::local_time_zone());
    }
    ...
    //   melon::time_point t = ...;
    //   melon::time_zone tz = ...;
    //   const auto cd = melon::to_chrono_day(t, tz);
    MELON_FORCE_INLINE chrono_second to_chrono_second(time_point t, time_zone tz) {
        return tz.at(t).cs;  // already a chrono_second
    }

    MELON_FORCE_INLINE chrono_minute to_chrono_minute(time_point t, time_zone tz) {
        return chrono_minute(tz.at(t).cs);
    }

    MELON_FORCE_INLINE chrono_hour to_chrono_hour(time_point t, time_zone tz) {
        return chrono_hour(tz.at(t).cs);
    }

    MELON_FORCE_INLINE chrono_day to_chrono_day(time_point t, time_zone tz) {
        return chrono_day(tz.at(t).cs);
    }

    MELON_FORCE_INLINE chrono_month to_chrono_month(time_point t, time_zone tz) {
        return chrono_month(tz.at(t).cs);
    }

    MELON_FORCE_INLINE chrono_year to_chrono_year(time_point t, time_zone tz) {
    ...
    MELON_FORCE_INLINE time_point from_chrono(chrono_second ct, time_zone tz) {
    ...
    // time_conversion
    //
    // An `melon::time_conversion` represents the conversion of year, month, day,
    // hour, minute, and second values (i.e., a civil time), in a particular
    // `melon::time_zone`, to a time instant (an absolute time), as returned by
    // `melon::convert_date_time()`. Legacy version of `melon::time_zone::time_info`.
    //
    // Deprecated. Use `melon::time_zone::time_info`.
    struct
    time_conversion {
        time_point pre;    // time calculated using the pre-transition offset
        time_point trans;  // when the civil-time discontinuity occurred
    ...
    // convert_date_time()
    //
    // Legacy version of `melon::time_zone::At(melon::chrono_second)` that takes
    ...
    //   melon::time_conversion tc =
    //       melon::convert_date_time(2013, 10, 32, 8, 30, 0, lax);
    //   // tc.kind == time_conversion::UNIQUE && tc.normalized == true
    //   // melon::to_chrono_day(tc.pre, tz).month() == 11
    //   // melon::to_chrono_day(tc.pre, tz).day() == 1
    //
    // Deprecated. Use `melon::time_zone::At(chrono_second)`.
    time_conversion convert_date_time(int64_t year, int mon, int day, int hour,
                                      int min, int sec, time_zone tz);

    // format_date_time()
    //
    // A convenience wrapper for `melon::convert_date_time()` that simply returns
    // the "pre" `melon::time_point`.  That is, the unique result, or the instant that
    // is correct using the pre-transition offset (as if the transition never
    // happened).
    //
    // Example:
    //
    //   melon::time_point t = melon::format_date_time(2017, 9, 26, 9, 30, 0, lax);
    //   // t = 2017-09-26 09:30:00 -0700
    //
    // Deprecated. Use `melon::from_chrono(chrono_second, time_zone)`. Note that the
    // behavior of `from_chrono()` differs from `format_date_time()` for skipped civil
    // times. If you care about that see `melon::time_zone::At(melon::chrono_second)`.
    MELON_FORCE_INLINE time_point format_date_time(int64_t year, int mon, int day, int hour,
                                                   int min, int sec, time_zone tz) {
    ...
    // `tm_sec` fields to an `melon::time_point` using the given time zone. See ctime(3)
    // for a description of the expected values of the tm fields. If the indicated
    // time instant is not unique (see `melon::time_zone::at(melon::chrono_second)`
    ...
    // to_tm()
    //
    // Converts the given `melon::time_point` to a struct tm using the given time zone.
    // See ctime(3) for a description of the values of the tm fields.
    struct tm to_tm(time_point t, time_zone tz);

    inline struct tm local_tm(time_point t) {
        return to_tm(t, melon::local_time_zone());
    }

    inline struct tm utc_tm(time_point t) {
        return to_tm(t, melon::utc_time_zone());
    }
    ...
    // format_time()
    //
    // Formats the given `melon::time_point` in the `melon::time_zone` according to the
    ...
    // Example:
    //
    //   melon::chrono_second cs(2013, 1, 2, 3, 4, 5);
    //   melon::time_point t = melon::from_chrono(cs, lax);
    //   std::string f = melon::format_time("%H:%M:%S", t, lax);  // "03:04:05"
    //   f = melon::format_time("%H:%M:%E3S", t, lax);  // "03:04:05.000"
    //
    // Note: If the given `melon::time_point` is `melon::infinite_future()`, the returned
    // string will be exactly "infinite-future". If the given `melon::time_point` is
    // `melon::infinite_past()`, the returned string will be exactly "infinite-past".
    // In both cases the given format string and `melon::time_zone` are ignored.
    //
    ...
    MELON_FORCE_INLINE std::ostream &operator<<(std::ostream &os, time_point t) {
    ...
    // returns the corresponding `melon::time_point`. Uses strftime()-like formatting
    ...
    //
    // For example, parsing a string of "15:45" (%H:%M) will return an melon::time_point
    ...
    // %Ez, or %E*z).
    //
    // Note also that `melon::parse_time()` only heeds the fields year, month, day,
    // hour, minute, (fractional) second, and UTC offset.  Other fields, like
    // weekday (%a or %A), while parsed for syntactic validity, are ignored
    // in the conversion.
    //
    // Date and time fields that are out-of-range will be treated as errors
    // rather than normalizing them like `melon::chrono_second` does.  For example,
    ...
    // `melon::time_point` will be `melon::infinite_future()` and `true` will be returned.
    // If the input string is "infinite-past", the returned `melon::time_point` will be
    // `melon::infinite_past()` and `true` will be returned.
    //
    ...
    inline int utc_minutes_offset(const std::tm &tm) {
        ...
        if (rv == TIME_ZONE_ID_INVALID)
            throw melon::log_ex("Failed getting timezone info. ", errno);
...
extern "C" {
void melon_internal_sleep_for(melon::duration duration);
}  // extern "C"

MELON_FORCE_INLINE void melon::sleep_for(melon::duration duration) {
    melon_internal_sleep_for(duration);
}

#endif  // MELON_TIMES_TIME_H_
