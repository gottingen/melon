...
#ifndef MELON_BASE_ANNOTATION_H_
#define MELON_BASE_ANNOTATION_H_
...
#define MELON_INTERNAL_USE_ASAN 1
#endif  // __has_feature(address_sanitizer)

#if __has_feature(thread_sanitizer)
#define MELON_INTERNAL_USE_TSAN 1
#endif  // __has_feature(thread_sanitizer)

#define MELON_INTERNAL_NO_SANITIZE_ADDRESS [[clang::no_sanitize("address")]]
#define MELON_INTERNAL_NO_SANITIZE_THREAD [[clang::no_sanitize("thread")]]
#endif  // __clang__
...
#define MELON_INTERNAL_NO_SANITIZE_ADDRESS [[gnu::no_sanitize("address")]]
#define MELON_INTERNAL_NO_SANITIZE_THREAD [[gnu::no_sanitize("thread")]]

#endif  // defined(__GNUC__) && !defined(__clang__)

#ifdef MELON_INTERNAL_USE_ASAN
extern "C" {
...
#endif  // MELON_INTERNAL_USE_ASAN

// FIXME: TSan support does not work yet.
#ifdef MELON_INTERNAL_USE_TSAN
extern "C" {
...
#define MELON_INTERNAL_TSAN_MUTEX_CREATE(...) __tsan_mutex_create(__VA_ARGS__)
#define MELON_INTERNAL_TSAN_MUTEX_DESTROY(...) __tsan_mutex_destroy(__VA_ARGS__)
#define MELON_INTERNAL_TSAN_MUTEX_PRE_LOCK(...) \
  __tsan_mutex_pre_lock(__VA_ARGS__)
#define MELON_INTERNAL_TSAN_MUTEX_POST_LOCK(...) \
  __tsan_mutex_post_lock(__VA_ARGS__)
#define MELON_INTERNAL_TSAN_MUTEX_PRE_UNLOCK(...) \
  __tsan_mutex_pre_unlock(__VA_ARGS__)
#define MELON_INTERNAL_TSAN_MUTEX_POST_UNLOCK(...) \
  __tsan_mutex_post_unlock(__VA_ARGS__)

#else

#define MELON_INTERNAL_TSAN_MUTEX_CREATE(...) (void)0
#define MELON_INTERNAL_TSAN_MUTEX_DESTROY(...) (void)0
#define MELON_INTERNAL_TSAN_MUTEX_PRE_LOCK(...) (void)0
#define MELON_INTERNAL_TSAN_MUTEX_POST_LOCK(...) (void)0
#define MELON_INTERNAL_TSAN_MUTEX_PRE_UNLOCK(...) (void)0
#define MELON_INTERNAL_TSAN_MUTEX_POST_UNLOCK(...) (void)0

#endif  // MELON_INTERNAL_USE_TSAN

// Using `initial-exec` prevents binary from being loaded via `dlopen`. melon is
// not intended to be used that way (via `dlopen`). Yet in case there's such
// need, you can define `MELON_USE_SLOW_TLS_MODEL` to accomplish that.
//
// @sa: `doc/performance-guide.md` for more details.
#if !defined(MELON_USE_SLOW_TLS_MODEL)
#define MELON_INTERNAL_TLS_MODEL [[gnu::tls_model("initial-exec")]]
#else
#define MELON_INTERNAL_TLS_MODEL /* Nothing, using the default model. */
#endif
...
#define MELON_INTERNAL_ASAN_CHECK(fp) \
  MELON_DCHECK(fp, "Your compiler is too old to use ASan with fibers.");
...
inline void start_switch_fiber(void** shadow_stack, const void* stack_bottom,
  ...
  MELON_INTERNAL_ASAN_CHECK(__sanitizer_start_switch_fiber);
...
inline void complete_switch_fiber(void* shadow_stack) {
  MELON_INTERNAL_ASAN_CHECK(__sanitizer_finish_switch_fiber);
...
inline void poison_memory_region(const void* ptr, std::size_t size) {
  MELON_INTERNAL_ASAN_CHECK(__asan_poison_memory_region);
  __asan_poison_memory_region(ptr, size);
}

inline void UnpoisonMemoryRegion(const void* ptr, std::size_t size) {
  MELON_INTERNAL_ASAN_CHECK(__asan_unpoison_memory_region);
  __asan_unpoison_memory_region(ptr, size);
}

#undef MELON_INTERNAL_ASAN_CHECK
...
#define MELON_INTERNAL_TSAN_CHECK(fp) \
  MELON_DCHECK(fp, "Your compiler is too old to use TSan with fibers.");
...
inline void* get_current_fiber() {
  MELON_INTERNAL_TSAN_CHECK(__tsan_get_current_fiber);
...
inline void SetFiberName(void* fiber, const char* name) {
  MELON_INTERNAL_TSAN_CHECK(__tsan_set_fiber_name);
  __tsan_set_fiber_name(fiber, name);
}

#undef MELON_INTERNAL_TSAN_CHECK
...
#endif  // MELON_BASE_ANNOTATION_H_
