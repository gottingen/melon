...
#ifndef MELON_BASE_RESULT_STATUS_H_
#define MELON_BASE_RESULT_STATUS_H_
...
namespace melon {
    ...
    public:
        ...
        static result_status from_melon_error(int err);

        static result_status from_melon_error(int err, const std::string_view &ext);

        template<typename ...Args>
        static result_status from_melon_error(int err, const std::string_view &fmt, Args &&...args) {
            auto msg = string_format(fmt, std::forward<Args>(args)...);
            return from_melon_error(err, msg);
        }
...
#endif  // MELON_BASE_RESULT_STATUS_H_
