...
// elements to be joined, using the same formatting that `melon::string_cat()` uses.
...
//
// Because the default formatter uses the `melon::alpha_num` class,
// `melon::string_join()`, like `melon::string_cat()`, will work out-of-the-box on
// collections of strings, ints, floats, doubles, etc.
//
// Example:
//
//   std::vector<std::string> v = {"foo", "bar", "baz"};
//   std::string s = melon::string_join(v, "-");
//   EXPECT_EQ("foo-bar-baz", s);
//
// See comments on the `melon::string_join()` function for more examples.

#ifndef MELON_STRINGS_STR_JOIN_H_
#define MELON_STRINGS_STR_JOIN_H_
...
#include "melon/strings/internal/str_join_internal.h"
#include <string_view>
...
// functions. You may provide your own Formatter to enable `melon::string_join()` to
...
// argument to `melon::string_join()`:
//
//   std::vector<int> v = {1, 2, 3, 4};
//   std::string s = melon::string_join(v, "-", MyFormatter());
//   EXPECT_EQ("1-2-3-4", s);
//
...
// alpha_num_formatter()
//
// Default formatter used if none is specified. Uses `melon::alpha_num` to convert
// numeric arguments to strings.
MELON_FORCE_INLINE strings_internal::alpha_num_formatter_impl alpha_num_formatter() {
...
MELON_FORCE_INLINE strings_internal::stream_formatter_impl stream_formatter() {
...
MELON_FORCE_INLINE strings_internal::pair_formatter_impl<FirstFormatter, SecondFormatter>
...
MELON_FORCE_INLINE strings_internal::pair_formatter_impl<
...
// `alpha_num_formatter()`.
MELON_FORCE_INLINE strings_internal::dereference_formatter_impl<
        strings_internal::alpha_num_formatter_impl>
dereference_formatter() {
...
// `melon::string_join()` takes a range, a separator string to use between the
...
//   std::string s = melon::string_join({"foo", "bar", "baz"}, "-");
//   EXPECT_EQ("foo-bar-baz", s);
//
// Example 3:
...
//       std::make_pair("b", 2),
//       std::make_pair("c", 3)};
//   std::string s = melon::string_join(m, ",", melon::pair_formatter("="));
//   EXPECT_EQ("a=1,b=2,c=3", s);
//
// Example 7:
//   // These examples show how `melon::string_join()` handles a few common edge
//   // cases:
//   std::vector<std::string> v_empty;
//   EXPECT_EQ("", melon::string_join(v_empty, "-"));
//
//   std::vector<std::string> v_one_item = {"foo"};
//   EXPECT_EQ("foo", melon::string_join(v_one_item, "-"));
//
//   std::vector<std::string> v_empty_string = {""};
//   EXPECT_EQ("", melon::string_join(v_empty_string, "-"));
//
//   std::vector<std::string> v_one_item_empty_string = {"a", ""};
//   EXPECT_EQ("a-", melon::string_join(v_one_item_empty_string, "-"));
//
//   std::vector<std::string> v_two_empty_string = {"", ""};
//   EXPECT_EQ("-", melon::string_join(v_two_empty_string, "-"));
//
// Example 8:
//   // Joins a `std::tuple<T...>` of heterogeneous types, converting each to
//   // a std::string using the `melon::alpha_num` class.
...
#endif  // MELON_STRINGS_STR_JOIN_H_
