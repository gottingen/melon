...
#ifndef MELON_STRINGS_INTERNAL_CHAR_TRAITS_H_
#define MELON_STRINGS_INTERNAL_CHAR_TRAITS_H_
#include <cstddef>
#include <cstring>
#include "melon/base/profile.h"  // disable some warnings on Windows
#include "melon/strings/ascii.h"  // for melon::ascii_tolower


namespace melon {
namespace strings_internal {

MELON_FORCE_INLINE char *char_cat(char *dest, size_t destlen, const char *src,
                                 size_t srclen) {
...
const char *int_char_match(const char *haystack, size_t haylen,
    ...
    for (; haystack < hayend; ++haystack) {
        char hay = case_sensitive
                   ? *haystack
                   : melon::ascii::to_lower(static_cast<unsigned char>(*haystack));
        char nee = case_sensitive
                   ? *needle
                   : melon::ascii::to_lower(static_cast<unsigned char>(*needle));
        if (hay == nee) {
            if (++needle == needleend) {
...
MELON_FORCE_INLINE const char *char_str(const char *phaystack, size_t haylen,
                                       const char *pneedle) {
    return int_char_match<true>(phaystack, haylen, pneedle, strlen(pneedle));
}

MELON_FORCE_INLINE const char *char_case_str(const char *phaystack, size_t haylen,
                                            const char *pneedle) {
    return int_char_match<false>(phaystack, haylen, pneedle, strlen(pneedle));
}

MELON_FORCE_INLINE const char *char_mem(const char *phaystack, size_t haylen,
                                       const char *pneedle, size_t needlelen) {
    return int_char_match<true>(phaystack, haylen, pneedle, needlelen);
}

MELON_FORCE_INLINE const char *char_case_mem(const char *phaystack, size_t haylen,
                                            const char *pneedle, size_t needlelen) {
...
#endif  // MELON_STRINGS_INTERNAL_CHAR_TRAITS_H_
