...
#ifndef MELON_UNICODE_H_
#define MELON_UNICODE_H_

#include <string_view>
#include "melon/base/profile.h"
#include "melon/base/throw_delegate.h"
#include <string>
...
MELON_FORCE_INLINE void append_utf8(std::string &str, uint32_t unicode) {
    if (unicode <= 0x7f) {
...
#ifdef MELON_RAISE_UNICODE_ERRORS
...
MELON_FORCE_INLINE bool is_valid_utf8(const std::string &utf8String) {
...
MELON_FORCE_INLINE int unicode_to_utf8_bytes(uint32_t ui) {
    if (ui <= 0x7f) {
        return 1;
    } else if (ui <= 0x7ff) {
        return 2;
