...
#ifndef MELON_CONTAINER_ROBIN_MAP_H_
#define MELON_CONTAINER_ROBIN_MAP_H_
...
#include <type_traits>
#include <utility>
#include "melon/container/robin_hash.h"
...
namespace melon {
     ...
     * as we then compare the stored hashes before comparing the keys. When `melon::rh::power_of_two_growth_policy` is used
     ...
     * `sizeof(melon::detail_robin_hash::bucket_entry<ValueType, true>) ==
     * sizeof(melon::detail_robin_hash::bucket_entry<ValueType, false>)`) and `melon::rh::power_of_two_growth_policy` is
     ...
     * By default the map uses `melon::rh::power_of_two_growth_policy`. This policy keeps the number of buckets
     ...
     * check `melon::rh::power_of_two_growth_policy` for the interface.
     *
    ...
    template<class Key,
            ...
            class GrowthPolicy = melon::rh::power_of_two_growth_policy<2>>
    class robin_map {
    private:
        template<typename U>
        using has_is_transparent = melon::detail_robin_hash::has_is_transparent<U>;
    ...
    /**
     * Same as `melon::robin_map<Key, T, Hash, KeyEqual, Allocator, StoreHash, melon::rh::prime_growth_policy>`.
     */
    template<class Key,
            class T,
            class Hash = std::hash<Key>,
            ...
            class Allocator = std::allocator<std::pair<Key, T>>,
            bool StoreHash = false>
    using robin_pg_map = robin_map<Key, T, Hash, KeyEqual, Allocator, StoreHash, melon::rh::prime_growth_policy>;

}  // namespace melon

#endif  // MELON_CONTAINER_ROBIN_MAP_H_
