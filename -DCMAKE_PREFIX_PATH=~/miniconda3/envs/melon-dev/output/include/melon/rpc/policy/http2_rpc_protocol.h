...
namespace melon::rpc {
    namespace policy {
        ...
        inline H2ParseResult MakeH2Error(H2Error err, int stream_id) { return H2ParseResult(err, stream_id); }

        inline H2ParseResult MakeH2Error(H2Error err) { return H2ParseResult(err, 0); }

        inline H2ParseResult MakeH2Message(H2StreamContext *msg) { return H2ParseResult(msg); }
...
// https://tools.ietf.org/html/rfc7540#section-4.1
...
#ifndef NDEBUG
        struct h2_variables {
            melon::gauge<int> h2_unsent_request_count;
            melon::gauge<int> h2_stream_context_count;
        ...
        private:
            std::string &push(const std::string &name) { return (new(&_list[_size++]) HPacker::Header(name))->value; }
...
#ifndef NDEBUG
                get_h2_variables()->h2_unsent_request_count << 1;
#endif
            }
