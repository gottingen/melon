...
#include "melon/thread/thread.h"           // MELON_THREAD_LOCAL
...
#undef BAIDU_OBJECT_POOL_GET
    ...
    private:
        ...
        static melon::static_atomic<ObjectPool *> _singleton;
        ...
        static melon::static_atomic<BlockGroup *> _block_groups[OP_MAX_BLOCK_NGROUP];
...
// Declare template static variables:
    ...
    template<typename T>
    melon::static_atomic<long> ObjectPool<T>::_nlocal = MELON_STATIC_ATOMIC_INIT(0);

    template<typename T>
    melon::static_atomic<size_t> ObjectPool<T>::_ngroup = MELON_STATIC_ATOMIC_INIT(0);
    ...
    template<typename T>
    melon::static_atomic<typename ObjectPool<T>::BlockGroup *>
            ObjectPool<T>::_block_groups[OP_MAX_BLOCK_NGROUP] = {};

#ifdef MELON_OBJECT_POOL_NEED_FREE_ITEM_NUM
    template <typename T>
    melon::static_atomic<size_t> ObjectPool<T>::_global_nfree = MELON_STATIC_ATOMIC_INIT(0);
#endif
