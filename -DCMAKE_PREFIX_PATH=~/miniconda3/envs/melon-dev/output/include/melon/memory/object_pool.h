...
#ifndef MELON_MEMORY_OBJECT_POOL_H_
#define MELON_MEMORY_OBJECT_POOL_H_

#include <cstddef>                       // size_t
#include "melon/memory/ref_ptr.h"
...
#include "melon/memory/object_pool_inl.h"
...
namespace melon {
    ...
    template<class T>
    using pool_ref_counted = melon::ref_counted<T, object_pool_deleter<T>>;

    // Interface of `melon::object_pool::get` does not align very well with
    ...
    ref_ptr<T> get_ref_counted() {
        ...
        MELON_DCHECK_EQ(1, ptr->unsafe_ref_count());
        return ptr;
#else
    ...
    void object_pool_deleter<T>::operator()(T *p) const noexcept {
        MELON_DCHECK_EQ(p->ref_count_.load(std::memory_order_relaxed), 0);
...
#endif  // MELON_MEMORY_OBJECT_POOL_H_
