...
#ifndef MELON_THREAD_THREAD_H_
#define MELON_THREAD_THREAD_H_
...
#include <memory>
#include <functional>
#include "melon/thread/affinity.h"
...
namespace melon {
    ...
    public:
        ...
        template<typename F, typename = typename std::enable_if<std::is_same<thread,
                typename melon::remove_cvref<F>::type>::value == false>>
        explicit thread(F &&f) {
        ...
        template<typename F,
                typename = typename std::enable_if<
                        std::is_same<thread, typename melon::remove_cvref<F>::type>::value == false>>
        void initialize(F &&f) {
        ...
        static void set_name(const std::string_view &fmt, Args &&... args) {
            std::string name = melon::string_format(fmt, std::forward<Args>(args)...);
            set_name(name);
        }
