...
#ifndef MELON_THREAD_SPINLOCK_H_
#define MELON_THREAD_SPINLOCK_H_
...
namespace std {
    ...
    template<>
    class lock_guard<melon::spinlock> {
    public:
        explicit lock_guard(melon::spinlock &mutex) : _pmutex(&mutex) {
            _pmutex->lock();
        }
    ...
    private:
        ...
        melon::spinlock *_pmutex;
    };

    template<>
    class unique_lock<melon::spinlock> {
        MELON_DISALLOW_COPY_AND_ASSIGN(unique_lock);

    public:
        typedef melon::spinlock mutex_type;
...
#endif  // MELON_BASE_THREAD_SPINLOCK_H_
