...
TEMP=`getopt -o v: --long headers:,libs:,cc:,cxx:,with-glog,with-thrift,with-rdma,with-mesalink,nodebugsymbols -n 'config_melon' -- "$@"`
WITH_RDMA=0
WITH_MESALINK=0
DEBUGSYMBOLS=-g
...
if [ -z "$HDRS_IN" ] || [ -z "$LIBS_IN" ]; then
    >&2 $ECHO "config_melon: --headers=HDRPATHS --libs=LIBPATHS must be specified"
    exit 1
fi
...
OUTPUT_CONTENT="# Generated by config_melon.sh, don't modify manually"
append_to_output() {
