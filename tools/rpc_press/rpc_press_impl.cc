...
namespace pbrpcframework {
    ...
    public:
        ...
        virtual void AddError(const std::string &filename, int line,
            ...
            TURBO_LOG_AT(ERROR, filename.c_str(), line) << message;
        }
    };
    ...
    int PressClient::init() {
        ...
        if (nullptr == _method_descriptor) {
            TURBO_LOG(ERROR) << "Fail to find method=" << _options->service << '.'
                             << _options->method;
            return -1;
        }
    ...
    int RpcPress::init(const PressOptions *options) {
        if (nullptr == options) {
            TURBO_LOG(ERROR) << "Param[options] is nullptr";
            return -1;
        }
        _options = *options;
        ...
        if (_options.proto_file.empty()) {
            TURBO_LOG(ERROR) << "-proto is required";
            return -1;
        }
        ...
        if (!_options.proto_includes.empty()) {
            turbo::StringSplitter sp(_options.proto_includes.c_str(), ';');
            for (; sp; ++sp) {
        ...
        if (_importer->Import(proto_file.c_str()) == nullptr) {
            TURBO_LOG(ERROR) << "Fail to import " << proto_file;
            return -1;
        }
        ...
        if (!_options.output.empty()) {
            ...
            auto dir = turbo::file_path(_options.output).parent_path();
            std::error_code ec;
            ...
            TURBO_LOG_IF(ERROR, !_output_json) << "Fail to open " << _options.output;
        }

        int ret = _pbrpc_client->init();
        if (0 != ret) {
            TURBO_LOG(ERROR) << "Fail to initialize rpc client";
            return ret;
        }

        if (_options.input.empty()) {
            TURBO_LOG(ERROR) << "-input is empty";
            return -1;
        }
        melon::rpc::JsonLoader json_util(_importer, &_factory,
                                         _options.service, _options.method);
        if (turbo::exists(_options.input)) {
            int fd = open(_options.input.c_str(), O_RDONLY);
            if (fd < 0) {
                TURBO_PLOG(ERROR) << "Fail to open " << _options.input;
                return -1;
            }
        ...
        if (_msgs.empty()) {
            TURBO_LOG(ERROR) << "Fail to load requests";
            return -1;
        }
        TURBO_LOG(INFO) << "Loaded " << _msgs.size() << " requests";
    ...
    void RpcPress::handle_response(melon::rpc::Controller *cntl,
                                   ...
                                   int64_t start_time) {
        if (!cntl->Failed()) {
            int64_t rpc_call_time_us = turbo::get_current_time_micros() - start_time;
            ...
            if (_output_json) {
                ...
                if (!json2pb::ProtoMessageToJson(*response, &response_json, &error)) {
                    TURBO_LOG(WARNING) << "Fail to convert to json: " << error;
                }
                fprintf(_output_json, "%s\n", response_json.c_str());
            }
        } else {
            TURBO_LOG(WARNING) << "error_code=" << cntl->ErrorCode() << ", "
                               << cntl->ErrorText();
            _error_count << 1;
        }
        delete response;
    ...
    void RpcPress::sync_client() {
        ...
        while (!_stop) {
            ...
            if (_options.test_req_rate <= 0) {
                ...
                int64_t end_time = turbo::time_now().to_unix_nanos();
    ...
    int RpcPress::start() {
        ...
        for (int i = 0; i < _options.test_thread_num; i++) {
            if ((ret = pthread_create(&_ttid[i], nullptr, sync_call_thread, this)) != 0) {
                TURBO_LOG(ERROR) << "Fail to create sending threads";
                return -1;
            }
        }
        ...
        if (!_info_thr.start(info_thr_opt)) {
            TURBO_LOG(ERROR) << "Fail to create stats thread";
            return -1;
        }
        _started = true;
        return 0;
