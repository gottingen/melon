...
public:
    ...
    void TrackMe(google::protobuf::RpcController *cntl_base,
        ...
        turbo::end_point server_addr;
        TURBO_CHECK_EQ(0, turbo::str2endpoint(request->server_addr().c_str(), &server_addr));
        ...
        TURBO_LOG(INFO) << "Pinged by " << server_addr << " (r"
                  << request->rpc_version() << ")";
    }
...
int main(int argc, char *argv[]) {
    ...
    if (!bugs.start(FLAGS_bug_file)) {
        TURBO_LOG(ERROR) << "Fail to start BugsLoader";
        return -1;
    }
    ...
    if (server.Start(FLAGS_port, &options) != 0) {
        TURBO_LOG(ERROR) << "Fail to start TrackMeServer";
        return -1;
    }
...
bool BugsLoader::start(const std::string &bugs_file) {
    ...
    if (pthread_create(&_tid, nullptr, run_this, this) != 0) {
        TURBO_LOG(ERROR) << "Fail to create loading thread";
        return false;
    }
    _started = true;
    return true;
...
void BugsLoader::run() {
    ...
    if (fw.init(_bugs_file.c_str()) < 0) {
        TURBO_LOG(ERROR) << "Fail to init file_watcher on `" << _bugs_file << "'";
        return;
    }
    while (!_stop) {
        load_bugs();
        while (!_stop) {
            turbo::file_watcher::Change change = fw.check_and_consume();
            if (change > 0) {
                break;
            }
            if (change < 0) {
                TURBO_LOG(ERROR) << "`" << _bugs_file << "' was deleted";
            }
            sleep(1);
        }
    }
...
void BugsLoader::load_bugs() {
    ...
    if (!status.is_ok()) {
        TURBO_PLOG(WARNING) << "Fail to open `" << _bugs_file << '\'';
        return;
    }
    ...
    for (auto line : lines) {
        ...
        std::vector<std::string> sps = turbo::string_split(line, " \t");
        auto sp = sps.begin();
        if (sp == sps.end()) {
            continue;
        }
        long long min_rev;
        if (!turbo::simple_atoi(*sp, &min_rev)) {
            TURBO_LOG(WARNING) << "[line" << nline << "] Fail to parse column1 as min_rev";
            continue;
        }
        ++sp;
        long long max_rev;
        if (sp == sps.end()|| !turbo::simple_atoi(*sp, &max_rev)) {
            TURBO_LOG(WARNING) << "[line" << nline << "] Fail to parse column2 as max_rev";
            continue;
        }
        if (max_rev < min_rev) {
            TURBO_LOG(WARNING) << "[line" << nline << "] max_rev=" << max_rev
                         << " is less than min_rev=" << min_rev;
            continue;
        }
        ++sp;
        if (sp == sps.end()) {
            TURBO_LOG(WARNING) << "[line" << nline << "] Fail to parse column3 as severity";
            continue;
        }
        ...
        if (severity_str == "f" || severity_str == "F") {
            ...
            TURBO_LOG(WARNING) << "[line" << nline << "] Invalid severity=" << severity_str;
            continue;
        }
        ++sp;
        if (sp == sps.end()) {
            TURBO_LOG(WARNING) << "[line" << nline << "] Fail to parse column4 as string";
            continue;
        }
    ...
    TURBO_LOG(INFO) << "Loaded " << m->size() << " bugs";
