...
#ifndef ABEL_SCHEDULE_FUTURE_H_
#define ABEL_SCHEDULE_FUTURE_H_
...
namespace abel {
    ...
    struct future_state {
        ...
        void set(const std::tuple<T...> &value) noexcept {
            ABEL_ASSERT(_state == state::future);
        ...
        std::exception_ptr get_exception() && noexcept {
            ABEL_ASSERT(_state == state::exception);
        ...
        std::tuple<T...> get_value() && noexcept {
            ABEL_ASSERT(_state == state::result);
    ...
    struct future_state<> {
        ...
        void set(const std::tuple<> &value) noexcept {
            ABEL_ASSERT(_u.st == state::future);
            _u.st = state::result;
        }
        ...
        void set_exception(std::exception_ptr ex) noexcept {
            ABEL_ASSERT(_u.st == state::future);
            new(&_u.ex) std::exception_ptr(ex);
            ABEL_ASSERT(_u.st >= state::exception_min);
        }
        ...
        std::tuple<> get_value() const noexcept {
            ABEL_ASSERT(_u.st == state::result);
            return {};
        }
    ...
    struct continuation final : continuation_base<T...> {
        continuation(Func &&func, future_state<T...> &&state) : continuation_base<T...>(std::move(state)),
                                                                _func(std::move(func)) {}
    ...
    ABEL_CONCEPT(
            ...
            template<typename T>
            concept
            bool Future = is_future<T>::value;

            template<typename Func, typename... T>
            concept
            bool CanApply = requires(Func f, T... args) {
                f(std::forward<T>(args)...);
            };

            template<typename Func, typename Return, typename... T>
            concept
            bool ApplyReturns = requires(Func f, T... args) {
                { f(std::forward<T>(args)...) } -> Return;
            };

            template<typename Func, typename... T>
            concept
            bool ApplyReturnsAnyFuture = requires(Func f, T... args) {
    ...
    private:
        ...
        void schedule(Func &&func) {
            if (state()->available()) {
                ::abel::schedule(std::make_unique<continuation<Func, T...>>(std::move(func), std::move(*state())));
            } else {
                ABEL_ASSERT(_promise);
    ...
    public:
        ...
        ///
        /// If get() is called in a \ref abel::thread context,
        ...
        /// Wait for the future to be available (in a abel::thread)
        ///
        /// When called from a abel::thread, this function blocks the
    ...
    public:
        ...
        ABEL_CONCEPT(requires ::abel::CanApply<Func, T...>)
        Result
        then(Func &&func) noexcept {
            ...
            try {
                memory_internal::disable_failure_guard dfg;
                schedule([pr = std::move(pr), func = std::forward<Func>(func)](auto &&state) mutable {
                    if (state.failed()) {
                        pr.set_exception(std::move(state).get_exception());
                    } else {
                        futurator::apply(std::forward<Func>(func), std::move(state).get_value()).forward_to(
                                std::move(pr));
                    }
                });
            } catch (...) {
        ...
        ABEL_CONCEPT(requires ::abel::CanApply<Func, future>)
        Result
         ...
         */
        template<typename Func>
        ABEL_CONCEPT(requires ::abel::CanApply<Func>)
        ...
        template<typename Func>
        /* Broken?
        ABEL_CONCEPT( requires ::abel::ApplyReturns<Func, future<T...>, std::exception_ptr>
                        || (sizeof...(T) == 0 && ::abel::ApplyReturns<Func, void, std::exception_ptr>)
                        || (sizeof...(T) == 1 && ::abel::ApplyReturns<Func, T..., std::exception_ptr>)
        ) */
    ...
    private:
        void set_callback(std::unique_ptr<continuation_base<T...>> callback) {
            if (state()->available()) {
                ...
                ::abel::schedule(std::move(callback));
            } else {
                ABEL_ASSERT(_promise);
        ...
        friend void schedule_internal::set_callback(future<U...> &, std::unique_ptr<V>);
    };

    ABEL_FORCE_INLINE void future_state<>::forward_to(promise<> &pr) noexcept {
        ABEL_ASSERT(_u.st != state::future && _u.st != state::invalid);
        if (_u.st >= state::exception_min) {
            pr.set_urgent_exception(std::move(_u.ex));
            _u.ex.~exception_ptr();
        } else {
    ...
    template<typename... T>
    ABEL_FORCE_INLINE future<T...> promise<T...>::get_future() noexcept {
        ABEL_ASSERT(!_future && _state && !_task);
    ...
    void promise<T...>::make_ready() noexcept {
        if (_task) {
            ...
            if (Urgent == urgent::yes && !need_preempt()) {
                ::abel::schedule_urgent(std::move(_task));
            } else {
                ::abel::schedule(std::move(_task));
            }
        }
    }
    ...
    void promise<T...>::abandoned() noexcept {
        if (_future) {
            ABEL_ASSERT(_state);
            ABEL_ASSERT(_state->available() || !_task);
    ...
    typename futurize<T>::type futurize<T>::apply(Func &&func, std::tuple<FuncArgs...> &&args) noexcept {
        try {
            return convert(abel::apply(std::forward<Func>(func), std::move(args)));
        } catch (...) {
    ...
    struct do_void_futurize_helper<void> {
        ...
        static future<> apply_tuple(Func &&func, std::tuple<FuncArgs...> &&args) noexcept {
            try {
                ::abel::apply(std::forward<Func>(func), std::move(args));
                return make_ready_future<>();
            } catch (...) {
                return make_exception_future(std::current_exception());
            }
    ...
    struct do_void_futurize_helper<future<>> {
        ...
        static future<> apply_tuple(Func &&func, std::tuple<FuncArgs...> &&args) noexcept {
            try {
                return ::abel::apply(std::forward<Func>(func), std::move(args));
            } catch (...) {
                return make_exception_future(std::current_exception());
            }
        }
    ...
    typename futurize<void>::type futurize<void>::apply(Func &&func, FuncArgs &&... args) noexcept {
        return void_futurize_helper<Func, FuncArgs...>::apply(std::forward<Func>(func),
                                                              std::forward<FuncArgs>(args)...);
    }

    template<typename... Args>
    template<typename Func, typename... FuncArgs>
    typename futurize<future<Args...>>::type
    futurize<future<Args...>>::apply(Func &&func, std::tuple<FuncArgs...> &&args) noexcept {
        try {
    ...
    template<typename Func, typename... FuncArgs>
    typename futurize<future<Args...>>::type
    futurize<future<Args...>>::apply(Func &&func, FuncArgs &&... args) noexcept {
        try {
            return func(std::forward<FuncArgs>(args)...);
        } catch (...) {
            return make_exception_future(std::current_exception());
