...
//
//  * `abel::abel_time` defines an absolute, specific instance in time
//  * `abel::duration` defines a signed, fixed-length span of time
//  * `abel::time_zone` defines geopolitical time zone regions (as collected
//     within the IANA abel_time Zone database (https://www.iana.org/time-zones)).
//
...
// (`abel::time_zone` within this API). That is:
//
...
//
//   abel::time_zone nyc;
//   // load_time_zone() may fail so it's always better to check for success.
//   if (!abel::load_time_zone("America/New_York", &nyc)) {
//      // handle error case
//   }
//
//   // My flight leaves NYC on Jan 2, 2017 at 03:04:05
//   abel::CivilSecond cs(2017, 1, 2, 3, 4, 5);
//   abel::abel_time takeoff = abel::from_civil(cs, nyc);
//
//   abel::duration flight_duration = abel::hours(21) + abel::minutes(35);
//   abel::abel_time landing = takeoff + flight_duration;
//
//   abel::time_zone syd;
//   if (!abel::load_time_zone("Australia/Sydney", &syd)) {
//      // handle error case
//   }
//   std::string s = abel::format_time(
//       "My flight will land in Sydney on %Y-%m-%d at %H:%M:%S",
//       landing, syd);

#ifndef ABEL_TIME_TIME_H_
#define ABEL_TIME_TIME_H_
...
ABEL_FORCE_INLINE duration MakePosDoubleDuration(double n);
...
ABEL_FORCE_INLINE int64_t operator/(duration lhs, duration rhs) {
  return time_internal::integer_div_duration(true, lhs, rhs,
...
ABEL_FORCE_INLINE int64_t integer_div_duration(duration num, duration den, duration* rem) {
  return time_internal::integer_div_duration(true, num, den,
...
// `abel::hours(24)`. If you actually want a civil day, use abel::CivilDay
// from civil_time.h.
//
// Example:
//
...
class abel_time {
 public:
  ...
  // `abel::time_zone`.
  //
  // Deprecated. Use `abel::time_zone::CivilInfo`.
  struct breakdown {
 ...
 private:
  friend constexpr abel_time time_internal::from_unix_duration(duration d);
  friend constexpr duration time_internal::to_unix_duration(abel_time t);
...
constexpr abel_time universal_epoch() {
  ...
  return abel_time(time_internal::MakeDuration(-24 * 719162 * int64_t{3600}, 0U));
}
...
class time_zone {
 public:
  ...
  // than passing a `time_zone::CivilInfo` to a function, pass an `abel::abel_time`
  ...
  //   // epoch.is_dst == false
  //   // epoch.abbr == "PST"
  CivilInfo At(abel_time t) const;
  ...
  // To account for these possibilities, `abel::time_zone::TimeInfo` is
  ...
  // time_zone::At(CivilSecond)
  //
  // Returns an `abel::TimeInfo` containing the absolute time(s) for this
  // time_zone at an `abel::CivilSecond`. When the civil time is skipped or
  ...
  //
  //   // A unique civil time
  //   const auto jan01 = lax.At(abel::CivilSecond(2011, 1, 1, 0, 0, 0));
  //   // jan01.kind == time_zone::TimeInfo::UNIQUE
  //   // jan01.pre    is 2011-01-01 00:00:00 -0800
  //   // jan01.trans  is 2011-01-01 00:00:00 -0800
  //   // jan01.post   is 2011-01-01 00:00:00 -0800
  //
  //   // A Spring DST transition, when there is a gap in civil time
  //   const auto mar13 = lax.At(abel::CivilSecond(2011, 3, 13, 2, 15, 0));
  //   // mar13.kind == time_zone::TimeInfo::SKIPPED
  //   // mar13.pre   is 2011-03-13 03:15:00 -0700
  //   // mar13.trans is 2011-03-13 03:00:00 -0700
  //   // mar13.post  is 2011-03-13 01:15:00 -0800
  //
  //   // A Fall DST transition, when civil times are repeated
  //   const auto nov06 = lax.At(abel::CivilSecond(2011, 11, 6, 1, 15, 0));
  //   // nov06.kind == time_zone::TimeInfo::REPEATED
  //   // nov06.pre   is 2011-11-06 01:15:00 -0700
  //   // nov06.trans is 2011-11-06 01:00:00 -0800
  //   // nov06.post  is 2011-11-06 01:15:00 -0800
  TimeInfo At(CivilSecond ct) const;

  // time_zone::NextTransition()
  // time_zone::PrevTransition()
  //
  // Finds the time of the next/previous offset change in this time zone.
  //
  // By definition, `NextTransition(t, &trans)` returns false when `t` is
  // `infinite_future()`, and `PrevTransition(t, &trans)` returns false
  ...
  bool NextTransition(abel_time t, CivilTransition* trans) const;
  bool PrevTransition(abel_time t, CivilTransition* trans) const;
...
//   abel::abel_time t = ...;
//   abel::time_zone tz = ...;
//   const auto cd = abel::to_civil_day(t, tz);
ABEL_FORCE_INLINE CivilSecond to_civil_second(abel_time t, time_zone tz) {
  return tz.At(t).cs;  // already a CivilSecond
}
ABEL_FORCE_INLINE CivilMinute to_civil_minute(abel_time t, time_zone tz) {
  return CivilMinute(tz.At(t).cs);
}
ABEL_FORCE_INLINE CivilHour to_civil_hour(abel_time t, time_zone tz) {
  return CivilHour(tz.At(t).cs);
}
ABEL_FORCE_INLINE CivilDay to_civil_day(abel_time t, time_zone tz) {
  return CivilDay(tz.At(t).cs);
}
ABEL_FORCE_INLINE CivilMonth to_civil_month(abel_time t, time_zone tz) {
  return CivilMonth(tz.At(t).cs);
}
ABEL_FORCE_INLINE CivilYear to_civil_year(abel_time t, time_zone tz) {
  return CivilYear(tz.At(t).cs);
}

// from_civil()
//
// Helper for time_zone::At(CivilSecond) that provides "order-preserving
...
ABEL_FORCE_INLINE abel_time from_civil(CivilSecond ct, time_zone tz) {
  const auto ti = tz.At(ct);
  if (ti.kind == time_zone::TimeInfo::SKIPPED) return ti.trans;
  return ti.pre;
}

// TimeConversion
//
// An `abel::TimeConversion` represents the conversion of year, month, day,
// hour, minute, and second values (i.e., a civil time), in a particular
// `abel::time_zone`, to a time instant (an absolute time), as returned by
// `abel::convert_date_time()`. Legacy version of `abel::time_zone::TimeInfo`.
//
// Deprecated. Use `abel::time_zone::TimeInfo`.
struct
    TimeConversion {
  abel_time pre;    // time calculated using the pre-transition offset
  abel_time trans;  // when the civil-time discontinuity occurred
...
// convert_date_time()
//
// Legacy version of `abel::time_zone::At(abel::CivilSecond)` that takes
...
//   // abel::to_civil_day(tc.pre, tz).month() == 11
//   // abel::to_civil_day(tc.pre, tz).day() == 1
//
// Deprecated. Use `abel::time_zone::At(CivilSecond)`.
...
//
// Deprecated. Use `abel::from_civil(CivilSecond, time_zone)`. Note that the
// behavior of `from_civil()` differs from `format_date_time()` for skipped civil
// times. If you care about that see `abel::time_zone::At(abel::CivilSecond)`.
...
// time instant is not unique (see `abel::time_zone::At(abel::CivilSecond)`
...
abel_time FromTM(const struct tm& tm, time_zone tz);

// ToTM()
//
// Converts the given `abel::abel_time` to a struct tm using the given time zone.
// See ctime(3) for a description of the values of the tm fields.
struct tm ToTM(abel_time t, time_zone tz);
...
// Example:
//
//   abel::CivilSecond cs(2013, 1, 2, 3, 4, 5);
//   abel::abel_time t = abel::from_civil(cs, lax);
...
// rather than normalizing them like `abel::CivilSecond` does.  For example,
...
// by time_zone::TimeInfo) is returned.  For these reasons we recommend that
...
constexpr abel_time from_unix_nanos(int64_t ns) {
  return time_internal::from_unix_duration(nanoseconds(ns));
}

constexpr abel_time from_unix_micros(int64_t us) {
  return time_internal::from_unix_duration(microseconds(us));
}

constexpr abel_time from_unix_millis(int64_t ms) {
  return time_internal::from_unix_duration(milliseconds(ms));
}

constexpr abel_time from_unix_seconds(int64_t s) {
  return time_internal::from_unix_duration(seconds(s));
}
