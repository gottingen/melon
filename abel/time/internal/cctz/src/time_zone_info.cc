...
// data provided by the IANA abel_time Zone Database (i.e., the only real game
// in town).
//
...
ABEL_FORCE_INLINE bool IsLeap(year_t year) {
...
ABEL_FORCE_INLINE time_zone::civil_lookup MakeUnique(const time_point<seconds>& tp) {
...
ABEL_FORCE_INLINE time_zone::civil_lookup MakeUnique(std::int_fast64_t unix_time) {
  return MakeUnique(from_unix_seconds(unix_time));
}

ABEL_FORCE_INLINE time_zone::civil_lookup MakeSkipped(const Transition& tr,
...
ABEL_FORCE_INLINE time_zone::civil_lookup MakeRepeated(const Transition& tr,
                                            const civil_second& cs) {
  time_zone::civil_lookup cl;
...
ABEL_FORCE_INLINE civil_second YearShift(const civil_second& cs, year_t shift) {
