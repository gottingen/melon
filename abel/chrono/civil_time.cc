...
// Formats the given chrono_second according to the given format.
        std::string FormatYearAnd(string_view fmt, chrono_second cs) {
...
// argument of type CivilT2.
    ...
    bool parse_chrono_time(string_view s, chrono_second *c) {
        return ParseYearAnd("-%m-%dT%H:%M:%S", s, c);
    }

    bool parse_chrono_time(string_view s, chrono_minute *c) {
        return ParseYearAnd("-%m-%dT%H:%M", s, c);
    }

    bool parse_chrono_time(string_view s, chrono_hour *c) {
        return ParseYearAnd("-%m-%dT%H", s, c);
    }

    bool parse_chrono_time(string_view s, chrono_day *c) {
        return ParseYearAnd("-%m-%d", s, c);
    }

    bool parse_chrono_time(string_view s, chrono_month *c) {
        return ParseYearAnd("-%m", s, c);
    }

    bool parse_chrono_time(string_view s, chrono_year *c) {
        return ParseYearAnd("", s, c);
    }

    bool ParseLenientCivilTime(string_view s, chrono_second *c) {
        return ParseLenient(s, c);
    }

    bool ParseLenientCivilTime(string_view s, chrono_minute *c) {
        return ParseLenient(s, c);
    }

    bool ParseLenientCivilTime(string_view s, chrono_hour *c) {
        return ParseLenient(s, c);
    }

    bool ParseLenientCivilTime(string_view s, chrono_day *c) {
        return ParseLenient(s, c);
    }

    bool ParseLenientCivilTime(string_view s, chrono_month *c) {
        return ParseLenient(s, c);
    }

    bool ParseLenientCivilTime(string_view s, chrono_year *c) {
        return ParseLenient(s, c);
    }
