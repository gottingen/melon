...
void WriteHead (std::ostream &out, const std::string &name, const cache_metrics &metric, const std::string &suffix = "",
...
void SerializeMetrics (std::ostream &out, const cache_metrics &metrics) {
    ...
    switch (metrics.type) {
    case metrics_type::mt_counter:out << "# TYPE " << metricsName << " counter\n";
        SerializeCounter(out, metricsName, metrics);
        break;
    case metrics_type::mt_gauge:out << "# TYPE " << metricsName << " gauge\n";
        SerializeGauge(out, metricsName, metrics);
        break;
    case metrics_type::mt_histogram:out << "# TYPE " << metricsName << " histogram\n";
        SerializeHistogram(out, metricsName, metrics);
        break;
    case metrics_type::mt_timer:out << "# TYPE " << metricsName << " histogram\n";
        SerializeHistogram(out, metricsName, metrics);
    default:break;
    }
}
