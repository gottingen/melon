...
#ifndef ABEL_BASE_ANNOTATION_H_
#define ABEL_BASE_ANNOTATION_H_
...
#define ABEL_INTERNAL_USE_ASAN 1
#endif  // __has_feature(address_sanitizer)

#if __has_feature(thread_sanitizer)
#define ABEL_INTERNAL_USE_TSAN 1
#endif  // __has_feature(thread_sanitizer)

#define ABEL_INTERNAL_NO_SANITIZE_ADDRESS [[clang::no_sanitize("address")]]
#define ABEL_INTERNAL_NO_SANITIZE_THREAD [[clang::no_sanitize("thread")]]
#endif  // __clang__
...
#define ABEL_INTERNAL_NO_SANITIZE_ADDRESS [[gnu::no_sanitize("address")]]
#define ABEL_INTERNAL_NO_SANITIZE_THREAD [[gnu::no_sanitize("thread")]]
...
#define ABEL_INTERNAL_TSAN_MUTEX_CREATE(...) __tsan_mutex_create(__VA_ARGS__)
#define ABEL_INTERNAL_TSAN_MUTEX_DESTROY(...) __tsan_mutex_destroy(__VA_ARGS__)
#define ABEL_INTERNAL_TSAN_MUTEX_PRE_LOCK(...) \
  __tsan_mutex_pre_lock(__VA_ARGS__)
#define ABEL_INTERNAL_TSAN_MUTEX_POST_LOCK(...) \
  __tsan_mutex_post_lock(__VA_ARGS__)
#define ABEL_INTERNAL_TSAN_MUTEX_PRE_UNLOCK(...) \
  __tsan_mutex_pre_unlock(__VA_ARGS__)
#define ABEL_INTERNAL_TSAN_MUTEX_POST_UNLOCK(...) \
  __tsan_mutex_post_unlock(__VA_ARGS__)

#else

#define ABEL_INTERNAL_TSAN_MUTEX_CREATE(...) (void)0
#define ABEL_INTERNAL_TSAN_MUTEX_DESTROY(...) (void)0
#define ABEL_INTERNAL_TSAN_MUTEX_PRE_LOCK(...) (void)0
#define ABEL_INTERNAL_TSAN_MUTEX_POST_LOCK(...) (void)0
#define ABEL_INTERNAL_TSAN_MUTEX_PRE_UNLOCK(...) (void)0
#define ABEL_INTERNAL_TSAN_MUTEX_POST_UNLOCK(...) (void)0

#endif  // ABEL_INTERNAL_USE_TSAN

// Using `initial-exec` prevents binary from being loaded via `dlopen`. abel is
// not intended to be used that way (via `dlopen`). Yet in case there's such
// need, you can define `ABEL_USE_SLOW_TLS_MODEL` to accomplish that.
//
// @sa: `doc/performance-guide.md` for more details.
#if !defined(ABEL_USE_SLOW_TLS_MODEL)
#define ABEL_INTERNAL_TLS_MODEL [[gnu::tls_model("initial-exec")]]
#else
#define ABEL_INTERNAL_TLS_MODEL /* Nothing, using the default model. */
#endif
...
inline void start_switch_fiber(void** shadow_stack, const void* stack_bottom,
  ...
  ABEL_INTERNAL_ASAN_CHECK(__sanitizer_start_switch_fiber);
...
inline void complete_switch_fiber(void* shadow_stack) {
  ABEL_INTERNAL_ASAN_CHECK(__sanitizer_finish_switch_fiber);
...
inline void poison_memory_region(const void* ptr, std::size_t size) {
  ABEL_INTERNAL_ASAN_CHECK(__asan_poison_memory_region);
  __asan_poison_memory_region(ptr, size);
}

inline void UnpoisonMemoryRegion(const void* ptr, std::size_t size) {
  ABEL_INTERNAL_ASAN_CHECK(__asan_unpoison_memory_region);
  __asan_unpoison_memory_region(ptr, size);
}

#undef ABEL_INTERNAL_ASAN_CHECK
...
inline void* get_current_fiber() {
  ABEL_INTERNAL_TSAN_CHECK(__tsan_get_current_fiber);
...
inline void SetFiberName(void* fiber, const char* name) {
  ABEL_INTERNAL_TSAN_CHECK(__tsan_set_fiber_name);
  __tsan_set_fiber_name(fiber, name);
}

#undef ABEL_INTERNAL_TSAN_CHECK
