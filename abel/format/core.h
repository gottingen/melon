...
#ifndef ABEL_FORMAT_CORE_H_
#define ABEL_FORMAT_CORE_H_
...
public:
    ...
    value (abel::basic_string_view<char_type> val) {
...
FMT_MAKE_VALUE(string_type, abel::basic_string_view<typename C::char_type>,
               abel::basic_string_view<typename C::char_type>)
FMT_MAKE_VALUE(string_type,
               typename abel::basic_string_view<typename C::char_type>::type,
               abel::basic_string_view<typename C::char_type>)
...
inline typename std::enable_if<
    !convert_to_int<T, Char>::value &&
        !std::is_convertible<T, abel::basic_string_view<Char>>::value,
...
private:
    abel::basic_string_view<Char> format_str_;
    int next_arg_id_;

public:
    typedef Char char_type;
    typedef typename abel::basic_string_view<Char>::iterator iterator;

    explicit FMT_CONSTEXPR basic_parse_context (
        abel::basic_string_view<Char> format_str, ErrorHandler eh = ErrorHandler())
    ...
    void check_arg_id (abel::basic_string_view<Char>) { }
...
private:
    ...
    struct entry {
        abel::basic_string_view<char_type> name;
...
public:
    ...
    basic_format_arg<Context> find (abel::basic_string_view<char_type> name) const {
...
protected:
    ...
    context_base (OutputIt out, abel::basic_string_view<char_type> format_str,
...
public:
     ...
     */
    basic_format_context (OutputIt out, abel::basic_string_view<char_type> format_str,
    ...
    // specified name.
    format_arg get_arg (abel::basic_string_view<char_type> name);
};
...
struct named_arg_base {
    ...
    named_arg_base (abel::basic_string_view<Char> nm) : name(nm) { }
...
struct named_arg : named_arg_base<Char> {
    ...
    named_arg (abel::basic_string_view<Char> name, const T &val)
 ...
 */
template<typename T>
inline internal::named_arg<T, char> arg (abel::string_view name, const T &arg) {
    return internal::named_arg<T, char>(name, arg);
}

template<typename T>
inline internal::named_arg<T, wchar_t> arg (abel::wstring_view name, const T &arg) {
...
FMT_API void vprint_colored (color c, abel::string_view format, format_args args);
FMT_API void vprint_colored (color c, abel::wstring_view format, wformat_args args);
 ...
 */
template<typename... Args>
inline void print_colored (color c, abel::string_view format_str,
                           const Args &... args) {
    vprint_colored(c, format_str, make_format_args(args...));
}

template<typename... Args>
inline void print_colored (color c, abel::wstring_view format_str,
                           const Args &... args) {
...
vformat_to (std::back_insert_iterator<Container> out,
            abel::string_view format_str, format_args args) {
...
typename std::enable_if<
    is_contiguous<Container>::value, std::back_insert_iterator<Container>>::type
vformat_to (std::back_insert_iterator<Container> out,
            abel::wstring_view format_str, wformat_args args) {
    auto &container = internal::get_container(out);
    internal::container_buffer<Container> buf(container);
    vformat_to(buf, format_str, args);
    return std::back_inserter(container);
...
std::string vformat (abel::string_view format_str, format_args args);
std::wstring vformat (abel::wstring_view format_str, wformat_args args);
...
*/
template<typename... Args>
inline std::string format (abel::string_view format_str, const Args &... args) {
    // This should be just
...
template<typename... Args>
inline std::wstring format (abel::wstring_view format_str, const Args &... args) {
    format_arg_store < wformat_context, Args...> as {args...};
    return vformat(format_str, as);
}

FMT_API void vprint (std::FILE *f, abel::string_view format_str, format_args args);
FMT_API void vprint (std::FILE *f, abel::wstring_view format_str, wformat_args args);
 ...
 */
template<typename... Args>
inline void print (std::FILE *f, abel::string_view format_str, const Args &... args) {
    format_arg_store < format_context, Args...> as(args...);
    vprint(f, format_str, as);
}
template<typename... Args>
inline void print (std::FILE *f, abel::wstring_view format_str, const Args &... args) {
    format_arg_store < wformat_context, Args...> as(args...);
    vprint(f, format_str, as);
}

FMT_API void vprint (abel::string_view format_str, format_args args);
FMT_API void vprint (abel::wstring_view format_str, wformat_args args);
...
template<typename... Args>
inline void print (abel::wstring_view format_str, const Args &... args) {
    format_arg_store < wformat_context, Args...> as(args...);
    vprint(format_str, as);
}
FMT_END_NAMESPACE

#endif  // ABEL_FORMAT_CORE_H_
