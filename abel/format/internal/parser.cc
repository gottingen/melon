...
#define ABEL_FORMAT_PARSER_INTERNAL_GET_CHAR()          \
  do {                                                  \
    ...
    const auto parse_digits = [&] {
        ...
        for (;;) {
            if (ABEL_UNLIKELY(pos == end))
                break;
            c = *pos++;
            if (!std::isdigit(c))
                break;
            --num_digits;
            if (ABEL_UNLIKELY(!num_digits))
                break;
    ...
    if (c < 'A') {
        ...
        for (; c <= '0';) {
            ...
            // can |= a value.
            switch (c) {
            case '-':conv->flags.left = true;
                break;
            case '+':conv->flags.show_pos = true;
                break;
            case ' ':conv->flags.sign_col = true;
                break;
            case '#':conv->flags.alt = true;
                break;
            case '0':conv->flags.zero = true;
                break;
...
bool ParsedFormatBase::MatchesConversions (
    ...
    for (const ConversionItem &item : items_) {
        ...
        if (conv.width.is_from_arg() &&
            ...
            return false;
        if (!add_if_valid_conv(conv.arg_position, conv.conv.Char()))
            return false;
    }
