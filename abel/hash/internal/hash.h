...
// returns the size of these chunks.
        constexpr size_t piecewise_chunk_size() { return 1024; }
        ...
        private:
            unsigned char buf_[piecewise_chunk_size()];
            size_t position_;
        };
...
// abel_hash_value for hashing tuples
        ...
        template<typename H>
        H abel_hash_value(H hash_state, std::string_view str) {
            return H::combine(
...
// abel_hash_value for hashing std::set
        ...
        abel_hash_value(H hash_state, const std::variant<T...> &v) {
            if (!v.valueless_by_exception()) {
                hash_state = std::visit(variant_visitor<H>{std::move(hash_state)}, v);
            }
...
#define ABEL_HASH_INTERNAL_SUPPORT_LEGACY_HASH_ 0
        ...
        struct hash_select {
        private:
            struct select_state : hash_state_base<select_state> {
                static select_state combine_contiguous(select_state hash_state, const unsigned char *,
                                                       size_t);

                using select_state::hash_state_base::combine_contiguous;
            };

            struct uniquely_represented_probe {
                template<typename H, typename T>
            ...
            struct hash_probe : Hash {
            private:
                template<typename H, typename = decltype(H::invoke(
                        std::declval<select_state>(), std::declval<const T &>()))>
        ...
        public:
            // hash_probe each implementation in order.
            // disjunction provides short circuiting wrt instantiation.
            template<typename T>
            using apply = abel::disjunction<         //
                    hash_probe<uniquely_represented_probe, T>,  //
                    hash_probe<hash_value_probe, T>,            //
                    hash_probe<legacy_hash_probe, T>,           //
                    hash_probe<std_hash_probe, T>,              //
                    std::false_type>;
        };
        ...
        public:
            ...
            static city_hash_state combine_contiguous(city_hash_state hash_state,
                ...
                return city_hash_state(
                        combine_contiguous_impl(hash_state._state, first, size,
            ...
            template<typename T, abel::enable_if_t<integral_fast_path<T>::value, int> = 0>
            static size_t hash(T value) {
                return static_cast<size_t>(read_mix(hash_seed(), static_cast<uint64_t>(value)));
            }

            // Overload of city_hash_state::hash()
            template<typename T, abel::enable_if_t<!integral_fast_path<T>::value, int> = 0>
            static size_t hash(const T &value) {
                return static_cast<size_t>(combine(city_hash_state{}, value)._state);
            }

        private:
            // Invoked only once for a given argument; that plus the fact that this is
            // move-only ensures that there is only one non-moved-from object.
            city_hash_state() : _state(hash_seed()) {}
            ...
            explicit city_hash_state(uint64_t state) : _state(state) {}
            ...
            // larger than piecewise_chunk_size().  Has the same effect as calling
            ...
            // .second.
            static std::pair<uint64_t, uint64_t> read_9_to_16(const unsigned char *p,
                                                              size_t len) {
            ...
            static uint64_t read_4_to_8(const unsigned char *p, size_t len) {
            ...
            static uint32_t read_1_to_3(const unsigned char *p, size_t len) {
                return static_cast<uint32_t>((p[0]) |                         //
                                             (p[len / 2] << (len / 2 * 8)) |  //
                                             (p[len - 1] << ((len - 1) * 8)));
            }

            ABEL_FORCE_INLINE static uint64_t read_mix(uint64_t state, uint64_t v) {
                using MultType =
            ...
            ABEL_FORCE_INLINE static uint64_t hash_seed() {
        ...
        ABEL_FORCE_INLINE uint64_t city_hash_state::combine_contiguous_impl(
            ...
            uint64_t v;
            if (len > 8) {
                if (ABEL_UNLIKELY(len > piecewise_chunk_size())) {
                ...
                v = read_4_to_8(first, len);
            } else if (len > 0) {
                v = read_1_to_3(first, len);
            } else {
                // Empty ranges have no effect.
                return state;
            }
            return read_mix(state, v);
        }
        ...
        ABEL_FORCE_INLINE uint64_t city_hash_state::combine_contiguous_impl(
            ...
            if (len > 16) {
                ...
                auto p = read_9_to_16(first, len);
                state = read_mix(state, p.first);
                v = p.second;
            } else if (len >= 4) {
                v = read_4_to_8(first, len);
            } else if (len > 0) {
        ...
        template<typename T>
        struct city_hash_impl {
            size_t operator()(const T &value) const { return city_hash_state::hash(value); }
        };

        template<typename T>
        struct city_hash
                : abel::conditional_t<is_hashable<T>::value, city_hash_impl<T>, poisoned_hash> {
        };
        ...
        H piecewise_combiner::add_buffer(H state, const unsigned char *data,
                                         size_t size) {
            if (position_ + size < piecewise_chunk_size()) {
            ...
            const size_t bytes_needed = piecewise_chunk_size() - position_;
            memcpy(buf_ + position_, data, bytes_needed);
            state = H::combine_contiguous(std::move(state), buf_, piecewise_chunk_size());
            data += bytes_needed;
            size -= bytes_needed;

            // Hash whatever chunks we can without copying
            while (size >= piecewise_chunk_size()) {
                state = H::combine_contiguous(std::move(state), data, piecewise_chunk_size());
                data += piecewise_chunk_size();
                size -= piecewise_chunk_size();
            }
