...
#ifndef ABEL_STRINGS_INTERNAL_COLOR_H_
#define ABEL_STRINGS_INTERNAL_COLOR_H_
...
struct color_type {
    ...
    constexpr color_type(color rgb_color) noexcept: is_rgb(true),
                                                    value{} {
    ...
    constexpr color_type(terminal_color term_color) noexcept: is_rgb(),
                                                              value{} {
...
class text_style {
  public:
    ...
    constexpr text_style &operator|=(const text_style &rhs) {
        if (!set_foreground_color) {
            ...
            if (!foreground_color.is_rgb || !rhs.foreground_color.is_rgb)
                ABEL_THROW(format_error("can't OR a terminal color"));
    ...
    constexpr text_style &operator&=(const text_style &rhs) {
        if (!set_foreground_color) {
            ...
            if (!foreground_color.is_rgb || !rhs.foreground_color.is_rgb)
                ABEL_THROW(format_error("can't AND a terminal color"));
            foreground_color.value.rgb_color &= rhs.foreground_color.value.rgb_color;
        }
    ...
    constexpr detail::color_type get_foreground() const noexcept {
        ABEL_ASSERT_MSG(has_foreground(), "no foreground specified for this style");
        return foreground_color;
    }

    constexpr detail::color_type get_background() const noexcept {
        ABEL_ASSERT_MSG(has_background(), "no background specified for this style");
        return background_color;
    }

    constexpr emphasis get_emphasis() const noexcept {
        ABEL_ASSERT_MSG(has_emphasis(), "no emphasis specified for this style");
        return ems;
    }
  ...
  private:
    ...
    friend constexpr text_style fg(detail::color_type foreground)
    noexcept;

    friend constexpr text_style bg(detail::color_type background)
    noexcept;
...
constexpr text_style operator|(emphasis lhs, emphasis rhs) noexcept {
...
template<typename Char>
inline void fputs(const Char *chars, FILE *stream) noexcept {
    std::fputs(chars, stream);
}

template<>
inline void fputs<wchar_t>(const wchar_t *chars, FILE *stream) noexcept {
    std::fputws(chars, stream);
}

template<typename Char>
inline void reset_color(FILE *stream) noexcept {
    fputs(detail::data::reset_color, stream);
}

template<>
inline void reset_color<wchar_t>(FILE *stream) noexcept {
    fputs(detail::data::wreset_color, stream);
}

template<typename Char>
inline void reset_color(basic_memory_buffer <Char> &buffer) noexcept {
...
//  Example:
//    abel::print(abel::emphasis::bold | fg(abel::color::red),
...
//    #include <fmt/color.h>
//
//    std::string message = abel::format(abel::emphasis::bold | fg(abel::color::red),
//                                      "The answer is {}", 42);
...
#endif  // ABEL_STRINGS_INTERNAL_COLOR_H_
