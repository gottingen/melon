...
#ifndef ABEL_STRINGS_STR_CAT_H_
#define ABEL_STRINGS_STR_CAT_H_
...
// within `alpha_num` string conversions.
    struct hex {
        ...
        explicit hex(
                Int v, pad_spec spec = abel::kNoPad,
        ...
        explicit hex(Pointee *v, pad_spec spec = abel::kNoPad)
                : hex(spec, reinterpret_cast<uintptr_t>(v)) {}

    private:
        hex(pad_spec spec, uint64_t v)
                : value(v),
                  width(spec == abel::kNoPad
                        ? 1
                        : spec >= abel::kSpacePad2 ? spec - abel::kSpacePad2 + 2
                                                   : spec - abel::kZeroPad2 + 2),
                  fill(spec >= abel::kSpacePad2 ? ' ' : '0') {}
    };
...
// integer conversion, so use it only if you need padding.
    struct dec {
        ...
        template<typename Int>
        explicit dec(Int v, pad_spec spec = abel::kNoPad,
...
// hexadecimal values (through the `hex` type) into strings.
    ...
    public:
        ...
        alpha_num(abel::string_view pc) : piece_(pc) {}  // NOLINT(runtime/explicit)
        ...
        abel::string_view::size_type size() const { return piece_.size(); }
...
// Do not call directly - this is not part of the public API.
    ...
    ABEL_MUST_USE_RESULT ABEL_FORCE_INLINE std::string string_cat() { return std::string(); }

    ABEL_MUST_USE_RESULT ABEL_FORCE_INLINE std::string string_cat(const alpha_num &a) {
        return std::string(a.data(), a.size());
    }

    ABEL_MUST_USE_RESULT std::string string_cat(const alpha_num &a, const alpha_num &b);
...
// Support 5 or more arguments
    ...
    ABEL_MUST_USE_RESULT ABEL_FORCE_INLINE std::string string_cat(
...
//   string_append(&s, s);
//
// This output is undefined as well, since `abel::string_view` does not own its
// data:
//
//   std::string s = "foobar";
//   abel::string_view p = s;
//   string_append(&s, p);

    ABEL_FORCE_INLINE void string_append(std::string *) {}
...
// Support 5 or more arguments
    template<typename... AV>
    ABEL_FORCE_INLINE void string_append(std::string *dest, const alpha_num &a, const alpha_num &b,
...
// This is fast.
    ABEL_FORCE_INLINE strings_internal::alpha_num_buffer<
