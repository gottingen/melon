...
// -----------------------------------------------------------------------------
//
// This file defines `abel::string_replace_all()`, a general-purpose string
...
// vary, `abel::Substitute()` may be a better choice.
//
// Example:
//
// std::string html_escaped = abel::string_replace_all(user_input, {
//                                                {"&", "&amp;"},
//                                                {"<", "&lt;"},
//                                                {">", "&gt;"},
//                                                {"\"", "&quot;"},
//                                                {"'", "&#39;"}});
#ifndef ABEL_STRINGS_STR_REPLACE_H_
#define ABEL_STRINGS_STR_REPLACE_H_
...
// Example:
//
//   std::string s = abel::string_replace_all(
//       "$who bought $count #Noun. Thanks $who!",
//       {{"$count", abel::string_cat(5)},
//        {"$who", "Bob"},
//        {"#Noun", "Apples"}});
//   EXPECT_EQ("Bob bought 5 Apples. Thanks Bob!", s);
    ABEL_MUST_USE_RESULT std::string string_replace_all(
            abel::string_view s,
            std::initializer_list<std::pair<abel::string_view, abel::string_view>>
            replacements);
...
// Examples:
//
//   std::map<const abel::string_view, const abel::string_view> replacements;
...
//   std::vector<std::pair<const abel::string_view, std::string>> replacements;
//   replacements.push_back({"&", "&amp;"});
//   replacements.push_back({"<", "&lt;"});
//   replacements.push_back({">", "&gt;"});
//   std::string s = abel::string_replace_all("if (ptr < &foo)",
//                                  replacements);
//   EXPECT_EQ("if (ptr &lt; &amp;foo)", s);
    template<typename StrToStrMapping>
    std::string string_replace_all(abel::string_view s,
...
//   int count = abel::string_replace_all({{"&", "&amp;"},
//                                    {"<", "&lt;"},
//                                    {">", "&gt;"}}, &s);
//  EXPECT_EQ(count, 2);
//  EXPECT_EQ("if (ptr &lt; &amp;foo)", s);
...
// Implementation details only, past this point.
    namespace strings_internal {
        ...
        struct viable_substitution {
            abel::string_view old;
            abel::string_view replacement;
            size_t offset;

            viable_substitution(abel::string_view old_str,
                                abel::string_view replacement_str, size_t offset_val)
...
// overhead of such a queue isn't worth it.
        ...
        std::vector<viable_substitution> find_substitutions(
                abel::string_view s, const StrToStrMapping &replacements) {
            std::vector<viable_substitution> subs;
            subs.reserve(replacements.size());

            for (const auto &rep : replacements) {
                using std::get;
                abel::string_view old(get<0>(rep));
        ...
        int apply_substitutions(abel::string_view s,
