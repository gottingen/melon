...
#ifndef ABEL_STRINGS_NUMBERS_H_
#define ABEL_STRINGS_NUMBERS_H_
...
ABEL_MUST_USE_RESULT bool simple_atoi(std::string_view str, int_type *out);
...
ABEL_MUST_USE_RESULT bool simple_atof(std::string_view str, float *out);
...
// always use the "C" locale. If any errors are encountered, this function
// returns `false`, leaving `out` in an unspecified state.
ABEL_MUST_USE_RESULT bool simple_atod(std::string_view str, double *out);
...
// unspecified state.
ABEL_MUST_USE_RESULT bool simple_atob(std::string_view str, bool *out);
...
//   // buf[0] == '4'
//   // buf[1] == '2'
ABEL_FORCE_INLINE void put_two_digits(size_t i, char *buf) {
    assert(i < 100);
...
bool safe_strtou128_base(std::string_view text, abel::uint128 *value,
                         int base);
...
template<typename int_type>
ABEL_MUST_USE_RESULT bool safe_strtoi_base(std::string_view s, int_type *out,
                                           int base) {
...
ABEL_FORCE_INLINE size_t fast_hex_to_buffer_zero_pad16(uint64_t val, char *out) {
#ifdef __SSE4_2__
    uint64_t be = big_endian::from_host64(val);
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        auto *hex = &abel::numbers_internal::kHexTable[byte * 2];
...
ABEL_MUST_USE_RESULT ABEL_FORCE_INLINE bool simple_atoi(std::string_view str,
                                                        abel::uint128 *out) {
