...
// elements to be joined, using the same formatting that `abel::string_cat()` uses.
...
// objects. The separator string is specified as an `abel::string_view`.
//
// Because the default formatter uses the `abel::alpha_num` class,
// `abel::string_join()`, like `abel::string_cat()`, will work out-of-the-box on
...
//
// See comments on the `abel::string_join()` function for more examples.

#ifndef ABEL_STRINGS_STR_JOIN_H_
#define ABEL_STRINGS_STR_JOIN_H_
...
// functions. You may provide your own Formatter to enable `abel::string_join()` to
...
// argument to `abel::string_join()`:
//
//   std::vector<int> v = {1, 2, 3, 4};
//   std::string s = abel::string_join(v, "-", MyFormatter());
//   EXPECT_EQ("1-2-3-4", s);
//
...
// `abel::string_join()` takes a range, a separator string to use between the
...
//   // of `abel::string_view` or even `const char*`.
//   std::vector<std::string> v = {"foo", "bar", "baz"};
//   std::string s = abel::string_join(v, "-");
//   EXPECT_EQ("foo-bar-baz", s);
//
...
//   // of ints or `abel::string_view` -- any `alpha_num`-compatible type.
//   std::string s = abel::string_join({"foo", "bar", "baz"}, "-");
//   EXPECT_EQ("foo-bar-baz", s);
//
// Example 3:
...
//       std::make_pair("b", 2),
//       std::make_pair("c", 3)};
//   std::string s = abel::string_join(m, ",", abel::pair_formatter("="));
//   EXPECT_EQ("a=1,b=2,c=3", s);
//
// Example 7:
//   // These examples show how `abel::string_join()` handles a few common edge
//   // cases:
//   std::vector<std::string> v_empty;
//   EXPECT_EQ("", abel::string_join(v_empty, "-"));
//
//   std::vector<std::string> v_one_item = {"foo"};
//   EXPECT_EQ("foo", abel::string_join(v_one_item, "-"));
//
//   std::vector<std::string> v_empty_string = {""};
//   EXPECT_EQ("", abel::string_join(v_empty_string, "-"));
//
//   std::vector<std::string> v_one_item_empty_string = {"a", ""};
//   EXPECT_EQ("a-", abel::string_join(v_one_item_empty_string, "-"));
//
//   std::vector<std::string> v_two_empty_string = {"", ""};
//   EXPECT_EQ("-", abel::string_join(v_two_empty_string, "-"));
//
// Example 8:
//   // Joins a `std::tuple<T...>` of heterogeneous types, converting each to
//   // a std::string using the `abel::alpha_num` class.
