...
#ifndef ABEL_UNICODE_H_
#define ABEL_UNICODE_H_
...
ABEL_FORCE_INLINE bool in_range(uint32_t c, uint32_t lo, uint32_t hi) {
    return (static_cast<uint32_t>(c - lo) < (hi - lo + 1));
}

ABEL_FORCE_INLINE bool is_surrogate(uint32_t c) {
    return in_range(c, 0xd800, 0xdfff);
}

ABEL_FORCE_INLINE bool is_high_surrogate(uint32_t c) {
    return (c & 0xfffffc00) == 0xd800;
}

ABEL_FORCE_INLINE bool is_low_surrogate(uint32_t c) {
    return (c & 0xfffffc00) == 0xdc00;
}

ABEL_FORCE_INLINE void append_utf8(std::string &str, uint32_t unicode) {
    if (unicode <= 0x7f) {
...
ABEL_FORCE_INLINE bool is_valid_utf8(const std::string &utf8String) {
...
ABEL_FORCE_INLINE int unicode_to_utf8_bytes(uint32_t ui) {
    if (ui <= 0x7f) {
        return 1;
    } else if (ui <= 0x7ff) {
        return 2;
