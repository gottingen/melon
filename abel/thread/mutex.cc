...
using abel::thread_internal::current_thread_identity_if_present;
...
using abel::thread_internal::per_thread_sem;
...
static abel::spin_lock deadlock_graph_mu(
...
static abel::spin_lock synch_event_mu(
        abel::base_internal::kLinkerInitialized);
...
// post on "w"'s associated per_thread_sem.
ABEL_FORCE_INLINE void mutex::increment_synch_sem(mutex *mu, per_thread_synch *w) {
    if (mu) {
        ABEL_TSAN_MUTEX_PRE_DIVERT(mu, 0);
    }
    per_thread_sem::post(w->thread_identity());
    if (mu) {
        ABEL_TSAN_MUTEX_POST_DIVERT(mu, 0);
    }
}

// wait on "w"'s associated per_thread_sem; returns false if timeout expired.
bool mutex::decrement_synch_sem(mutex *mu, per_thread_synch *w, kernel_timeout t) {
    ...
    static_cast<void>(w);
    bool res = per_thread_sem::wait(t);
    if (mu) {
        ABEL_TSAN_MUTEX_POST_DIVERT(mu, 0);
    }
    return res;
...
// prevent certain DCHECK_MSG() statements from being triggered when
// re-rentry is detected.  The DCHECK_MSG() statements are those in the
// mutex code checking that the "waitp" field has not been reused.
void mutex::internal_attempt_to_use_mutex_in_fatal_signal_handler() {
    // Fix the per-thread state only if it exists.
    thread_identity *identity = current_thread_identity_if_present();
    if (identity != nullptr) {
...
static per_thread_synch *DequeueAllWakeable(per_thread_synch *head,
    ...
    do {
        if (w->wake) {                    // remove this element
            DCHECK_MSG(pw->skip == nullptr, "bad skip in DequeueAllWakeable");
...
void mutex::block(per_thread_synch *s) {
    ...
    DCHECK_MSG(s->waitp != nullptr || s->suppress_fatal_errors,
...
static graph_id DeadlockCheck(mutex *mu) {
    ...
    abel::spin_lock_holder lock(&deadlock_graph_mu);
...
void mutex::await(const condition &cond) {
    if (cond.eval()) {    // condition already true; nothing to do
        ...
        DCHECK_MSG(this->await_common(cond, kernel_timeout::never()),
        ...
        if ((v & waitp->how->slow_need_zero) == 0) {
            ...
            if ((v & (kMuSpin | kMuWait)) == 0) {   // no waiters
                ...
                intptr_t nv = (v & zap_desig_waker[flags & kMuHasBlocked] & kMuLow) |
                              kMuWait;
                DCHECK_MSG(new_h != nullptr, "Enqueue to empty list failed");
                ...
                intptr_t wr_wait = 0;
                DCHECK_MSG(new_h != nullptr, "Enqueue to list failed");
                if (waitp->how == kExclusive && (v & kMuReader) != 0) {
...
void mutex::unlock_slow(synch_wait_params *waitp) {
    ...
    DCHECK_MSG(waitp == nullptr || waitp->thread->waitp == nullptr ||
    ...
    for (;;) {
        ...
        if ((v & kMuWriter) != 0 && (v & (kMuWait | kMuDesig)) != kMuWait &&
            ...
            DCHECK_MSG(old_h == nullptr || h->maybe_unlocking,
            ...
            if (old_h != nullptr &&
                ...
                DCHECK_MSG(old_h->skip == nullptr, "illegal skip from head");
            ...
            if (h->next->waitp->how == kExclusive &&
                ...
                DCHECK_MSG(h->skip == nullptr, "illegal skip from head");
            ...
            DCHECK_MSG(pw->next == w, "pw not w's predecessor");
            ...
            DCHECK_MSG(wake_list != kPerThreadSynchNull,
...
void mutex::fer(per_thread_synch *w) {
    int c = 0;
    DCHECK_MSG(w->waitp->cond == nullptr,
               "mutex::fer while waiting on condition");
    DCHECK_MSG(!w->waitp->timeout.has_timeout(),
               "mutex::fer while in timed wait");
    DCHECK_MSG(w->waitp->cv_word == nullptr,
...
static void CondVarEnqueue(synch_wait_params *waitp) {
    ...
    DCHECK_MSG(waitp->thread->waitp == nullptr, "waiting when shouldn't be");
...
bool cond_var::WaitCommon(mutex *mutex, kernel_timeout t) {
    ...
    DCHECK_MSG(waitp.thread->waitp != nullptr, "not waiting when should be");
