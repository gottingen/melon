...
#ifndef ABEL_THREAD_INTERNAL_OBJECT_ARRAY_H_
#define ABEL_THREAD_INTERNAL_OBJECT_ARRAY_H_
    ...
    namespace thread_internal {
        ...
        public:
            ...
            void initialize_at(std::size_t index, F &&f) {
                DCHECK_LT(index, initialized_.size());
                DCHECK(!initialized_[index]);
        ...
        public:
            static object_array_registry *instance() {
                static non_destroyed_singleton<object_array_registry> instance;
                return instance.get();
            }

            void registery(object_array<T> *array) {
                std::scoped_lock _(lock_);
                DCHECK(std::find(arrays_.begin(), arrays_.end(), array) ==
                            arrays_.end());
        ...
        class object_array_layout {
            using InitializerPtr = abel::function<void(void *)> *;
        ...
        public:
            ...
            template<class F>
            std::size_t create_entry(abel::function<void(void *)> *initializer, F &&cb) {
                std::scoped_lock _(lock_);

                // Let's see if we can reuse a slot.
                if (unused_.empty()) {
                    DCHECK_EQ(max_index_, initializers_.size());
        ...
        object_array<T> *get_newest_local_object_array() {
            ...
            object_array_layout<T>::instance()->with_wewest_layout_locked([&](auto &&layout) {
                ...
                DCHECK_LT(array.objects.size(), layout.size());
...
// method is in hot path, it's explicitly marked as `noinline`.
    ...
    [[gnu::noinline]] T* get_local_object_array_at(std::ptrdiff_t offset) {
            ...
            DCHECK_EQ(offset % sizeof(T), 0);
            DCHECK_GE(offset, 0);
            if (ABEL_LIKELY(offset < cache.limit)) {
...
#endif  // ABEL_THREAD_INTERNAL_OBJECT_ARRAY_H_
