...
static abel::spin_lock freelist_lock(
...
void reclaim_thread_identity(void *v) {
    ...
    per_thread_sem::destroy(identity);
    ...
    thread_internal::clear_current_thread_identity();
    {
        abel::spin_lock_holder l(&freelist_lock);
...
thread_internal::thread_identity *create_thread_identity() {
    ...
    per_thread_sem::init(identity);
    // Associate the value with the current thread, and attach our destructor.
    thread_internal::set_current_thread_identity(identity, reclaim_thread_identity);
    return identity;
}
