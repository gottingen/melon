 ...
 */
    ...
    namespace digest_detail {
        ...
        static ABEL_FORCE_INLINE uint32_t min(uint32_t x, uint32_t y) {
            return x < y ? x : y;
        }

        static ABEL_FORCE_INLINE void store64(uint64_t x, unsigned char *y) {
            for (int i = 0; i != 8; ++i)
                y[i] = (x >> ((7 - i) * 8)) & 255;
        }

        static ABEL_FORCE_INLINE uint64_t load64(const unsigned char *y) {
            uint64_t res = 0;
            for (int i = 0; i != 8; ++i)
        ...
        static ABEL_FORCE_INLINE uint64_t Sh(uint64_t x, uint64_t n) {
            return x >> n;
        }

        static ABEL_FORCE_INLINE uint64_t Sigma0(uint64_t x) {
            return ror(x, 28) ^ ror(x, 34) ^ ror(x, 39);
        }

        static ABEL_FORCE_INLINE uint64_t Sigma1(uint64_t x) {
            return ror(x, 14) ^ ror(x, 18) ^ ror(x, 41);
        }

        static ABEL_FORCE_INLINE uint64_t Gamma0(uint64_t x) {
            return ror(x, 1) ^ ror(x, 8) ^ Sh(x, 7);
        }

        static ABEL_FORCE_INLINE uint64_t Gamma1(uint64_t x) {
            return ror(x, 19) ^ ror(x, 61) ^ Sh(x, 6);
        }
