 ...
 */
    ...
    namespace digest_detail {
        ...
        static ABEL_FORCE_INLINE uint32_t load32l(const uint8_t *y) {
            uint32_t res = 0;
        ...
        static ABEL_FORCE_INLINE void store32l(uint32_t x, uint8_t *y) {
            for (size_t i = 0; i != 4; ++i)
                y[i] = (x >> (i * 8)) & 255;
        }

        static ABEL_FORCE_INLINE void store64l(uint64_t x, uint8_t *y) {
