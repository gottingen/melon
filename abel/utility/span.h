...
// array of data. The `span` object, much like the `std::string_view` object,
...
#ifndef ABEL_UTILITY_SPAN_H_
#define ABEL_UTILITY_SPAN_H_
...
// `std::vector<T>`, `abel::inlined_vector<T, N>`). All implicit conversions to
...
// accept a variety of array types (e.g. a `std::vector`, `abel::inlined_vector`,
...
//   MyRoutine(v);
//
//   abel::inlined_vector<int, 4> my_inline_vector;
...
class span {
  ...
  public:
    ...
    explicit span(V &v) noexcept  // NOLINT(runtime/references)
            : span(span_internal::get_data(v), v.size()) {}
...
bool operator==(span<T> a, span<T> b) {
    return span_internal::equal_impl<span, const T>(a, b);
}
...
-> decltype(abel::make_span(span_internal::get_data(c), c.size())) {
