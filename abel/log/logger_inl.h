...
// protected methods
    ...
    logger::log_it_impl(const abel::details::log_msg &log_msg, bool log_enabled, bool traceback_enabled) {
        if (log_enabled) {
            sink_it_impl(log_msg);
        }
    ...
    ABEL_FORCE_INLINE void logger::sink_it_impl(const details::log_msg &msg) {
    ...
    ABEL_FORCE_INLINE void logger::flush_impl() {
        for (auto &sink : sinks_) {
            ABEL_TRY {
                sink->flush();
            }
    ...
    ABEL_FORCE_INLINE void logger::dump_backtrace_() {
        ...
        if (tracer_.enabled()) {
            sink_it_impl(log_msg{name(), level::info, "****************** Backtrace Start ******************"});
            tracer_.foreach_pop([this](const log_msg &msg) { this->sink_it_impl(msg); });
            sink_it_impl(log_msg{name(), level::info, "****************** Backtrace End ********************"});
        }
    }
