...
class level_formatter final : public flag_formatter {
  public:
    ...
    void format(const details::log_msg &msg, const std::tm &, memory_buf_t &dest) override {
        std::string_view &level_name = level::to_string_view(msg.level);
...
// use by default full log_formatter for if pattern is not given
...
ABEL_FORCE_INLINE std::unique_ptr<log_formatter> pattern_formatter::clone() const {
