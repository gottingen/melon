...
#ifdef _WIN32
    auto color_sink = std::make_shared<sinks::wincolor_stderr_sink_mt>();
#else
    auto color_sink = std::make_shared<sinks::ansicolor_stderr_sink_mt>();
#endif
...
ABEL_FORCE_INLINE void registry::set_formatter(std::unique_ptr<log_formatter> formatter) {
    std::lock_guard<std::mutex> lock(logger_map_mutex_);
