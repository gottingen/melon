...
namespace melon {
    ...
    void *thread::thread_func(void *arg) {
        ...
        impl->name = melon::string_format("{}#{}", impl->prefix, impl->index);
        thread::set_name(impl->name);
        MELON_LOG(INFO) << "start thread: " << impl->name;
        ptr->set_affinity();
        impl->start_latch.count_down();
        impl->func();
        local_impl = nullptr;
        MELON_LOG(INFO) << "exit thread: " << impl->name;
        return nullptr;
    }
...
#else
        MELON_CHECK(!melon::core_affinity::supported) << "Attempting to use thread affinity on a unsupported platform";
#endif
         */
    }
    ...
    thread::~thread() {
        if (_impl) {
            MELON_LOG(WARNING) << "thread: " << _impl->name
    ...
    int32_t thread::thread_index() {
        if (MELON_UNLIKELY(local_thread_id == -1)) {
            if (melon::sysinfo::get_tid() != melon::sysinfo::get_main_thread_pid()) {
    ...
    thread::native_handler_type thread::native_handler() {
        ...
        // in main thread or create not by melon thread
        pthread_t mid = pthread_self();
        return mid;
    }
    // for main thread will index '0'
    MELON_BOOTSTRAP(0, [] { thread::thread_index(); });
