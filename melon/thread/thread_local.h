...
#ifndef MELON_THREAD_THREAD_LOCAL_H_
#define MELON_THREAD_THREAD_LOCAL_H_
...
#include "melon/thread/internal/always_initialized.h"
#include "melon/memory/atomic_ptr.h"
...
namespace melon {
    ...
    public:
        ...
        T *get() const {
            ...
            return MELON_LIKELY(!!ptr) ? ptr : get_slow();
        }
    ...
    private:
        ...
        mutable thread_internal::thread_local_always_initialized<melon::atomic_scoped_ptr<T>> raw_tls_;
        mutable std::mutex init_lock_;
        std::function<std::unique_ptr<T>()> creator_;
    };

}  // namespace melon
#endif  // MELON_THREAD_THREAD_LOCAL_H_
