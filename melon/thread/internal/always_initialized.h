...
#ifndef MELON_THREAD_INTERNAL_ALWAYS_INITIALIZED_H_
#define MELON_THREAD_INTERNAL_ALWAYS_INITIALIZED_H_


#include <utility>
#include <functional>
#include "melon/thread/internal/object_array.h"
...
namespace melon::thread_internal {
    ...
    template<class T>
    thread_local_always_initialized<T>::~thread_local_always_initialized() {
        MELON_CHECK_EQ(static_cast<size_t>(offset_) % sizeof(T), 0ul);
    ...
    void thread_local_always_initialized<T>::for_each(F &&f) const {
        MELON_DCHECK_EQ(static_cast<size_t>(offset_) % sizeof(T), 0ul);
        auto index = offset_ / sizeof(T);
...
#endif  // MELON_THREAD_INTERNAL_ALWAYS_INITIALIZED_H_
