...
#include "melon/base/md5.h"
#include "melon/strings/hex_dump.h"
...
namespace melon::base {
    ...
    namespace digest_detail {
        ...
        static MELON_FORCE_INLINE uint32_t min(uint32_t x, uint32_t y) {
            return x < y ? x : y;
        }

        static MELON_FORCE_INLINE uint32_t load32l(const uint8_t *y) {
            uint32_t res = 0;
        ...
        static MELON_FORCE_INLINE void store32l(uint32_t x, uint8_t *y) {
            for (size_t i = 0; i != 4; ++i)
                y[i] = (x >> (i * 8)) & 255;
        }

        static MELON_FORCE_INLINE void store64l(uint64_t x, uint8_t *y) {
        ...
        static MELON_FORCE_INLINE void FF(uint32_t &a, uint32_t &b, uint32_t &c, uint32_t &d,
                                         uint32_t M, uint32_t s, uint32_t t) {
            a = (a + F(b, c, d) + M + t);
            a = rotl(a, s) + b;
        }

        static MELON_FORCE_INLINE void GG(uint32_t &a, uint32_t &b, uint32_t &c, uint32_t &d,
                                         uint32_t M, uint32_t s, uint32_t t) {
            a = (a + G(b, c, d) + M + t);
            a = rotl(a, s) + b;
        }

        static MELON_FORCE_INLINE void HH(uint32_t &a, uint32_t &b, uint32_t &c, uint32_t &d,
                                         uint32_t M, uint32_t s, uint32_t t) {
            a = (a + H(b, c, d) + M + t);
            a = rotl(a, s) + b;
        }

        static MELON_FORCE_INLINE void II(uint32_t &a, uint32_t &b, uint32_t &c, uint32_t &d,
                                         uint32_t M, uint32_t s, uint32_t t) {
            a = (a + I(b, c, d) + M + t);
            a = rotl(a, s) + b;
        }
    ...
    std::string MD5::digest_hex() {
        ...
        finalize(digest);
        return melon::hex_dump_lc(digest, kDigestLength);
    }

    std::string MD5::digest_hex_uc() {
        uint8_t digest[kDigestLength];
        finalize(digest);
        return melon::hex_dump(digest, kDigestLength);
    }
