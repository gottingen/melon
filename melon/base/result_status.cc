...
namespace melon {
    result_status result_status::from_melon_error(int err) {
        const char * msg = melon_error(err);
        return result_status(err, msg);
    }

    result_status result_status::from_melon_error(int err, const std::string_view &ext) {
        const char * msg = melon_error(err);
