...
namespace melon::base {
    ...
    int ip2hostname(ip_t ip, char *host, size_t host_len) {
        ...
        if (melon::ends_with(host, ".baidu.com")) {
            host[str.size() - 10] = '\0';
        }
        return 0;
    }
        ...
        if (self_port != nullptr) {
            ...
            if (get_local_side(sockfd, &pt) == 0) {
                *self_port = pt.port;
                ...
                MELON_CHECK(false) << "Fail to get the local port of sockfd=" << sockfd;
            }
        }
    ...
    int tcp_listen(end_point point) {
...
#else
            MELON_LOG(ERROR) << "Missing def of SO_REUSEADDR while -reuse_addr is on";
            return -1;
#endif
        }
        ...
        if (FLAGS_reuse_port) {
            ...
            if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEPORT,
                           &on, sizeof(on)) != 0) {
                MELON_LOG(WARNING) << "Fail to setsockopt SO_REUSEPORT of sockfd=" << sockfd;
            }
#else
            MELON_LOG(ERROR) << "Missing def of SO_REUSEPORT while -reuse_port is on";
            return -1;
#endif
        }
