...
#include "melon/log/config.h"        // to get the program name
#include "melon/log/logging.h"
#include "melon/log/raw_logging.h"
#include "melon/log/init.h"
#include "melon/log/utility.h"
#include "melon/thread/thread.h"
#include "melon/system/sysinfo.h"
...
# include "melon/debugging/stacktrace.h"
...
namespace melon::log {
    ...
    // Safely get melon_max_log_size, overriding to 1 if it somehow gets defined as 0
    static int32_t MaxLogSize() {
        return (FLAGS_melon_max_log_size > 0 && FLAGS_melon_max_log_size < 4096 ? FLAGS_melon_max_log_size : 1);
    }
    ...
    struct log_message::log_message_data {
        ...
        const char *basename_;        // basename of file that called MELON_LOG
        const char *fullname_;        // fullname of file that called MELON_LOG
    ...
    namespace {
        ...
        private:
            ...
            melon::time_point next_flush_time_;         // cycle count at which to flush log
            melon::time_point start_time_;
...
/* static */
    const string &log_destination::hostname() {
        if (hostname_.empty()) {
            hostname_ = melon::sysinfo::get_host_name();
            if (hostname_.empty()) {
                hostname_ = "(unknown)";
            }
        }
    ...
    inline void log_destination::set_log_symlink(log_severity severity,
                                                 const char *symlink_basename) {
        MELON_CHECK_GE(severity, 0);
        MELON_CHECK_LT(severity, NUM_SEVERITIES);
        std::unique_lock<std::mutex> l(log_mutex);
    ...
    inline void log_destination::set_stderr_logging(log_severity min_severity) {
        ...
        // all this stuff.
        std::unique_lock<std::mutex> l(log_mutex);
        FLAGS_melon_stderrthreshold = min_severity;
    }
    ...
    static void colored_write_to_stderr(log_severity severity,
        ...
        const log_color color =
                (log_destination::terminal_supports_color() && FLAGS_melon_colorlogtostderr) ?
    ...
    inline void log_destination::maybe_log_to_stderr(log_severity severity,
        ...
        if ((severity >= FLAGS_melon_stderrthreshold) || FLAGS_melon_also_logtostderr) {
    ...
    inline void log_destination::maybe_log_to_email(log_severity severity,
        ...
        if (severity >= email_logging_severity_ ||
            severity >= FLAGS_melon_log_email_level) {
            std::string to(FLAGS_melon_also_log_to_email);
            if (!addresses_.empty()) {
                if (!to.empty()) {
                    to += ",";
                }
            ...
            const string subject(string("[MELON_LOG] ") + log_severity_names[severity] + ": " +
            ...
            // log_mutex and SendEmail() calls MELON_LOG/MELON_VLOG which will block trying to
    ...
    inline void log_destination::maybe_log_to_logfile(log_severity severity,
                                                      ...
                                                      const char *message,
                                                      size_t len) {
        const bool should_flush = severity > FLAGS_melon_logbuflevel;
    ...
    inline void log_destination::log_to_all_logfiles(log_severity severity,
        ...
        if (FLAGS_melon_logtostderr) {           // global flag: never log to file
    ...
    namespace {
        ...
        string pretty_duration(const melon::duration &d) {
        ...
        void log_file_object::flush_unlocked() {
            ...
            const melon::duration next = melon::duration::seconds(FLAGS_melon_logbufsecs);  // in usec
            next_flush_time_ = melon::time_now() + next;
        }
        ...
        bool log_file_object::create_logfile(const string &time_pid_string) {
            ...
            int fd = open(filename, flags, FLAGS_melon_logfile_mode);
      ...
      // same file. This applies particularly when !FLAGS_melon_timestamp_in_logfile_name
      ...
      if (wlock_ret == -1) {
          ...
          return false;
            ...
            if (!symlink_basename_.empty()) {
                ...
                // FLAGS_melon_log_link, if indicated
                if (!FLAGS_melon_log_link.empty()) {
                    linkpath = FLAGS_melon_log_link + "/" + linkname;
                    unlink(linkpath.c_str());                  // delete old one if it exists
        ...
        void log_file_object::write(bool force_flush,
            ...
            if (file_ == nullptr) {
                ...
                time_pid_stream << 1900 + tm_time.tm_year
                                ...
                                << '.'
                                << melon::sysinfo::get_main_thread_pid();
                ...
                if (base_filename_selected_) {
                    ...
                    string stripped_filename(
                            log_internal::program_invocation_short_name());
                    string hostname = melon::sysinfo::get_host_name();

                    string uidname = melon::sysinfo::user_name();
                    // We should not call MELON_CHECK() here because this function can be
                ...
                file_header_stream << "Log file created at: "
                                   ...
                                   << setw(2) << tm_time.tm_hour << ':'
                                   << setw(2) << tm_time.tm_min << ':'
                                   << setw(2) << tm_time.tm_sec << (FLAGS_melon_log_utc_time ? " UTC\n" : "\n")
                                   << "Running on machine: "
                ...
                file_header_stream << "Running duration (h:mm:ss): "
                                   << pretty_duration(melon::time_now() - start_time_)
                                   << '\n'
            ...
            if (!stop_writing) {
                ...
                if (FLAGS_melon_stop_logging_if_full_disk &&
                ...
                if (melon::time_now() >= next_flush_time_)
                    stop_writing = false;  // check to see if disk has free space.
                return;  // no need to flush
            }

            // See important msgs *now*.  Also, flush logs at least every 10^6 chars,
            // or every "FLAGS_melon_logbufsecs" seconds.
            if (force_flush ||
                (bytes_since_flush_ >= 1000000) ||
                (melon::time_now() >= next_flush_time_)) {
                flush_unlocked();
#ifdef MELON_PLATFORM_LINUX
...
// Static log data space to avoid alloc failures in a MELON_LOG(FATAL)
//
// Since multiple threads may call MELON_LOG(FATAL), and we want to preserve
...
// all other threads.
    ...
    void log_message::init(const char *file,
        ...
        if (FLAGS_melon_log_prefix && (line != kNoLogPrefix)) {
            stream() << log_severity_names[severity][0]
                     ...
                     << ' '
                     << setfill(' ') << setw(5)
                     << static_cast<unsigned int>(melon::thread::thread_index()) << setfill('0')
                     << ' '
    ...
    void log_message::flush() {
        if (data_->has_been_flushed_ || data_->severity_ < FLAGS_melon_minloglevel)
            return;
    ...
    void reprint_fatal_message() {
        if (fatal_message[0]) {
            ...
            log_destination::log_to_all_logfiles(MELON_ERROR, fatal_time, fatal_message, n);
        }
    }
...
// L >= log_mutex (callers must hold the log_mutex).
    void log_message::send_to_log() EXCLUSIVE_LOCKS_REQUIRED(log_mutex) {
        ...
        // program name.
        if (FLAGS_melon_logtostderr || !is_logging_initialized()) {
        ...
        if (data_->severity_ == MELON_FATAL && FLAGS_melon_crash_on_fatal_log) {
    ...
    void log_message::record_crash_reason(
        ...
        reason->depth = melon::debugging::get_stack_trace(reason->stack, MELON_ARRAY_SIZE(reason->stack), 4);
#else
        reason->depth = 0;
#endif
    }
...
# define ATTRIBUTE_NORETURN
    ...
    static void logging_fail() {
        if (FLAGS_melon_crash_on_fatal_log) {
            abort();
        }
    }
    ...
    MELON_EXPORT logging_fail_func_t g_logging_fail_func = &logging_fail;
...
// log_stream.
    ...
    string log_sink::ToString(log_severity severity, const char *file, int line,
        ...
        stream << log_severity_names[severity][0]
               ...
               << setw(6) << usecs
               << ' '
               << setfill(' ') << setw(5) << melon::sysinfo::get_tid() << setfill('0')
               << ' '
               << file << ':' << line << "] ";
    ...
    // use_logging controls whether the logging functions MELON_LOG/MELON_VLOG are used
    ...
    static bool SendEmailInternal(const char *dest, const char *subject,
        ...
        if (dest && *dest) {
            if (use_logging) {
                MELON_VLOG(1) << "Trying to send TITLE:" << subject
                        << " BODY:" << body << " to " << dest;
            } else {
            ...
            std::string melon_log_mailer = FLAGS_melon_log_mailer;
            if (melon_log_mailer.empty()) {
                melon_log_mailer = "/bin/mail";
            }
            std::string cmd =
                    melon_log_mailer + " -s" +
            ...
            if (pipe != nullptr) {
                ...
                if (!ok) {
                    if (use_logging) {
                        MELON_LOG(ERROR) << "Problems sending mail to " << dest << ": "
                                   << StrError(errno);
                    } else {
                ...
                if (use_logging) {
                    MELON_LOG(ERROR) << "Unable to send mail to " << dest;
                } else {
    ...
    static void GetTempDirectories(vector<string> *list) {
        ...
        for (size_t i = 0; i < MELON_ARRAY_SIZE(candidates); i++) {
    ...
    const vector<string> &GetLoggingDirectories() {
        ...
        if (logging_directories_list == nullptr) {
            ...
            if (!FLAGS_melon_log_dir.empty()) {
                ...
                logging_directories_list->push_back(FLAGS_melon_log_dir.c_str());
            } else {
    ...
    void truncate_log_file(const char *path, int64_t limit, int64_t keep) {
        ...
        if (fd == -1) {
            if (errno == EFBIG) {
                ...
                if (truncate(path, 0) == -1) {
                    MELON_PLOG(ERROR) << "Unable to truncate " << path;
                } else {
                    MELON_LOG(ERROR) << "Truncated " << path << " due to EFBIG error";
                }
            } else {
                MELON_PLOG(ERROR) << "Unable to open " << path;
            }
            return;
        }

        if (fstat(fd, &statbuf) == -1) {
            MELON_PLOG(ERROR) << "Unable to fstat()";
            goto out_close_fd;
        }
        ...
        MELON_LOG(INFO) << "Truncating " << path << " to " << keep << " bytes";
        ...
        while ((bytesin = ::pread(fd, copybuf, sizeof(copybuf), read_offset)) > 0) {
            ...
            if (bytesout == -1) {
                MELON_PLOG(ERROR) << "Unable to write to " << path;
                break;
            } else if (bytesout != bytesin) {
                MELON_LOG(ERROR) << "Expected to write " << bytesin << ", wrote " << bytesout;
            }
            read_offset += bytesin;
            write_offset += bytesout;
        }
        if (bytesin == -1) MELON_PLOG(ERROR) << "Unable to read from " << path;
...
#undef DEFINE_CHECK_STROP_IMPL
    ...
    int posix_strerror_r(int err, char *buf, size_t len) {
        ...
        if (!rc) {
            ...
            if (rc == buf) {
                ...
                buf[0] = '\000';
#if defined(MELON_PLATFORM_OSX) || defined(MELON_PLATFORM_FREEBSD) || defined(MELON_PLATFORM_OPENBSD)
    ...
    log_message_fatal::log_message_fatal(const char *file, int line) :
            log_message(file, line, MELON_FATAL) {}
...
namespace melon::internal::logging {
/*
    namespace {
