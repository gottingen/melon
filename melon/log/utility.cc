...
MELON_LOG_DEFINE_bool(symbolize_stacktrace, true,
...
namespace melon::log {
    ...
    static void DumpPCAndSymbol(DebugWriter *writerfn, void *arg, void *pc,
        ...
        // a call to a function annotated noreturn (e.g. MELON_CHECK).
        if (melon::debugging::symbolize(reinterpret_cast<char *>(pc) - 1, tmp, sizeof(tmp))) {
            symbol = tmp;
        }
        char buf[1024];
...
// Dump current stack trace as directed by writerfn
    static void DumpStackTrace(int skip_count, DebugWriter *writerfn, void *arg) {
        // Print stack trace
        void *stack[32];
        int depth = melon::debugging::get_stack_trace(stack, MELON_ARRAY_SIZE(stack), skip_count + 1);
