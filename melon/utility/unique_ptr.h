...
template <class T1, class T2, bool = mutil::is_empty<T2>::value>
class UniquePtrStorage {
    T1 t1_;
    T2 t2_;

    typedef typename mutil::add_reference<T2>::type T2_reference;
    typedef typename mutil::add_reference<const T2>::type T2_const_reference;
...
struct default_delete {
    ...
    template <class U>
        default_delete(const default_delete<U>&,
            typename mutil::enable_if<up_detail::is_convertible<U*, T*>::value>::type* = 0) {}

    void operator()(T* ptr) const {
        MELON_CASSERT(sizeof(T) > 0, T_is_empty);
        delete ptr;
    }
};
...
struct pointer_type {
    typedef typename pointer_type_imp::pointer_type<T,
        typename mutil::remove_reference<D>::type>::type type;
};
...
private:
    ...
    typedef typename mutil::add_reference<deleter_type>::type deleter_reference;
    typedef typename mutil::add_reference<const deleter_type>::type deleter_const_reference;
...
public:
    ...
    unique_ptr(pointer p, typename mutil::conditional<mutil::is_reference<D>::value,
               volatile typename mutil::remove_reference<D>::type&, D>::type d)
        : ptr_(move(p), forward<D>(const_cast<typename mutil::add_reference<D>::type>(d))) {}

    template <class U, class E>
    unique_ptr(unique_ptr<U, E> u,
               typename mutil::enable_if <
               !mutil::is_array<U>::value &&
               up_detail::is_convertible<typename unique_ptr<U>::pointer, pointer>::value
&&
               up_detail::is_convertible<E, deleter_type>::value &&
               (
                   !mutil::is_reference<deleter_type>::value ||
                   mutil::is_same<deleter_type, E>::value)
               >::type* = 0)
        : ptr_(u.release(), forward<D>(forward<E>(u.get_deleter()))) {}

    ~unique_ptr() {
        MELON_CASSERT(!mutil::is_reference<deleter_type>::value, deleter_is_ref);
        MELON_CASSERT(!mutil::is_pointer<deleter_type>::value, deleter_is_pointer);
        reset();
    }
    ...
    typename mutil::add_reference<T>::type operator*() const {return *get();}
...
private:
    template <class U>
    explicit unique_ptr(
        U, typename mutil::enable_if<up_detail::is_convertible<U, pointer>::value>::type* = 0);

    template <class U>
    unique_ptr(U, typename mutil::conditional<mutil::is_reference<D>::value,
               volatile typename mutil::remove_reference<D>::type&, D>::type,
               typename mutil::enable_if<up_detail::is_convertible<U, pointer>::value>::type* = 0);
};
