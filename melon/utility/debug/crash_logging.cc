...
typedef std::map<mutil::StringPiece, CrashKey> CrashKeyMap;
...
void SetCrashKeyValue(const mutil::StringPiece& key,
  ...
  for (size_t i = chunks.size();
       ...
       ++i) {
    g_clear_key_func_(mutil::StringPrintf(kChunkFormatString, key.data(), i+1));
  }

  // Set the chunked keys.
  for (size_t i = 0; i < chunks.size(); ++i) {
    g_set_key_func_(mutil::StringPrintf(kChunkFormatString, key.data(), i+1),
                    chunks[i]);
  }
}

void ClearCrashKey(const mutil::StringPiece& key) {
...
void SetCrashKeyToStackTrace(const mutil::StringPiece& key,
...
void SetCrashKeyFromAddresses(const mutil::StringPiece& key,
  ...
  if (addresses && count) {
    ...
    for (size_t i = 0; i < count; ++i) {
      std::string s = mutil::StringPrintf("%p", addresses[i]);
      length += s.length() + 1;
...
ScopedCrashKey::ScopedCrashKey(const mutil::StringPiece& key,
                               const mutil::StringPiece& value)
    : key_(key.as_string()) {
  SetCrashKeyValue(key, value);
}
...
const CrashKey* LookupCrashKey(const mutil::StringPiece& key) {
  if (!g_crash_keys_)
    return NULL;
