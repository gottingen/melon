...
int MELON_WEAK fiber_key_create(fiber_key_t* key,
                                  void (*destructor)(void* data));
int MELON_WEAK fiber_setspecific(fiber_key_t key, void* data);
void* MELON_WEAK fiber_getspecific(fiber_key_t key);
}
...
// static
mutil::Mutex* LoggingLock::log_lock = NULL;
// static
...
struct MELON_CACHELINE_ALIGNMENT LogRequest {
...
class AsyncLogger : public mutil::SimpleThread {
public:
...
friend struct DefaultSingletonTraits<AsyncLogger>;
    ...
    mutil::atomic<LogRequest*> _log_head;
    mutil::Mutex _mutex;
    mutil::ConditionVariable _cond;
    LogRequest* _current_log_request;
    mutil::atomic<int32_t> _log_request_count;
...
AsyncLogger::AsyncLogger()
    : mutil::SimpleThread("async_log_thread")
    , _log_head(NULL)
    , _cond(&_mutex)
...
void AsyncLogger::Log(const std::string& log) {
    ...
    bool is_full = FLAGS_max_async_log_queue_size > 0 &&
        _log_request_count.fetch_add(1, mutil::memory_order_relaxed) >
        FLAGS_max_async_log_queue_size;
    if (is_full || _stop.load(mutil::memory_order_relaxed)) {
        // Async logger is full or stopped, fallback to sync log.
        DoLog(log);
        return;
    }

    auto log_req = mutil::get_object<LogRequest>();
    if (!log_req) {
...
void AsyncLogger::LogImpl(LogRequest* log_req) {
    ...
    LogRequest* const prev_head =
        _log_head.exchange(log_req, mutil::memory_order_release);
    if (prev_head != NULL) {
    ...
    if (!FLAGS_async_log_in_background_always) {
        ...
        if (IsLogComplete(log_req)) {
            mutil::return_object(log_req);
            return;
        }
    }

    MELON_SCOPED_LOCK(_mutex);
    if (_stop.load(mutil::memory_order_relaxed)) {
...
void AsyncLogger::StopAndJoin() {
    if (!_stop.exchange(true, mutil::memory_order_relaxed)) {
        MELON_SCOPED_LOCK(_mutex);
        _cond.Signal();
    }
    if (!HasBeenJoined()) {
...
void PrintLogPrefix(std::ostream& os, int severity,
    ...
    os << ' ' << std::setfill(' ') << std::setw(5)
       << mutil::PlatformThread::CurrentId() << std::setfill('0');
...
static void PrintLogPrefixAsJSON(std::ostream& os, int severity,
    ...
    os << "\"tid\":\"" << mutil::PlatformThread::CurrentId() << "\",";
    if (FLAGS_log_hostname) {
        mutil::StringPiece hostname(mutil::my_hostname());
        if (hostname.ends_with(".baidu.com")) { // make it shorter
            hostname.remove_suffix(10);
...
void EscapeJson(std::ostream& os, const mutil::StringPiece& s) {
...
inline void OutputLog(std::ostream& os, const mutil::StringPiece& s) {
    if (FLAGS_escape_log) {
        EscapeJson(os, s);
    } else {
...
class DoublyBufferedLogSink : public mutil::DoublyBufferedData<LogSink*> {
public:
...
void DisplayDebugMessageInDialog(const std::string& str) {
    ...
    std::wstring cmdline = mutil::UTF8ToWide(str);
    if (cmdline.empty())
        return;
...
bool StringSink::OnLogMessage(int severity, const char* file,
    ...
    {
        mutil::AutoLock lock_guard(_lock);
        append(msg);
    }
    return true;
}
...
public:
    ...
    bool OnLogMessage(int severity, const char* file, int line,
                      const mutil::StringPiece& content) override {
        return OnLogMessage(severity, file, line, "", content);
    }
    ...
    if (FLAGS_print_stack_on_check && _is_check && _severity == BLOG_FATAL) {
        ...
        *this << std::endl;  // Newline to separate from log message.
        if (count > 3) {
            ...
            // #2 may be ~LogStream
            //   #0 0x00000059ccae mutil::debug::StackTrace::StackTrace()
            //   #1 0x0000005947c7 logging::LogStream::FlushWithoutReset()
            //   #2 0x000000594b88 logging::LogMessage::~LogMessage()
            mutil::debug::StackTrace trace_stripped(addrs + 3, count - 3);
...
FINISH_LOGGING:
    if (FLAGS_crash_on_fatal_log && _severity == BLOG_FATAL) {
        ...
        mutil::StringPiece str = content();
        char str_stack[1024];
        str.copy(str_stack, arraysize(str_stack));
        mutil::debug::Alias(str_stack);
...
MUTIL_EXPORT std::string SystemErrorCodeToString(SystemErrorCode error_code) {
    ...
    if (len) {
        ...
        return mutil::CollapseWhitespaceASCII(msgbuf, true) +
            mutil::StringPrintf(" (0x%X)", error_code);
    }
    return mutil::StringPrintf("Error (0x%X) while retrieving error. (0x%X)",
...
struct VLogSite {
    VLogSite(const char* filename, int required_v, int line_no)
        ...
        if (pos != mutil::StringPiece::npos) {
            if (s[pos] == '.') {
                ...
                if (pos2 != mutil::StringPiece::npos) {
    ...
    VLogSite* next() { return (VLogSite*)mutil::subtle::Acquire_Load(&_next); }
    const VLogSite* next() const
    { return (VLogSite*)mutil::subtle::Acquire_Load(&_next); }
    void set_next(VLogSite* next)
    { mutil::subtle::Release_Store(&_next, (mutil::subtle::AtomicWord)next); }
...
struct VModuleList {
    ...
    int init(const char* vmodules) {
        ...
        for (mutil::StringSplitter sp(vmodules, ','); sp; ++sp) {
    ...
    if (old_module_list) {
        ...
        deleting_vmodule_list->push_back(
            std::make_pair(old_module_list,
                           mutil::gettimeofday_us() + DELAY_DELETION_SEC * 1000000L));
        while (!deleting_vmodule_list->empty() &&
               deleting_vmodule_list->front().second <= mutil::gettimeofday_us()) {
...
std::ostream& operator<<(std::ostream& out, const wchar_t* wstr) {
    return out << mutil::WideToUTF8(std::wstring(wstr));
}
