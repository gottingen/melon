...
using mutil::internal::PlatformThreadLocalStorage;
...
mutil::subtle::AtomicWord g_native_tls_key =
...
mutil::subtle::Atomic32 g_last_used_tls_key = 0;
...
volatile mutil::ThreadLocalStorage::TLSDestructorFunc
...
void** ConstructTlsVector() {
  ...
  if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES) {
    ...
    if (PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES !=
        mutil::subtle::NoBarrier_CompareAndSwap(&g_native_tls_key,
...
void OnThreadExitInternal(void* value) {
  ...
  while (need_to_scan_destructors) {
    ...
    mutil::subtle::Atomic32 last_used_tls_key =
        mutil::subtle::NoBarrier_Load(&g_last_used_tls_key);
    for (int slot = last_used_tls_key; slot > 0; --slot) {
      void* value = stack_allocated_tls_data[slot];
      if (value == NULL)
        continue;

      mutil::ThreadLocalStorage::TLSDestructorFunc destructor =
    ...
    if (--remaining_attempts <= 0) {
      DCHECK(false);  // Destructors might not have been called.
      break;
    }
  }
