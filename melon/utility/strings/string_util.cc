...
TrimPositions TrimWhitespace(const string16& input,
                             TrimPositions positions,
                             string16* output) {
  return TrimStringT(input, mutil::string16(kWhitespaceUTF16), positions,
                     output);
}

TrimPositions TrimWhitespace(const mutil::StringPiece16& input,
                             TrimPositions positions,
                             mutil::StringPiece16* output) {
  return TrimStringT(input, mutil::StringPiece16(kWhitespaceUTF16), positions,
                     output);
}
...
TrimPositions TrimWhitespaceASCII(const mutil::StringPiece& input,
                                  TrimPositions positions,
                                  mutil::StringPiece* output) {
  return TrimStringT(input, mutil::StringPiece(kWhitespaceASCII), positions, output);
}
...
TrimPositions TrimWhitespace(const mutil::StringPiece& input,
                             TrimPositions positions,
                             mutil::StringPiece* output) {
  return TrimWhitespaceASCII(input, positions, output);
}
...
bool EqualsASCII(const string16& a, const mutil::StringPiece& b) {
...
bool StartsWithASCII(const std::string& str,
  ...
  if (case_sensitive)
    ...
    return mutil::strncasecmp(str.c_str(), search.c_str(), search.length()) == 0;
}
...
bool StartsWithT(const STR& str, const STR& search, bool case_sensitive) {
  if (case_sensitive) {
    ...
    return std::equal(search.begin(), search.end(), str.begin(),
                      mutil::CaseInsensitiveCompare<typename STR::value_type>());
  }
}
...
string16 FormatBytesUnlocalized(int64_t bytes) {
  ...
  if (bytes != 0 && dimension > 0 && unit_amount < 100) {
    mutil::snprintf(buf, arraysize(buf), "%.1lf%s", unit_amount,
                   kByteStringsUnlocalized[dimension]);
  } else {
    mutil::snprintf(buf, arraysize(buf), "%.0lf%s", unit_amount,
                   kByteStringsUnlocalized[dimension]);
  }

  return mutil::ASCIIToUTF16(buf);
}
...
size_t Tokenize(const mutil::StringPiece& str,
...
std::string ReplaceStringPlaceholders(const mutil::StringPiece& format_string,
...
bool MatchPattern(const mutil::StringPiece& eval,
...
size_t mutil::strlcpy(char* dst, const char* src, size_t dst_size) {
  return lcpyT<char>(dst, src, dst_size);
}
size_t mutil::wcslcpy(wchar_t* dst, const wchar_t* src, size_t dst_size) {
