...
#ifndef MUTIL_MACROS_H_
#define MUTIL_MACROS_H_
...
#define MUTIL_DELETE_FUNCTION(decl) decl
#else
#define MUTIL_DELETE_FUNCTION(decl) decl = delete
#endif

// Put this in the private: declarations for a class to be uncopyable.
#define DISALLOW_COPY(TypeName)                         \
    MUTIL_DELETE_FUNCTION(TypeName(const TypeName&))

// Put this in the private: declarations for a class to be unassignable.
#define DISALLOW_ASSIGN(TypeName)                               \
    MUTIL_DELETE_FUNCTION(void operator=(const TypeName&))
...
#define DISALLOW_IMPLICIT_CONSTRUCTORS(TypeName) \
    MUTIL_DELETE_FUNCTION(TypeName());            \
...
#define arraysize(array) (sizeof(::mutil::ArraySizeHelper(array)))
...
// the mutil::LINKER_INITIALIZED argument.  Normally, it is unsafe to declare a
// static variable that has a constructor or a destructor because invocation
// order is undefined.  However, IF the type can be initialized by filling with
// zeroes (which the loader does for static variables), AND the destructor also
// does nothing to the storage, AND there are no virtual methods, then a
// constructor declared as
//       explicit MyClass(mutil::LinkerInitialized x) {}
// and invoked as
//       static MyClass my_variable_name(mutil::LINKER_INITIALIZED);
namespace mutil {
...
#ifndef MELON_TYPEOF
# if defined(MUTIL_CXX11_ENABLED)
#  define MELON_TYPEOF decltype
# else
#  ifdef _MSC_VER
#   include <boost/typeof/typeof.hpp>
#   define MELON_TYPEOF BOOST_TYPEOF
#  else
#   define MELON_TYPEOF typeof
#  endif
# endif // MUTIL_CXX11_ENABLED
#endif  // MELON_TYPEOF
...
# define container_of(ptr, type, member) ({                             \
            const MELON_TYPEOF( ((type *)0)->member ) *__mptr = (ptr);  \
...
# define DEFINE_SMALL_ARRAY(Tp, name, size, maxsize)                    \
    ...
    ::mutil::internal::ArrayDeleter<Tp> name##_array_deleter;            \
...
# define DEFINE_SMALL_ARRAY(Tp, name, size, maxsize)                    \
    ...
    ::mutil::internal::ArrayDeleter<char> name##_array_deleter;          \
    if (name##_stack_array_size) {                                      \
    ...
    const ::mutil::internal::ArrayCtorDtor<Tp> name##_array_ctor_dtor(name, name##_size);
...
//   * Use ASSERT_*.
//   * Have multiple MELON_GLOBAL_INIT() in one scope.
// 
// Since the code run in global scope, quit with exit() or similar functions.

#if defined(__cplusplus)
# define MELON_GLOBAL_INIT                                      \
namespace {  /*anonymous namespace */                           \
    struct MELON_CONCAT(MelonGlobalInit, __LINE__) {            \
        MELON_CONCAT(MelonGlobalInit, __LINE__)() { init(); }   \
        void init();                                            \
    } MELON_CONCAT(melon_global_init_dummy_, __LINE__);         \
}  /* anonymous namespace */                                    \
    void MELON_CONCAT(MelonGlobalInit, __LINE__)::init
#else
# define MELON_GLOBAL_INIT                      \
    static void __attribute__((constructor))    \
    MELON_CONCAT(baidu_global_init_, __LINE__)
