...
public:
    struct NamedMeta {
        std::string name;
        std::shared_ptr<mutil::IOBuf> data;
    };
    ...
    const mutil::IOBuf* Meta(const char* name) const;
    ...
    // are a lot.
    mutil::IOBuf* MutableMeta(const char* name, bool null_on_found = false);
    mutil::IOBuf* MutableMeta(const std::string& name, bool null_on_found = false);

    // Remove meta with the name. The impl. may scan all fields.
    // Returns true on erased, false on absent.
    bool RemoveMeta(const mutil::StringPiece& name);

    // Get the payload. Empty by default.
    const mutil::IOBuf& Payload() const { return _payload; }

    // Get a mutable pointer to the payload.
    mutil::IOBuf* MutablePayload() { return &_payload; }
