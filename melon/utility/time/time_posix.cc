...
#include "melon/utility/os_compat_nacl.h"
#endif
...
mutil::TimeTicks ClockNow(clockid_t clk_id) {
  ...
  absolute_micro =
      (static_cast<int64_t>(ts.tv_sec) * mutil::Time::kMicrosecondsPerSecond) +
      (static_cast<int64_t>(ts.tv_nsec) / mutil::Time::kNanosecondsPerMicrosecond);

  return mutil::TimeTicks::FromInternalValue(absolute_micro);
}
