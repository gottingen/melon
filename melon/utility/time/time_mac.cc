...
uint64_t ComputeCurrentTicks() {
  ...
  absolute_micro =
      mach_absolute_time() / mutil::Time::kNanosecondsPerMicrosecond *
...
uint64_t ComputeThreadTicks() {
  ...
  return 0;
#else
  mutil::mac::ScopedMachSendRight thread(mach_thread_self());
...
Time Time::FromExploded(bool is_local, const Exploded& exploded) {
  ...
  mutil::ScopedCFTypeRef<CFTimeZoneRef> time_zone(
