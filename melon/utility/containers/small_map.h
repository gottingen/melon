...
#ifndef MUTIL_CONTAINERS_SMALL_MAP_H_
#define MUTIL_CONTAINERS_SMALL_MAP_H_
...
//
//  - mutil::hash_map should be used if you need O(1) lookups. It may waste
...
//
//   mutil::SmallMap<std::map<MyKey, MyValue>, 4, std::equal_to<KyKey> >
//
//
// USAGE
// -----
...
//
// example:
//   mutil::SmallMap< std::map<string, int> > days;
//   days["sunday"   ] = 0;
//   days["monday"   ] = 1;
//   days["tuesday"  ] = 2;
//   days["wednesday"] = 3;
...
//
// If we switch to using std::unordered_map for mutil::hash_map, then the
...
template <typename KeyType, typename ValueType>
struct select_equal_key< mutil::hash_map<KeyType, ValueType>, false> {
  struct equal_key {
    bool operator()(const KeyType& left, const KeyType& right) {
      return left == right;
    }
...
#endif  // MUTIL_CONTAINERS_SMALL_MAP_H_
