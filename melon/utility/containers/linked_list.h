...
//
// Q. Should I use std::list or mutil::LinkedList?
//
// A. The main reason to use mutil::LinkedList over std::list is
//    performance. If you don't care about the performance differences
//    then use an STL container, as it makes for better code readability.
//
//    Comparing the performance of mutil::LinkedList<T> to std::list<T*>:
//
//    * Erasing an element of type T* from mutil::LinkedList<T> is
//      an O(1) operation. Whereas for std::list<T*> it is O(n).
//      That is because with std::list<T*> you must obtain an
//      iterator to the T* element before you can call erase(iterator).
//
//    * Insertion operations with mutil::LinkedList<T> never require
//      heap allocations.
//
// Q. How does mutil::LinkedList implementation differ from std::list?
//
// A. Doubly-linked lists are made up of nodes that contain "next" and
//    "previous" pointers that reference other nodes in the list.
//
//    With mutil::LinkedList<T>, the type being inserted already reserves
//    space for the "next" and "previous" pointers (mutil::LinkNode<T>*).
...
#endif  // MUTIL_CONTAINERS_LINKED_LIST_H_
