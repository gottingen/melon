...
#define MELON_RECORDIO_MAGIC "RDIO"
...
const mutil::IOBuf* Record::Meta(const char* name) const {
...
mutil::IOBuf* Record::MutableMeta(const char* name_cstr, bool null_on_found) {
    const mutil::StringPiece name = name_cstr;
    for (size_t i = 0; i < _metas.size(); ++i) {
        if (_metas[i].name == name) {
    ...
    p.data = std::make_shared<mutil::IOBuf>();
    _metas.push_back(p);
    return p.data.get();
}

mutil::IOBuf* Record::MutableMeta(const std::string& name, bool null_on_found) {
    for (size_t i = 0; i < _metas.size(); ++i) {
        if (_metas[i].name == name) {
            return null_on_found ? NULL : _metas[i].data.get();
        }
...
bool Record::RemoveMeta(const mutil::StringPiece& name) {
    for (size_t i = 0; i < _metas.size(); ++i) {
        if (_metas[i].name == name) {
