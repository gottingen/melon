...
bool UncachedAmIBundled() {
  ...
  // Yes, this is cheap.
  return [[mutil::mac::OuterBundle() bundlePath] hasSuffix:@".app"];
#endif
}
...
bool IsBackgroundOnlyProcess() {
  ...
  NSDictionary* info_dictionary = [mutil::mac::MainBundle() infoDictionary];
...
FilePath PathForFrameworkBundleResource(CFStringRef resourceName) {
  NSBundle* bundle = mutil::mac::FrameworkBundle();
...
std::string GetValueFromDictionaryErrorMessage(
  ...
  return "Expected value for key " +
      mutil::SysCFStringRefToUTF8(key) +
      " to be " +
      expected_type +
      " but it was " +
      mutil::SysCFStringRefToUTF8(actual_type_ref) +
      " instead";
}
...
std::ostream& operator<<(std::ostream& o, const CFStringRef string) {
  return o << mutil::SysCFStringRefToUTF8(string);
}

std::ostream& operator<<(std::ostream& o, const CFErrorRef err) {
  mutil::ScopedCFTypeRef<CFStringRef> desc(CFErrorCopyDescription(err));
  mutil::ScopedCFTypeRef<CFDictionaryRef> user_info(CFErrorCopyUserInfo(err));
