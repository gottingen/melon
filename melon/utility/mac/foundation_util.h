...
#ifndef MUTIL_MAC_FOUNDATION_UTIL_H_
#define MUTIL_MAC_FOUNDATION_UTIL_H_
...
MUTIL_EXPORT bool AmIBundled();
MUTIL_EXPORT void SetOverrideAmIBundled(bool value);

// Returns true if this process is marked as a "Background only process".
MUTIL_EXPORT bool IsBackgroundOnlyProcess();

// Returns the path to a resource within the framework bundle.
MUTIL_EXPORT FilePath PathForFrameworkBundleResource(CFStringRef resourceName);
...
MUTIL_EXPORT OSType CreatorCodeForApplication();
...
MUTIL_EXPORT bool GetSearchPathDirectory(NSSearchPathDirectory directory,
...
// If found, fills result (which must always be non-NULL) with the
// first found directory and returns true.  Otherwise, returns false.
MUTIL_EXPORT bool GetLocalDirectory(NSSearchPathDirectory directory,
                                   FilePath* result);

// Searches for directories for the given key in only the user domain.
// If found, fills result (which must always be non-NULL) with the
// first found directory and returns true.  Otherwise, returns false.
MUTIL_EXPORT bool GetUserDirectory(NSSearchPathDirectory directory,
                                  FilePath* result);

// Returns the ~/Library directory.
MUTIL_EXPORT FilePath GetUserLibraryPath();
...
MUTIL_EXPORT FilePath GetAppBundlePath(const FilePath& exec_name);

#define TYPE_NAME_FOR_CF_TYPE_DECL(TypeCF) \
MUTIL_EXPORT std::string TypeNameForCFType(TypeCF##Ref);
...
MUTIL_EXPORT void NSObjectRetain(void* obj);
MUTIL_EXPORT void NSObjectRelease(void* obj);
...
MUTIL_EXPORT void* CFTypeRefToNSObjectAutorelease(CFTypeRef cf_object);
...
namespace mutil { \
namespace mac { \
MUTIL_EXPORT TypeNS* CFToNSCast(TypeCF##Ref cf_val); \
MUTIL_EXPORT TypeCF##Ref NSToCFCast(TypeNS* ns_val); \
} \
}
...
namespace mutil { \
namespace mac { \
MUTIL_EXPORT NSMutable##name* CFToNSCast(CFMutable##name##Ref cf_val); \
MUTIL_EXPORT CFMutable##name##Ref NSToCFCast(NSMutable##name* ns_val); \
} \
}
...
// CFStringRef some_string = mutil::mac::CFCastStrict<CFStringRef>(hello);
...
template<> MUTIL_EXPORT TypeCF##Ref \
...
//
// Example usage:
// NSString* version = mutil::mac::ObjCCast<NSString>(
//     [bundle objectForInfoDictionaryKey:@"CFBundleShortVersionString"]);
//
// NSString* str = mutil::mac::ObjCCastStrict<NSString>(
...
MUTIL_EXPORT std::string GetValueFromDictionaryErrorMessage(
...
MUTIL_EXPORT NSString* FilePathToNSString(const FilePath& path);

// Converts |str| to a FilePath. Returns an empty path if |str| is nil.
MUTIL_EXPORT FilePath NSStringToFilePath(NSString* str);
...
#endif  // MUTIL_MAC_FOUNDATION_UTIL_H_
