...
mutil::static_atomic<size_t> g_nblock = MUTIL_STATIC_ATOMIC_INIT(0);
mutil::static_atomic<size_t> g_blockmem = MUTIL_STATIC_ATOMIC_INIT(0);
mutil::static_atomic<size_t> g_newbigview = MUTIL_STATIC_ATOMIC_INIT(0);

}  // namespace iobuf

size_t IOBuf::block_count() {
    return iobuf::g_nblock.load(mutil::memory_order_relaxed);
}

size_t IOBuf::block_memory() {
    return iobuf::g_blockmem.load(mutil::memory_order_relaxed);
}

size_t IOBuf::new_bigview_count() {
    return iobuf::g_newbigview.load(mutil::memory_order_relaxed);
}
...
struct IOBuf::Block {
    ...
    Block(char* data_in, uint32_t data_size)
        ...
        , u({NULL})
        , data(data_in) {
        iobuf::g_nblock.fetch_add(1, mutil::memory_order_relaxed);
    ...
    void inc_ref() {
        check_abi();
        nshared.fetch_add(1, mutil::memory_order_relaxed);
    }
        
    void dec_ref() {
        check_abi();
        if (nshared.fetch_sub(1, mutil::memory_order_release) == 1) {
            mutil::atomic_thread_fence(mutil::memory_order_acquire);
            if (!flags) {
                iobuf::g_nblock.fetch_sub(1, mutil::memory_order_relaxed);
    ...
    int ref_count() const {
        return nshared.load(mutil::memory_order_relaxed);
    }
...
static mutil::static_atomic<size_t> g_num_hit_tls_threshold = MUTIL_STATIC_ATOMIC_INIT(0);
...
IOBuf::Block* share_tls_block() {
    ...
    if (b) {
        ...
        mutil::thread_atexit(remove_tls_block_chain);
    }
    if (!new_block) {
...
inline void release_tls_block(IOBuf::Block* b) {
    ...
    if (b->full()) {
        ...
        b->dec_ref();
        g_num_hit_tls_threshold.fetch_add(1, mutil::memory_order_relaxed);
    } else {
...
void release_tls_block_chain(IOBuf::Block* b) {
    ...
    if (tls_data.num_blocks >= MAX_BLOCKS_PER_THREAD) {
        ...
        g_num_hit_tls_threshold.fetch_add(n, mutil::memory_order_relaxed);
        return;
    }
...
inline IOBuf::BlockRef* acquire_blockref_array(size_t cap) {
    iobuf::g_newbigview.fetch_add(1, mutil::memory_order_relaxed);
...
size_t IOBuf::block_count_hit_tls_threshold() {
    return iobuf::g_num_hit_tls_threshold.load(mutil::memory_order_relaxed);
}
...
void IOBuf::append(const Movable& movable_other) {
    if (empty()) {
        ...
        mutil::IOBuf& other = movable_other.value();
        const size_t nref = other._ref_num();
        for (size_t i = 0; i < nref; ++i) {
...
IOBufCutter::IOBufCutter(mutil::IOBuf* buf)
    : _data(NULL)
    , _data_end(NULL)
    , _block(NULL)
    , _buf(buf) {
...
size_t IOBufCutter::cutn(mutil::IOBuf* out, size_t n) {
    if (n == 0) {
        return 0;
    }
...
void IOBufAsZeroCopyOutputStream::BackUp(int count) {
    while (!_buf->empty()) {
        ...
        if (MELON_LIKELY(r.length > (uint32_t)count)) {
            r.length -= count;
            if (!_buf->_small()) {
...
IOBufAsSnappySink::IOBufAsSnappySink(mutil::IOBuf& buf)
...
char* IOBufAsSnappySink::GetAppendBuffer(size_t length, char* scratch) {
    // TODO: mutil::IOBuf supports dynamic sized blocks.
...
size_t IOBufBytesIterator::append_and_forward(mutil::IOBuf* buf, size_t n) {
    size_t nc = 0;
...
void* fast_memcpy(void *__restrict dest, const void *__restrict src, size_t n) {
    return mutil::iobuf::cp(dest, src, n);
}
