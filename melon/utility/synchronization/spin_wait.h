...
#define SPIN_FOR_1_SECOND_OR_UNTIL_TRUE(expression) \
    SPIN_FOR_TIMEDELTA_OR_UNTIL_TRUE(mutil::TimeDelta::FromSeconds(1), \
                                     (expression))

#define SPIN_FOR_TIMEDELTA_OR_UNTIL_TRUE(delta, expression) do { \
  mutil::TimeTicks start = mutil::TimeTicks::Now(); \
  const mutil::TimeDelta kTimeout = delta; \
    while (!(expression)) { \
      if (kTimeout < mutil::TimeTicks::Now() - start) { \
      EXPECT_LE((mutil::TimeTicks::Now() - start).InMilliseconds(), \
                kTimeout.InMilliseconds()) << "Timed out"; \
        break; \
      } \
      mutil::PlatformThread::Sleep(mutil::TimeDelta::FromMilliseconds(50)); \
    } \
  } while (0)

#endif  // MUTIL_SYNCHRONIZATION_SPIN_WAIT_H_
