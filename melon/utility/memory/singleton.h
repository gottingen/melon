...
#ifndef MUTIL_MEMORY_SINGLETON_H_
...
MUTIL_EXPORT subtle::AtomicWord WaitForInstance(subtle::AtomicWord* instance);

}  // namespace internal
}  // namespace mutil

// TODO(joth): Move more of this file into namespace mutil
...
struct StaticMemorySingletonTraits {
  ...
  static Type* New() {
    ...
    if (mutil::subtle::NoBarrier_AtomicExchange(&dead_, 1))
      return NULL;
  ...
  static void Resurrect() {
    mutil::subtle::NoBarrier_Store(&dead_, 0);
  }

 private:
  static mutil::AlignedMemory<sizeof(Type), ALIGNOF(Type)> buffer_;
  // Signal the object was already deleted, so it is not revived.
  static mutil::subtle::Atomic32 dead_;
};

template <typename Type> mutil::AlignedMemory<sizeof(Type), ALIGNOF(Type)>
    StaticMemorySingletonTraits<Type>::buffer_;
template <typename Type> mutil::subtle::Atomic32
...
// process exit. More precisely it uses mutil::AtExitManager which requires an
...
class Singleton {
 private:
  ...
  static Type* get() {
    ...
    mutil::subtle::AtomicWord value = mutil::subtle::Acquire_Load(&instance_);
    if (value != 0 && value != mutil::internal::kBeingCreatedMarker) {
    ...
    if (mutil::subtle::Acquire_CompareAndSwap(
          &instance_, 0, mutil::internal::kBeingCreatedMarker) == 0) {
      ...
      mutil::subtle::Release_Store(
          &instance_, reinterpret_cast<mutil::subtle::AtomicWord>(newval));

      if (newval != NULL && Traits::kRegisterAtExit)
        mutil::AtExitManager::RegisterCallback(OnExit, NULL);

      return newval;
    }

    // We hit a race. Wait for the other thread to complete it.
    value = mutil::internal::WaitForInstance(&instance_);
  ...
  static void OnExit(void* /*unused*/) {
    ...
    Traits::Delete(
        reinterpret_cast<Type*>(mutil::subtle::NoBarrier_Load(&instance_)));
    instance_ = 0;
  }
  static mutil::subtle::AtomicWord instance_;
};

template <typename Type, typename Traits, typename DifferentiatingType>
mutil::subtle::AtomicWord Singleton<Type, Traits, DifferentiatingType>::
    instance_ = 0;

#endif  // MUTIL_MEMORY_SINGLETON_H_
