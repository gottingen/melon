...
bool ScopedTempDir::CreateUniqueTempDir() {
  ...
  // for the unique name.
  if (!mutil::CreateNewTempDirectory(FILE_PATH_LITERAL("scoped_dir"), &path_))
    return false;
...
bool ScopedTempDir::CreateUniqueTempDirUnderPath(const FilePath& base_path) {
  ...
  if (!mutil::CreateDirectory(base_path))
    return false;

  // Create a new, uniquely named directory under |base_path|.
  if (!mutil::CreateTemporaryDirInDir(base_path,
...
bool ScopedTempDir::Set(const FilePath& path) {
  ...
  if (!DirectoryExists(path) && !mutil::CreateDirectory(path))
    return false;
...
bool ScopedTempDir::Delete() {
  ...
  bool ret = mutil::DeleteFile(path_, true);
  if (ret) {
