...
namespace mcpack2pb {
    ...
    inline void OutputStream::Area::assign(const void *data) const {
        if (_addr1) {
            ...
            for (std::vector<std::string_view>::const_iterator iter =
    ...
    inline void Serializer::begin_mcpack_array(FieldType item_type) { begin_array_internal(item_type, false); }
    ...
    inline void Serializer::begin_compack_array(FieldType item_type) { begin_array_internal(item_type, true); }

    inline void Serializer::begin_object(const StringWrapper &name) { begin_object_internal(name); }
