...
#ifndef MELON_CONTAINER_CONTAINERS_H_
#define MELON_CONTAINER_CONTAINERS_H_
...
namespace melon {
    namespace containers {
        ...
        // Unlike std::vector, melon::containers::vector keeps the first
        ...
        private:
            using TStorage = typename melon::aligned_storage<sizeof(T), alignof(T)>::type;
        ...
        vector<T, BASE_CAPACITY>::vector(
                melon::allocator *allocator /* = allocator::Default */)
                : _allocator(allocator) {}

        template<typename T, int BASE_CAPACITY>
        template<int BASE_CAPACITY_2>
        vector<T, BASE_CAPACITY>::vector(
                const vector<T, BASE_CAPACITY_2> &other, melon::allocator *alloc /* = allocator::Default */)
                : _allocator(alloc) {
            *this = other;
        }
        ...
        void vector<T, BASE_CAPACITY>::pop_back() {
            MELON_CHECK(count > 0) << "pop_back() called on empty vector";
            count--;
        ...
        T &vector<T, BASE_CAPACITY>::front() {
            MELON_CHECK(count > 0) << "front() called on empty vector";
            return reinterpret_cast<T *>(elements)[0];
        }

        template<typename T, int BASE_CAPACITY>
        T &vector<T, BASE_CAPACITY>::back() {
            MELON_CHECK(count > 0) << "back() called on empty vector";
        ...
        void list<T>::grow(size_t count) {
            ...
            auto const allocChainOffset = melon::base::align_up(entriesSize, alignof(AllocationChain));
        ...
        void list<T>::link(Entry *entry, Entry *&list) {
            MELON_CHECK(entry->next == nullptr);
            MELON_CHECK(entry->prev == nullptr);
            if (list) {
                entry->next = list;
                list->prev = entry;
            }
...
#endif  // MELON_CONTAINER_CONTAINERS_H_
