...
#ifndef MELON_CONTAINER_INTRUSIVE_PTR_H_
#define MELON_CONTAINER_INTRUSIVE_PTR_H_
...
namespace MELON_HASH_NAMESPACE {
    ...
    template<typename T>
    struct hash<melon::container::intrusive_ptr < T> > {
    std::size_t operator()(const melon::container::intrusive_ptr <T> &p) const {
        return hash<T *>()(p.get());
    }
};
#elif defined(MELON_COMPILER_MSVC)
template<typename T>
inline size_t hash_value(const melon::container::intrusive_ptr<T>& sp) {
...
#endif  // MELON_CONTAINER_INTRUSIVE_PTR_H_
