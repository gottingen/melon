...
#ifndef MELON_CONTAINER_DOUBLY_BUFFERED_DATA_H_
#define MELON_CONTAINER_DOUBLY_BUFFERED_DATA_H_
...
// Called when thread quits.
    ...
    DoublyBufferedData<T, TLS>::DoublyBufferedData()
        ...
        if (rc != 0) {
            MELON_LOG(FATAL) << "Fail to pthread_key_create: " << melon_error(rc);
        } else {
            _created_key = true;
        }
    ...
    int DoublyBufferedData<T, TLS>::Read(
        ...
        if (MELON_UNLIKELY(!_created_key)) {
            return -1;
        }
    ...
    size_t DoublyBufferedData<T, TLS>::Modify(Fn &fn) {
        ...
        MELON_CHECK_EQ(ret2, ret) << "index=" << _index.load(std::memory_order_relaxed);
        return ret2;
    }
...
#endif  // MELON_CONTAINER_DOUBLY_BUFFERED_DATA_H_
