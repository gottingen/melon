...
#include "melon/strings/internal/charconv_bigint.h"
#include "melon/strings/internal/charconv_parse.h"

#ifdef MELON_BIT_PACK_FLOATS
#error MELON_BIT_PACK_FLOATS cannot be directly set
#elif defined(__x86_64__) || defined(_M_X64)
#define MELON_BIT_PACK_FLOATS 1
#endif
...
struct FloatTraits<double> {
    ...
    static double Make(uint64_t mantissa, int exponent, bool sign) {
        ...
        dbl += mantissa;
        return melon::base::bit_cast<double>(dbl);
#endif  // MELON_BIT_PACK_FLOATS
    }
};
...
struct FloatTraits<float> {
    ...
    return sign ? -ldexpf(mantissa, exponent) : ldexpf(mantissa, exponent);
        ...
        flt += mantissa;
        return melon::base::bit_cast<float>(flt);
#endif  // MELON_BIT_PACK_FLOATS
    }
};
...
int BitWidth(melon::uint128 value) {
    if (uint128_high64(value) == 0) {
        return 64 - melon::base::countl_zero(uint128_low64(value));
    }
    return 128 - melon::base::countl_zero(uint128_high64(value));
}
...
int TruncateToBitWidth(int bit_width, melon::uint128 *value) {
...
void EncodeResult(const CalculatedFloat &calculated, bool negative,
                  melon::from_chars_result *result, FloatType *value) {
...
uint64_t ShiftRightAndRound(melon::uint128 value, int shift, bool input_exact,
                            bool *output_exact) {
    if (shift <= 0) {
    ...
    *output_exact = true;
    const melon::uint128 shift_mask = (melon::uint128(1) << shift) - 1;
    const melon::uint128 halfway_point = melon::uint128(1) << (shift - 1);

    const melon::uint128 shifted_bits = value & shift_mask;
    value >>= shift;
...
bool MustRoundUp(uint64_t guess_mantissa, int guess_exponent,
    ...
    melon::strings_internal::big_unsigned<84> exact_mantissa;
    ...
    guess_exponent -= 1;
    melon::strings_internal::big_unsigned<84> &lhs = exact_mantissa;
    int comparison;
    if (exact_exponent >= 0) {
        lhs.MultiplyByFiveToTheNth(exact_exponent);
        melon::strings_internal::big_unsigned<84> rhs(guess_mantissa);
        ...
        melon::strings_internal::big_unsigned<84> rhs =
                melon::strings_internal::big_unsigned<84>::FiveToTheNth(-exact_exponent);
...
CalculatedFloat CalculateFromParsedHexadecimal(
    ...
    int mantissa_width = 64 - melon::base::countl_zero(mantissa);
...
CalculatedFloat CalculateFromParsedDecimal(
    ...
    melon::uint128 wide_binary_mantissa = parsed_decimal.mantissa;
