...
//
// NOTE: Use of `alpha_num` outside of the //melon/strings package is unsupported
...
#ifndef MELON_STRINGS_STR_CAT_H_
#define MELON_STRINGS_STR_CAT_H_
...
// a size.  Please don't use outside of melon, yet.
...
struct hex {
    ...
    explicit hex(
            Int v, pad_spec spec = melon::kNoPad,
    ...
    explicit hex(Pointee *v, pad_spec spec = melon::kNoPad)
            : hex(spec, reinterpret_cast<uintptr_t>(v)) {}

  private:
    hex(pad_spec spec, uint64_t v)
            : value(v),
              width(spec == melon::kNoPad
                    ? 1
                    : spec >= melon::kSpacePad2 ? spec - melon::kSpacePad2 + 2
                                               : spec - melon::kZeroPad2 + 2),
              fill(spec >= melon::kSpacePad2 ? ' ' : '0') {}
};
...
struct dec {
    ...
    template<typename Int>
    explicit dec(Int v, pad_spec spec = melon::kNoPad,
...
MELON_MUST_USE_RESULT MELON_FORCE_INLINE std::string string_cat() { return std::string(); }

MELON_MUST_USE_RESULT MELON_FORCE_INLINE std::string string_cat(const alpha_num &a) {
    return std::string(a.data(), a.size());
}

MELON_MUST_USE_RESULT std::string string_cat(const alpha_num &a, const alpha_num &b);
...
MELON_MUST_USE_RESULT MELON_FORCE_INLINE std::string string_cat(
...
MELON_FORCE_INLINE void string_append(std::string *) {}
...
// Support 5 or more arguments
template<typename... AV>
MELON_FORCE_INLINE void string_append(std::string *dest, const alpha_num &a, const alpha_num &b,
...
// This is fast.
MELON_FORCE_INLINE strings_internal::alpha_num_buffer<
...
#endif  // MELON_STRINGS_STR_CAT_H_
