...
#ifndef MELON_STRINGS_UTF8_GREEDY_TABLE_DECODER_H_
#define MELON_STRINGS_UTF8_GREEDY_TABLE_DECODER_H_
...
namespace melon::utf8_detail {
    ...
    MELON_FORCE_INLINE void greedy_table_decoder::skip_by_greedy_2(const char8_t *&pSrc) noexcept {
        if ((pSrc[0] & 0xC0) == 0x80) {
            pSrc += 1;
        }
    }

    MELON_FORCE_INLINE void greedy_table_decoder::advance_by_greedy_2(const char8_t *&pSrc,
    ...
    MELON_FORCE_INLINE void greedy_table_decoder::skip_by_greedy_3(const char8_t *&pSrc) noexcept {
        if (((pSrc[0] & 0xC0) == 0x80) && ((pSrc[1] & 0xC0) == 0x80)) {
            pSrc += 2;
        }
    }

    MELON_FORCE_INLINE void greedy_table_decoder::advance_by_greedy_3(const char8_t *&pSrc,
                                                                  char32_t &cdpt) noexcept {
        char32_t unit2;  //- The second UTF-8 code unit
    ...
    MELON_FORCE_INLINE void greedy_table_decoder::skip_by_greedy_4(const char8_t *&pSrc) noexcept {
        if (((pSrc[0] & 0xC0) == 0x80) && ((pSrc[1] & 0xC0) == 0x80) && ((pSrc[2] & 0xC0) == 0x80)) {
            pSrc += 3;
        }
    }

    MELON_FORCE_INLINE void greedy_table_decoder::advance_by_greedy_4(const char8_t *&pSrc,
                                                                  char32_t &cdpt) noexcept {
        char32_t unit2;  //- The second UTF-8 code unit
        char32_t unit3;  //- The third UTF-8 code unit
...
#endif  // MELON_STRINGS_UTF8_GREEDY_TABLE_DECODER_H_
