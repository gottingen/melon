...
#include "melon/debugging/internal/address_is_readable.h"
...
static uint64_t Pack(uint64_t pid, uint64_t read_fd, uint64_t write_fd) {
  MELON_DCHECK((read_fd >> 24) == 0 && (write_fd >> 24) == 0)<<"fd out of range";
...
bool address_is_readable(const void *addr) {
  ...
  do {  // until we do not get EBADF trying to use file descriptors
    ...
    while (current_pid != pid) {
      ...
      // new pipe
      if (pipe(p) != 0) {
        MELON_LOG(FATAL)<<"Failed to create pipe, errno="<<errno;
      }
