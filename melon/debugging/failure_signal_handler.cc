...
#include "melon/debugging/failure_signal_handler.h"
...
#include "melon/debugging/internal/examine_stack.h"
#include "melon/debugging/stacktrace.h"

#ifndef _WIN32
#define MELON_HAVE_SIGACTION
#endif
...
#define MAP_ANONYMOUS MAP_ANON
        ...
        if (sigstk.ss_sp == MAP_FAILED) {
             MELON_DLOG(FATAL)<<"mmap() for alternate signal stack failed";
        }
#else
        sigstk.ss_sp = malloc(sigstk.ss_size);
        if (sigstk.ss_sp == nullptr) {
            MELON_DLOG(FATAL) << "malloc() for alternate signal stack failed";
        }
#endif

        if (sigaltstack(&sigstk, nullptr) != 0) {
            MELON_DLOG(FATAL) << "sigaltstack() failed with errno={}" << errno;
        }
        return true;
    }
...
// if the system supports using an alternate stack.
    ...
    static void install_one_failure_handler(failure_signal_data *data,
        ...
        MELON_DCHECK(sigaction(data->signo, &act, &data->previous_action) == 0) << "sigaction() failed";
    }
    ...
    static void install_one_failure_handler(failure_signal_data* data,
      ...
      MELON_CHECK(data->previous_handler != SIG_ERR)<< "signal() failed";
    }

#endif

    static void safe_write_to_stderr(const char *s, size_t len) {
#if defined(MELON_HAVE_SYSCALL_WRITE)
        syscall(SYS_write, STDERR_FILENO, s, len);
#elif defined(MELON_HAVE_POSIX_WRITE)
        write(STDERR_FILENO, s, len);
#elif defined(MELON_HAVE_RAW_IO)
...
// `void*` might not be big enough to store `void(*)(const char*)`.
    ...
    // Many of the melon::debugging::::debugging_internal::Dump* functions in
    ...
    MELON_NO_INLINE static void write_stack_trace(
        ...
        int depth = melon::debugging::get_stack_frames_with_context(
                stack, frame_sizes, kNumStackFrames,
                1,  // Do not include this function in stack trace.
                ucontext, &min_dropped_frames);
        melon::debugging::debugging_internal::DumpPCAndFrameSizesAndStackTrace(
                melon::debugging::debugging_internal::GetProgramCounter(ucontext), stack, frame_sizes,
                depth, min_dropped_frames, symbolize_stacktrace, writerfn, writerfn_arg);
    }

// Called by melon_failure_signal_handler() to write the failure info. It is
...
#ifdef MELON_HAVE_ALARM

    // melon_failure_signal_handler() installs this as a signal handler for
    // SIGALRM, then sets an alarm to be delivered to the program after a
    // set amount of time. If melon_failure_signal_handler() hangs for more than
...
// melon::debugging::get_tid() returns pid_t on most platforms, but
// returns melon::debugging::base_internal::pid_t on Windows.
    using GetTidType = decltype(melon::thread::thread_index());

    static std::atomic<GetTidType> failed_tid(0);

#ifndef MELON_HAVE_SIGACTION
    static void melon_failure_signal_handler(int signo) {
      void* ucontext = nullptr;
#else

    static void melon_failure_signal_handler(int signo, siginfo_t *, void *ucontext) {
#endif

        const GetTidType this_tid = melon::thread::thread_index();
        ...
        if (!failed_tid.compare_exchange_strong(
            ...
            MELON_DLOG(ERROR) <<
                              ...
                              << melon::debugging::debugging_internal::GetProgramCounter(ucontext)
                              << " while already in melon_failure_signal_handler()";

            if (this_tid != previous_failed_tid) {
                // Another thread is already in melon_failure_signal_handler(), so wait
    ...
    void install_failure_signal_handler(const failure_signal_handler_options &options) {
        ...
        for (auto &it : failure_signal_array) {
            install_one_failure_handler(&it, melon_failure_signal_handler);
        }
    }
