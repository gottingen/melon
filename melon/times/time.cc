...
// The implementation of the melon::time_point class, which is declared in
// //melon/times/time.h.
//
// The representation for an melon::time_point is an melon::duration offset from the
...
//
// tz: An melon::time_zone
// ci: An melon::time_zone::chrono_info
// ti: An melon::time_zone::time_info
// cd: An melon::chrono_day or a melon::times_internal::civil_day
// cs: An melon::chrono_second or a melon::times_internal::civil_second
// bd: An melon::time_point::breakdown
// cl: A melon::times_internal::time_zone::civil_lookup
// al: A melon::times_internal::time_zone::absolute_lookup
...
#include "melon/times/internal/unscaled_cycle_clock.h"
...
namespace melon {
    ...
    namespace {
        ...
        MELON_FORCE_INLINE melon::times_internal::time_point<melon::times_internal::seconds> internal_unix_epoch() {
        ...
        MELON_FORCE_INLINE int64_t FloorToUnit(melon::duration d, melon::duration unit) {
            melon::duration rem;
        ...
        MELON_FORCE_INLINE melon::time_point::breakdown InfiniteFutureBreakdown() {
            melon::time_point::breakdown bd;
            ...
            bd.minute = 59;
            bd.second = 59;
            bd.subsecond = melon::infinite_duration();
            bd.weekday = 4;
            bd.yearday = 365;
            bd.offset = 0;
            bd.is_dst = false;
        ...
        MELON_FORCE_INLINE melon::time_point::breakdown InfinitePastBreakdown() {
        ...
        MELON_FORCE_INLINE melon::time_zone::chrono_info InfiniteFutureCivilInfo() {
        ...
        MELON_FORCE_INLINE melon::time_zone::chrono_info InfinitePastCivilInfo() {
            time_zone::chrono_info ci;
        ...
        MELON_FORCE_INLINE melon::time_conversion InfiniteFutureTimeConversion() {
            melon::time_conversion tc;
            tc.pre = tc.trans = tc.post = melon::time_point::infinite_future();
            tc.kind = melon::time_conversion::UNIQUE;
            tc.normalized = true;
            return tc;
        }

        MELON_FORCE_INLINE time_conversion InfinitePastTimeConversion() {
            melon::time_conversion tc;
            tc.pre = tc.trans = tc.post = melon::time_point::infinite_past();
            tc.kind = melon::time_conversion::UNIQUE;
            tc.normalized = true;
            return tc;
        }
...
// necessary. If sec is min/max, then consult cs+tz to check for overlow.
        time_point MakeTimeWithOverflow(const melon::times_internal::time_point<melon::times_internal::seconds> &sec,
                                        const melon::times_internal::civil_second &cs,
                                        const melon::times_internal::time_zone &tz,
                                        bool *normalized = nullptr) {
            const auto max = melon::times_internal::time_point<melon::times_internal::seconds>::max();
            const auto min = melon::times_internal::time_point<melon::times_internal::seconds>::min();
            if (sec == max) {
                const auto al = tz.lookup(max);
                if (cs > al.cs) {
                    if (normalized)
                        *normalized = true;
                    return melon::time_point::infinite_future();
                }
            }
            if (sec == min) {
                ...
                if (cs < al.cs) {
                    if (normalized)
                        *normalized = true;
                    return melon::time_point::infinite_past();
                }
            }
...
// Returns Mon=1..Sun=7.
        MELON_FORCE_INLINE int MapWeekday(const melon::times_internal::weekday &wd) {
            switch (wd) {
                case melon::times_internal::weekday::monday:
                    return 1;
                case melon::times_internal::weekday::tuesday:
                    return 2;
                case melon::times_internal::weekday::wednesday:
                    return 3;
                case melon::times_internal::weekday::thursday:
                    return 4;
                case melon::times_internal::weekday::friday:
                    return 5;
                case melon::times_internal::weekday::saturday:
                    return 6;
                case melon::times_internal::weekday::sunday:
                    return 7;
            }
            return 1;
        }

        bool FindTransition(const melon::times_internal::time_zone &tz,
                            bool (melon::times_internal::time_zone::*find_transition)(
                                    const melon::times_internal::time_point<melon::times_internal::seconds> &tp,
                                    melon::times_internal::time_zone::civil_transition *trans) const,
                            time_point t, time_zone::chrono_transition *trans) {
            // Transitions are second-aligned, so we can discard any fractional part.
            const auto tp = internal_unix_epoch() + melon::times_internal::seconds(t.to_unix_seconds());
            melon::times_internal::time_zone::civil_transition tr;
    ...
    melon::time_point::breakdown time_point::in(melon::time_zone tz) const {
        if (*this == melon::time_point::infinite_future())
            return InfiniteFutureBreakdown();
        if (*this == melon::time_point::infinite_past())
            return InfinitePastBreakdown();

        const auto tp = internal_unix_epoch() + melon::times_internal::seconds(duration::get_rep_hi(rep_));
        const auto al = melon::times_internal::time_zone(tz).lookup(tp);
        const auto cs = al.cs;
        const auto cd = melon::times_internal::civil_day(cs);
        ...
        bd.weekday = MapWeekday(melon::times_internal::get_weekday(cd));
        bd.yearday = melon::times_internal::get_yearday(cd);
        bd.offset = al.offset;
        bd.is_dst = al.is_dst;
    ...
    melon::time_point time_point::from_date(double udate) {
        return time_point::from_unix_duration(duration::milliseconds(udate));
    }

    melon::time_point time_point::from_universal(int64_t universal) {
        return melon::time_point::universal_epoch() + 100 * duration::nanoseconds(universal);
    }
    ...
    int64_t time_point::to_universal() const {
        return melon::FloorToUnit(*this - melon::time_point::universal_epoch(), duration::nanoseconds(100));
    }

    melon::time_point time_point::from_timespec(timespec ts) {
        return from_unix_duration(duration::from_timespec(ts));
    }

    melon::time_point time_point::from_timeval(timeval tv) {
        return from_unix_duration(duration::from_timeval(tv));
    }

    timespec time_point::to_timespec() const {
        timespec ts{0, 0};
        melon::duration d = to_unix_duration(*this);
...
//
    ...
    melon::time_zone::chrono_info time_zone::at(time_point t) const {
        ...
        const auto tp = internal_unix_epoch() + melon::times_internal::seconds(duration::get_rep_hi(ud));
    ...
    melon::time_zone::time_info time_zone::at(chrono_second ct) const {
        const melon::times_internal::civil_second cs(ct);
        const auto cl = cz_.lookup(cs);

        time_zone::time_info ti;
        switch (cl.kind) {
            case melon::times_internal::time_zone::civil_lookup::UNIQUE:
                ti.kind = time_zone::time_info::UNIQUE;
                break;
            case melon::times_internal::time_zone::civil_lookup::SKIPPED:
                ti.kind = time_zone::time_info::SKIPPED;
                break;
            case melon::times_internal::time_zone::civil_lookup::REPEATED:
    ...
    bool time_zone::next_transition(time_point t, chrono_transition *trans) const {
        return FindTransition(cz_, &melon::times_internal::time_zone::next_transition, t, trans);
    }

    bool time_zone::prev_transition(time_point t, chrono_transition *trans) const {
        return FindTransition(cz_, &melon::times_internal::time_zone::prev_transition, t, trans);
    }
...
//
    ...
    melon::time_conversion convert_date_time(int64_t year, int mon, int day, int hour,
    ...
    melon::time_point from_tm(const struct tm &tm, melon::time_zone tz) {
    ...
    struct tm to_tm(melon::time_point t, melon::time_zone tz) {
        struct tm tm = {};
...
namespace melon {
    ...
    time_point time_now() {
        ...
        int64_t n = melon::get_current_time_nanos();
        if (n >= 0) {
            return time_point::from_unix_duration(
                    duration::make_duration(n / 1000000000, n % 1000000000 * 4));
        }
        return time_point::from_unix_duration(melon::duration::nanoseconds(n));
    }
...
// -DMELON_USE_CYCLECLOCK_FOR_GET_CURRENT_TIME_NANOS=[0|1]
#ifndef MELON_USE_CYCLECLOCK_FOR_GET_CURRENT_TIME_NANOS
#if MELON_USE_UNSCALED_CYCLECLOCK
#define MELON_USE_CYCLECLOCK_FOR_GET_CURRENT_TIME_NANOS 1
#else
#define MELON_USE_CYCLECLOCK_FOR_GET_CURRENT_TIME_NANOS 0
#endif
#endif

#if defined(__APPLE__) || defined(_WIN32)

#include "melon/times/internal/chrono_time.h"

#else
#include "melon/times/internal/chrono_posix_time.h"
#endif

// Allows override by test.
#ifndef GET_CURRENT_TIME_NANOS_FROM_SYSTEM
#define GET_CURRENT_TIME_NANOS_FROM_SYSTEM() \
  ::melon::times_internal::get_current_time_nanos_from_system()
#endif

#if !MELON_USE_CYCLECLOCK_FOR_GET_CURRENT_TIME_NANOS
namespace melon {
...
#define GET_CURRENT_TIME_NANOS_CYCLECLOCK_NOW() \
  ::melon::unscaled_cycle_clock_wrapper_for_get_current_time::now()
#endif
...
// Acquire seqlock (*seq) and return the value to be written to unlock.
    static MELON_FORCE_INLINE uint64_t SeqAcquire(std::atomic<uint64_t> *seq) {
...
// SeqAcquire.
    static MELON_FORCE_INLINE void SeqRelease(std::atomic<uint64_t> *seq, uint64_t x) {
...
// have at least a bit left over for 64-bit calculations.
    ...
    static int64_t get_current_time_nanos_slow_path() MELON_COLD;
...
// preserve precision without overflow.
    ...
    //
    // TODO(melon-team): Remove this attribute when our compiler is smart enough
    // to do the right thing.
    MELON_NO_INLINE
    static int64_t get_current_time_nanos_slow_path() MELON_LOCKS_EXCLUDED(lock) {
    ...
    static uint64_t update_last_sample(uint64_t now_cycles, uint64_t now_ns,
                                       ...
                                       const struct time_sample *sample) MELON_EXCLUSIVE_LOCKS_REQUIRED(lock) {
...
#endif  // MELON_USE_CYCLECLOCK_FOR_GET_CURRENT_TIME_NANOS
...
// Returns the maximum duration that SleepOnce() can sleep for.
        constexpr melon::duration max_sleep() {
#ifdef _WIN32
            // Windows Sleep() takes unsigned long argument in milliseconds.
            return melon::milliseconds(
                std::numeric_limits<unsigned long>::max());  // NOLINT(runtime/int)
#else
            return melon::duration::seconds(std::numeric_limits<time_t>::max());
#endif
        }

// Sleeps for the given duration.
// REQUIRES: to_sleep <= max_sleep().
        void sleep_once(melon::duration to_sleep) {
#ifdef _WIN32
            Sleep(to_sleep / melon::milliseconds(1));
#else
...
MELON_WEAK void melon_internal_sleep_for(melon::duration duration) {
    while (duration > melon::zero_duration()) {
        melon::duration to_sleep = std::min(duration, melon::max_sleep());
        melon::sleep_once(to_sleep);
        duration -= to_sleep;
    }
}
