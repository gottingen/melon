...
namespace melon {
    ...
    namespace {
        ...
        // Since a civil time has a larger year range than melon::time_point (64-bit years vs
        ...
        // melon::time_point can handle.
        MELON_FORCE_INLINE chrono_year_t
        ...
        std::string FormatYearAnd(std::string_view fmt, chrono_second cs) {
            ...
            // TODO(melon-team): Avoid conversion of fmt std::string.
        ...
        bool ParseYearAnd(std::string_view fmt, std::string_view s, CivilT *c) {
            // Civil times support a larger year range than melon::time_point, so we need to
            // parse the year separately, normalize it, then use melon::parse_time on the
            // normalized std::string.
            const std::string ss = std::string(s);  // TODO(melon-team): Avoid conversion.
