   ...
   melon::var::Adder<int64_t> g_num_nodes("raft_node_count");
    ...
    NodeImpl::~NodeImpl() {
        ...
        if (_closure_queue) {
            ...
            _closure_queue = nullptr;
        }
        if (_options.node_owns_log_storage) {
            if (_log_storage) {
                delete _log_storage;
                _log_storage = nullptr;
            }
        }
        if (_meta_storage) {
            delete _meta_storage;
            _meta_storage = nullptr;
        }
...
// in lock
    void NodeImpl::step_down(const int64_t term, bool wakeup_a_candidate,
        ...
        if (_stop_transfer_arg != nullptr) {
            ...
            // mark _stop_transfer_arg to nullptr
            _stop_transfer_arg = nullptr;
        }
...
// in lock
    ...
    void NodeImpl::apply(LogEntryAndClosure tasks[], size_t size) {
        ...
        for (size_t i = 0; i < size; ++i) {
            ...
            _ballot_box->append_pending_task(_conf.conf,
                                             _conf.stable() ? nullptr : &_conf.old_conf,
                                             tasks[i].done);
        }
    ...
    void *NodeImpl::handle_append_entries_cache_timedout(void *arg) {
        ...
        if (node->_append_entries_cache &&
            ...
            if (node->_append_entries_cache->empty()) {
                ...
                node->_append_entries_cache = nullptr;
            }
        }
        lck.unlock();
        delete timer_arg;
    ...
    void NodeImpl::ConfigurationCtx::flush(const Configuration &conf,
        ...
        _node->unsafe_apply_configuration(conf, old_conf.empty() ? nullptr : &old_conf,
                                          true);
    ...
    void NodeImpl::ConfigurationCtx::next_stage() {
        ...
        switch (_stage) {
            ...
            case STAGE_JOINT:
                _stage = STAGE_STABLE;
                return _node->unsafe_apply_configuration(
                        Configuration(_new_peers), nullptr, false);
            case STAGE_STABLE: {
    ...
    void NodeImpl::VoteBallotCtx::init(NodeImpl *node, bool triggered) {
        reset(node);
        _ballot.init(node->_conf.conf, node->_conf.stable() ? nullptr : &(node->_conf.old_conf));
