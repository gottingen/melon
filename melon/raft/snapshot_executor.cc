...
namespace melon::raft {
    ...
    SnapshotExecutor::SnapshotExecutor()
            : _last_snapshot_term(0), _last_snapshot_index(0), _term(0), _saving_snapshot(false),
              _loading_snapshot(false), _stopped(false), _snapshot_storage(NULL), _cur_copier(NULL), _fsm_caller(NULL),
              _node(NULL), _log_manager(NULL), _downloading_snapshot(NULL), _running_jobs(0), _snapshot_throttle(NULL) {
    }
    ...
    void SnapshotExecutor::do_snapshot(Closure *done) {
        ...
        if (_stopped) {
            ...
            if (done) {
                ...
                run_closure_in_fiber(done, _usercode_in_pthread);
            }
            return;
        }
        ...
        if (_fsm_caller->on_snapshot_save(snapshot_save_done) != 0) {
            ...
            if (done) {
                ...
                run_closure_in_fiber(snapshot_save_done, _usercode_in_pthread);
            }
            return;
        }
    ...
    int SnapshotExecutor::on_snapshot_save_done(
            const mutil::Status &st, const SnapshotMeta &meta, SnapshotWriter *writer) {
        std::unique_lock<raft_mutex_t> lck(_mutex);
    ...
    void *SaveSnapshotDone::continue_run(void *arg) {
        ...
        if (self->_done) {
            run_closure_in_fiber(self->_done, true);
        }
        return NULL;
    }
