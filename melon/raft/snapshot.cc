...
#define MELON_RAFT_SNAPSHOT_META_FILE "__raft_snapshot_meta"
...
namespace melon::raft {
    ...
    int LocalSnapshotWriter::init() {
        ...
        // to avoid dirty data
        {
            ...
            while (dir_reader->next()) {
                ...
                if (filename != MELON_RAFT_SNAPSHOT_META_FILE) {
    ...
    int LocalSnapshotStorage::init() {
        ...
        while (dir_reader->next()) {
            int64_t index = 0;
            int match = sscanf(dir_reader->name(), MELON_RAFT_SNAPSHOT_PATTERN, &index);
            if (match == 1) {
...
// LocalSnapshotCopier
    ...
    void LocalSnapshotCopier::load_meta_table() {
        ...
        scoped_refptr<RemoteFileCopier::Session> session
                = _copier.start_to_copy_to_iobuf(MELON_RAFT_SNAPSHOT_META_FILE,
                                                 &meta_buf, NULL);
