...
namespace melon::raft {
    ...
    static melon::var::LatencyRecorder g_load_kv_raft_meta("raft_load_kv_raft_meta");
    static melon::var::LatencyRecorder g_save_kv_raft_meta("raft_save_kv_raft_meta");
    static melon::var::LatencyRecorder g_delete_kv_raft_meta("raft_delete_kv_raft_meta");

    static melon::var::CounterRecorder g_save_kv_raft_meta_batch_counter(
...
//     local-mixed://merged_path={merged_path}&&single_path={single_path}
    int MixedMetaStorage::parse_mixed_path(const std::string &uri,
        ...
        auto sv = copied_uri.substr(0, pos);
        merged_path.assign(sv.data(), sv.size());
        copied_uri.remove_prefix(pos + 14/* length of '&&single_path=' */);
        single_path.assign(copied_uri.data(), copied_uri.size());
