...
namespace melon {
    ...
    result_status sequential_read_file::open(const melon::file_path &path) noexcept {
        MELON_CHECK(_fd == -1) << "do not reopen";
        result_status rs;
        _path = path;
        _fd = ::open(path.c_str(), O_RDONLY | O_CLOEXEC, 0644);
        if (_fd < 0) {
            MELON_LOG(ERROR) << "open file: " << path << "error: " << errno << " " << strerror(errno);
    ...
    result_status sequential_read_file::read(melon::cord_buf *buf, size_t n) {
        size_t left = n;
        melon::IOPortal portal;
        result_status frs;
        while (left > 0) {
            ...
            if (read_len > 0) {
                ...
                MELON_LOG(WARNING) << "read failed, err: " << melon_error()
                                   << " fd: " << _fd << " size: " << n;
