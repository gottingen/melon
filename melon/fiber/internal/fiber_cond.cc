...
int fiber_cond_signal(fiber_cond_t *c) {
    melon::fiber_internal::CondInternal *ic = reinterpret_cast<melon::fiber_internal::CondInternal *>(c);
    ...
    melon::fiber_internal::waitable_event_wake(saved_seq);
    return 0;
}
...
int fiber_cond_broadcast(fiber_cond_t *c) {
    ...
    melon::fiber_internal::waitable_event_requeue(saved_seq, saved_event);
    return 0;
}
...
int fiber_cond_timedwait(fiber_cond_t *__restrict c,
    ...
    fiber_mutex_unlock(m);
    int rc1 = 0;
    if (melon::fiber_internal::waitable_event_wait(ic->seq, expected_seq, abstime) < 0 &&
        errno != EWOULDBLOCK && errno != EINTR/*note*/) {
