...
#include "melon/fiber/internal/log.h"
...
// Skip frames which are always same: the unlock function and submit_contention()
    ...
    struct SampledContention : public melon::Collected {
        ...
        // Implement melon::Collected
        void dump_and_destroy(size_t round) override;

        void destroy() override;

        melon::CollectorSpeedLimit *speed_limit() override { return &g_cp_sl; }
...
#define FIBER_MUTEX_CONTENDED (*(const unsigned*)&melon::fiber_internal::MUTEX_CONTENDED_RAW)
#define FIBER_MUTEX_LOCKED (*(const unsigned*)&melon::fiber_internal::MUTEX_LOCKED_RAW)
    ...
    inline int mutex_timedlock_contended(
        ...
        std::atomic<unsigned> *whole = (std::atomic<unsigned> *) m->event;
        while (whole->exchange(FIBER_MUTEX_CONTENDED) & FIBER_MUTEX_LOCKED) {
            if (melon::fiber_internal::waitable_event_wait(whole, FIBER_MUTEX_CONTENDED, abstime) < 0 &&
                errno != EWOULDBLOCK && errno != EINTR/*note*/) {
                // a mutex lock should ignore interrruptions in general since
                // user code is unlikely to check the return value.
                return errno;
    ...
    namespace internal {
        ...
        int fast_pthread_mutex::lock_contended() {
        ...
        void fast_pthread_mutex::lock() {
            melon::fiber_internal::MutexInternal *split = (melon::fiber_internal::MutexInternal *) &_futex;
            if (split->locked.exchange(1, std::memory_order_acquire)) {
                (void) lock_contended();
            }
        }

        bool fast_pthread_mutex::try_lock() {
            melon::fiber_internal::MutexInternal *split = (melon::fiber_internal::MutexInternal *) &_futex;
            return !split->locked.exchange(1, std::memory_order_acquire);
        }

        void fast_pthread_mutex::unlock() {
            std::atomic<unsigned> *whole = (std::atomic<unsigned> *) &_futex;
...
int fiber_mutex_init(fiber_mutex_t *__restrict m,
    ...
    melon::fiber_internal::make_contention_site_invalid(&m->csite);
    m->event = melon::fiber_internal::waitable_event_create_checked<unsigned>();
    if (!m->event) {
        return ENOMEM;
    }
    *m->event = 0;
...
int fiber_mutex_destroy(fiber_mutex_t *m) {
    melon::fiber_internal::waitable_event_destroy(m->event);
    return 0;
}

int fiber_mutex_trylock(fiber_mutex_t *m) {
    melon::fiber_internal::MutexInternal *split = (melon::fiber_internal::MutexInternal *) m->event;
    if (!split->locked.exchange(1, std::memory_order_acquire)) {
        return 0;
    }
    return EBUSY;
...
int fiber_mutex_lock(fiber_mutex_t *m) {
    ...
    if (!melon::fiber_internal::g_cp) {
        return melon::fiber_internal::mutex_lock_contended(m);
    }
    // Ask Collector if this (contended) locking should be sampled.
    const size_t sampling_range = melon::is_collectable(&melon::fiber_internal::g_cp_sl);
    if (!sampling_range) { // Don't sample
        return melon::fiber_internal::mutex_lock_contended(m);
    }
    // Start sampling.
...
int fiber_mutex_unlock(fiber_mutex_t *m) {
    std::atomic<unsigned> *whole = (std::atomic<unsigned> *) m->event;
    fiber_contention_site_t saved_csite = {0, 0};
    if (melon::fiber_internal::is_contention_site_valid(m->csite)) {
        saved_csite = m->csite;
        melon::fiber_internal::make_contention_site_invalid(&m->csite);
    }
    const unsigned prev = whole->exchange(0, std::memory_order_release);
    ...
    // Wakeup one waiter
    if (!melon::fiber_internal::is_contention_site_valid(saved_csite)) {
...
int pthread_mutex_lock(pthread_mutex_t *__mutex) {
    return melon::fiber_internal::pthread_mutex_lock_impl(__mutex);
}
int pthread_mutex_unlock(pthread_mutex_t *__mutex) {
    return melon::fiber_internal::pthread_mutex_unlock_impl(__mutex);
}
