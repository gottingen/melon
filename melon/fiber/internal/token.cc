...
// This queue reduces the chance to allocate memory for deque
    ...
    struct MELON_CACHELINE_ALIGNMENT token {
        ...
        token() {
            ...
            event = melon::fiber_internal::waitable_event_create_checked<uint32_t>();
            join_butex = melon::fiber_internal::waitable_event_create_checked<uint32_t>();
            *event = 0;
            *join_butex = 0;
        }

        ~token() {
            melon::fiber_internal::waitable_event_destroy(event);
            melon::fiber_internal::waitable_event_destroy(join_butex);
        }
    ...
    inline bool token_exists_with_true_negatives(fiber_token_t tn) {
        ...
        const uint32_t token_ver = melon::fiber_internal::get_version(tn);
...
// required by unittest
    ...
    void token_status(fiber_token_t tn, std::ostream &os) {
        melon::fiber_internal::token *const meta = address_resource(melon::fiber_internal::get_slot(tn));
        if (!meta) {
        ...
        if (valid) {
            os << "First token: "
               << melon::fiber_internal::make_id(first_ver, melon::fiber_internal::get_slot(tn)).value << '\n'
    ...
    struct token_traits {
        ...
        static bool exists(fiber_token_t tn) { return melon::fiber_internal::token_exists_with_true_negatives(tn); }
    };
    ...
    size_t get_sizes(const fiber_token_list_t *list, size_t *cnt, size_t n) {
        ...
        return static_cast<melon::fiber_internal::token_list *>(list->impl)->get_sizes(cnt, n);
    }
...
int fiber_token_create(
        fiber_token_t *tn, void *data,
        int (*on_error)(fiber_token_t, void *, int)) {
    return melon::fiber_internal::token_create_impl(
            tn, data,
            (on_error ? on_error : melon::fiber_internal::default_fiber_token_on_error), nullptr);
}

int fiber_token_create_ranged(fiber_token_t *tn, void *data,
                              int (*on_error)(fiber_token_t, void *, int),
                              int range) {
    return melon::fiber_internal::token_create_ranged_impl(
            tn, data,
            (on_error ? on_error : melon::fiber_internal::default_fiber_token_on_error),
            nullptr, range);
}
    ...
    while (meta->has_version(token_ver)) {
        if (*event == meta->first_ver) {
            ...
            *event = meta->contended_ver();
            ...
            meta->mutex.unlock();
            ever_contended = true;
            if (melon::fiber_internal::waitable_event_wait(event, expected_ver, nullptr) < 0 &&
...
int fiber_token_about_to_destroy(fiber_token_t tn) {
    ...
    *event = meta->unlockable_ver();
    ...
    if (contended) {
        ...
        melon::fiber_internal::waitable_event_wake_except(event, 0);
    }
    return 0;
}
...
int fiber_token_cancel(fiber_token_t tn) {
    ...
    *event = meta->end_ver();
    ...
    meta->mutex.unlock();
    return_resource(melon::fiber_internal::get_slot(tn));
    return 0;
}

int fiber_token_join(fiber_token_t tn) {
    const melon::fiber_internal::IdResourceId slot = melon::fiber_internal::get_slot(tn);
    melon::fiber_internal::token *const meta = address_resource(slot);
    if (!meta) {
    ...
    while (1) {
        ...
        if (melon::fiber_internal::waitable_event_wait(join_butex, expected_ver, nullptr) < 0 &&
            errno != EWOULDBLOCK && errno != EINTR) {
            return errno;
        }
    }
    ...
    if (meta->pending_q.pop(&front)) {
        ...
        *event = meta->first_ver;
        ...
        if (contended) {
            ...
            melon::fiber_internal::waitable_event_wake(event);
        }
        return 0;
    }
}
...
int fiber_token_unlock_and_destroy(fiber_token_t tn) {
    ...
    *join_butex = next_ver;
    ...
    melon::fiber_internal::waitable_event_wake_except(event, 0);
    melon::fiber_internal::waitable_event_wake_all(join_butex);
    return_resource(melon::fiber_internal::get_slot(tn));
    return 0;
}
...
void fiber_token_list_destroy(fiber_token_list_t *list) {
    delete static_cast<melon::fiber_internal::token_list *>(list->impl);
    list->impl = nullptr;
}

int fiber_token_list_add(fiber_token_list_t *list, fiber_token_t tn) {
    if (list->impl == nullptr) {
        list->impl = new(std::nothrow) melon::fiber_internal::token_list;
        if (nullptr == list->impl) {
            return ENOMEM;
        }
    }
    return static_cast<melon::fiber_internal::token_list *>(list->impl)->add(tn);
}
...
int fiber_token_create2(
    ...
    return melon::fiber_internal::token_create_impl(
            tn, data, nullptr,
            (on_error ? on_error : melon::fiber_internal::default_fiber_token_on_error2));
}
...
int fiber_token_create2_ranged(
    ...
    return melon::fiber_internal::token_create_ranged_impl(
            tn, data, nullptr,
            (on_error ? on_error : melon::fiber_internal::default_fiber_token_on_error2), range);
}
...
int fiber_token_error2_verbose(fiber_token_t tn, int error_code,
    ...
    if (*event == meta->first_ver) {
        *event = meta->locked_ver;
        ...
        melon::fiber_internal::PendingError e;
        e.tn = tn;
...
int fiber_token_reset2(fiber_token_list_t *list,
                       ...
                       const std::string &error_text) {
    if (list->impl != nullptr) {
        static_cast<melon::fiber_internal::token_list *>(list->impl)->apply(
                melon::fiber_internal::token_resetter(error_code, error_text));
    }
    return 0;
}
