...
#include "melon/fiber/internal/offset_inl.list"
    };
    ...
    void fiber_worker::run_main_task() {
        melon::status_gauge<double> cumulated_cputime(
                get_cumulated_cputime_from_this, this);
        std::unique_ptr<melon::per_second<melon::status_gauge<double> > > usage_variable;
...
#endif
                usage_variable.reset(new melon::per_second<melon::status_gauge<double> >
...
// To be consistent with sys_usleep, set errno and return -1 on error.
    int fiber_worker::usleep(fiber_worker **pg, uint64_t timeout_us) {
        ...
        if (e.meta->interrupted) {
            ...
            // pespective, however many RPC code expects melon::fiber_sleep_for to set
...
// by race conditions.
// TODO: fibers created by FIBER_ATTR_PTHREAD blocking on melon::fiber_sleep_for()
