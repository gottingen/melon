        ...
        void clear(uint32_t offset) {
            for (uint32_t i = 0; i < KEY_2NDLEVEL_SIZE; ++i) {
                ...
                if (p) {
                    ...
                    KeyInfo info = melon::fiber_internal::s_key_info[offset + i];
...
// Caller of this function must hold the KeyTable
    ...
    static melon::status_gauge<int> s_fiber_key_count(
            "fiber_key_count", get_key_count, nullptr);
    static melon::status_gauge<size_t> s_fiber_keytable_count(
            "fiber_keytable_count", get_keytable_count, nullptr);
    static melon::status_gauge<size_t> s_fiber_keytable_memory(
...
int fiber_keytable_pool_destroy(fiber_keytable_pool_t *pool) {
    ...
    melon::fiber_internal::KeyTable *old_kt = melon::fiber_internal::tls_bls.keytable;
    while (saved_free_keytables) {
        melon::fiber_internal::KeyTable *kt = saved_free_keytables;
        saved_free_keytables = kt->next;
        melon::fiber_internal::tls_bls.keytable = kt;
        if (g) {
    ...
    melon::fiber_internal::tls_bls.keytable = old_kt;
    if (g) {
...
int fiber_keytable_pool_getstat(fiber_keytable_pool_t *pool,
    ...
    std::unique_lock<pthread_mutex_t> mu(pool->mutex);
    size_t count = 0;
    melon::fiber_internal::KeyTable *p = (melon::fiber_internal::KeyTable *) pool->free_keytables;
...
void fiber_keytable_pool_reserve(fiber_keytable_pool_t *pool,
    ...
    for (size_t i = stat.nfree; i < nfree; ++i) {
        melon::fiber_internal::KeyTable *kt = new(std::nothrow) melon::fiber_internal::KeyTable;
        if (kt == nullptr) {
            break;
        }
        ...
        kt->next = (melon::fiber_internal::KeyTable *) pool->free_keytables;
        pool->free_keytables = kt;
        if (data == nullptr) {
            break;
        }
...
int fiber_key_create2(fiber_local_key *key,
    ...
    uint32_t index = 0;
    {
        ...
        if (melon::fiber_internal::nfreekey > 0) {
            ...
            index = melon::fiber_internal::nkey++;
        } else {
            return EAGAIN;  // what pthread_key_create returns in this case.
        }
    }
    melon::fiber_internal::s_key_info[index].dtor = dtor;
    melon::fiber_internal::s_key_info[index].dtor_args = dtor_args;
    key->index = index;
    key->version = melon::fiber_internal::s_key_info[index].version;
    if (key->version == 0) {
        ++melon::fiber_internal::s_key_info[index].version;
        ++key->version;
    }
    return 0;
}
...
int fiber_key_create(fiber_local_key *key, void (*dtor)(void *)) {
    if (dtor == nullptr) {
        ...
        return fiber_key_create2(key, melon::fiber_internal::arg_as_dtor, (const void *) dtor);
    }
}
...
int fiber_key_delete(fiber_local_key key) {
    if (key.index < melon::fiber_internal::KEYS_MAX &&
        ...
        if (key.version == melon::fiber_internal::s_key_info[key.index].version) {
            ...
            melon::fiber_internal::s_key_info[key.index].dtor = nullptr;
            melon::fiber_internal::s_key_info[key.index].dtor_args = nullptr;
            melon::fiber_internal::s_free_keys[melon::fiber_internal::nfreekey++] = key.index;
            return 0;
        }
    }
...
int fiber_setspecific(fiber_local_key key, void *data) {
    melon::fiber_internal::KeyTable *kt = melon::fiber_internal::tls_bls.keytable;
    if (nullptr == kt) {
        kt = new(std::nothrow) melon::fiber_internal::KeyTable;
        if (nullptr == kt) {
            return ENOMEM;
        }
        melon::fiber_internal::tls_bls.keytable = kt;
...
void *fiber_getspecific(fiber_local_key key) {
    ...
    melon::fiber_internal::fiber_worker *const g = melon::fiber_internal::tls_task_group;
    if (g) {
        melon::fiber_internal::fiber_entity *const task = g->current_task();
        kt = melon::fiber_internal::borrow_keytable(task->attr.keytable_pool);
        if (kt) {
            g->current_task()->local_storage.keytable = kt;
            melon::fiber_internal::tls_bls.keytable = kt;
            return kt->get_data(key);
...
void fiber_assign_data(void *data) {
    melon::fiber_internal::tls_bls.assigned_data = data;
}

void *fiber_get_assigned_data() {
    return melon::fiber_internal::tls_bls.assigned_data;
}
