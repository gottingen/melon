...
// space is not allowed by rfc3986, but allowed by melon
...
// ignoring of scheme("http://")
    ...
    QueryRemover::QueryRemover(const std::string *str)
            : _query(str), _qs(str->data(), str->data() + str->size()), _iterated_len(0),
              _removed_current_key_value(false), _ever_removed(false) {
    }
