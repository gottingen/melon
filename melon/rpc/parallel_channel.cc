...
// Not see difference when memory is cached.
    ...
    private:
        ParallelChannelDone(int fail_limit, int ndone, int nchan, int memsize,
                ...
                : _fail_limit(fail_limit), _ndone(ndone), _nchan(nchan), _memsize(memsize), _current_fail(0),
                  _current_done(0), _cntl(cntl), _user_done(user_done), _callmethod_fiber(INVALID_FIBER),
...
// Used in controller.cpp
    ...
    void ParallelChannel::CallMethod(
        ...
        const int rc = fiber_session_lock(cid, nullptr);
        if (rc != 0) {
            CHECK_EQ(EINVAL, rc);
