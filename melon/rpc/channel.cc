...
namespace melon {
    ...
    ChannelOptions::ChannelOptions()
            : connect_timeout_ms(200), timeout_ms(500), backup_request_ms(-1), max_retry(3),
              ...
              succeed_without_server(true), log_succeed_without_server(true), use_rdma(false), auth(nullptr),
              retry_policy(nullptr), ns_filter(nullptr) {}
    ...
    Channel::Channel(ProfilerLinker)
            : _server_id(INVALID_SOCKET_ID), _serialize_request(nullptr), _pack_request(nullptr),
              _get_method_name(nullptr), _preferred_index(-1) {
    }
