...
// Utility functions to combine and extract Socket::_versioned_ref
    ...
    inline SocketOptions::SocketOptions()
            : fd(-1), user(nullptr), on_edge_triggered_events(nullptr), health_check_interval_s(-1), force_ssl(false),
              use_rdma(false), keytable_pool(nullptr), conn(nullptr), app_connect(nullptr),
              initial_parsing_context(nullptr), fiber_tag(FIBER_TAG_DEFAULT) {}
    ...
    inline void Socket::PushPipelinedInfo(const PipelinedInfo &pi) {
        std::unique_lock mu(_pipeline_mutex);
