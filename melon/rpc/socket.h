...
#ifndef MELON_RPC_SOCKET_H_
#define MELON_RPC_SOCKET_H_
...
namespace melon::rpc {
    ...
    struct SocketVarsCollector {
        SocketVarsCollector()
                : nsocket("rpc_socket_count"), channel_conn("rpc_channel_connection_count"),
                  neventthread_second("rpc_event_thread_second", &neventthread), nhealthcheck("rpc_health_check_count"),
        ...
        melon::gauge<int64_t> nsocket;
        melon::gauge<int64_t> channel_conn;
        melon::gauge<int> neventthread;
        melon::per_second<melon::gauge<int> > neventthread_second;
        melon::gauge<int64_t> nhealthcheck;
        melon::gauge<int64_t> nkeepwrite;
        melon::per_second<melon::gauge<int64_t> > nkeepwrite_second;
        melon::gauge<int64_t> nwaitepollout;
        melon::per_second<melon::gauge<int64_t> > nwaitepollout_second;
    };
...
// NOTE: accessed by multiple threads(frequently), align it by cacheline.
    ...
    public:
        ...
        bool DidReleaseAdditionalRereference() const { return _recycle_flag.load(std::memory_order_relaxed); }
        ...
        void set_correlation_id(uint64_t correlation_id) { _correlation_id = correlation_id; }
        ...
        // the body part fully.
        void read_will_be_progressive(ConnectionType t) { _connection_type_for_progressive_read = t; }

        // True if read_will_be_progressive() was called.
        bool is_read_progressive() const { return _connection_type_for_progressive_read != CONNECTION_TYPE_UNKNOWN; }
...
#define MELON_RPC_HANDLE_EOVERCROWDED(write_expr)                       \
    ({                                                                  \
        int64_t __ret_code__;                                           \
        int sleep_time = 250;                                           \
        while (true) {                                                  \
            __ret_code__ = (write_expr);                                \
            if (__ret_code__ >= 0 || errno != ::melon::rpc::EOVERCROWDED) { \
                break;                                                  \
            }                                                           \
            sleep_time *= 2;                                            \
            if (sleep_time > 2000) { sleep_time = 2000; }               \
            melon::fiber_sleep_for(sleep_time);                               \
        }                                                               \
        __ret_code__;                                                   \
    })
...
#define MELON_RPC_HANDLE_EOVERCROWDED_N(write_expr, nretry)                  \
    ({                                                                  \
