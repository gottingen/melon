...
// _s = per second, _m = per minute
    ...
    struct SocketKeepaliveOptions {
        SocketKeepaliveOptions()
                : keepalive_idle_s(-1), keepalive_interval_s(-1), keepalive_count(-1) {}
...
// NOTE: accessed by multiple threads(frequently), align it by cacheline.
    ...
    public:
        ...
        struct WriteOptions {
            ...
            WriteOptions()
                    : id_wait(INVALID_FIBER_ID), abstime(NULL), pipelined_count(0), auth_flags(0),
...
#define MELON_HANDLE_EOVERCROWDED(write_expr)                       \
    ({                                                                  \
...
#define MELON_HANDLE_EOVERCROWDED_N(write_expr, nretry)                  \
    ({                                                                  \
