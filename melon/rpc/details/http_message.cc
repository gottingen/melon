...
namespace melon::rpc {
    ...
    int HttpMessage::OnBody(const char *at, const size_t length) {
        ...
        if (!_read_body_progressively) {
            // Normal read.
            // TODO: The input data is from cord_buf as well, possible to append
            // data w/o copying.
...
#define MELON_RPC_CRLF "\r\n"
    ...
    void MakeRawHttpRequest(melon::cord_buf *request,
        ...
        os << " HTTP/" << h->major_version() << '.'
           << h->minor_version() << MELON_RPC_CRLF;
    ...
    void MakeRawHttpResponse(melon::cord_buf *response,
        ...
        if (content) {
            ...
            os << "Content-Length: " << content->length() << MELON_RPC_CRLF;
        }
        if (!h->content_type().empty()) {
            os << "Content-Type: " << h->content_type()
               << MELON_RPC_CRLF;
