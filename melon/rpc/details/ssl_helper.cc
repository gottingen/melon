...
static mutil::Mutex* g_ssl_mutexs = nullptr;
...
int SSLThreadInit() {
#if OPENSSL_VERSION_NUMBER < 0x10100000L
    g_ssl_mutexs = new mutil::Mutex[CRYPTO_num_locks()];
...
std::string ALPNProtocolToString(const AdaptiveProtocolType& protocol) {
    std::string_view name = protocol.name();
    // Default use http 1.1 version
    static const std::string_view kHTTP11 = "http/1.1";
    if (turbo::starts_with(name, "http")) {
        name = kHTTP11;
    }
