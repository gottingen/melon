...
TURBO_FLAG(int, usercode_backup_threads, 5, "# of backup threads to run user code"
                                            " when too many pthread worker of fibers are used");
TURBO_FLAG(int, max_pending_in_each_backup_thread, 10,
...
// Store pending user code.
    ...
    int UserCodeBackupPool::Init() {
        ...
        for (int i = 0; i < turbo::get_flag(FLAGS_usercode_backup_threads); ++i) {
            pthread_t th;
...
// Entry of backup thread for running user code.
        ...
        while (true) {
            ...
            UserCode usercode = {NULL, NULL};
            {
                ...
                if (g_too_many_usercode &&
                    (int) queue.size() <= turbo::get_flag(FLAGS_usercode_backup_threads)) {
    ...
    void EndRunningUserCodeInPool(void (*fn)(void *), void *arg) {
        ...
        if ((int) s_usercode_pool->queue.size() >=
            (turbo::get_flag(FLAGS_usercode_backup_threads) *
                    turbo::get_flag(FLAGS_max_pending_in_each_backup_thread))) {
