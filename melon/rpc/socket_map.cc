...
// NOTE: Must be limited to positive to guarantee correctness of SocketMapRemove.
    ...
    MELON_VALIDATE_GFLAG(idle_timeout_second, PassValidate);
    ...
    DEFINE_int32(defer_close_second, 0,
                 ...
                 "non-positive values.");
    MELON_VALIDATE_GFLAG(defer_close_second, PassValidate);

    DEFINE_bool(show_socketmap_in_vars, false,
                "[DEBUG] Describe SocketMaps in /vars");
    MELON_VALIDATE_GFLAG(show_socketmap_in_vars, PassValidate);
...
// ========== SocketMap impl. ============
    ...
    SocketMap::SocketMap()
            : _exposed_in_var(false), _this_map_var(NULL), _has_close_idle_thread(false) {
    }
    ...
    void SocketMap::ShowSocketMapInVarIfNeed() {
        if (FLAGS_show_socketmap_in_vars &&
            !_exposed_in_var.exchange(true, std::memory_order_release)) {
            char namebuf[32];
