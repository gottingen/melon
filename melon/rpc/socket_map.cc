...
namespace melon {
    ...
    static mutil::static_atomic<SocketMap *> g_socket_map = MUTIL_STATIC_ATOMIC_INIT(nullptr);
...
// ========== SocketMap impl. ============
    ...
    SocketMapOptions::SocketMapOptions()
            : socket_creator(nullptr), suggested_map_size(1024), idle_timeout_second_dynamic(), idle_timeout_second(0),
              defer_close_second_dynamic(), defer_close_second(0) {
    }

    SocketMap::SocketMap()
            : _exposed_in_var(false), _this_map_var(nullptr), _has_close_idle_thread(false) {
    }
