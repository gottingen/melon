...
// NOTE: Must be limited to positive to guarantee correctness of SocketMapRemove.
    MELON_RPC_VALIDATE_GFLAG(health_check_interval, PositiveInteger);
    ...
    MELON_RPC_VALIDATE_GFLAG(idle_timeout_second, PassValidate);
    ...
    DEFINE_int32(defer_close_second, 0,
                 ...
                 "non-positive values.");
    MELON_RPC_VALIDATE_GFLAG(defer_close_second, PassValidate);
    ...
    int SocketMap::Insert(const SocketMapKey &key, SocketId *id,
        ...
        *id = tmp_id;
        ...
        if (need_to_create_variable) {
            ...
            _this_map_var = new melon::status_gauge<std::string>(
    ...
    void SocketMap::WatchConnections() {
        ...
        while (melon::fiber_sleep_for(CHECK_INTERVAL_US) == 0) {
            // NOTE: save the gflag which may be reloaded at any time.
