...
namespace melon::rpc {
    ...
    MELON_RPC_VALIDATE_GFLAG(enable_rpcz, PassValidate);

    DEFINE_bool(rpcz_hex_log_id, false, "Show log_id in hexadecimal");
    MELON_RPC_VALIDATE_GFLAG(rpcz_hex_log_id, PassValidate);
    ...
    void RpczService::enable(::google::protobuf::RpcController *cntl_base,
                             const ::melon::rpc::RpczRequest *,
                             ::melon::rpc::RpczResponse *,
    ...
    public:
        RpczSpanFilter()
                : _min_latency(std::numeric_limits<int64_t>::min()), _min_request_size(std::numeric_limits<int>::min()),
                  _min_response_size(std::numeric_limits<int>::min()), _log_id(0), _check_log_id(false),
                  _check_error_code(false), _error_code(0) {}
