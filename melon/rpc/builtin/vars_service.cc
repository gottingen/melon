...
#include "melon/rpc/builtin/vars_service.h"
...
// trailing colon from $1
    ...
    void VarsService::default_method(::google::protobuf::RpcController *cntl_base,
                                     const ::melon::rpc::VarsRequest *,
                                     ::melon::rpc::VarsResponse *,
        ...
        melon::variable_dump_options options;
        options.question_mark = '$';
        options.filter =
                (use_html ? melon::DISPLAY_ON_HTML : melon::DISPLAY_ON_PLAIN_TEXT);
        options.white_wildcards = cntl->http_request().unresolved_path();
        const int ndump = melon::variable_base::dump_exposed(&dumper, &options);
        if (ndump < 0) {
