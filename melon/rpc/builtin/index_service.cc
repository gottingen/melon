...
namespace melon::rpc {
    ...
    void IndexService::default_method(::google::protobuf::RpcController *controller,
        ...
        os << '\n';
        if (use_html) {
            os << "<a href=\"https://github.com/melon-rpc/melon-cpp\">github</a>";
        } else {
            os << "github : https://github.com/melon-rpc/melon-cpp";
        }
        os << NL << NL;
        if (!as_more) {
            os << Path("/status", html_addr) << " : Status of services" << NL
               ...
               << Path("/vars", html_addr) << " : List all exposed variables" << NL
               << SP << Path("/vars/rpc_num_sockets", html_addr)
               << " : List the variable" << NL
               << SP << Path("/vars/rpc_server*_count;iobuf_blo$k_*", html_addr)
               << " : List multiple variables with glob patterns"
