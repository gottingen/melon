...
// <rpc_dump_dir>/<DUMPED_FILE_PREFIX>.yyyymmdd_hhmmss_uuuuus
    ...
    MELON_RPC_VALIDATE_GFLAG(rpc_dump, PassValidate);
    MELON_RPC_VALIDATE_GFLAG(rpc_dump_max_requests_in_one_file, PositiveInteger);
    MELON_RPC_VALIDATE_GFLAG(rpc_dump_max_files, PositiveInteger);
...
// Dump a request.
    ...
    SampleIterator::SampleIterator(const std::string_view &dir)
            : _cur_fd(-1), _enum(), _dir(std::string(dir.data(), dir.size())) {
    }
