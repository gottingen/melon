...
namespace melon::rpc {
    ...
    // }
    // MELON_RPC_VALIDATE_GFLAG(rpcz_max_span_per_second,
    //                          validate_rpcz_max_span_per_second);

    DEFINE_int32(rpcz_keep_span_seconds, 3600,
                 "Keep spans for at most so many seconds");
    MELON_RPC_VALIDATE_GFLAG(rpcz_keep_span_seconds, PositiveInteger);
    ...
    bool CanAnnotateSpan() {
        return melon::fiber_internal::tls_bls.rpcz_parent_span;
    }

    void AnnotateSpan(const char *fmt, ...) {
        Span *span = (Span *) melon::fiber_internal::tls_bls.rpcz_parent_span;
        va_list ap;
        va_start(ap, fmt);
...
// Can't use intrusive_ptr which has ctor/dtor issues.
    ...
    melon::CollectorSpeedLimit g_span_sl = VARIABLE_COLLECTOR_SPEED_LIMIT_INITIALIZER;
    static melon::DisplaySamplingRatio s_display_sampling_ratio(
            "rpcz_sampling_ratio", &g_span_sl);

    struct SpanEarlier {
        bool operator()(melon::Collected *c1, melon::Collected *c2) const {
            return ((Span *) c1)->GetStartRealTimeUs() < ((Span *) c2)->GetStartRealTimeUs();
        }
    };

    class SpanPreprocessor : public melon::CollectorPreprocessor {
    public:
        void process(std::vector<melon::Collected *> &list) {
    ...
    melon::CollectorPreprocessor *Span::preprocessor() {
        return g_span_prep;
    }
