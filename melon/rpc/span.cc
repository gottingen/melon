...
//                          validate_rpcz_max_span_per_second);
    ...
    MELON_VALIDATE_GFLAG(rpcz_keep_span_seconds, PositiveInteger);
    ...
    void Span::Annotate(const std::string &info) {
        ...
        turbo::str_append_format(&_info, MELON_SPAN_INFO_SEP "%lld ",
                                 (long long) anno_time);
        _info.append(info);
    }
    ...
    private:
        ...
        ~SpanDB() {
            ...
            if (!FLAGS_rpcz_keep_span_db) {
                std::string cmd = turbo::str_format("rm -rf %s %s",
                                                    id_db_name.c_str(),
                                                    time_db_name.c_str());
