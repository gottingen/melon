...
#define MELON_SPAN_INFO_SEP "\1"


TURBO_FLAG(std::string, rpcz_database_dir, "./rpc_data/rpcz",
...
// }
// MELON_VALIDATE_GFLAG(rpcz_max_span_per_second,
//                          validate_rpcz_max_span_per_second);

TURBO_FLAG(int, rpcz_keep_span_seconds, 3600,
           "Keep spans for at most so many seconds").on_validate(turbo::ClosedClosedInRangeValidator<int, 1800, 5400>::validate);

TURBO_FLAG(bool, rpcz_keep_span_db, false, "Don't remove DB of rpcz at program's exit");
...
namespace melon {
    ...
    void Span::Annotate(const char *fmt, ...) {
        ...
        mutil::string_appendf(&_info, MELON_SPAN_INFO_SEP "%lld ",
                              (long long) anno_time);
        va_list ap;
        va_start(ap, fmt);
    ...
    private:
        ...
        ~SpanDB() {
            ...
            delete id_db;
            delete time_db;
            if (!turbo::get_flag(FLAGS_rpcz_keep_span_db)) {
...
// Can't use intrusive_ptr which has ctor/dtor issues.
    ...
    SpanDB *SpanDB::Open() {
        ...
        local.id_db_name.append(turbo::get_flag(FLAGS_rpcz_database_dir));
        ...
        local.time_db_name.append(turbo::get_flag(FLAGS_rpcz_database_dir));
    ...
    void Span::dump_and_destroy(size_t /*round*/) {
        ...
        if (now > g_last_delete_tm + SPAN_DELETE_INTERVAL_US) {
            ...
            leveldb::Status st = db->RemoveSpansBefore(
                    now - turbo::get_flag(FLAGS_rpcz_keep_span_seconds) * 1000000L);
            if (!st.ok()) {
