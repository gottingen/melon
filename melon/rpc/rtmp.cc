...
namespace melon::rpc {
    ...
    struct rtmp_variables {
        melon::gauge<int> client_count;
        melon::gauge<int> client_stream_count;
        melon::gauge<int> retrying_client_stream_count;
        melon::gauge<int> server_stream_count;
    ...
    AVCDecoderConfigurationRecord::AVCDecoderConfigurationRecord()
            : width(0), height(0), avc_profile((AVCProfile) 0), avc_level((AVCLevel) 0), length_size_minus1(-1) {
    }
    ...
    static bool find_avc_annexb_nalu_start_code(const melon::cord_buf &buf,
        ...
        for (melon::cord_buf_bytes_iterator it(buf); it != NULL; ++it) {
            char c = *it;
            if (c == 0) {
    ...
    RtmpClientOptions::RtmpClientOptions()
            : fpad(false), audioCodecs((RtmpAudioCodec) 3575) // Copy from SRS
            , videoCodecs((RtmpVideoCodec) 252)  // Copy from SRS
            , videoFunction(RTMP_VIDEO_FUNCTION_CLIENT_SEEK), timeout_ms(1000), connect_timeout_ms(500),
              buffer_length_ms(1000), chunk_size(policy::RTMP_DEFAULT_CHUNK_SIZE),
              window_ack_size(policy::RTMP_DEFAULT_WINDOW_ACK_SIZE), simplified_rtmp(false) {
    }
...
// Shared by RtmpClient and RtmpClientStream(s)
    ...
    public:
        RtmpClientImpl() {
            get_rtmp_variables()->client_count << 1;
        }
    ...
    void RtmpConnect::StartConnect(
        ...
        if (ctx == NULL) {
            MELON_LOG(FATAL) << "RtmpContext of " << *s << " is NULL";
...
// ============ RtmpClientStream =============
    ...
    void RtmpClientStream::OnStopInternal() {
        ...
        if (ctx != NULL) {
            ...
            MELON_LOG(FATAL) << "RtmpContext of " << *_rtmpsock << " is NULL";
        }
        if (_from_socketmap) {
...
// ========= RtmpRetryingClientStream ============
    ...
    RtmpRetryingClientStreamOptions::RtmpRetryingClientStreamOptions()
            : retry_interval_ms(1000), max_retry_duration_ms(-1), fast_retry_count(2), quit_when_no_data_ever(true) {
    }

    RtmpRetryingClientStream::RtmpRetryingClientStream()
            : RtmpStreamBase(true), _destroying(false), _called_on_stop(false), _changed_stream(false),
              _has_timer_ever(false), _is_server_accepted_ever(false), _num_fast_retries(0), _last_creation_time_us(0),
              _last_retry_start_time_us(0), _create_timer_id(0), _sub_stream_creator(NULL) {
        get_rtmp_variables()->retrying_client_stream_count << 1;
        _self_ref.reset(this);
    }
    ...
    void RtmpRetryingClientStream::Init(
        ...
        if (sub_stream_creator == NULL) {
            MELON_LOG(ERROR) << "sub_stream_creator is NULL";
            return CallOnStopIfNeeded();
        }
    ...
    void RetryingClientMessageHandler::OnCuePoint(melon::rpc::RtmpCuePoint *cuepoint) {
        _parent->CallOnCuePoint(cuepoint);
    }

    void RetryingClientMessageHandler::OnMetaData(melon::rpc::RtmpMetaData *metadata, const std::string_view &name) {
        _parent->CallOnMetaData(metadata, name);
    }

    void RetryingClientMessageHandler::OnAudioMessage(melon::rpc::RtmpAudioMessage *msg) {
        _parent->CallOnAudioMessage(msg);
    }

    void RetryingClientMessageHandler::OnVideoMessage(melon::rpc::RtmpVideoMessage *msg) {
