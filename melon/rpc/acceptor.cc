...
namespace melon {
    ...
    Acceptor::Acceptor(fiber_keytable_pool_t *pool)
            : InputMessenger(), _keytable_pool(pool), _status(UNINITIALIZED), _idle_timeout_sec(-1),
              _close_idle_tid(INVALID_FIBER), _listened_fd(-1), _acception_id(0), _map_mutex(), _empty_cond(),
              _force_ssl(false), _ssl_ctx(nullptr), _use_rdma(false), _fiber_tag(FIBER_TAG_DEFAULT) {
    }
...
// NOTE: Join() can happen before StopAccept()
    ...
    void Acceptor::OnNewConnectionsUntilEAGAIN(Socket *acception) {
        while (1) {
            ...
            // ChannelTest.skip_parallel in test/melon_channel_unittest.cpp (running
