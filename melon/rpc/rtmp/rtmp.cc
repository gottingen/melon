    ...
    mutil::Status AVCDecoderConfigurationRecord::Create(const void *data, size_t len) {
        ...
        for (int i = 0; i < num_sps; ++i) {
            ...
            if (sps_length > 0) {
                ...
                sps_list.push_back(std::string(sv.data(), sv.size()));
            }
        ...
        for (int i = 0; i < num_pps; ++i) {
            ...
            if (pps_length > 0) {
                pps_list.push_back(std::string(buf.data() + 2, pps_length));
            }
...
// Shared by RtmpClient and RtmpClientStream(s)
    ...
    RtmpStreamBase::RtmpStreamBase(bool is_client)
            : _is_client(is_client), _paused(false), _stopped(false), _processing_msg(false), _has_data_ever(false),
              _message_stream_id(0), _chunk_stream_id(0), _create_realtime_us(mutil::gettimeofday_us()),
...
// ============ RtmpClientStream =============
    ...
    RtmpClientStream::RtmpClientStream()
            : RtmpStreamBase(true), _onfail_id(INVALID_FIBER_ID), _create_stream_rpc_id(INVALID_FIBER_ID),
...
// =========== RtmpService ===============
    ...
    int RtmpServerStream::SendStopMessage(const std::string_view &error_desc) {
        ...
        RtmpInfo info;
        {
            ...
            if (!error_desc.empty()) {
                info.set_description(std::string(error_desc.data(), error_desc.size()));
            }
