...
namespace melon::rpc {
    ...
    static melon::status_gauge<std::string> s_lb_st(
            "rpc_load_balancer", PrintSupportedLB, nullptr);

    static melon::status_gauge<std::string> s_ns_st(
            "rpc_naming_service", PrintSupportedNS, nullptr);

    static melon::status_gauge<std::string> s_proto_st(
            "rpc_protocols", PrintSupportedProtocols, nullptr);

    static melon::status_gauge<std::string> s_comp_st(
            "rpc_compressions", PrintSupportedCompressions, nullptr);

    static melon::status_gauge<std::string> s_prof_st(
    ...
    static melon::Vector<unsigned, 2> GetSessionLocalDataCount(void *arg) {
        melon::Vector<unsigned, 2> v;
    ...
    void *Server::UpdateDerivedVars(void *arg) {
        ...
        melon::status_gauge<timeval> uptime_st(
                prefix, "uptime", GetUptime, (void *) (intptr_t) start_us);

        melon::status_gauge<std::string> start_time_st(
                prefix, "start_time", PrintStartTime, server);

        melon::status_gauge<int32_t> nconn_st(
                prefix, "connection_count", GetConnectionCount, server);

        melon::status_gauge<int32_t> nservice_st(
                prefix, "service_count", GetServiceCount, server);

        melon::status_gauge<int32_t> nbuiltinservice_st(
                prefix, "builtin_service_count", GetBuiltinServiceCount, server);

        melon::status_gauge<melon::Vector<unsigned, 2> > nsessiondata_st(
        ...
        for (MethodMap::iterator it = server->_method_map.begin();
            ...
            if (!it->second.is_builtin_service) {
                ...
                mprefix.push_back('_');
                melon::to_underscored_name(&mprefix, it->second.method->full_name());
    ...
    int Server::StartInternal(const melon::base::ip_t &ip,
        ...
        for (int port = port_range.min_port; port <= port_range.max_port; ++port) {
            ...
            if (sockfd < 0) {
                ...
                if (port_range.min_port != port_range.max_port) {
                    MELON_LOG(ERROR) << "Fail to listen " << ip
                                     << ":[" << port_range.min_port << '-'
                                     << port_range.max_port << ']';
                } else {
