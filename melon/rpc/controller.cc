...
MELON_REGISTER_ERRNO(melon::rpc::ENOSERVICE, "No such service");
MELON_REGISTER_ERRNO(melon::rpc::ENOMETHOD, "No such method");
MELON_REGISTER_ERRNO(melon::rpc::EREQUEST, "Bad request");
MELON_REGISTER_ERRNO(melon::rpc::ERPCAUTH, "Authentication failed");
MELON_REGISTER_ERRNO(melon::rpc::ETOOMANYFAILS, "Too many sub channels failed");
MELON_REGISTER_ERRNO(melon::rpc::EPCHANFINISH, "ParallelChannel finished");
MELON_REGISTER_ERRNO(melon::rpc::EBACKUPREQUEST, "Sending backup request");
MELON_REGISTER_ERRNO(melon::rpc::ERPCTIMEDOUT, "RPC call is timed out");
MELON_REGISTER_ERRNO(melon::rpc::EFAILEDSOCKET, "Broken socket");
MELON_REGISTER_ERRNO(melon::rpc::EHTTP, "Bad http call");
MELON_REGISTER_ERRNO(melon::rpc::EOVERCROWDED, "The server is overcrowded");
MELON_REGISTER_ERRNO(melon::rpc::ERTMPPUBLISHABLE, "RtmpRetryingClientStream is publishable");
MELON_REGISTER_ERRNO(melon::rpc::ERTMPCREATESTREAM, "createStream was rejected by the RTMP server");
MELON_REGISTER_ERRNO(melon::rpc::EEOF, "Got EOF");
MELON_REGISTER_ERRNO(melon::rpc::EUNUSED, "The socket was not needed");
MELON_REGISTER_ERRNO(melon::rpc::ESSL, "SSL related operation failed");
MELON_REGISTER_ERRNO(melon::rpc::EH2RUNOUTSTREAMS, "The H2 socket was run out of streams");

MELON_REGISTER_ERRNO(melon::rpc::EINTERNAL, "General internal error");
MELON_REGISTER_ERRNO(melon::rpc::ERESPONSE, "Bad response");
MELON_REGISTER_ERRNO(melon::rpc::ELOGOFF, "Server is stopping");
MELON_REGISTER_ERRNO(melon::rpc::ELIMIT, "Reached server's max_concurrency");
MELON_REGISTER_ERRNO(melon::rpc::ECLOSE, "Close socket initiatively");
MELON_REGISTER_ERRNO(melon::rpc::EITP, "Bad Itp response");
...
// For definitely false branch in melon/rpc/profiler_link.h
    ...
    Controller::~Controller() {
        *g_ncontroller << -1;
        if (_session_kv != nullptr && _session_kv->Count() != 0) {
            MELON_LOG(INFO) << SessionKVFlusher{this};
        }
        ResetNonPods();
    }
    ...
    void Controller::ResetNonPods() {
        ...
        if (!is_used_by_rpc() && _correlation_id != INVALID_FIBER_TOKEN) {
            MELON_CHECK_NE(EPERM, fiber_token_cancel(_correlation_id));
        }
        ...
        MELON_CHECK(_unfinished_call == nullptr);
    }
    ...
    Controller::Call::~Call() {
        MELON_CHECK(sending_sock.get() == nullptr);
    }
    ...
    void Controller::set_timeout_ms(int64_t timeout_ms) {
        if (timeout_ms <= 0x7fffffff) {
            ...
            MELON_LOG(WARNING) << "timeout_ms is limited to 0x7fffffff (roughly 24 days)";
        }
    }
    ...
    void Controller::set_backup_request_ms(int64_t timeout_ms) {
        if (timeout_ms <= 0x7fffffff) {
            ...
            MELON_LOG(WARNING) << "backup_request_ms is limited to 0x7fffffff (roughly 24 days)";
        }
    }

    void Controller::set_max_retry(int max_retry) {
        if (max_retry > MAX_RETRY_COUNT) {
            MELON_LOG(WARNING) << "Retry count can't be larger than "
    ...
    void Controller::StartCancel() {
        MELON_LOG(FATAL) << "You must call melon::rpc::StartCancel(id) instead!"
    ...
    void Controller::SetFailed(const std::string &reason) {
        ...
        if (_current_call.nretry != 0) {
            melon::string_appendf(&_error_text, "[R%d]", _current_call.nretry);
        } else {
            AppendServerIdentiy();
        }
    ...
    void Controller::SetFailed(int error_code, const char *reason_fmt, ...) {
        ...
        if (_error_code != -1) {
            melon::string_appendf(&_error_text, "[E%d]", _error_code);
        }
        va_list ap;
        va_start(ap, reason_fmt);
        melon::string_vappendf(&_error_text, reason_fmt, ap);
        va_end(ap);
        if (_span) {
            _span->set_error_code(_error_code);
    ...
    public:
        ...
        void Run() {
            _cb->Run();
            MELON_CHECK_EQ(0, fiber_token_unlock_and_destroy(_id));
            delete this;
        }
    ...
    int Controller::RunOnCancel(fiber_token_t id, void *data, int error_code) {
        if (error_code == 0) {
            ...
            MELON_CHECK_EQ(0, fiber_token_unlock_and_destroy(id));
            return 0;
        }
        ...
        fiber_id_t th;
        MELON_CHECK_EQ(0, fiber_start_urgent(&th, nullptr, RunOnCancelThread::RunThis, arg));
        return 0;
    }

    void Controller::NotifyOnCancel(google::protobuf::Closure *callback) {
        if (nullptr == callback) {
            MELON_LOG(WARNING) << "Parameter `callback' is NLLL";
            return;
        }

        ClosureGuard guard(callback);
        if (_oncancel_id != INVALID_FIBER_TOKEN) {
            MELON_LOG(FATAL) << "NotifyCancel a single call more than once!";
            return;
        }
        if (fiber_token_create(&_oncancel_id, callback, RunOnCancel) != 0) {
            MELON_PLOG(FATAL) << "Fail to create fiber_token";
            return;
        }
        SocketUniquePtr sock;
    ...
    void Controller::OnVersionedRPCReturned(const CompletionInfo &info,
        ...
        while (info.id != _correlation_id && info.id != current_id()) {
            ...
            MELON_CHECK_EQ(0, fiber_token_unlock(info.id));
            return;
        }
        ...
        if (_error_code == EBACKUPREQUEST) {
            ...
            if (timeout_ms() >= 0) {
                rc = fiber_timer_add(
                        &_timeout_id,
                        melon::time_point::from_unix_micros(_deadline_us).to_timespec(),
            ...
            MELON_CHECK(_unfinished_call == nullptr);  // only one backup request now.
            ...
            return IssueRPC(melon::get_current_time_micros());
            ...
            MELON_CHECK_EQ(current_id(), info.id) << "error_code=" << _error_code;
            if (!SingleServer()) {
                if (_accessed == nullptr) {
                    _accessed = ExcludedServers::Create(
                            std::min(_max_retry, RETRY_AVOIDANCE));
...
//      entire RPC (specified by c->FailedInline()).
        ...
        if (sending_sock != nullptr) {
            ...
            if (enable_circuit_breaker) {
                sending_sock->FeedbackCircuitBreaker(error_code,
                                                     melon::get_current_time_micros() - begin_time_us);
            }
        }
    ...
    void Controller::EndRPC(const CompletionInfo &info) {
        ...
        if (info.id == current_id() || info.id == _correlation_id) {
            ...
            if (_unfinished_call == nullptr) {
                MELON_CHECK(false) << "A previous non-backup request responded, cid="
            ...
            if (_unfinished_call != nullptr) {
                ...
                if (get_id(_unfinished_call->nretry) == info.id) {
                    ...
                    MELON_CHECK(false) << "A previous non-backup request responded";
        ...
        if (_done) {
            if (!FLAGS_usercode_in_pthread || _done == DoNothing()/*Note*/) {
                ...
                OnRPCEnd(melon::get_current_time_micros());
    ...
    void Controller::IssueRPC(int64_t start_realtime_us) {
        ...
        if (_stream_creator) {
            ...
            MELON_CHECK_EQ(_remote_side, tmp_sock->remote_side());
        }
        ...
        if (_auth != nullptr) {
            ...
            if (_current_call.sending_sock->FightAuthentication(&auth_error) == 0) {
                ...
                SetFailed(auth_error, "Fail to authenticate, %s",
                          melon_error(auth_error));
                return HandleSendFailed();
            }
        }
        // Make request
        melon::cord_buf packet;
        ...
        if (_connect_timeout_ms > 0) {
            if (_deadline_us >= 0) {
                connect_abstime = melon::time_point::from_unix_micros(
    ...
    CallId Controller::call_id() {
        ...
        MELON_CHECK_EQ(0, fiber_token_create2(&cid, this, HandleSocketFailed));
...
// TODO: Is percent encoding better?
    ...
    void Controller::set_stream_creator(StreamCreator *sc) {
        if (_stream_creator) {
            MELON_LOG(FATAL) << "A StreamCreator has been set previously";
            return;
        }
        _stream_creator = sc;
    }
    ...
    Controller::CreateProgressiveAttachment(StopStyle stop_style) {
        if (has_progressive_writer()) {
            MELON_LOG(ERROR) << "One controller can only have one ProgressiveAttachment";
            return nullptr;
        }
        if (_request_protocol != PROTOCOL_HTTP) {
            MELON_LOG(ERROR) << "Only http supports ProgressiveAttachment now";
            return nullptr;
        }
        if (_current_call.sending_sock == nullptr) {
            MELON_LOG(ERROR) << "sending_sock is nullptr";
            return nullptr;
        }
    ...
    void Controller::ReadProgressiveAttachmentBy(ProgressiveReader *r) {
        if (r == nullptr) {
            MELON_LOG(FATAL) << "Param[r] is nullptr";
            return;
        }
        if (!is_response_read_progressively()) {
            return r->OnEndOfMessage(
                    melon::result_status(EINVAL, "Can't read progressive attachment from a "
                                                      "controller without calling "
                                                      "response_will_be_read_progressively() before"));
        }
        if (_rpa == nullptr) {
            return r->OnEndOfMessage(
                    melon::result_status(EINVAL, "ReadableProgressiveAttachment is nullptr"));
        }
        if (has_progressive_reader()) {
            return r->OnEndOfMessage(
                    melon::result_status(EPERM, "{} can't be called more than once",
                                              __FUNCTION__));
        }
    ...
    static void RegisterQuitSignalOrDie() {
        ...
        if (prev != SIG_DFL &&
            ...
            if (prev == SIG_ERR) {
                MELON_LOG(ERROR) << "Fail to register SIGINT, abort";
                abort();
            } else {
                s_prev_sigint_handler = prev;
                MELON_LOG(WARNING) << "SIGINT was installed with " << prev;
            }
        }
        ...
        if (FLAGS_graceful_quit_on_sigterm) {
            ...
            if (prev != SIG_DFL &&
                ...
                if (prev == SIG_ERR) {
                    MELON_LOG(ERROR) << "Fail to register SIGTERM, abort";
                    abort();
                } else {
                    s_prev_sigterm_handler = prev;
                    MELON_LOG(WARNING) << "SIGTERM was installed with " << prev;
                }
            }
        }
    }
