...
// For definitely false branch in src/melon/profiler_link.h
...
#define MELON_SESSION_END_MSG "Session ends."
#define MELON_REQ_ID "@rid"
#define MELON_KV_SEP "="
...
void Controller::FlushSessionKV(std::ostream& os) {
    ...
    if (FLAGS_log_as_json) {
        if (pRID) {
            os << "\"" MELON_REQ_ID "\":\"" << *pRID << "\",";
        }
        os << "\"M\":\"" MELON_SESSION_END_MSG "\"";
        for (auto it = _session_kv->Begin(); it != _session_kv->End(); ++it) {
            os << ",\"" << it->first << "\":\"" << it->second << '"';
        }
    } else {
        if (pRID) {
            os << MELON_REQ_ID MELON_KV_SEP << *pRID << " ";
        }
        os << MELON_SESSION_END_MSG;
        for (auto it = _session_kv->Begin(); it != _session_kv->End(); ++it) {
            os << ' ' << it->first << MELON_KV_SEP << it->second;
        }
    }
}
