...
#include "melon/rpc/server.h"   // Server::_session_local_data_pool
...
TURBO_REGISTER_ERRNO(melon::rpc::ENOSERVICE, "No such service");
TURBO_REGISTER_ERRNO(melon::rpc::ENOMETHOD, "No such method");
TURBO_REGISTER_ERRNO(melon::rpc::EREQUEST, "Bad request");
TURBO_REGISTER_ERRNO(melon::rpc::ERPCAUTH, "Authentication failed");
TURBO_REGISTER_ERRNO(melon::rpc::ETOOMANYFAILS, "Too many sub channels failed");
TURBO_REGISTER_ERRNO(melon::rpc::EPCHANFINISH, "ParallelChannel finished");
TURBO_REGISTER_ERRNO(melon::rpc::EBACKUPREQUEST, "Sending backup request");
TURBO_REGISTER_ERRNO(melon::rpc::ERPCTIMEDOUT, "RPC call is timed out");
TURBO_REGISTER_ERRNO(melon::rpc::EFAILEDSOCKET, "Broken socket");
TURBO_REGISTER_ERRNO(melon::rpc::EHTTP, "Bad http call");
TURBO_REGISTER_ERRNO(melon::rpc::EOVERCROWDED, "The server is overcrowded");
TURBO_REGISTER_ERRNO(melon::rpc::ERTMPPUBLISHABLE, "RtmpRetryingClientStream is publishable");
TURBO_REGISTER_ERRNO(melon::rpc::ERTMPCREATESTREAM, "createStream was rejected by the RTMP server");
TURBO_REGISTER_ERRNO(melon::rpc::EEOF, "Got EOF");
TURBO_REGISTER_ERRNO(melon::rpc::EUNUSED, "The socket was not needed");
TURBO_REGISTER_ERRNO(melon::rpc::ESSL, "SSL related operation failed");
TURBO_REGISTER_ERRNO(melon::rpc::EH2RUNOUTSTREAMS, "The H2 socket was run out of streams");

TURBO_REGISTER_ERRNO(melon::rpc::EINTERNAL, "General internal error");
TURBO_REGISTER_ERRNO(melon::rpc::ERESPONSE, "Bad response");
TURBO_REGISTER_ERRNO(melon::rpc::ELOGOFF, "Server is stopping");
TURBO_REGISTER_ERRNO(melon::rpc::ELIMIT, "Reached server's max_concurrency");
TURBO_REGISTER_ERRNO(melon::rpc::ECLOSE, "Close socket initiatively");
TURBO_REGISTER_ERRNO(melon::rpc::EITP, "Bad Itp response");
...
// For definitely false branch in melon/rpc/profiler_link.h
    int PROFILER_LINKER_DUMMY = 0;

    static void PrintRevision(std::ostream &os, void *) {
#if defined(MELON_RPC_REVISION)
        os << MELON_RPC_REVISION;
#else
        os << "undefined";
#endif
    }

    static melon::status_gauge<std::string> s_rpc_revision(
    ...
    static melon::gauge<int64_t> *g_ncontroller = nullptr;

    static pthread_once_t s_create_vars_once = PTHREAD_ONCE_INIT;

    static void CreateVars() {
        g_ncontroller = new melon::gauge<int64_t>("rpc_controller_count");
    }
    ...
    Controller::~Controller() {
        *g_ncontroller << -1;
        if (_session_kv != nullptr && _session_kv->Count() != 0) {
            TURBO_LOG(INFO) << SessionKVFlusher{this};
        }
        ResetNonPods();
    }
    ...
    void Controller::ResetNonPods() {
        if (_span) {
            Span::Submit(_span, turbo::get_current_time_micros());
        }
        _error_text.clear();
        _remote_side = turbo::end_point();
        ...
        if (!is_used_by_rpc() && _correlation_id != INVALID_FIBER_TOKEN) {
            TURBO_CHECK_NE(EPERM, fiber_token_cancel(_correlation_id));
        }
        ...
        TURBO_CHECK(_unfinished_call == nullptr);
    }
    ...
    Controller::Call::~Call() {
        TURBO_CHECK(sending_sock.get() == nullptr);
    }
    ...
    void Controller::set_timeout_ms(int64_t timeout_ms) {
        if (timeout_ms <= 0x7fffffff) {
            ...
            TURBO_LOG(WARNING) << "timeout_ms is limited to 0x7fffffff (roughly 24 days)";
        }
    }
    ...
    void Controller::set_backup_request_ms(int64_t timeout_ms) {
        if (timeout_ms <= 0x7fffffff) {
            ...
            TURBO_LOG(WARNING) << "backup_request_ms is limited to 0x7fffffff (roughly 24 days)";
        }
    }

    void Controller::set_max_retry(int max_retry) {
        if (max_retry > MAX_RETRY_COUNT) {
            TURBO_LOG(WARNING) << "Retry count can't be larger than "
    ...
    void Controller::StartCancel() {
        TURBO_LOG(FATAL) << "You must call melon::rpc::StartCancel(id) instead!"
    ...
    void Controller::AppendServerIdentiy() {
        ...
        if (is_security_mode()) {
            ...
            int len = snprintf(ipbuf, sizeof(ipbuf), "%s:%d",
                               turbo::my_ip_cstr(), _server->listen_address().port);
            ...
            turbo::string_appendf(&_error_text, "[%s:%d]",
                                  turbo::my_ip_cstr(), _server->listen_address().port);
        }
    }

    inline void UpdateResponseHeader(Controller *cntl) {
        TURBO_DCHECK(cntl->Failed());
    ...
    void Controller::SetFailed(const std::string &reason) {
        ...
        if (_current_call.nretry != 0) {
            turbo::string_appendf(&_error_text, "[R%d]", _current_call.nretry);
        } else {
            AppendServerIdentiy();
        }
    ...
    void Controller::SetFailed(int error_code, const char *reason_fmt, ...) {
        ...
        if (_error_code != -1) {
            turbo::string_appendf(&_error_text, "[E%d]", _error_code);
        }
        va_list ap;
        va_start(ap, reason_fmt);
        turbo::string_vappendf(&_error_text, reason_fmt, ap);
        va_end(ap);
        if (_span) {
            _span->set_error_code(_error_code);
    ...
    public:
        ...
        void Run() {
            _cb->Run();
            TURBO_CHECK_EQ(0, fiber_token_unlock_and_destroy(_id));
            delete this;
        }
    ...
    int Controller::RunOnCancel(fiber_token_t id, void *data, int error_code) {
        if (error_code == 0) {
            ...
            TURBO_CHECK_EQ(0, fiber_token_unlock_and_destroy(id));
            return 0;
        }
        ...
        fiber_id_t th;
        TURBO_CHECK_EQ(0, fiber_start_urgent(&th, nullptr, RunOnCancelThread::RunThis, arg));
        return 0;
    }

    void Controller::NotifyOnCancel(google::protobuf::Closure *callback) {
        if (nullptr == callback) {
            TURBO_LOG(WARNING) << "Parameter `callback' is NLLL";
            return;
        }

        ClosureGuard guard(callback);
        if (_oncancel_id != INVALID_FIBER_TOKEN) {
            TURBO_LOG(FATAL) << "NotifyCancel a single call more than once!";
            return;
        }
        if (fiber_token_create(&_oncancel_id, callback, RunOnCancel) != 0) {
            TURBO_PLOG(FATAL) << "Fail to create fiber_token";
            return;
        }
        SocketUniquePtr sock;
    ...
    void Controller::OnVersionedRPCReturned(const CompletionInfo &info,
        ...
        while (info.id != _correlation_id && info.id != current_id()) {
            ...
            TURBO_CHECK_EQ(0, fiber_token_unlock(info.id));
            return;
        }
        ...
        if (_error_code == EBACKUPREQUEST) {
            ...
            if (timeout_ms() >= 0) {
                rc = fiber_timer_add(
                        &_timeout_id,
                        turbo::time_point::from_unix_micros(_deadline_us).to_timespec(),
            ...
            TURBO_CHECK(_unfinished_call == nullptr);  // only one backup request now.
            ...
            return IssueRPC(turbo::get_current_time_micros());
            ...
            TURBO_CHECK_EQ(current_id(), info.id) << "error_code=" << _error_code;
            if (!SingleServer()) {
                if (_accessed == nullptr) {
                    _accessed = ExcludedServers::Create(
                            std::min(_max_retry, RETRY_AVOIDANCE));
...
//      entire RPC (specified by c->FailedInline()).
        ...
        if (sending_sock != nullptr) {
            ...
            if (enable_circuit_breaker) {
                sending_sock->FeedbackCircuitBreaker(error_code,
                                                     turbo::get_current_time_micros() - begin_time_us);
            }
        }
    ...
    void Controller::EndRPC(const CompletionInfo &info) {
        ...
        if (info.id == current_id() || info.id == _correlation_id) {
            ...
            if (_unfinished_call == nullptr) {
                TURBO_CHECK(false) << "A previous non-backup request responded, cid="
            ...
            if (_unfinished_call != nullptr) {
                ...
                if (get_id(_unfinished_call->nretry) == info.id) {
                    ...
                    TURBO_CHECK(false) << "A previous non-backup request responded";
    ...
    void Controller::IssueRPC(int64_t start_realtime_us) {
        ...
        if (_stream_creator) {
            ...
            TURBO_CHECK_EQ(_remote_side, tmp_sock->remote_side());
        }
        ...
        if (_auth != nullptr) {
            ...
            if (_current_call.sending_sock->FightAuthentication(&auth_error) == 0) {
                ...
                SetFailed(auth_error, "Fail to authenticate, %s",
                          turbo_error(auth_error));
                return HandleSendFailed();
            }
        }
        // Make request
        turbo::cord_buf packet;
        ...
        if (_connect_timeout_ms > 0) {
            if (_deadline_us >= 0) {
                connect_abstime = turbo::time_point::from_unix_micros(
    ...
    int Controller::HandleSocketFailed(fiber_token_t id, void *data, int error_code,
        ...
        if (error_code == ERPCTIMEDOUT) {
            cntl->SetFailed(error_code, "Reached timeout=%" PRId64 "ms @%s",
                            cntl->timeout_ms(),
                            turbo::endpoint2str(cntl->remote_side()).c_str());
            ...
            cntl->SetFailed(error_code, "%s @%s", turbo_error(error_code),
                            turbo::endpoint2str(cntl->remote_side()).c_str());
        }
    ...
    CallId Controller::call_id() {
        ...
        TURBO_CHECK_EQ(0, fiber_token_create2(&cid, this, HandleSocketFailed));
...
// TODO: Is percent encoding better?
    ...
    void Controller::set_stream_creator(StreamCreator *sc) {
        if (_stream_creator) {
            TURBO_LOG(FATAL) << "A StreamCreator has been set previously";
            return;
        }
        _stream_creator = sc;
    }
    ...
    Controller::CreateProgressiveAttachment(StopStyle stop_style) {
        if (has_progressive_writer()) {
            TURBO_LOG(ERROR) << "One controller can only have one ProgressiveAttachment";
            return nullptr;
        }
        if (_request_protocol != PROTOCOL_HTTP) {
            TURBO_LOG(ERROR) << "Only http supports ProgressiveAttachment now";
            return nullptr;
        }
        if (_current_call.sending_sock == nullptr) {
            TURBO_LOG(ERROR) << "sending_sock is nullptr";
            return nullptr;
        }
    ...
    void Controller::ReadProgressiveAttachmentBy(ProgressiveReader *r) {
        if (r == nullptr) {
            TURBO_LOG(FATAL) << "Param[r] is nullptr";
            return;
        }
        if (!is_response_read_progressively()) {
            return r->OnEndOfMessage(
                    turbo::result_status(EINVAL, "Can't read progressive attachment from a "
                                                      "controller without calling "
                                                      "response_will_be_read_progressively() before"));
        }
        if (_rpa == nullptr) {
            return r->OnEndOfMessage(
                    turbo::result_status(EINVAL, "ReadableProgressiveAttachment is nullptr"));
        }
        if (has_progressive_reader()) {
            return r->OnEndOfMessage(
                    turbo::result_status(EPERM, "{} can't be called more than once",
                                              __FUNCTION__));
        }
    ...
    static void RegisterQuitSignalOrDie() {
        ...
        if (prev != SIG_DFL &&
            ...
            if (prev == SIG_ERR) {
                TURBO_LOG(ERROR) << "Fail to register SIGINT, abort";
                abort();
            } else {
                s_prev_sigint_handler = prev;
                TURBO_LOG(WARNING) << "SIGINT was installed with " << prev;
            }
        }
        ...
        if (FLAGS_graceful_quit_on_sigterm) {
            ...
            if (prev != SIG_DFL &&
                ...
                if (prev == SIG_ERR) {
                    TURBO_LOG(ERROR) << "Fail to register SIGTERM, abort";
                    abort();
                } else {
                    s_prev_sigterm_handler = prev;
                    TURBO_LOG(WARNING) << "SIGTERM was installed with " << prev;
                }
            }
        }
    }
    ...
    google::protobuf::Closure *DoNothing() {
        return turbo::get_leaky_singleton<DoNothingClosure>();
    }
...
#define MELON_RPC_SESSION_END_MSG "Session ends."
#define MELON_RPC_REQ_ID "@rid"
#define MELON_RPC_KV_SEP "="
    ...
    void Controller::FlushSessionKV(std::ostream &os) {
        ...
        if (FLAGS_turbo_log_as_json) {
            if (pRID) {
                os << "\"" MELON_RPC_REQ_ID "\":\"" << *pRID << "\",";
            }
            os << "\"M\":\"" MELON_RPC_SESSION_END_MSG "\"";
            for (auto it = _session_kv->Begin(); it != _session_kv->End(); ++it) {
                os << ",\"" << it->first << "\":\"" << it->second << '"';
            }
        } else {
            if (pRID) {
                os << MELON_RPC_REQ_ID MELON_RPC_KV_SEP << *pRID << " ";
            }
            os << MELON_RPC_SESSION_END_MSG;
            for (auto it = _session_kv->Begin(); it != _session_kv->End(); ++it) {
                os << ' ' << it->first << MELON_RPC_KV_SEP << it->second;
            }
        }
    }
