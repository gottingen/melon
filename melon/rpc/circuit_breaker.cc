...
#define EPSILON (FLAGS_circuit_breaker_epsilon_value)
    ...
    CircuitBreaker::EmaErrorRecorder::EmaErrorRecorder(int window_size,
            ...
            : _window_size(window_size), _max_error_percent(max_error_percent),
              _smooth(std::pow(EPSILON, 1.0 / window_size)), _sample_count_when_initializing(0),
              _error_count_when_initializing(0), _ema_error_cost(0), _ema_latency(0) {
    }
