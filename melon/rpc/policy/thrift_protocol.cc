...
#include "melon/rpc/policy/thrift_protocol.h"
...
ReadThriftMessageBegin(melon::cord_buf* body,
    ...
    if (k != sizeof(version_and_len_buf) ) {
        return melon:result_status(-1, "Fail to copy {} bytes from body",
    ...
    *mtype = (apache::thrift::protocol::TMessageType)
    ...
    if (method_name_length > MAX_THRIFT_METHOD_NAME_LENGTH) {
        return melon:result_status(-1, "method_name_length={} is too long",
                             method_name_length);
    }
    ...
    if (k != sizeof(buf)) {
        return melon:result_status(-1, "Fail to cut {} bytes", sizeof(buf));
    }
...
void ProcessThriftRequest(InputMessageBase* msg_base) {
    ...
    uint32_t seq_id;
    ::apache::thrift::protocol::TMessageType mtype;
    melon:result_status st = ReadThriftMessageBegin(
...
void ProcessThriftResponse(InputMessageBase* msg_base) {
    ...
    do {
        ...
        melon:result_status st = ReadThriftMessageBegin(&msg->payload, &fname, &mtype, &seq_id);
        if (!st.is_ok()) {
        ...
        if (mtype == ::apache::thrift::protocol::T_EXCEPTION) {
            ...
            // TODO: Convert exception type to melon errors.
