...
// Modified from code in SRS2 (src/protocol/srs_rtmp_handshake.cpp:94)
            int openssl_HMACsha256(const void *key, int key_size,
                ...
                if (NULL == EVP_sha256) {
                    LOG_FIRST_N(ERROR, 1) << "Fail to find EVP_sha256, fall back to simple handshaking";
                    return -1;
                }
...
// Common part of C1 and S1.
            ...
            protected:
                bool ComputeDigest(void *digest) const { return ComputeDigestBase(GenuineFPKey, 30, digest); }
            };
...
// The common part of C2 and S2.
            ...
            public:
                bool Generate(const void *s1_digest) { return C2S2Base::Generate(GenuineFPKey, 62, s1_digest); }
            ...
            public:
                bool Generate(const void *c1_digest) { return C2S2Base::Generate(GenuineFMSKey, 68, c1_digest); }
            ...
            while (!buf->empty()) {
                ...
                if (nw < 0) {
                    ...
                    if (errno == EAGAIN) {
                        ...
                        // writable.
                        LOG_EVERY_N_SEC(ERROR, 1) << "Impossible: meet EAGAIN!";
                        fiber_usleep(1000);
                        continue;
                    }
                    return -1;
...
// double the increment of `_trans_id_allocator' on each try.
        ...
        static int SendAck(Socket *socket, uint64_t received_bytes) {
            const uint32_t data = turbo::ghtonl(received_bytes);
        ...
        RtmpChunkStream::ReadParams::ReadParams()
                : last_has_extended_ts(false), first_chunk_of_message(true), last_timestamp_delta(0),
        ...
        bool RtmpChunkStream::OnAudioMessage(
            ...
            if (!connection_context()->FindMessageStream(mh.stream_id, &stream)) {
                LOG_EVERY_N_SEC(WARNING, 1) << socket->remote_side()
        ...
        bool RtmpChunkStream::OnSharedObjectMessageAMF0(
            ...
            LOG_EVERY_N_SEC(ERROR, 1) << socket->remote_side() << ": Not implemented";
            return false;
        }
