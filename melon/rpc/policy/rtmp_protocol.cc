...
static mutil::IOBuf* s_rtmp_handshake_server_random = nullptr;
...
static mutil::IOBuf* s_rtmp_handshake_client_random = nullptr;
...
RtmpContext::~RtmpContext() {
    ...
    for (size_t i = 0; i < RTMP_CHUNK_ARRAY_1ST_SIZE; ++i) {
        ...
        if (p) {
            _cstream_ctx[i].store(nullptr, mutil::memory_order_relaxed);
            delete p;
        }
    }
...
RtmpContext::SubChunkArray::~SubChunkArray() {
    for (size_t i = 0; i < RTMP_CHUNK_ARRAY_2ND_SIZE; ++i) {
        ...
        if (stream) {
            ptrs[i].store(nullptr, mutil::memory_order_relaxed);
            delete stream;
        }
    }
}
...
void RtmpContext::ClearChunkStream(uint32_t cs_id) {
    ...
    delete sub_array->ptrs[index2].exchange(
        nullptr, mutil::memory_order_acquire);
}
...
bool RtmpChunkStream::OnCreateStream(const RtmpMessageHeader& mh,
    ...
    if (stream_name_field != nullptr && stream_name_field->IsString()) {
        stream_name = stream_name_field->AsString();
    }
    if (is_publish) {
