...
inline void PackMStdHeader(char* rpc_header, uint32_t meta_size, int payload_size) {
...
static void SerializeMStdHeaderAndMeta(
    ...
    if (meta_size <= 244) { // most common cases
        ...
        PackMStdHeader(header_and_meta, meta_size, payload_size);
        ...
        char header[12];
        PackMStdHeader(header, meta_size, payload_size);
...
ParseResult ParseMStdMessage(mutil::IOBuf* source, Socket* socket,
                            bool /*read_eof*/, const void*) {
    char header_buf[12];
    const size_t n = source->copy_to(header_buf, sizeof(header_buf));
    if (n >= 4) {
        void* dummy = header_buf;
        if (*(const uint32_t*)dummy != *(const uint32_t*)"MRPC") {
            return MakeParseError(PARSE_ERROR_TRY_OTHERS);
        }
    } else {
        if (memcmp(header_buf, "MRPC", n) != 0) {
            return MakeParseError(PARSE_ERROR_TRY_OTHERS);
        }
    }
...
void SendRpcResponse(int64_t correlation_id,
    ...
    mutil::IOBuf res_buf;
    SerializeMStdHeaderAndMeta(&res_buf, meta, res_size + attached_size);
    if (append_body) {
...
void MStdEndRunningCallMethodInPool(
    ::google::protobuf::Service* service,
    const ::google::protobuf::MethodDescriptor* method,
    ::google::protobuf::RpcController* controller,
    const ::google::protobuf::Message* request,
...
void ProcessMStdRequest(InputMessageBase* msg_base) {
    ...
    do {
        ...
        if (BeginRunningUserCode()) {
            ...
            return MStdEndRunningCallMethodInPool(
...
bool VerifyMStdRequest(const InputMessageBase* msg_base) {
...
void ProcessMStdResponse(InputMessageBase* msg_base) {
    const int64_t start_parse_us = mutil::cpuwide_time_us();
    DestroyingPtr<MostCommonMessage> msg(static_cast<MostCommonMessage*>(msg_base));
    RpcMeta meta;
    if (!ParsePbFromIOBuf(&meta, msg->meta)) {
...
void PackMStdRequest(mutil::IOBuf* req_buf,
                    SocketMessage**,
    ...
    SerializeMStdHeaderAndMeta(req_buf, meta, req_size + attached_size);
