...
public:
    ...
    // @SocketMessage
    mutil::Status AppendAndDestroySelf(mutil::IOBuf* out, Socket*);
};
...
public:
    ...
    // to be thread-safe.
    ParseResult Feed(mutil::IOBuf* source, Socket* socket);
    ...
    void SetState(const mutil::EndPoint& remote_side, State new_state);
    ...
    int SendConnectRequest(const mutil::EndPoint& remote_side, int fd, bool simplified_rtmp);

private:
    ParseResult WaitForC0C1orSimpleRtmp(mutil::IOBuf* source, Socket* socket);
    ParseResult WaitForC2(mutil::IOBuf* source, Socket* socket);
    ParseResult WaitForS0S1(mutil::IOBuf* source, Socket* socket);
    ParseResult WaitForS2(mutil::IOBuf* source, Socket* socket);
    ParseResult OnChunks(mutil::IOBuf* source, Socket* socket);
...
private:
    ...
    mutil::FlatMap<uint32_t, MessageStreamInfo> _mstream_map;

    // Mapping transaction id to handlers.
    mutil::Mutex _trans_mutex;
    uint32_t _trans_id_allocator;
    mutil::FlatMap<uint32_t, RtmpTransactionHandler*> _trans_map;
...
public:
    ...
    int SerializeMessage(mutil::IOBuf* buf, const RtmpMessageHeader& mh,
                         mutil::IOBuf* body);
    
    bool OnMessage(
        const RtmpBasicHeader& bh, const RtmpMessageHeader& mh,
        mutil::IOBuf* msg_body, Socket* socket);
    ...
    bool OnStreamBegin(const RtmpMessageHeader&,
                       const mutil::StringPiece& event_data, Socket* socket);
