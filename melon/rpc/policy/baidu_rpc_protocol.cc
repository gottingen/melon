...
inline void PackBRPCHeader(char* rpc_header, uint32_t meta_size, int payload_size) {
...
static void SerializeBRPCHeaderAndMeta(
    mutil::IOBuf* out, const RpcMeta& meta, int payload_size) {
    const uint32_t meta_size = GetProtobufByteSize(meta);
    if (meta_size <= 244) { // most common cases
        char header_and_meta[12 + meta_size];
        PackBRPCHeader(header_and_meta, meta_size, payload_size);
...
ParseResult ParseBRPCMessage(mutil::IOBuf* source, Socket* socket,
...
static void SendBRPCResponse(int64_t correlation_id,
                     Controller* cntl, 
    ...
    mutil::IOBuf res_buf;
    SerializeBRPCHeaderAndMeta(&res_buf, meta, res_size + attached_size);
    if (append_body) {
...
void ProcessBRPCRequest(InputMessageBase* msg_base) {
    ...
    if (sample) {
        ...
        sample->meta.set_protocol_type(PROTOCOL_MELON_STD);
    ...
    accessor.set_server(server)
        ...
        .set_request_protocol(PROTOCOL_MELON_STD)
    ...
    if (IsTraceable(request_meta.has_trace_id())) {
        ...
        span->set_protocol(PROTOCOL_MELON_STD);
    ...
    do {
        ...
        mutil::IOBuf req_buf;
        mutil::IOBuf* req_buf_ptr = &msg->payload;
        ...
        google::protobuf::Closure* done = ::melon::NewCallback<
            ...
            MethodStatus*, int64_t>(
                &SendBRPCResponse, meta.correlation_id(), cntl.get(), 
    ...
    // `cntl', `req' and `res' will be deleted inside `SendBRPCResponse'
    // `socket' will be held until response has been sent
    SendBRPCResponse(meta.correlation_id(), cntl.release(), 
                    req.release(), res.release(), server,
                    method_status, msg->received_us());
}

bool VerifyBRPCRequest(const InputMessageBase* msg_base) {
...
void ProcessBRPCResponse(InputMessageBase* msg_base) {
    const int64_t start_parse_us = mutil::cpuwide_time_us();
    DestroyingPtr<MostCommonMessage> msg(static_cast<MostCommonMessage*>(msg_base));
    RpcMeta meta;
    if (!ParsePbFromIOBuf(&meta, msg->meta)) {
    ...
    do {
        ...
        mutil::IOBuf* res_buf_ptr = &msg->payload;
        if (meta.has_attachment_size()) {
...
void PackBRPCRequest(mutil::IOBuf* req_buf,
                    SocketMessage**,
                    uint64_t correlation_id,
                    const google::protobuf::MethodDescriptor* method,
                    Controller* cntl,
                    const mutil::IOBuf& request_body,
    ...
    SerializeBRPCHeaderAndMeta(req_buf, meta, req_size + attached_size);
