...
namespace melon::rpc {
    ...
    namespace policy {
        ...
        CommonStrings::CommonStrings()
                : ACCEPT("accept"), DEFAULT_ACCEPT("*/*"), USER_AGENT("user-agent"),
                  DEFAULT_USER_AGENT("melon/1.0 curl/7.0"), CONTENT_TYPE("content-type"),
                  CONTENT_TYPE_TEXT("text/plain"), CONTENT_TYPE_JSON("application/json"),
                  CONTENT_TYPE_PROTO("application/proto"), CONTENT_TYPE_SPRING_PROTO("application/x-protobuf"),
                  ERROR_CODE("x-bd-error-code"), AUTHORIZATION("authorization"), ACCEPT_ENCODING("accept-encoding"),
                  CONTENT_ENCODING("content-encoding"), GZIP("gzip"), CONNECTION("connection"),
                  KEEP_ALIVE("keep-alive"), CLOSE("close"), LOG_ID("log-id"), DEFAULT_METHOD("default_method"),
                  NO_METHOD("no_method"), H2_SCHEME(":scheme"), H2_SCHEME_HTTP("http"), H2_SCHEME_HTTPS("https"),
                  H2_AUTHORITY(":authority"), H2_PATH(":path"), H2_STATUS(":status"), STATUS_200("200"),
                  H2_METHOD(":method"), METHOD_GET("GET"), METHOD_POST("POST"), TE("te"), TRAILERS("trailers"),
                  GRPC_ENCODING("grpc-encoding"), GRPC_ACCEPT_ENCODING("grpc-accept-encoding"),
        ...
        void ProcessHttpResponse(InputMessageBase *msg) {
            ...
            do {
                ...
                if (content_type == HTTP_CONTENT_PROTO) {
                    if (!ParsePbFromCordBuf(cntl->response(), res_body)) {
        ...
        public:
            ...
            HttpResponseSender(HttpResponseSender &&s)
                    : _cntl(std::move(s._cntl)), _req(std::move(s._req)), _res(std::move(s._res)),
                      _method_status(std::move(s._method_status)), _received_us(s._received_us),
...
// put it into `unresolved_path'
        ...
        ParseResult ParseHttpMessage(melon::cord_buf *source, Socket *socket,
            ...
            if (read_eof) {
                ...
                rc = http_imsg->ParseFromCordBuf(*source);
            }
