    ...
    namespace policy {
        ...
        void PackStreamMessage(turbo::cord_buf *out,
            ...
            *(uint32_t *) dummy = *(const uint32_t *) "STRM";
            turbo::raw_packer(head + 4)
                    .pack32(data_length + meta_length)
                    .pack32(meta_length);
            out->append(head, TURBO_ARRAY_SIZE(head));
            turbo::cord_buf_as_zero_copy_output_stream wrapper(out);
            TURBO_CHECK(fm.SerializeToZeroCopyStream(&wrapper));
            if (data != nullptr) {
                out->append(*data);
            }
        }

        ParseResult ParseStreamingMessage(turbo::cord_buf *source,
            ...
            if (TURBO_UNLIKELY(meta_size > body_size)) {
            ...
            do {
                ...
                if (!ParsePbFromCordBuf(&fm, meta_buf)) {
                    TURBO_LOG(WARNING) << "Fail to Parse StreamFrameMeta from " << *socket;
                    break;
                }
                SocketUniquePtr ptr;
