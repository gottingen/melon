...
AutoConcurrencyLimiter::AutoConcurrencyLimiter()
    ...
    , _min_latency_us(-1)
    , _ema_max_qps(-1)
    , _explore_ratio(turbo::get_flag(FLAGS_auto_cl_max_explore_ratio))
...
int64_t AutoConcurrencyLimiter::NextResetTime(int64_t sampling_time_us) {
    int64_t reset_start_us = sampling_time_us + 
        ...
        mutil::fast_rand_less_than(turbo::get_flag(FLAGS_auto_cl_noload_latency_remeasure_interval_ms) / 2)) * 1000;
...
void AutoConcurrencyLimiter::AdjustMaxConcurrency(int next_max_concurrency) {
    next_max_concurrency = std::max(turbo::get_flag(FLAGS_fiber_concurrency), next_max_concurrency);
