...
// The done to sub channels.
        ...
        public:
            explicit SubDone(Sender *owner)
                    : _owner(owner), _cid(INVALID_FIBER_ID), _peer_id(INVALID_SOCKET_ID) {
            }
...
// ===============================================
        ...
        int ChannelBalancer::AddChannel(ChannelBase *sub_channel,
            ...
            options.health_check_interval_s = turbo::get_flag(FLAGS_channel_check_interval);
...
// ===================================
        ...
        Sender::Sender(Controller *cntl,
                       ...
                       google::protobuf::Message *response,
                       google::protobuf::Closure *user_done)
                : _main_cntl(cntl), _request(request), _response(response), _user_done(user_done), _nfree(0),
                  _nalloc(0), _finished(false), _sub_done0(this) {
        }
