...
// version of RPC.
// Since the code for getting MELON_RPC_REVISION often fails,
// MELON_RPC_REVISION must be defined to string and be converted to number
// within our code.
// The code running before main() may see g_rpc_version=0, should be OK.
#if defined(MELON_RPC_REVISION)
    const int64_t g_rpc_version = atoll(MELON_RPC_REVISION);
#else
    ...
    static void TrackMeNow(std::unique_lock<pthread_mutex_t> &mu) {
        ...
        google::protobuf::Closure *done =
                ::melon::rpc::NewCallback(&HandleTrackMeResponse, cntl, res);
