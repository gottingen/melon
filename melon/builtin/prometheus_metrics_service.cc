...
namespace melon {
    ...
    PrometheusMetricsDumper::ProcessLatencyRecorderSuffix(const std::string_view &name,
        ...
        static std::string latency_names[] = {
                ...
                mutil::string_printf("_latency_%d", (int) turbo::get_flag(FLAGS_var_latency_p3)),
    ...
    bool PrometheusMetricsDumper::DumpLatencyRecorderSuffix(
        ...
        *_os << "# HELP " << si->metric_name << '\n'
             ...
             << (double) (turbo::get_flag(FLAGS_var_latency_p1)) / 100 << "\"} "
             << si->latency_percentiles[0] << '\n'
             << si->metric_name << "{quantile=\""
             << (double) (turbo::get_flag(FLAGS_var_latency_p2)) / 100 << "\"} "
             << si->latency_percentiles[1] << '\n'
             << si->metric_name << "{quantile=\""
             << (double) (turbo::get_flag(FLAGS_var_latency_p3)) / 100 << "\"} "
    ...
    int DumpPrometheusMetricsToIOBuf(mutil::IOBuf *output) {
        ...
        if (turbo::get_flag(FLAGS_var_max_dump_multi_dimension_metric_number) > 0) {
