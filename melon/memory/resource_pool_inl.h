...
#ifndef MELON_MEMORY_RESOURCE_POOL_INL_H_
#define MELON_MEMORY_RESOURCE_POOL_INL_H_
...
private:
    ...
    inline LocalPool* get_or_new_local_pool() {
        ...
        _local_pool = lp;
        melon::thread::atexit(LocalPool::delete_local_pool, lp);
...
private:
    ...
    static melon::static_atomic<ResourcePool*> _singleton;
    static pthread_mutex_t _singleton_mutex;
    static MELON_THREAD_LOCAL LocalPool* _local_pool;
    static melon::static_atomic<long> _nlocal;
    static melon::static_atomic<size_t> _ngroup;
    static pthread_mutex_t _block_group_mutex;
    static pthread_mutex_t _change_thread_mutex;
    static melon::static_atomic<BlockGroup*> _block_groups[RP_MAX_BLOCK_NGROUP];

    std::vector<DynamicFreeChunk*> _free_chunks;
    pthread_mutex_t _free_chunks_mutex;

#ifdef MELON_RESOURCE_POOL_NEED_FREE_ITEM_NUM
    static melon::static_atomic<size_t> _global_nfree;
#endif
};
...
template <typename T>
melon::static_atomic<long> ResourcePool<T>::_nlocal = MELON_STATIC_ATOMIC_INIT(0);

template <typename T>
melon::static_atomic<size_t> ResourcePool<T>::_ngroup = MELON_STATIC_ATOMIC_INIT(0);
...
template <typename T>
melon::static_atomic<typename ResourcePool<T>::BlockGroup*>
ResourcePool<T>::_block_groups[RP_MAX_BLOCK_NGROUP] = {};

#ifdef MELON_RESOURCE_POOL_NEED_FREE_ITEM_NUM
template <typename T>
melon::static_atomic<size_t> ResourcePool<T>::_global_nfree = MELON_STATIC_ATOMIC_INIT(0);
#endif
...
#endif  // MELON_MEMORY_RESOURCE_POOL_INL_H_
