...
namespace {
    ...
    inline void *allocate_pages(size_t count) {
        ...
        MELON_CHECK(mapping != MAP_FAILED) << "Failed to allocate" << int(count) << " pages";
    ...
    inline void freePages(void *ptr, size_t count) {
        ...
        (void) res;
        MELON_CHECK(res == 0) << "Failed to free " << int(count) << " pages at " << ptr;
    }

    inline void protect_page(void *addr) {
        auto res = mprotect(addr, page_size(), PROT_NONE);
        (void) res;
        MELON_CHECK(res == 0) << "Failed to protect page at " << addr;
    }
}  // anonymous namespace
...
inline void* allocate_pages(size_t count) {
  ...
  zx_handle_close(vmo);
  (void)status;
  MELON_CHECK(status == ZX_OK, "Failed to allocate %d pages", int(count));
...
inline void freePages(void* ptr, size_t count) {
  ...
  (void)status;
  MELON_CHECK(status == ZX_OK, "Failed to free %d pages at %p", int(count),
              ptr);
}
inline void protect_page(void* addr) {
  ...
  (void)status;
  MELON_CHECK(status == ZX_OK, "Failed to protect page at %p", addr);
}
}  // anonymous namespace
#elif defined(_WIN32)
...
inline void* allocate_pages(size_t count) {
  ...
  MELON_CHECK(mapping != nullptr, "Failed to allocate %d pages", int(count));
  return mapping;
}
inline void freePages(void* ptr, size_t count) {
  (void)count;
  ...
  (void)res;
  MELON_CHECK(res != 0, "Failed to free %d pages at %p", int(count), ptr);
}
inline void protect_page(void* addr) {
  DWORD oldVal = 0;
  auto res = VirtualProtect(addr, page_size(), PAGE_NOACCESS, &oldVal);
  (void)res;
  MELON_CHECK(res != 0, "Failed to protect page at %p", addr);
}
}  // anonymous namespace
#else
...
// The pointer returned must be freed with pagedFree().
    void *pagedMalloc(size_t alignment,
                      ...
                      bool guardHigh) {
        (void) alignment;
        MELON_CHECK(alignment < page_size());
    ...
    inline void *aligned_malloc(size_t alignment, size_t size) {
        ...
        auto aligned = reinterpret_cast<uint8_t *>(melon::base::align_up(
    ...
    class default_allocator : public melon::allocator {
    public:
        static default_allocator instance;

        virtual melon::allocation allocate(
                const melon::allocation::required_info &request) override {
            void *ptr = nullptr;
            ...
            MELON_CHECK(ptr != nullptr)<<"allocation failed";
            MELON_CHECK(reinterpret_cast<uintptr_t>(ptr) % request.alignment == 0)<<"allocation gave incorrect alignment";

            melon::allocation allocation;
            allocation.ptr = ptr;
            allocation.request = request;
            return allocation;
        }

        virtual void free(const melon::allocation &allocation) override {
