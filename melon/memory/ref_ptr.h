...
#ifndef MELON_MEMORY_REF_PTR_H_
#define MELON_MEMORY_REF_PTR_H_
...
namespace melon {
    ...
    // is subject to change. UNLESS YOU'RE A DEVELOPER OF MELON, YOU MUST NOT RELY
    // ON THIS.
    ...
    struct ref_traits<ref_counted<T, Deleter>> {
        static void reference(ref_counted<T, Deleter> *ptr) noexcept {
            //MELON_DCHECK_GT(ptr->unsafe_ref_count(), 0);
            ptr->add_ref();
        }
...
namespace std {
    ...
    // Specialization for `std::atomic<melon::ref_ptr<T>>`.
    //
    // @sa: https://en.cppreference.com/w/cpp/memory/shared_ptr/atomic2
    template<class T>
    class atomic<melon::ref_ptr<T>> {
        using Traits = melon::ref_traits<T>;
    ...
    public:
        ...
        // Receiving a pointer.
        constexpr /* implicit */ atomic(melon::ref_ptr<T> ptr) noexcept
                : ptr_(ptr.leak()) {}

        atomic &operator=(melon::ref_ptr<T> ptr) noexcept {
            store(std::move(ptr));
            return *this;
        }
        ...
        void store(melon::ref_ptr<T> ptr,
        ...
        melon::ref_ptr<T> load(
                std::memory_order order = std::memory_order_seq_cst) const noexcept {
            return melon::ref_ptr<T>(melon::ref_ptr_v, ptr_.load(order));
        }

        // Same as `load()`.
        operator melon::ref_ptr<T>() const noexcept { return load(); }

        // Exchanges with a (possibly) different ref-ptr.
        melon::ref_ptr<T> exchange(
                melon::ref_ptr<T> ptr,
                std::memory_order order = std::memory_order_seq_cst) noexcept {
            return melon::ref_ptr(melon::adopt_ptr_v, ptr_.exchange(ptr.leak(), order));
        }

        // Compares if this atomic holds the `expected` pointer, and exchanges it with
        // the new `desired` one if the comparsion holds.
        bool compare_exchange_strong(
                melon::ref_ptr<T> &expected, melon::ref_ptr<T> desired,
                std::memory_order order = std::memory_order_seq_cst) noexcept {
        ...
        bool compare_exchange_strong(melon::ref_ptr<T> &expected,
                                     melon::ref_ptr<T> desired,
        ...
        bool compare_exchange_weak(melon::ref_ptr<T> &expected,
                                   melon::ref_ptr<T> desired,
                                   std::memory_order success,
                                   std::memory_order failure) noexcept {
            return compare_exchange_impl(
  ...
  void notify_all() noexcept { ptr_.notify_all(); }
    ...
    private:
        ...
        bool compare_exchange_impl(F &&f, melon::ref_ptr<T> &expected,
                                   melon::ref_ptr<T> desired, Orders... orders) {
            ...
            if (std::forward<F>(f)(current, desired.get(), orders...)) {
                ...
                melon::ref_ptr(melon::adopt_ptr_v, current);
                return true;
            }
