...
// Combine two circular linked list into one.
    ...
    class Collector : public melon::variable_reducer<Collected *, CombineCollected> {
    public:
        Collector();
    ...
    void Collector::grab_thread() {
        ...
        melon::per_second<melon::status_gauge<double> > busy_seconds_second(
                "variable_collector_grab_thread_usage", &busy_seconds_var);

        melon::status_gauge<int64_t> ngrab_var(deref_value<int64_t>, &_ngrab);
        melon::per_second<melon::status_gauge<int64_t> > ngrab_second(
...
// Call user's callbacks in this thread.
    void Collector::dump_thread() {
        ...
        melon::status_gauge<int64_t> ndumped_var(deref_value<int64_t>, &_ndump);
