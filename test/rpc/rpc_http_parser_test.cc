...
using melon::rpc::http_parser;
using melon::rpc::http_parser_init;
using melon::rpc::http_parser_settings;
...
TEST_F(HttpParserTest, init_perf) {
    ...
    for (size_t i = 0; i < loops; ++i) {
        http_parser parser;
        http_parser_init(&parser, melon::rpc::HTTP_REQUEST);
    }
    timer.stop();
