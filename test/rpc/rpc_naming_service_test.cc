...
bool IsIPListEqual(const std::set<melon::base::ip_t>& s1, const std::set<melon::base::ip_t>& s2) {
    if (s1.size() != s2.size()) {
        return false;
    }
...
TEST(NamingServiceTest, sanity) {
    ...
    melon::rpc::policy::DomainNamingService dns;
    ...
    const std::set<melon::base::ip_t> expected_ips{servers[0].addr.ip, servers[1].addr.ip};

    ASSERT_EQ(0, dns.GetServers("baidu.com", &servers));
    ASSERT_EQ(2u, servers.size());
    const std::set<melon::base::ip_t> ip_list1{servers[0].addr.ip, servers[1].addr.ip};
    ...
    ASSERT_EQ(2u, servers.size());
    const std::set<melon::base::ip_t> ip_list2{servers[0].addr.ip, servers[1].addr.ip};
    ...
    ASSERT_EQ(2u, servers.size());
    const std::set<melon::base::ip_t> ip_list3{servers[0].addr.ip, servers[1].addr.ip};
...
TEST(NamingServiceTest, remotefile) {
    ...
    melon::base::end_point n2;
    ASSERT_EQ(0, melon::base::str2endpoint("0.0.0.0:8636", &n2));
    ...
    expected_servers.push_back(melon::rpc::ServerNode(n1, "tag3"));
    expected_servers.push_back(melon::rpc::ServerNode(n2));
    std::sort(expected_servers.begin(), expected_servers.end());

    std::vector<melon::rpc::ServerNode> servers;
    melon::rpc::policy::RemoteFileNamingService rfns;
...
TEST(NamingServiceTest, consul_with_backup_file) {
    melon::rpc::policy::FLAGS_consul_enable_degrade_to_file_naming_service = true;
    const int saved_hc_interval = melon::rpc::FLAGS_health_check_interval;
    melon::rpc::FLAGS_health_check_interval = 1;
    const char *address_list[] =  {
        "10.127.0.1:1234",
        "10.128.0.1:1234",
        "10.129.0.1:1234",
    ...
    std::string restful_map(melon::rpc::policy::FLAGS_consul_service_discovery_url);
    restful_map.append("/");
    ...
    melon::base::end_point n1;
    ASSERT_EQ(0, melon::base::str2endpoint("10.121.36.189:8003", &n1));
    melon::base::end_point n2;
    ASSERT_EQ(0, melon::base::str2endpoint("10.121.36.190:8003", &n2));
    std::vector<melon::rpc::ServerNode> expected_servers;
    ...
    melon::rpc::FLAGS_health_check_interval = saved_hc_interval;
}
...
TEST(NamingServiceTest, discovery_sanity) {
    melon::rpc::policy::FLAGS_discovery_api_addr = "http://127.0.0.1:8635/discovery/nodes";
    melon::rpc::policy::FLAGS_discovery_renew_interval_s = 1;
    melon::rpc::Server server;
    ...
    melon::rpc::policy::DiscoveryRegisterParam dparam;
    ...
    dparam.version = "v1";
    {
        melon::rpc::policy::DiscoveryClient dc;
    }
