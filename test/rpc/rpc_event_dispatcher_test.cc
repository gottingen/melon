...
TEST_F(EventDispatcherTest, versioned_ref) {
    ...
    versioned_ref.fetch_add(melon::rpc::MakeVRef(0, -1),
                            std::memory_order_release);
    ASSERT_EQ(melon::rpc::MakeVRef(1, 1), versioned_ref);
}
...
struct MELON_CACHELINE_ALIGNMENT SocketExtra : public melon::rpc::SocketUser {
    ...
    virtual void BeforeRecycle(melon::rpc::Socket *m) {
    ...
    static int OnEdgeTriggeredEventOnce(melon::rpc::Socket *m) {
        ...
        do {
            ...
            if (n == 0
                ...
                ++e->times;
#ifdef MELON_RPC_SOCKET_HAS_EOF
                if ((size_t)n < e->buf_cap && melon::rpc::has_epollrdhup) {
                    break;
                }
#endif
            } else {
    ...
    static void OnEdgeTriggeredEvents(melon::rpc::Socket *m) {
        int progress = melon::rpc::Socket::PROGRESS_INIT;
        do {
...
TEST_F(EventDispatcherTest, dispatch_tasks) {
    ...
    for (size_t i = 0; i < NCLIENT; ++i) {
        ...
        ASSERT_EQ(0, melon::rpc::Socket::Create(options, &socket_id));
        cm[i] = new ClientMeta;
        cm[i]->fd = fds[i * 2 + 1];
        cm[i]->times = 0;
        cm[i]->bytes = 0;
