...
extern mutil::atomic<bool> g_rdma_available;
extern bool g_skip_rdma_init;
}
}

static std::string g_ip = "127.0.0.1";
static mutil::EndPoint g_ep;
...
protected:
    RdmaTest() {
        mutil::ip_t ip;
        EXPECT_EQ(0, mutil::str2ip(g_ip.c_str(), &ip));
        mutil::EndPoint ep(ip, PORT);
        g_ep = ep;
        EXPECT_EQ(0, _server_list.save(mutil::endpoint2str(g_ep).c_str()));
...
TEST_F(RdmaTest, client_hello_msg_invalid_magic_str) {
    ...
    memcpy(data, "PRPC", 4);  // send as normal melon_std protocol
    memset(data + 4, 0, 32);
...
TEST_F(RdmaTest, client_close_during_hello_send) {
    ...
    mutil::fd_guard sockfd1(socket(AF_INET, SOCK_STREAM, 0));
    ...
    mutil::fd_guard sockfd2(socket(AF_INET, SOCK_STREAM, 0));
    ...
    mutil::fd_guard sockfd3(socket(AF_INET, SOCK_STREAM, 0));
...
TEST_F(RdmaTest, client_hello_msg_invalid_len) {
    ...
    ASSERT_EQ(rdma::RdmaEndpoint::UNINIT, s->_rdma_ep->_state);
    memcpy(data, "RDMA", 4);
    uint16_t len = mutil::HostToNet16(35);
    memcpy(data + 4, &len, 2);
    memset(data + 6, 0, 32);
...
TEST_F(RdmaTest, client_hello_msg_invalid_version) {
    ...
    Socket* s = NULL;
    uint8_t data[RDMA_HELLO_MSG_LEN];
    uint16_t len = mutil::HostToNet16(38);
    uint16_t ver = mutil::HostToNet16(1);
    ...
    usleep(100000);  // wait for server to handle the msg
    ASSERT_EQ(rdma::RdmaEndpoint::S_ACK_WAIT, s->_rdma_ep->_state);
    uint32_t flag = mutil::HostToNet32(1);
...
TEST_F(RdmaTest, client_close_during_ack_send) {
    ...
    usleep(100000);  // wait for server to handle the msg
    ASSERT_EQ(rdma::RdmaEndpoint::S_ACK_WAIT, s->_rdma_ep->_state);
    uint32_t flags = mutil::HostToNet32(0);
...
TEST_F(RdmaTest, server_miss_before_hello_send) {
    mutil::fd_guard sockfd(mutil::tcp_listen(g_ep));
    ...
    mutil::fd_guard acc_fd(accept(sockfd, NULL, NULL));
    ASSERT_TRUE(acc_fd >= 0);
    fiber_session_join(cntl.call_id());
...
TEST_F(RdmaTest, server_hello_invalid_version) {
    ...
    ASSERT_EQ(0, mutil::NetToHost32(*tmp));
    fiber_session_join(cntl.call_id());
...
TEST_F(RdmaTest, server_miss_after_ack) {
    ...
    ASSERT_EQ(4, read(acc_fd, data, 4));
    uint32_t* tmp = (uint32_t*)data;
    ASSERT_EQ(1, mutil::NetToHost32(*tmp));
    fiber_session_join(cntl.call_id());
...
TEST_F(RdmaTest, try_global_disable_rdma) {
    StartServer();
    rdma::g_rdma_available.store(false, mutil::memory_order_relaxed);
    ...
    StopServer();
    rdma::g_rdma_available.store(true, mutil::memory_order_relaxed);
}
...
TEST_F(RdmaTest, channel_option_invalid) {
    ...
    // ssl and rdma are incompatible
    chan_options.protocol = "melon_std";
...
TEST_F(RdmaTest, send_rpcs_in_one_qp) {
    ...
    for (int i = 0; i < RPC_NUM; ++i) {
        fiber_session_join(cntl[i].call_id());
...
TEST_F(RdmaTest, send_rpc_in_many_qp) {
    ...
    test::EchoResponse res[RPC_NUM];
    mutil::ip_t ip;
    mutil::str2ip(g_ip.c_str(), &ip);
    for (int i = 0; i < RPC_NUM; ++i) {
        svr[i] = &server[i % num];
        mutil::EndPoint ep(ip, 8000 + ((port++) % num));
...
TEST_F(RdmaTest, send_rpcs_with_user_defined_iobuf) {
    ...
    fiber_session_join(cntl[0].call_id());
