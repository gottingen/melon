...
namespace {
using melon::rpc::policy::FindMethodPropertyByURI;
using melon::rpc::policy::ParseHttpServerAddress;

TEST(HttpMessageTest, http_method) {
    ASSERT_STREQ("DELETE", melon::rpc::HttpMethod2Str(melon::rpc::HTTP_METHOD_DELETE));
    ASSERT_STREQ("GET", melon::rpc::HttpMethod2Str(melon::rpc::HTTP_METHOD_GET));
    ASSERT_STREQ("POST", melon::rpc::HttpMethod2Str(melon::rpc::HTTP_METHOD_POST));
    ASSERT_STREQ("PUT", melon::rpc::HttpMethod2Str(melon::rpc::HTTP_METHOD_PUT));

    melon::rpc::HttpMethod m;
    ASSERT_TRUE(melon::rpc::Str2HttpMethod("DELETE", &m));
    ASSERT_EQ(melon::rpc::HTTP_METHOD_DELETE, m);
    ASSERT_TRUE(melon::rpc::Str2HttpMethod("GET", &m));
    ASSERT_EQ(melon::rpc::HTTP_METHOD_GET, m);
    ASSERT_TRUE(melon::rpc::Str2HttpMethod("POST", &m));
    ASSERT_EQ(melon::rpc::HTTP_METHOD_POST, m);
    ASSERT_TRUE(melon::rpc::Str2HttpMethod("PUT", &m));
    ASSERT_EQ(melon::rpc::HTTP_METHOD_PUT, m);
    ...
    // non-existed
    ASSERT_FALSE(melon::rpc::Str2HttpMethod("DEL", &m));
    ASSERT_FALSE(melon::rpc::Str2HttpMethod("DELETE ", &m));
    ASSERT_FALSE(melon::rpc::Str2HttpMethod("GOT", &m));
}
...
TEST(HttpMessageTest, request_sanity) {
    ...
    const melon::rpc::HttpHeader& header = http_message.header();
    // Check all keys
    ...
    ASSERT_EQ(melon::rpc::HTTP_METHOD_POST, header.method());
    ASSERT_EQ(melon::rpc::HTTP_STATUS_OK, header.status_code());
...
TEST(HttpMessageTest, response_sanity) {
    ...
    ASSERT_EQ(melon::rpc::HTTP_METHOD_DELETE, header.method());
    ASSERT_EQ(melon::rpc::HTTP_STATUS_GONE, header.status_code());
    ASSERT_STREQ(melon::rpc::HttpReasonPhrase(header.status_code()), /*not GoneBlah*/
...
TEST(HttpMessageTest, find_method_property_by_uri) {
    ...
    mp = FindMethodPropertyByURI("/melon.rpc.flags/$*",
...
TEST(HttpMessageTest, http_header) {
    ...
    ASSERT_EQ(melon::rpc::HTTP_METHOD_GET, header.method());
    header.set_method(melon::rpc::HTTP_METHOD_POST);
    ASSERT_EQ(melon::rpc::HTTP_METHOD_POST, header.method());

    ASSERT_EQ(melon::rpc::HTTP_STATUS_OK, header.status_code());
    ASSERT_STREQ(melon::rpc::HttpReasonPhrase(header.status_code()),
                 header.reason_phrase());
    header.set_status_code(melon::rpc::HTTP_STATUS_CONTINUE);
    ASSERT_EQ(melon::rpc::HTTP_STATUS_CONTINUE, header.status_code());
    ASSERT_STREQ(melon::rpc::HttpReasonPhrase(header.status_code()),
                 header.reason_phrase());
    
    header.set_status_code(melon::rpc::HTTP_STATUS_GONE);
    ASSERT_EQ(melon::rpc::HTTP_STATUS_GONE, header.status_code());
    ASSERT_STREQ(melon::rpc::HttpReasonPhrase(header.status_code()),
                 header.reason_phrase());
}
...
TEST(HttpMessageTest, serialize_http_request) {
    ...
    header.set_method(melon::rpc::HTTP_METHOD_POST);
    turbo::end_point ep;
    ASSERT_EQ(0, turbo::str2endpoint("127.0.0.1:1234", &ep));
    turbo::cord_buf request;
    ...
    header.set_method(melon::rpc::HTTP_METHOD_GET);
    MakeRawHttpRequest(&request, &header, ep, &content);
...
TEST(HttpMessageTest, serialize_http_response) {
    ...
    header.SetHeader("Foo", "Bar");
    header.set_method(melon::rpc::HTTP_METHOD_POST);
    turbo::cord_buf response;
    turbo::cord_buf content;
    content.append("data");
    MakeRawHttpResponse(&response, &header, &content);
    ASSERT_EQ("HTTP/1.1 200 OK\r\nContent-Length: 4\r\nFoo: Bar\r\n\r\ndata", response);
    // content is cleared.
    TURBO_CHECK(content.empty());
