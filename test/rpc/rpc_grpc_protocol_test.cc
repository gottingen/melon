...
public:
    void Method(::google::protobuf::RpcController* cntl_base,
        ...
        if (req->return_error()) {
            cntl->SetFailed(melon::rpc::EINTERNAL, "%s", g_prefix.c_str());
            return;
        }
    ...
    void MethodTimeOut(::google::protobuf::RpcController* cntl_base,
              ...
              ::google::protobuf::Closure* done) {
        melon::rpc::ClosureGuard done_guard(done);
        melon::fiber_sleep_for(2000000 /*2s*/);
        res->set_message(g_prefix + req->message());
        return;
    }
};
...
protected:
    GrpcTest() {
        EXPECT_EQ(0, _server.AddService(&_svc, melon::rpc::SERVER_DOESNT_OWN_SERVICE));
    ...
    void CallMethod(bool req_gzip, bool res_gzip) {
        ...
        melon::rpc::Controller cntl;
        if (req_gzip) {
            cntl.set_request_compress_type(melon::rpc::COMPRESS_TYPE_GZIP);
        }
...
TEST_F(GrpcTest, percent_encode) {
    ...
    melon::rpc::PercentEncode(s1, &out);
    ...
    melon::rpc::PercentEncode(s2, &out);
...
TEST_F(GrpcTest, percent_decode) {
    ...
    melon::rpc::PercentDecode(s1, &out);
    EXPECT_TRUE(out == s1_out) << s1_out << " vs " << out;
    ...
    melon::rpc::PercentDecode(s2, &out);
    EXPECT_TRUE(out == s2_expected_out) << s2_expected_out << " vs " << out;
}
...
TEST_F(GrpcTest, RpcTimedOut) {
    ...
    EXPECT_TRUE(cntl.Failed());
    EXPECT_EQ(cntl.ErrorCode(), melon::rpc::ERPCTIMEDOUT);
}
