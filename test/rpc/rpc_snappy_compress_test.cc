...
typedef bool (*Compress)(const google::protobuf::Message &, turbo::cord_buf *);

typedef bool (*Decompress)(const turbo::cord_buf &, google::protobuf::Message *);
...
static bool SnappyDecompressCordBuf(char *input, size_t len, turbo::cord_buf *buf) {
    size_t decompress_length;
    if (!turbo::snappy::GetUncompressedLength(input, len, &decompress_length)) {
        return false;
    }
    char *output = new char[decompress_length];
    if (!turbo::snappy::RawUncompress(input, len, output)) {
        delete[] output;
        return false;
    }
...
TEST_F(test_compress_method, snappy_iobuf) {
    turbo::cord_buf buf, output_buf, check_buf;
...
TEST_F(test_compress_method, snappy_test) {
    ...
    ASSERT_TRUE(turbo::snappy::Compress(text, len, &output));
    size_t com_len1 = output.size();
    const char *s_text = "123456";
    ASSERT_TRUE(turbo::snappy::Compress(s_text, strlen(s_text), &append_string));
    output.append(append_string);
    std::string uncompress_str;
    std::string uncompress_str_t;
    char *ptr = const_cast<char *>(output.c_str());
    ASSERT_TRUE(turbo::snappy::Uncompress(ptr, com_len1, &uncompress_str));
    ptr = const_cast<char *>(append_string.c_str());
    ASSERT_TRUE(turbo::snappy::Uncompress(ptr, strlen(ptr), &uncompress_str_t));
    delete[] text;
}
...
TEST_F(test_compress_method, throughput_compare) {
    ...
    for (size_t num = 0; num < TURBO_ARRAY_SIZE(len_subs); ++num) {
        len = len_subs[num];
        snappy_message::SnappyMessageProto old_msg;
        char *text = new char[len + 1];
        for (int j = 0; j < len;) {
...
TEST_F(test_compress_method, mass_snappy_iobuf) {
    ...
    turbo::cord_buf output_buf, check_buf;
    ASSERT_TRUE(melon::rpc::policy::SnappyCompress(buf, &output_buf));
