...
typedef bool (*Compress)(const google::protobuf::Message &, melon::cord_buf *);

typedef bool (*Decompress)(const melon::cord_buf &, google::protobuf::Message *);
...
static bool SnappyDecompressCordBuf(char *input, size_t len, melon::cord_buf *buf) {
    size_t decompress_length;
    if (!melon::snappy::GetUncompressedLength(input, len, &decompress_length)) {
        return false;
    }
    char *output = new char[decompress_length];
    if (!melon::snappy::RawUncompress(input, len, output)) {
        delete[] output;
        return false;
    }
...
TEST_F(test_compress_method, snappy) {
    ...
    melon::cord_buf buf;
    ASSERT_TRUE(melon::rpc::policy::SnappyCompress(old_msg, &buf));
    snappy_message::SnappyMessageProto new_msg;
    ASSERT_TRUE(melon::rpc::policy::SnappyDecompress(buf, &new_msg));
...
TEST_F(test_compress_method, snappy_iobuf) {
    melon::cord_buf buf, output_buf, check_buf;
    const char *test = "this is a test";
    buf.append(test, strlen(test));
    ASSERT_TRUE(melon::rpc::policy::SnappyCompress(buf, &output_buf));
    ASSERT_TRUE(melon::rpc::policy::SnappyDecompress(output_buf, &check_buf));
...
TEST_F(test_compress_method, snappy_test) {
    ...
    ASSERT_TRUE(melon::snappy::Compress(text, len, &output));
    size_t com_len1 = output.size();
    const char *s_text = "123456";
    ASSERT_TRUE(melon::snappy::Compress(s_text, strlen(s_text), &append_string));
    output.append(append_string);
    std::string uncompress_str;
    std::string uncompress_str_t;
    char *ptr = const_cast<char *>(output.c_str());
    ASSERT_TRUE(melon::snappy::Uncompress(ptr, com_len1, &uncompress_str));
    ptr = const_cast<char *>(append_string.c_str());
    ASSERT_TRUE(melon::snappy::Uncompress(ptr, strlen(ptr), &uncompress_str_t));
    delete[] text;
}
...
TEST_F(test_compress_method, throughput_compare) {
    ...
    for (size_t num = 0; num < MELON_ARRAY_SIZE(len_subs); ++num) {
        len = len_subs[num];
        snappy_message::SnappyMessageProto old_msg;
        char *text = new char[len + 1];
        for (int j = 0; j < len;) {
        ...
        CompressMessage("Snappy", k, old_msg, len,
                        melon::rpc::policy::SnappyCompress,
                        melon::rpc::policy::SnappyDecompress);
        CompressMessage("Gzip", k, old_msg, len,
                        melon::rpc::policy::GzipCompress,
                        melon::rpc::policy::GzipDecompress);
        CompressMessage("Zlib", k, old_msg, len,
                        melon::rpc::policy::ZlibCompress,
                        melon::rpc::policy::ZlibDecompress);
        printf("\n");
        delete[] text;
    }
}
...
TEST_F(test_compress_method, mass_snappy_iobuf) {
    ...
    melon::cord_buf output_buf, check_buf;
    ASSERT_TRUE(melon::rpc::policy::SnappyCompress(buf, &output_buf));
