...
#define REDIS_SERVER_PORT "6479"
    ...
    void AssertReplyEqual(const melon::rpc::RedisReply &reply1,
                          const melon::rpc::RedisReply &reply2) {
        if (&reply1 == &reply2) {
            return;
        }
        ...
        switch (reply1.type()) {
            case melon::rpc::REDIS_REPLY_ARRAY:
                ...
                break;
            case melon::rpc::REDIS_REPLY_INTEGER:
                ASSERT_EQ(reply1.integer(), reply2.integer());
                break;
            case melon::rpc::REDIS_REPLY_NIL:
                break;
            case melon::rpc::REDIS_REPLY_STRING:
                // fall through
            case melon::rpc::REDIS_REPLY_STATUS:
                ASSERT_NE(reply1.c_str(), reply2.c_str()); // from different arena
                ASSERT_EQ(reply1.data(), reply2.data());
                break;
            case melon::rpc::REDIS_REPLY_ERROR:
    ...
    void AssertResponseEqual(const melon::rpc::RedisResponse &r1,
                             const melon::rpc::RedisResponse &r2,
    ...
    TEST_F(RedisTest, keys_with_spaces) {
        ...
        ASSERT_EQ(melon::rpc::REDIS_REPLY_STATUS, response.reply(0).type());
        ASSERT_EQ("OK", response.reply(0).data());
        ASSERT_EQ(melon::rpc::REDIS_REPLY_STATUS, response.reply(1).type());
        ASSERT_EQ("OK", response.reply(1).data());
        ASSERT_EQ(melon::rpc::REDIS_REPLY_STATUS, response.reply(2).type());
        ASSERT_EQ("OK", response.reply(2).data());
        ASSERT_EQ(melon::rpc::REDIS_REPLY_STRING, response.reply(3).type());
        ASSERT_EQ("he1 he1 da1", response.reply(3).data());
        ASSERT_EQ(melon::rpc::REDIS_REPLY_STRING, response.reply(4).type());
        ...
        ASSERT_EQ(melon::rpc::REDIS_REPLY_STRING, response.reply(6).type());
        ASSERT_EQ("he3 he3 da3", response.reply(6).data());

        melon::rpc::RedisResponse response2 = response;
    ...
    TEST_F(RedisTest, incr_and_decr) {
        ...
        ASSERT_EQ(melon::rpc::REDIS_REPLY_INTEGER, response.reply(0).type());
        ASSERT_EQ(1, response.reply(0).integer());
        ASSERT_EQ(melon::rpc::REDIS_REPLY_INTEGER, response.reply(1).type());
        ASSERT_EQ(0, response.reply(1).integer());
        ASSERT_EQ(melon::rpc::REDIS_REPLY_INTEGER, response.reply(2).type());
        ASSERT_EQ(10, response.reply(2).integer());
        ASSERT_EQ(melon::rpc::REDIS_REPLY_INTEGER, response.reply(3).type());
    ...
    TEST_F(RedisTest, auth) {
        ...
        // pass does not work on redis 6.0.6)
        {
            ...
            options.protocol = melon::rpc::PROTOCOL_REDIS;
            melon::rpc::Channel channel;
            melon::rpc::policy::RedisAuthenticator *auth =
                    new melon::rpc::policy::RedisAuthenticator(passwd1.c_str());
            options.auth = auth;
            ASSERT_EQ(0, channel.Init("0.0.0.0:" REDIS_SERVER_PORT, &options));
            melon::rpc::RedisRequest request;
            melon::rpc::RedisResponse response;
        ...
        // Auth with passwd2
        {
            ...
            options.protocol = melon::rpc::PROTOCOL_REDIS;
            melon::rpc::policy::RedisAuthenticator *auth =
                    new melon::rpc::policy::RedisAuthenticator(passwd2.c_str());
            options.auth = auth;
            melon::rpc::Channel channel;
            ASSERT_EQ(0, channel.Init("0.0.0.0:" REDIS_SERVER_PORT, &options));
            melon::rpc::RedisRequest request;
    ...
    TEST_F(RedisTest, command_parser) {
        ...
        melon::Arena arena;
        {
            ...
            ASSERT_TRUE(melon::rpc::RedisCommandNoFormat(&buf, command.c_str()).is_ok());
        ...
        }
        {
            ...
            buf.append("*3\r\n$3");
            ASSERT_EQ(melon::rpc::PARSE_ERROR_NOT_ENOUGH_DATA, parser.Consume(buf, &command_out, &arena));
            ASSERT_EQ((int) buf.size(), 2);    // left "$3"
            buf.append("\r\nset\r\n:123\r\n$3\r\ndef\r\n");
            ASSERT_EQ(melon::rpc::PARSE_ERROR_ABSOLUTELY_WRONG, parser.Consume(buf, &command_out, &arena));
            parser.Reset();
        }
        {
            // not array
            buf.append(":123456\r\n");
            ASSERT_EQ(melon::rpc::PARSE_ERROR_TRY_OTHERS, parser.Consume(buf, &command_out, &arena));
            parser.Reset();
        }
        {
            // not array
    ...
    TEST_F(RedisTest, redis_reply_codec) {
        ...
        // status
        {
            ...
            melon::rpc::RedisReply r2(&arena);
            melon::rpc::ParseError err = r2.ConsumePartialCordBuf(buf);
            ASSERT_EQ(err, melon::rpc::PARSE_OK);
        ...
        // string
        {
            ...
            ASSERT_STREQ("verylongstring verylongstring verylongstring verylongstring int:123 str:foobar fp:3.21",
                         r.c_str());

            melon::rpc::RedisReply r3(&arena);
        ...
        // array
        {
            ...
            melon::rpc::RedisReply r2(&arena);
            ASSERT_EQ(r2.ConsumePartialCordBuf(buf), melon::rpc::PARSE_OK);
            ...
            appender.move_to(buf);
            ASSERT_STREQ(buf.to_string().c_str(), "*-1\r\n");
            ASSERT_EQ(r.ConsumePartialCordBuf(buf), melon::rpc::PARSE_OK);
    ...
    public:
        ...
        melon::rpc::RedisCommandHandlerResult OnBatched(const std::vector<std::string_view> &args,
            ...
            if (flush_batched) {
                ...
                return melon::rpc::REDIS_CMD_BATCHED;
            }
        }

        void DoSet(const std::string &key, const std::string &value, melon::rpc::RedisReply *output) {
            m[key] = value;
            output->SetStatus("OK");
        }

        void DoGet(const std::string &key, melon::rpc::RedisReply *output) {
            auto it = m.find(key);
            if (it != m.end()) {
    ...
    public:
        SetCommandHandler(RedisServiceImpl *rs, bool batch_process = false)
                : _rs(rs), _batch_process(batch_process) {}
    ...
    class IncrCommandHandler : public melon::rpc::RedisCommandHandler {
    public:
        IncrCommandHandler() {}
    ...
    TEST_F(RedisTest, server_sanity) {
        ...
        melon::rpc::PortRange pr(8081, 8900);
        ...
        ASSERT_EQ(melon::rpc::REDIS_REPLY_STATUS, response.reply(4).type());
        ...
        ASSERT_EQ(melon::rpc::REDIS_REPLY_STATUS, response.reply(1).type());
        ASSERT_STREQ("OK", response.reply(1).c_str());
        ASSERT_EQ(melon::rpc::REDIS_REPLY_STRING, response.reply(2).type());
    ...
    void *incr_thread(void *arg) {
        melon::rpc::Channel *c = static_cast<melon::rpc::Channel *>(arg);
    ...
    TEST_F(RedisTest, server_concurrency) {
        ...
        std::vector<melon::rpc::Channel *> channels;
        for (int i = 0; i < N; ++i) {
    ...
    class MultiCommandHandler : public melon::rpc::RedisCommandHandler {
    public:
        MultiCommandHandler() {}
        ...
        melon::rpc::RedisCommandHandlerResult Run(const std::vector<std::string_view> &args,
            ...
            output->SetStatus("OK");
            return melon::rpc::REDIS_CMD_CONTINUE;
        }
        ...
        class MultiTransactionHandler : public melon::rpc::RedisCommandHandler {
        public:
            melon::rpc::RedisCommandHandlerResult Run(const std::vector<std::string_view> &args,
                                                      melon::rpc::RedisReply *output,
                                                      bool flush_batched) {
    ...
    TEST_F(RedisTest, server_command_continue) {
        ...
        }
        {
            ...
            ASSERT_EQ(melon::rpc::REDIS_REPLY_STATUS, response.reply(0).type());
            ASSERT_STREQ("OK", response.reply(0).c_str());
            ASSERT_EQ(melon::rpc::REDIS_REPLY_ERROR, response.reply(1).type());
            for (int i = 2; i < count + 2; ++i) {
                ASSERT_EQ(melon::rpc::REDIS_REPLY_STATUS, response.reply(i).type());
                ASSERT_STREQ("QUEUED", response.reply(i).c_str());
            }
            const melon::rpc::RedisReply &m = response.reply(count + 2);
