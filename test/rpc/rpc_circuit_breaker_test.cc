...
int main(int argc, char* argv[]) {
    melon::rpc::FLAGS_circuit_breaker_short_window_size = kShortWindowSize;
    melon::rpc::FLAGS_circuit_breaker_long_window_size = kLongWindowSize;
    melon::rpc::FLAGS_circuit_breaker_short_window_error_percent = kShortWindowErrorPercent;
    melon::rpc::FLAGS_circuit_breaker_long_window_error_percent = kLongWindowErrorPercent;
    melon::rpc::FLAGS_circuit_breaker_min_isolation_duration_ms = kMinIsolationDurationMs;
    melon::rpc::FLAGS_circuit_breaker_max_isolation_duration_ms = kMaxIsolationDurationMs;
...
struct FeedbackControl {
    FeedbackControl(int req_num, int error_percent,
                 melon::rpc::CircuitBreaker* circuit_breaker)
        : _req_num(req_num)
    ...
    int _req_num;
    int _error_percent;
    melon::rpc::CircuitBreaker* _circuit_breaker;
    int _healthy_cnt;
    int _unhealthy_cnt;
    bool _healthy;
};
...
TEST_F(CircuitBreakerTest, maximum_isolation_duration) {
    melon::rpc::FLAGS_circuit_breaker_max_isolation_duration_ms =
        melon::rpc::FLAGS_circuit_breaker_min_isolation_duration_ms + 1;
    ASSERT_LT(melon::rpc::FLAGS_circuit_breaker_max_isolation_duration_ms,
              2 * melon::rpc::FLAGS_circuit_breaker_min_isolation_duration_ms);
    std::vector<pthread_t> thread_list;
    std::vector<std::unique_ptr<FeedbackControl>> fc_list;
