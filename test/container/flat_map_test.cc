...
namespace {
    ...
    TEST_F(FlatMapTest, initialization_of_values) {
        ...
        melon::container::FlatMap<int, Foo> map;
        ...
        // Zeroize POD values.
        melon::container::FlatMap<int, Bar> map2;
    ...
    TEST_F(FlatMapTest, swap_pooled_allocator) {
        melon::container::details::PooledAllocator<int, 128> a1;
        a1.allocate(1);
        const void *p1 = a1._pool._blocks;

        melon::container::details::PooledAllocator<int, 128> a2;
        a2.allocate(1);
    ...
    TEST_F(FlatMapTest, copy_flat_map) {
        ...
        for (size_t i = 0; i < MELON_ARRAY_SIZE(bcs); ++i) {
            Map m5;
    ...
    TEST_F(FlatMapTest, seek_by_string_piece) {
        melon::container::FlatMap<std::string, int> m;
    ...
    TEST_F(FlatMapTest, to_lower) {
        for (int c = -128; c < 128; ++c) {
            ASSERT_EQ((char) ::tolower(c), melon::ascii::to_lower(c)) << "c=" << c;
        }
        ...
        tm2.stop();
        MELON_LOG(INFO) << "tm1=" << tm1.n_elapsed()
                        << " tm2=" << tm2.n_elapsed()
    ...
    TEST_F(FlatMapTest, __builtin_ctzl_perf) {
        ...
        tm1.stop();
        MELON_LOG(INFO) << "__builtin_ctzl takes " << tm1.n_elapsed() / (double) N << "ns";
    }

    TEST_F(FlatMapTest, case_ignored_map) {
        melon::container::CaseIgnoredFlatMap<int> m1;
    ...
    TEST_F(FlatMapTest, case_ignored_set) {
        melon::container::CaseIgnoredFlatSet s1;
    ...
    TEST_F(FlatMapTest, make_sure_all_methods_compile) {
        typedef melon::container::FlatMap<int, long> M1;
        M1 m1;
        ASSERT_EQ(0, m1.init(32));
        ...
        typedef melon::container::FlatSet<int> S1;
        S1 s1;
        ASSERT_EQ(0, s1.init(32));
    ...
    TEST_F(FlatMapTest, flat_map_of_string) {
        ...
        melon::container::FlatMap<std::string, size_t> m1;
        std::map<std::string, size_t> m2;
        melon::container::hash_map<std::string, size_t> m3;
        const size_t N = 10000;
        ASSERT_EQ(0, m1.init(N));
        melon::stop_watcher tm1, tm1_2, tm2, tm3;
        size_t sum = 0;
        keys.reserve(N);
        for (size_t i = 0; i < N; ++i) {
            keys.push_back(melon::string_printf("up_latency_as_key_%lu", i));
        }
        ...
        tm3.stop();
        MELON_LOG(INFO) << "inserting strings takes " << tm1.n_elapsed() / N
                        << " " << tm2.n_elapsed() / N
                        << " " << tm3.n_elapsed() / N;
        ...
        tm3.stop();
        MELON_LOG(INFO) << "finding strings takes " << tm1.n_elapsed() / N
        ...
        MELON_LOG(INFO) << "finding c_strings takes " << tm1.n_elapsed() / N
                        << " " << tm2.n_elapsed() / N << " " << tm3.n_elapsed() / N
                        << " " << tm1_2.n_elapsed() / N;
    ...
    TEST_F(FlatMapTest, fast_iterator) {
        typedef melon::container::FlatMap<uint64_t, uint64_t> M1;
        typedef melon::container::SparseFlatMap<uint64_t, uint64_t> M2;
        ...
        MELON_LOG(INFO) << "m1.insert=" << tm1.n_elapsed() / (double) N
        ...
        tm2.stop();
        MELON_LOG(INFO) << "m1.iterate=" << tm1.n_elapsed() / (double) N
    ...
    static void list_flat_map(std::vector<Key> *keys,
                              const melon::container::FlatMap<Key, Value> &map,
                              size_t max_one_pass,
                              OnPause &on_pause) {
        keys->clear();
        typedef melon::container::FlatMap<Key, Value> Map;
        size_t n = 0;
    ...
    typedef melon::container::FlatMap<uint64_t, uint64_t> PositionHintMap;
    ...
    static void fill_position_hint_map(PositionHintMap *map,
        ...
        MELON_LOG(INFO) << map->bucket_info();
    }
    ...
    struct RemoveInsertVisitedOnPause {
        ...
        melon::container::FlatSet<uint64_t> removed_keys;
        melon::container::FlatSet<uint64_t> inserted_keys;
    ...
    TEST_F(FlatMapTest, erase_hinted_during_iteration) {
        ...
        MELON_LOG_IF(INFO, keys_out.size() != old_keys_out_size)
    ...
    TEST_F(FlatMapTest, perf_cmp_with_map_storing_pointers) {
        const size_t REP = 4;
        int *ptr[2048];
        for (size_t i = 0; i < MELON_ARRAY_SIZE(ptr); ++i) {
            ptr[i] = new int;
        }

        std::set<int *> m1;
        melon::container::FlatSet<int *, PointerHasher<int> > m2;
        melon::container::hash_set<int *, PointerHasher<int> > m3;

        std::vector<int *> r;
        int sum;
        melon::stop_watcher tm;

        r.reserve(MELON_ARRAY_SIZE(ptr) * REP);
        ASSERT_EQ(0, m2.init(MELON_ARRAY_SIZE(ptr)));
        ...
        tm.stop();
        MELON_LOG(INFO) << "std::set takes " << tm.n_elapsed() / r.size();
    ...
    TEST_F(FlatMapTest, key_value_are_not_constructed_before_first_insertion) {
        melon::container::FlatMap<Key, Value, KeyHasher, KeyEqualTo> m;
    ...
    TEST_F(FlatMapTest, manipulate_uninitialized_map) {
        melon::container::FlatMap<int, int> m;
    ...
    TEST_F(FlatMapTest, perf_small_string_map) {
        ...
        melon::stop_watcher tm2;
        melon::stop_watcher tm3;
        melon::stop_watcher tm4;

        for (int i = 0; i < 10; ++i) {
            tm3.start();
            melon::container::PooledMap<std::string, std::string> m3;
            ...
            tm4.start();
            melon::container::CaseIgnoredFlatMap<std::string> m4;
            m4.init(16);
            ...
            tm1.start();
            melon::container::FlatMap<std::string, std::string> m1;
            m1.init(16);
            ...
            MELON_LOG(INFO) << "flatmap=" << tm1.n_elapsed()
    ...
    TEST_F(FlatMapTest, sanity) {
        typedef melon::container::FlatMap<uint64_t, long> Map;
        Map m;
    ...
    TEST_F(FlatMapTest, random_insert_erase) {
        ...
        {
            melon::container::hash_map<uint64_t, Value> ref[2];
            typedef melon::container::FlatMap<uint64_t, Value> Map;
            Map ht[2];
            ht[0].init(40);
            ht[1] = ht[0];
            ...
            for (int j = 0; j < 30; ++j) {
                ...
                MELON_LOG(INFO) << "Check j=" << j;
                // bi-check
                for (int i = 0; i < 2; ++i) {
                    for (Map::iterator it = ht[i].begin(); it != ht[i].end(); ++it) {
                        melon::container::hash_map<uint64_t, Value>::iterator it2 = ref[i].find(it->first);
        ...
        MELON_LOG(INFO) << "n_con:" << n_con << std::endl
    ...
    void perf_insert_erase(bool random, const T &value) {
        ...
        const size_t NPASS = MELON_ARRAY_SIZE(nkeys);

        std::vector<uint64_t> keys;
        melon::container::FlatMap<uint64_t, T> id_map;
        std::map<uint64_t, T> std_map;
        melon::container::PooledMap<uint64_t, T> pooled_map;
        melon::container::hash_map<uint64_t, T> hash_map;
        melon::stop_watcher id_tm, std_tm, pooled_tm, hash_tm;
        ...
        MELON_LOG(INFO) << "[ value = " << sizeof(T) << " bytes ]";
        for (size_t pass = 0; pass < NPASS; ++pass) {
            ...
            MELON_LOG(INFO) << (random ? "Randomly" : "Sequentially")
                            << " inserting " << keys.size()
                            << " into FlatMap/std::map/melon::container::PooledMap/melon::container::hash_map takes "
            ...
            MELON_LOG(INFO) << (random ? "Randomly" : "Sequentially")
                            << " erasing " << keys.size()
                            << " from FlatMap/std::map/melon::container::PooledMap/melon::container::hash_map takes "
                            << id_tm.n_elapsed() / keys.size()
                            << "/" << std_tm.n_elapsed() / keys.size()
                            << "/" << pooled_tm.n_elapsed() / keys.size()
                            << "/" << hash_tm.n_elapsed() / keys.size();
    ...
    void perf_seek(const T &value) {
        ...
        for (size_t pass = 0; pass < NPASS; ++pass) {
            ...
            MELON_LOG(INFO) << "Seeking " << keys.size()
                            << " from FlatMap/std::map/melon::container::PooledMap/melon::container::hash_map takes "
                            << id_tm.n_elapsed() / keys.size()
                            << "/" << std_tm.n_elapsed() / keys.size()
                            << "/" << pooled_tm.n_elapsed() / keys.size()
    ...
    TEST_F(FlatMapTest, copy) {
        melon::container::FlatMap<int, int> m1;
        melon::container::FlatMap<int, int> m2;
        ASSERT_EQ(0, m1.init(32));
        m1[1] = 1;
        m1[2] = 2;
        m2 = m1;
