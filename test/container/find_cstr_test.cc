...
namespace {
    ...
    TEST_F(FindCstrTest, sanity) {
        ...
        t1["hello"] = 0xdeadbeef;
        std::map<std::string, int>::iterator it = melon::container::find_cstr(t1, "hello");
    ...
    TEST_F(FindCstrTest, perf) {
        ...
        for (size_t i = 0; i < all_keys.size(); ++i) {
            sum += melon::container::find_cstr(t1, all_keys[i])->second;
        }
        tm.stop();
        ...
        MELON_LOG(INFO) << "elp1=" << elp1 / N << " elp2=" << elp2 / N;
    }
