...
TEST(Btree, InitializerListConstructor) {
  ...
  abel::btree_map<int, int> map({{1, 5}, {2, 10}});
  EXPECT_EQ(map[1], 5);
  EXPECT_EQ(map[2], 10);

  abel::btree_multimap<int, int> mmap({{1, 5}, {1, 10}});
...
TEST(Btree, MapAt) {
  abel::btree_map<int, int> map = {{1, 2}, {2, 4}};
...
TEST(Btree, ExtractAndInsertNodeHandleMap) {
  abel::btree_map<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
  auto nh = src1.extract(src1.find(3));
...
TEST(Btree, ExtractAndInsertNodeHandleMultiMap) {
  abel::btree_multimap<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
  auto nh = src1.extract(src1.find(3));
  EXPECT_THAT(src1, ElementsAre(Pair(1, 2), Pair(5, 6)));
...
struct IntCompareToCmp {
  abel::weak_ordering operator()(int a, int b) const {
    if (a < b) return abel::weak_ordering::less;
    if (a > b) return abel::weak_ordering::greater;
...
TEST(Btree, MergeIntoMultiMapsWithDifferentComparators) {
  abel::btree_map<int, int, IntCompareToCmp> src1 = {{1, 1}, {2, 2}, {3, 3}};
...
TEST(Btree, TryEmplaceWithBadHint) {
  abel::btree_map<int, int> m = {{1, 1}, {9, 9}};
