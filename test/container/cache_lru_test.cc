...
#include "melon/container/lru_cache.h"
...
namespace testing {
    TEST(TestCache, TestSet) {
        melon::cache_config config;
        config.worker_sleep_ms_ = 0;
        melon::lru_cache<int, int> cache(config);
        cache.start();
        {
    ...
    TEST(TestCache, TestPromoteTableFull) {
        ...
        config.promote_per_times_ = 3;
        config.item_expire_sec_ = melon::cache_config::kDefaultCacheItemExpireSec;
        config.item_gen_time_threshold_ms_ = melon::cache_config::kDefaultGenItemTimeThresholdMs;
    ...
    TEST(TestCache, TestWithRamPolicyDump) {
        melon::lru_cache<uint32_t, uint32_t> cache;
    ...
    TEST(TestCache, TestWithRamPolicyGc) {
        ...
        config.promote_per_times_ = 3;
        config.worker_sleep_ms_ = 0;
        melon::lru_cache<uint32_t, uint32_t> cache(config);
