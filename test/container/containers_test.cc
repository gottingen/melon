...
#include "melon/container/containers.h"
...
TEST_F(ContainersVectorTest, Empty) {
    melon::containers::vector<std::string, 4> vector(allocator);
...
TEST_F(ContainersVectorTest, BeyondFixedCapIndex) {
    melon::containers::vector<std::string, 1> vector(allocator);
    vector.resize(4);
    vector[0] = "A";
    vector[1] = "B";
    vector[2] = "C";
...
TEST_F(ContainersVectorTest, BeyondFixedCapPushPop) {
    melon::containers::vector<std::string, 2> vector(allocator);
    vector.push_back("A");
    vector.push_back("B");
    vector.push_back("C");
    vector.push_back("D");
...
TEST_F(ContainersVectorTest, CopyConstruct) {
    melon::containers::vector<std::string, 4> vectorA(allocator);
    ...
    melon::containers::vector<std::string, 4> vectorB(vectorA, allocator);
...
TEST_F(ContainersVectorTest, CopyConstructDifferentBaseCapacity) {
    ...
    melon::containers::vector<std::string, 2> vectorB(vectorA, allocator);
    ASSERT_EQ(vectorB.size(), size_t(3));
    ASSERT_EQ(vectorB[0], "A");
    ASSERT_EQ(vectorB[1], "B");
    ASSERT_EQ(vectorB[2], "C");
...
TEST_F(ContainersVectorTest, CopyAssignment) {
    ...
    melon::containers::vector<std::string, 4> vectorB(allocator);
    vectorB = vectorA;
    ASSERT_EQ(vectorB.size(), size_t(3));
    ASSERT_EQ(vectorB[0], "A");
    ASSERT_EQ(vectorB[1], "B");
...
TEST_F(ContainersVectorTest, CopyAssignmentDifferentBaseCapacity) {
    ...
    melon::containers::vector<std::string, 2> vectorB(allocator);
    vectorB = vectorA;
    ASSERT_EQ(vectorB.size(), size_t(3));
    ASSERT_EQ(vectorB[0], "A");
    ASSERT_EQ(vectorB[1], "B");
...
TEST_F(ContainersVectorTest, MoveConstruct) {
    ...
    melon::containers::vector<std::string, 2> vectorB(std::move(vectorA),
                                                      allocator);
    ASSERT_EQ(vectorB.size(), size_t(3));
    ASSERT_EQ(vectorB[0], "A");
    ASSERT_EQ(vectorB[1], "B");
...
TEST_F(ContainersListTest, Empty) {
    melon::containers::list<std::string> list(allocator);
