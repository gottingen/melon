...
namespace testing {
    TEST(TestRamPolicy, TestOnCacheNormalSetAndDel) {
        ...
        melon::ram_cache_policy<uint32_t, uint32_t> p(64, callback);
    ...
    TEST(TestRamPolicy, TestInitWithInvalidPram) {
        uint32_t i = 0;
        std::function<void(void)> callback = [&]() { i++; };
        melon::ram_cache_policy<uint32_t, uint32_t> p(0, callback);
    ...
    TEST(TestRamPolicy, TestConcurrentSetAndDel) {
        ...
        melon::ram_cache_policy<uint32_t, uint32_t> p(256, callback);
    ...
    TEST(TestRamPolicy, TestOnCacheDiyEstimator) {
        ...
        uint32_t i = 0;
        std::function<void(void)> callback = [&]() { i++; };
        melon::ram_cache_policy<uint32_t, std::vector<uint32_t>, melon::ram_usage<uint32_t>, VectorRamUsage> p(
                1 << 25, callback);
