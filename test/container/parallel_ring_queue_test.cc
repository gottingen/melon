...
namespace testing {
    TEST(TestParallelRingQueue, TestQueue) {
        melon::parallel_ring_queue<int> queue;
    ...
    TEST(TestQueue, TestConcurrent) {
        melon::parallel_ring_queue<int> q(1000);
    ...
    TEST(TestQueue, TestNoConcurrent) {
        melon::parallel_ring_queue<int> q(64);
    ...
    TEST(TestQueue, TestFront) {
        melon::parallel_ring_queue<int> q(10);
        for (int i = 0; i < 15; i++) {
            q.push_back(i);
        }
    ...
    TEST(TestQueue, TestFixCapacity) {
        ...
        {
            melon::parallel_ring_queue<int> q(200);
            EXPECT_EQ(q.fix_capacity(200), 256);
        }
        {
            melon::parallel_ring_queue<int> q(0);
            EXPECT_EQ(q.fix_capacity(0), 1024);
        }
        {
            melon::parallel_ring_queue<int> q(static_cast<uint32_t>(1 << 31) + 2);
    ...
    TEST(TestQueue, TestPowerOfTwoForSize) {
        ...
        {
            melon::parallel_ring_queue<int> q(1);
    ...
    TEST(TestQueue, TestHighestOneBit) {
        ...
        {
            melon::parallel_ring_queue<int> q(17);
