...
#include "melon/container/single_threaded_pool.h"
#include <new>
#include <map>
...
// Specialize for void
        ...
        private:
            melon::container::SingleThreadedPool<sizeof(T1), BLOCK_SIZE, 1> _pool;
        };
...
namespace std {
    template<class T1, size_t BLOCK_SIZE>
    inline void swap(::melon::container::details::PooledAllocator<T1, BLOCK_SIZE> &lhs,
                     ::melon::container::details::PooledAllocator<T1, BLOCK_SIZE> &rhs) {
        lhs.swap(rhs);
    }
}  // namespace std
