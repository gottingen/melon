...
namespace melon {
    namespace priv {
        ...
        struct StatefulTestingHash
                : melon::priv::hash_testing_internal::WithId<
                        StatefulTestingHash> {
            template<class T>
            size_t operator()(const T &t) const {
                return melon::hash<T>{}(t);
            }
        };
...
namespace std {
    ...
    // ----------------------------------------------------------------
    template<>
    struct hash<melon::priv::NonStandardLayout> {
        std::size_t operator()(melon::priv::NonStandardLayout const &p) const {
            return std::hash<std::string>()(p.value);
        }
    };
}

// MELON_MAP_UNORDERED_SUPPORTS_ALLOC_CTORS is false for glibcxx versions
...
#define MELON_MAP_UNORDERED_SUPPORTS_ALLOC_CTORS 0
#else
#define MELON_MAP_UNORDERED_SUPPORTS_ALLOC_CTORS 1
#endif
