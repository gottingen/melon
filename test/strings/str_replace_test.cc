...
TEST(string_replace_all, ManyReplacements) {
    ...
    s = abel::string_replace_all("abc", {{"a", "x"},
                                         ...
                                         {"c", "z"}});
    EXPECT_EQ(s, "xyz");
    s = abel::string_replace_all("zxy", {{"z", "x"},
                                         {"x", "y"},
                                         {"y", "z"}});
    EXPECT_EQ(s, "xyz");
    ...
    s = abel::string_replace_all("aaa", {{"aa", "x"},
                                         {"a",  "X"}});
    EXPECT_EQ(s, "xX");
    s = abel::string_replace_all("aaa", {{"a",  "X"},
                                         {"aa", "x"}});
    EXPECT_EQ(s, "xX");
...
TEST(string_replace_all, Inplace) {
    ...
    // Replace entire std::string, one char at a time
    s = "abc";
    reps = abel::string_replace_all({{"a", "x"},
                                     {"b", "y"},
                                     {"c", "z"}}, &s);
    EXPECT_EQ(reps, 3);
    EXPECT_EQ(s, "xyz");
    s = "zxy";
    reps = abel::string_replace_all({{"z", "x"},
                                     {"x", "y"},
                                     {"y", "z"}}, &s);
    EXPECT_EQ(reps, 3);
    EXPECT_EQ(s, "xyz");
    ...
    // Replace multiple times with varying lengths of original/replacement.
    s = "ababa";
    reps = abel::string_replace_all({{"a", "xxx"},
                                     {"b", "XXXX"}}, &s);
    EXPECT_EQ(reps, 5);
    EXPECT_EQ(s, "xxxXXXXxxxXXXXxxx");
    ...
    // Overlapping matches are replaced greedily.
    s = "aaa";
    reps = abel::string_replace_all({{"aa", "x"},
                                     {"a",  "X"}}, &s);
    EXPECT_EQ(reps, 2);
    EXPECT_EQ(s, "xX");
    s = "aaa";
