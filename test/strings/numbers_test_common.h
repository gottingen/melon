...
MELON_FORCE_INLINE bool Itoa(IntType value, int base, std::string *destination) {
    destination->clear();
    if (base <= 1 || base > 36) {
        return false;
    }
...
MELON_FORCE_INLINE const std::array<uint32_test_case, 27> &strtouint32_test_cases() {
...
MELON_FORCE_INLINE const std::array<uint64_test_case, 34> &strtouint64_test_cases() {
