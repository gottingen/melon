 ...
 *****************************************************************/
#include "melon/strings/internal/char_map.h"
...
namespace {
    ...
    constexpr melon::strings_internal::Charmap everything_map =
            ~melon::strings_internal::Charmap();
    constexpr melon::strings_internal::Charmap nothing_map{};

    TEST(Charmap, AllTests) {
        const melon::strings_internal::Charmap also_nothing_map("", 0);
        ...
        const melon::strings_internal::Charmap symbols("&@#@^!@?", 5);
        ...
        const melon::strings_internal::Charmap lets("^abcde", 3);
        const melon::strings_internal::Charmap lets2("fghij\0klmnop", 10);
        const melon::strings_internal::Charmap lets3("fghij\0klmnop");
    ...
    namespace {
        std::string Members(const melon::strings_internal::Charmap &m) {
            std::string r;
    ...
    TEST(Charmap, Constexpr) {
        constexpr melon::strings_internal::Charmap kEmpty = nothing_map;
        EXPECT_THAT(Members(kEmpty), "");
        constexpr melon::strings_internal::Charmap kA =
                melon::strings_internal::Charmap::Char('A');
        EXPECT_THAT(Members(kA), "A");
        constexpr melon::strings_internal::Charmap kAZ =
                melon::strings_internal::Charmap::Range('A', 'Z');
        EXPECT_THAT(Members(kAZ), "ABCDEFGHIJKLMNOPQRSTUVWXYZ");
        constexpr melon::strings_internal::Charmap kIdentifier =
                melon::strings_internal::Charmap::Range('0', '9') |
                melon::strings_internal::Charmap::Range('A', 'Z') |
                melon::strings_internal::Charmap::Range('a', 'z') |
                melon::strings_internal::Charmap::Char('_');
        EXPECT_THAT(Members(kIdentifier),
                    "0123456789"
                    "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                    "_"
                    "abcdefghijklmnopqrstuvwxyz");
        constexpr melon::strings_internal::Charmap kAll = everything_map;
        for (size_t i = 0; i < 256; ++i) {
            EXPECT_TRUE(kAll.contains(i)) << i;
        }
        constexpr melon::strings_internal::Charmap kHello =
                melon::strings_internal::Charmap::FromString("Hello, world!");
        EXPECT_THAT(Members(kHello), " !,Hdelorw");

        // test negation and intersection
        constexpr melon::strings_internal::Charmap kABC =
                melon::strings_internal::Charmap::Range('A', 'Z') &
                ~melon::strings_internal::Charmap::Range('D', 'Z');
    ...
    TEST(Charmap, Range) {
        ...
        for (auto lo = poi.begin(); lo != poi.end(); ++lo) {
            ...
            for (auto hi = lo; hi != poi.end(); ++hi) {
                SCOPED_TRACE(*hi);
                EXPECT_THAT(Members(melon::strings_internal::Charmap::Range(*lo, *hi)),
    ...
    TEST(CharmapCtype, Match) {
        for (int c = 0; c < 256; ++c) {
            ...
            EXPECT_EQ(AsBool(std::isupper(c)),
                      melon::strings_internal::UpperCharmap().contains(c));
            EXPECT_EQ(AsBool(std::islower(c)),
                      melon::strings_internal::LowerCharmap().contains(c));
            EXPECT_EQ(AsBool(std::isdigit(c)),
                      melon::strings_internal::DigitCharmap().contains(c));
            EXPECT_EQ(AsBool(std::isalpha(c)),
                      melon::strings_internal::AlphaCharmap().contains(c));
            EXPECT_EQ(AsBool(std::isalnum(c)),
                      melon::strings_internal::AlnumCharmap().contains(c));
            EXPECT_EQ(AsBool(std::isxdigit(c)),
                      melon::strings_internal::XDigitCharmap().contains(c));
            EXPECT_EQ(AsBool(std::isprint(c)),
                      melon::strings_internal::PrintCharmap().contains(c));
            EXPECT_EQ(AsBool(std::isspace(c)),
                      melon::strings_internal::SpaceCharmap().contains(c));
            EXPECT_EQ(AsBool(std::iscntrl(c)),
                      melon::strings_internal::CntrlCharmap().contains(c));
            EXPECT_EQ(AsBool(std::isblank(c)),
                      melon::strings_internal::BlankCharmap().contains(c));
            EXPECT_EQ(AsBool(std::isgraph(c)),
                      melon::strings_internal::GraphCharmap().contains(c));
            EXPECT_EQ(AsBool(std::ispunct(c)),
                      melon::strings_internal::PunctCharmap().contains(c));
        }
    }
