...
struct SpinLockTest {
    static uint32_t encode_wait_cycles(int64_t wait_start_time,
        ...
        return abel::spin_lock::encode_wait_cycles(wait_start_time, wait_end_time);
    }

    static uint64_t decode_wait_cycles(uint32_t lock_value) {
        return abel::spin_lock::decode_wait_cycles(lock_value);
    }
};
...
static abel::spin_lock static_spinlock(base_internal::kLinkerInitialized);
static abel::spin_lock static_cooperative_spinlock(
        base_internal::kLinkerInitialized,
        thread_internal::SCHEDULE_COOPERATIVE_AND_KERNEL);
static abel::spin_lock static_noncooperative_spinlock(
...
TEST(spin_lock, StackNonCooperativeDisablesScheduling) {
    abel::spin_lock spinlock(thread_internal::SCHEDULE_KERNEL_ONLY);
    spinlock.lock();
...
TEST(spin_lock, WaitCyclesEncoding) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        uint64_t decoded = SpinLockTest::decode_wait_cycles(lock_value);
    ...
    EXPECT_EQ(0, SpinLockTest::decode_wait_cycles(0));
    EXPECT_EQ(0, SpinLockTest::decode_wait_cycles(kLockwordReservedMask));
    ...
    uint64_t max_value_decoded = SpinLockTest::decode_wait_cycles(max_value);
