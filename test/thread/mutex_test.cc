...
static void CheckSumG0G1(void *v) {
    ...
    DCHECK_MSG(cxt->g0 == -cxt->g1, "Error in CheckSumG0G1");
...
static void TestRW(TestContext *cxt, int c) {
    if ((c & 1) == 0) {
        ...
        for (int i = 0; i != cxt->iterations; i++) {
            abel::reader_mutex_lock l(&cxt->mu);
            DCHECK_MSG(cxt->g0 == -cxt->g1, "Error in TestRW");
            cxt->mu.assert_reader_held();
        }
    }
}
...
static void TestAwait(TestContext *cxt, int c) {
    ...
    while (cxt->g0 < cxt->iterations) {
        ...
        DCHECK_MSG(mc.MyTurn(), "Error in TestAwait");
        cxt->mu.assert_held();
...
static void TestSignal(TestContext *cxt, int c) {
    DCHECK_MSG(cxt->threads == 2, "TestSignal should use 2 threads");
    int target = c;
    abel::mutex_lock l(&cxt->mu);
    cxt->mu.assert_held();
    while (cxt->g0 < cxt->iterations) {
...
static void TestTime(TestContext *cxt, int c, bool use_cv) {
    DCHECK_MSG(cxt->iterations == 1, "TestTime should only use 1 iteration");
    DCHECK_MSG(cxt->threads > 2, "TestTime should use more than 2 threads");
    ...
    if (c == 0) {
        ...
        if (use_cv) {
            ...
            DCHECK_MSG(cxt->mu.await_with_timeout(g0ge2, abel::duration::seconds(100)),
                       "TestTime failed");
        }
        cxt->g0++;
    } else {
...
static int RunTestWithInvariantDebugging(void (*test)(TestContext *cxt, int),
    ...
    DCHECK_MSG(GetInvariantChecked(), "Invariant not checked");
...
static bool ConditionWithAcquire(AcquireFromConditionStruct *x) {
    ...
    DCHECK_MSG(x->value < 4, "should not be invoked a fourth time");
