...
namespace melon {
    ...
    TEST(Latch, WaitFor) {
        latch l(1);
        ASSERT_FALSE(l.wait_for(melon::duration::milliseconds(100)));
        l.count_down();
        ASSERT_TRUE(l.wait_for(melon::zero_duration()));
    }

    TEST(Latch, WaitUntil) {
        latch l(1);
        ASSERT_FALSE(l.wait_until(melon::time_point::future_unix_millis(100)));
        l.count_down();
        ASSERT_TRUE(l.wait_until(melon::time_now()));
    }
}