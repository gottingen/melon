...
#include "melon/memory/scoped_ptr.h"
...
namespace {
    ...
    TEST(aligned_memoryTest, StaticAlignment) {
        ...
        EXPECT_EQ(8u, MELON_ALIGN_OF(raw8));
        EXPECT_EQ(16u, MELON_ALIGN_OF(raw16));
        EXPECT_EQ(256u, MELON_ALIGN_OF(raw256));
        EXPECT_EQ(4096u, MELON_ALIGN_OF(raw4096));
...
#define GOOD_GCC_STACK_ALIGNMENT
    ...
    TEST(aligned_memoryTest, StackAlignment) {
        ...
        EXPECT_EQ(8u, MELON_ALIGN_OF(raw8));
        EXPECT_EQ(16u, MELON_ALIGN_OF(raw16));
        EXPECT_EQ(128u, MELON_ALIGN_OF(raw128));
  ...
  EXPECT_ALIGNED(raw4096.void_data(), 4096);
    ...
    TEST(aligned_memoryTest, DynamicAllocation) {
        void* p = melon::aligned_alloc(8, 8);
        EXPECT_TRUE(p);
        EXPECT_ALIGNED(p, 8);
        melon::aligned_free(p);

        p = melon::aligned_alloc(8, 16);
        EXPECT_TRUE(p);
        EXPECT_ALIGNED(p, 16);
        melon::aligned_free(p);

        p = melon::aligned_alloc(8, 256);
        EXPECT_TRUE(p);
        EXPECT_ALIGNED(p, 256);
        melon::aligned_free(p);

        p = melon::aligned_alloc(8, 4096);
        EXPECT_TRUE(p);
    ...
    TEST(aligned_memoryTest, ScopedDynamicAllocation) {
        scoped_ptr<float, melon::aligned_free_deleter> p(
                static_cast<float*>(melon::aligned_alloc(8, 8)));
        EXPECT_TRUE(p.get());
