...
namespace melon {
    ...
    TEST(ref_ptr, AtomicDontLeak) {
        ...
        for (auto &&from : ps) {
            ...
            for (auto &&e : ts) {
                e = std::thread([&] {
                    auto op = melon::base::fast_rand() % 4;
                    while (!ever_success) {
                        if (op == 0) {
                            atomic.store(temps[melon::base::fast_rand_less_than(9)], std::memory_order_release);
                        } else if (op == 1) {
                            ...
                            atomic.exchange(temps[melon::base::fast_rand_less_than(9)]);
                        }
                        ASSERT_EQ(11, RefCounted1::instances);
                    }
                });
