...
namespace {
    ...
    TEST_F(ResourcePoolTest, change_config) {
        ...
        melon::ResourcePoolInfo info = melon::describe_resources<MyObject>();
        melon::ResourcePoolInfo zero_info = {0, 0, 0, 0, 3, 3, 0};
        ASSERT_EQ(0, memcmp(&info, &zero_info, sizeof(info)));

        melon::ResourceId<MyObject> id = {0};
        get_resource<MyObject>(&id);
        std::cout << melon::describe_resources<MyObject>() << std::endl;
        return_resource(id);
        std::cout << melon::describe_resources<MyObject>() << std::endl;
    }
    ...
    TEST_F(ResourcePoolTest, sanity) {
        ptr_set.clear();
        melon::ResourceId<NonDefaultCtorObject> id0 = {0};
        melon::get_resource<NonDefaultCtorObject>(&id0, 10);
        ASSERT_EQ(10, melon::address_resource(id0)->_value);
        melon::get_resource<NonDefaultCtorObject>(&id0, 100, 30);
        ASSERT_EQ(130, melon::address_resource(id0)->_value);

        printf("BLOCK_NITEM=%lu\n", melon::ResourcePool<YellObj>::BLOCK_NITEM);

        nc = 0;
        nd = 0;
        {
            melon::ResourceId<YellObj> id1;
            YellObj *o1 = melon::get_resource(&id1);
            ASSERT_TRUE(o1);
            ASSERT_EQ(o1, melon::address_resource(id1));

            ASSERT_EQ(1, nc);
            ASSERT_EQ(0, nd);

            melon::ResourceId<YellObj> id2;
            YellObj *o2 = melon::get_resource(&id2);
            ASSERT_TRUE(o2);
            ASSERT_EQ(o2, melon::address_resource(id2));
    ...
    TEST_F(ResourcePoolTest, validator) {
        ...
        int nfoo = 0;
        for (int i = 0; i < 100; ++i) {
            melon::ResourceId<Foo> id = {0};
            Foo *foo = melon::get_resource<Foo>(&id);
            if (foo) {
                ASSERT_EQ(1, foo->x);
                ++nfoo;
            }
        ...
        ASSERT_EQ((size_t) nfoo, melon::describe_resources<Foo>().item_num);
    }

    TEST_F(ResourcePoolTest, get_int) {
        melon::clear_resources<int>();

        // Perf of this test is affected by previous case.
        const size_t N = 100000;

        melon::stop_watcher tm;
        melon::ResourceId<int> id;

        // warm up
        if (melon::get_resource(&id)) {
            melon::return_resource(id);
        }
        ASSERT_EQ(0UL, id);
        delete (new int);
        ...
        for (size_t i = 0; i < N; ++i) {
            id.value = i;
            *melon::ResourcePool<int>::unsafe_address_resource(id) = i;
        }
        tm.stop();
        ...
        for (size_t i = 0; i < N; ++i) {
            id.value = i;
            *melon::address_resource(id) = i;
        }
        tm.stop();
        printf("address a int takes %.1fns\n", tm.n_elapsed() / (double) N);

        std::cout << melon::describe_resources<int>() << std::endl;
        melon::clear_resources<int>();
        std::cout << melon::describe_resources<int>() << std::endl;
    }
    ...
    TEST_F(ResourcePoolTest, get_perf) {
        ...
        new_list.reserve(N);
        melon::ResourceId<SilentObj> id;

        melon::stop_watcher tm1, tm2;
        ...
        std::cout << melon::describe_resources<SilentObj>() << std::endl;
    }
    ...
    void *get_and_return_int(void *) {
        ...
        v.reserve(N);
        melon::stop_watcher tm0, tm1, tm2;
        ResourceId<D> id = {0};
        D tmp = D();
        int sr = 0;
    ...
    TEST_F(ResourcePoolTest, get_and_return_int_multiple_threads) {
        pthread_t tid[16];
        for (size_t i = 0; i < MELON_ARRAY_SIZE(tid); ++i) {
        ...
        pthread_t tid2[16];
        for (size_t i = 0; i < MELON_ARRAY_SIZE(tid2); ++i) {
