...
#include "melon/files/temp_file.h"      // temp_file
#include <melon/container/flat_map.h>
#include "melon/times/time.h"                 // Timer
...
namespace melon {
    namespace iobuf {
        ...
        extern int32_t block_shared_count(melon::cord_buf::Block const *b);

        extern uint32_t block_cap(melon::cord_buf::Block const *b);
...
namespace {
    ...
    const int MELON_ALLOW_UNUSED check_dummy = melon::thread::atexit(check_tls_block);

    static melon::container::FlatSet<void *> s_set;
    ...
    inline bool is_debug_allocator_enabled() {
        return (melon::iobuf::blockmem_allocate == debug_block_allocate);
    }

    void install_debug_allocator() {
        if (!is_debug_allocator_enabled()) {
            melon::iobuf::remove_tls_block_chain();
            s_set.init(1024);
            melon::iobuf::blockmem_allocate = debug_block_allocate;
            melon::iobuf::blockmem_deallocate = debug_block_deallocate;
            MELON_LOG(INFO) << "<Installed debug create/destroy>";
        }
    }
    ...
    static void check_memory_leak() {
        if (is_debug_allocator_enabled()) {
            melon::cord_buf::Block *p = melon::iobuf::get_tls_block_head();
            size_t n = 0;
            while (p) {
                ASSERT_TRUE(s_set.seek(p)) << "Memory leak: " << p;
                p = melon::iobuf::get_portal_next(p);
                ++n;
            }
            ASSERT_EQ(n, s_set.size());
            ASSERT_EQ(n, (size_t) melon::iobuf::get_tls_block_count());
        }
    }
    ...
    std::string to_str(const melon::cord_buf &p) {
        return p.to_string();
    }
    ...
    TEST_F(CordBufTest, appendv) {
        ...
        const_iovec vec[] = {{"hello1",  6},
                             ...
                             {"hello5",  6},
                             {" world5", 7}};
        ASSERT_EQ(0, b.appendv(vec, MELON_ARRAY_SIZE(vec)));
        ...
        const_iovec vec2[] = {{str,               len1},
                              ...
                              {str + len1 + len2, len3}};
        b.clear();
        ASSERT_EQ(0, b.appendv(vec2, MELON_ARRAY_SIZE(vec2)));
    ...
    TEST_F(CordBufTest, reserve) {
        melon::cord_buf b;
        ASSERT_EQ(melon::cord_buf::INVALID_AREA, b.reserve(0));
        const size_t NRESERVED1 = 5;
        const melon::cord_buf::Area a1 = b.reserve(NRESERVED1);
        ASSERT_TRUE(a1 != melon::cord_buf::INVALID_AREA);
        ...
        const melon::cord_buf::Area a2 = b.reserve(NRESERVED2);
    ...
    TEST_F(CordBufTest, iobuf_as_queue) {
        ...
        // If INITIAL_CAP gets bigger, creating melon::cord_buf::Block are very
        // small. Since We don't access melon::cord_buf::Block::data in this case.
        // We replace melon::cord_buf::Block with FakeBlock with only nshared (in
        // the same offset)
        FakeBlock *blocks[melon::cord_buf::INITIAL_CAP + 16];
        const size_t NBLOCKS = MELON_ARRAY_SIZE(blocks);
        melon::cord_buf::BlockRef r[NBLOCKS];
        const size_t LENGTH = 7UL;
        for (size_t i = 0; i < NBLOCKS; ++i) {
            ASSERT_TRUE((blocks[i] = new FakeBlock));
            r[i].offset = 1;
            r[i].length = LENGTH;
            r[i].block = (melon::cord_buf::Block *) blocks[i];
        }
        ...
        ASSERT_EQ(2, melon::iobuf::block_shared_count(r[0].block));
        ...
        ASSERT_EQ(2, melon::iobuf::block_shared_count(r[1].block));
        ...
        //ASSERT_EQ(1, melon::iobuf::block_shared_count(r[0].block));
        ...
        for (size_t i = 0; i < melon::cord_buf::INITIAL_CAP + 2; ++i) {
            ...
            ASSERT_EQ(2, melon::iobuf::block_shared_count(r[i + 2].block));
        }
    ...
    TEST_F(CordBufTest, iobuf_sanity) {
        ...
        MELON_LOG(INFO) << "sizeof(melon::cord_buf)=" << sizeof(melon::cord_buf)
                  << " sizeof(IOPortal)=" << sizeof(melon::IOPortal);
    ...
    TEST_F(CordBufTest, copy_and_assign) {
        ...
        const size_t TARGET_SIZE = melon::cord_buf::DEFAULT_BLOCK_SIZE * 2;
        melon::cord_buf buf0;
        buf0.append("hello");
        ASSERT_EQ(1u, buf0._ref_num());

        // Copy-construct from SmallView
        melon::cord_buf buf1 = buf0;
    ...
    TEST_F(CordBufTest, cut_into_fd_tiny) {
        ...
        MELON_LOG(INFO) << "ref.size=" << ref.size();

        //ASSERT_EQ(0, pipe(fds));
        ASSERT_EQ(0, socketpair(AF_UNIX, SOCK_STREAM, 0, fds));

        melon::base::make_non_blocking(fds[0]);
        melon::base::make_non_blocking(fds[1]);
    ...
    TEST_F(CordBufTest, cut_multiple_into_fd_tiny) {
        ...
        for (size_t j = 0; j < MELON_ARRAY_SIZE(b1); ++j) {
            ...
            ref.append(s);
            melon::IOPortal *b = new melon::IOPortal();
            b->append(s);
            b1[j] = b;
        }
        ...
        ASSERT_EQ((ssize_t) ref.length(),
                  melon::cord_buf::cut_multiple_into_file_descriptor(
                          fds[1], b1, MELON_ARRAY_SIZE(b1)));
        for (size_t j = 0; j < MELON_ARRAY_SIZE(b1); ++j) {
    ...
    TEST_F(CordBufTest, cut_by_delim_perf) {
        melon::iobuf::reset_blockmem_allocate_and_deallocate();
        ...
        MELON_LOG(INFO) << "IOPortal::cut_by_delim takes "
                  << t.n_elapsed() / N << "ns, tp="
    ...
    TEST_F(CordBufTest, cut_perf) {
        ...
        if (!push_char) {
            ...
            t.stop();
            //ProfilerStop();
            MELON_LOG(INFO) << "IOPortal::append(std::string_" << s.length() << ") takes "
            ...
            t.stop();
            //ProfilerStop();
            MELON_LOG(INFO) << "IOPortal::push_back(char) takes "
        ...
        for (size_t i = 0; i < MELON_ARRAY_SIZE(w); ++i) {
            ...
            MELON_LOG(INFO) << "IOPortal::cutn(12+" << w[i] << ") takes "
            ...
            b.append(p);
            t.stop();
            MELON_LOG(INFO) << "IOPortal::append(melon::cord_buf) takes "
    ...
    TEST_F(CordBufTest, append_store_append_cut) {
        ...
        for (size_t i = 0; i < MELON_ARRAY_SIZE(w); ++i) {
            ...
            // MELON_LOG(INFO) << "ps.cap=" << ps.capacity();
            ...
            //MELON_LOG(INFO) << "ps.size=" << ps.size();
            ps.clear();
            MELON_LOG(INFO) << "Bandwidth of append(" << f.fname() << ")->cut(12+" << w[i]
    ...
    TEST_F(CordBufTest, conversion_with_protobuf) {
        ...
        melon::cord_buf_as_zero_copy_output_stream out_wrapper(&buf);
    ...
    TEST_F(CordBufTest, extended_backup) {
        for (int i = 0; i < 2; ++i) {
            ...
            melon::iobuf::remove_tls_block_chain();
            melon::cord_buf src;
            const int BLKSIZE = (i == 0 ? 1024 : melon::cord_buf::DEFAULT_BLOCK_SIZE);
            const int PLDSIZE = BLKSIZE - BLOCK_OVERHEAD;
            melon::cord_buf_as_zero_copy_output_stream out_stream1(&src, BLKSIZE);
    ...
    TEST_F(CordBufTest, backup_iobuf_never_called_next) {
        ...
        melon::cord_buf_as_zero_copy_output_stream out_stream(&src);
    ...
    TEST_F(CordBufTest, own_block) {
        ...
        melon::cord_buf::Block *saved_tls_block = melon::iobuf::get_tls_block_head();
        melon::cord_buf_as_zero_copy_output_stream wrapper(&buf, BLOCK_SIZE);
        int alloc_size = 0;
        for (int i = 0; i < 100; ++i) {
            void *data;
            int size;
        ...
        ASSERT_EQ(saved_tls_block, melon::iobuf::get_tls_block_head());
        ASSERT_EQ(melon::iobuf::block_cap(buf._front_ref().block), BLOCK_SIZE - BLOCK_OVERHEAD);
    }
    ...
    TEST_F(CordBufTest, as_ostream) {
        ...
        melon::cord_buf_builder builder;
        MELON_LOG(INFO) << "sizeof(cord_buf_builder)=" << sizeof(builder) << std::endl
                  << "sizeof(cord_buf)=" << sizeof(melon::cord_buf) << std::endl
                  << "sizeof(cord_buf_as_zero_copy_output_stream)="
                  << sizeof(melon::cord_buf_as_zero_copy_output_stream) << std::endl
                  << "sizeof(zero_copy_stream_as_stream_buf)="
                  << sizeof(melon::zero_copy_stream_as_stream_buf) << std::endl
    ...
    TEST_F(CordBufTest, append_from_fd_with_offset) {
        melon::temp_file file;
        file.save("dummy");
        melon::base::fd_guard fd(open(file.fname(), O_RDWR | O_TRUNC));
        ASSERT_TRUE(fd >= 0) << file.fname() << ' ' << melon_error();
        melon::IOPortal buf;
        char dummy[10 * 1024];
    ...
    TEST_F(CordBufTest, appender_perf) {
        ...
        tm2.start();
        melon::cord_buf_appender appender1;
        for (size_t i = 0; i < N1; ++i) {
            appender1.push_back(i);
        }
        tm2.stop();

        MELON_LOG(INFO) << "cord_buf.push_back=" << tm1.n_elapsed() / N1
        ...
        tm2.start();
        melon::cord_buf_appender appender2;
        for (size_t i = 0; i < N2; ++i) {
            appender2.append(s);
        }
        tm2.stop();

        melon::stop_watcher tm3;
        tm3.start();
        for (size_t i = 0; i < N2; ++i) {
            str2.append(s);
        }
        ...
        MELON_LOG(INFO) << "cord_buf.append=" << tm1.n_elapsed() / N2
    ...
    TEST_F(CordBufTest, printed_as_binary) {
        ...
        os << melon::to_printable(buf, 256);
        ASSERT_STREQ(OUTPUT, os.str().c_str());
        os.str("");
        os << melon::to_printable(str, 256);
        ASSERT_STREQ(OUTPUT, os.str().c_str());
    }
    ...
    TEST_F(CordBufTest, copy_to_string_from_iterator) {
        ...
        for (size_t i = 0; i < 1 * 1024 * 1024lu; ++i) {
            b0.push_back(melon::base::fast_rand_in('a', 'z'));
        }
        melon::cord_buf b1(b0);
        melon::cord_buf_bytes_iterator iter(b0);
        size_t nc = 0;
        while (nc < b0.length()) {
            size_t to_copy = melon::base::fast_rand_in(1024lu, 64 * 1024lu);
            melon::cord_buf b;
            b1.cutn(&b, to_copy);
            std::string s;
    ...
    TEST_F(CordBufTest, append_user_data_and_consume) {
        ...
        ASSERT_EQ(1, melon::iobuf::block_shared_count(r.block));
...

        {
            ...
            for (int i = 0; i < 256; ++i) {
                ...
                if (i != 255) {
                    ASSERT_EQ(1UL, b0._ref_num());
                    melon::cord_buf::BlockRef r = b0._front_ref();
                    ASSERT_EQ(i + 2, melon::iobuf::block_shared_count(r.block));
                } else {
    ...
    TEST_F(CordBufTest, share_tls_block) {
        melon::iobuf::remove_tls_block_chain();
        melon::cord_buf::Block *b = melon::iobuf::acquire_tls_block();
        ASSERT_EQ(0u, melon::iobuf::block_size(b));

        melon::cord_buf::Block *b2 = melon::iobuf::share_tls_block();
        melon::cord_buf buf;
        for (size_t i = 0; i < melon::iobuf::block_cap(b2); i++) {
            buf.push_back('x');
        }
    ...
    TEST_F(CordBufTest, acquire_tls_block) {
        melon::iobuf::remove_tls_block_chain();
        melon::cord_buf::Block *b = melon::iobuf::acquire_tls_block();
        const size_t block_cap = melon::iobuf::block_cap(b);
        melon::cord_buf buf;
        for (size_t i = 0; i < block_cap; i++) {
            buf.append("x");
        }
        ASSERT_EQ(1, melon::iobuf::get_tls_block_count());
        melon::cord_buf::Block *head = melon::iobuf::get_tls_block_head();
        ASSERT_EQ(melon::iobuf::block_cap(head), melon::iobuf::block_size(head));
        melon::iobuf::release_tls_block_chain(b);
        ASSERT_EQ(2, melon::iobuf::get_tls_block_count());
        for (size_t i = 0; i < block_cap; i++) {
            buf.append("x");
        }
        ASSERT_EQ(2, melon::iobuf::get_tls_block_count());
        head = melon::iobuf::get_tls_block_head();
        ASSERT_EQ(melon::iobuf::block_cap(head), melon::iobuf::block_size(head));
        b = melon::iobuf::acquire_tls_block();
        ASSERT_EQ(0, melon::iobuf::get_tls_block_count());
        ASSERT_NE(melon::iobuf::block_cap(b), melon::iobuf::block_size(b));
    }
