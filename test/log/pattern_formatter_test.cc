...
static std::string log_to_str (const std::string &msg, const Args &... args) {
    ...
    abel::logger oss_logger("pattern_tester", oss_sink);
    oss_logger.set_level(abel::level::info);

    oss_logger.set_formatter(std::unique_ptr<abel::formatter>(new abel::pattern_formatter(args...)));
...
TEST(pattern_formatter, empty) {
    ...
    std::unique_ptr<abel::formatter>(new abel::pattern_formatter("%v", abel::pattern_time_type::local, ";)"));
