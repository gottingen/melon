...
TEST(FileLog, flush_on) {
    ...
    logger->flush_on(abel::level::info);
...
TEST(FileLog, rotatingfilelogger1) {
    ...
    auto logger = abel::rotating_logger_mt("logger", basename, max_size, 0);
...
TEST(FileLog, rotatingfilelogger2) {
    ...
    size_t max_size = 1024 * 10;
    std::string basename = "logs/rotating_log";
    auto logger = abel::rotating_logger_mt("logger", basename, max_size, 1);
    for (int i = 0; i < 10; ++i)
        logger->info("Test message {}", i);
...
TEST(FileLog, dailyloggerdateonly) {
    ...
    auto logger = abel::create<sink_type>("logger", basename, 0, 0);
    for (int i = 0; i < 10; ++i) {
...
struct custom_daily_file_name_calculator {
    static abel::filename_t calc_filename (const abel::filename_t &basename, const tm &now_tm) {
        fmt::memory_buffer w;
