...
namespace {
    ...
    TEST(PopenTest, posix_popen) {
        ...
        int rc = melon::read_command_output_through_popen(oss, "echo \"Hello World\"");
        ASSERT_EQ(0, rc) << melon_error(errno);
        ASSERT_EQ("Hello World\n", oss.str());

        oss.str("");
        rc = melon::read_command_output_through_popen(oss, "exit 1");
        EXPECT_EQ(1, rc) << melon_error(errno);
        ASSERT_TRUE(oss.str().empty()) << oss.str();
        oss.str("");
        rc = melon::read_command_output_through_popen(oss, "kill -9 $$");
        ASSERT_EQ(-1, rc);
        ASSERT_EQ(errno, ECHILD);
        ASSERT_TRUE(melon::ends_with(oss.str(), "was killed by signal 9"));
        oss.str("");
        rc = melon::read_command_output_through_popen(oss, "kill -15 $$");
        ASSERT_EQ(-1, rc);
        ASSERT_EQ(errno, ECHILD);
        ASSERT_TRUE(melon::ends_with(oss.str(), "was killed by signal 15"));
    }
    ...
    TEST(PopenTest, clone) {
        std::ostringstream oss;
        int rc = melon::read_command_output_through_clone(oss, "echo \"Hello World\"");
        ASSERT_EQ(0, rc) << melon_error(errno);
        ASSERT_EQ("Hello World\n", oss.str());

        oss.str("");
        rc = melon::read_command_output_through_clone(oss, "exit 1");
        ASSERT_EQ(1, rc) << melon_error(errno);
        ASSERT_TRUE(oss.str().empty()) << oss.str();
        oss.str("");
        rc = melon::read_command_output_through_clone(oss, "kill -9 $$");
        ASSERT_EQ(-1, rc);
        ASSERT_EQ(errno, ECHILD);
        ASSERT_TRUE(melon::ends_with(oss.str(), "was killed by signal 9"));
        oss.str("");
        rc = melon::read_command_output_through_clone(oss, "kill -15 $$");
        ASSERT_EQ(-1, rc);
        ASSERT_EQ(errno, ECHILD);
        ASSERT_TRUE(melon::ends_with(oss.str(), "was killed by signal 15"));

        oss.str("");
        ASSERT_EQ(0, melon::read_command_output_through_clone(oss, "for i in `seq 1 100000`; do echo -n '=' ; done"));
