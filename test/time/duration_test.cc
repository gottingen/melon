...
TEST(duration, Negation) {
  ...
  EXPECT_TRUE(
      abel::time_internal::IsInfiniteDuration(negated_infinite_duration));

  // The largest duration is kint64max seconds and kTicksPerSecond - 1 ticks.
  // Using the abel::time_internal::MakeDuration API is the cleanest way to
  // construct that duration.
  constexpr abel::duration max_duration = abel::time_internal::MakeDuration(
      kint64max, abel::time_internal::kTicksPerSecond - 1);
  ...
  constexpr abel::duration nearly_min_duration =
      abel::time_internal::MakeDuration(kint64min, int64_t{1});
  ...
  constexpr abel::duration min_duration =
      abel::time_internal::MakeDuration(kint64min);
...
TEST(duration, ToDoubleSecondsCheckEdgeCases) {
  constexpr uint32_t kTicksPerSecond = abel::time_internal::kTicksPerSecond;
  constexpr auto duration_tick = abel::time_internal::MakeDuration(0, 1u);
  int misses = 0;
