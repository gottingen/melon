...
TEST(Format, TimePointResolution) {
  ...
  EXPECT_EQ(
      "03:04:05",
      format(kFmt,
             chrono::time_point_cast<abel::time_internal::cctz::seconds>(t0),
             utc));
  EXPECT_EQ("03:04:00",
...
TEST(Format, TimePointExtendedResolution) {
  const char kFmt[] = "%H:%M:%E*S";
  const time_zone utc = utc_time_zone();
  const time_point<abel::time_internal::cctz::seconds> tp =
      chrono::time_point_cast<abel::time_internal::cctz::seconds>(
...
TEST(Format, ExtendedOffset) {
  ...
  auto tz = fixed_time_zone(abel::time_internal::cctz::seconds::zero());
  TestFormatSpecifier(tp, tz, "%z", "+0000");
...
TEST(Parse, TimePointExtendedResolution) {
  ...
  time_point<abel::time_internal::cctz::seconds> tp;
...
TEST(Parse, MaxRange) {
  ...
  EXPECT_EQ(tp, time_point<abel::time_internal::cctz::seconds>::max());
  EXPECT_FALSE(
  ...
  EXPECT_EQ(tp, time_point<abel::time_internal::cctz::seconds>::min());
  EXPECT_FALSE(
...
TEST(FormatParse, RoundTripDistantFuture) {
  const time_zone utc = utc_time_zone();
  const time_point<abel::time_internal::cctz::seconds> in =
      time_point<abel::time_internal::cctz::seconds>::max();
  const std::string s = format(RFC3339_full, in, utc);
  time_point<abel::time_internal::cctz::seconds> out;
...
TEST(FormatParse, RoundTripDistantPast) {
  const time_zone utc = utc_time_zone();
  const time_point<abel::time_internal::cctz::seconds> in =
      time_point<abel::time_internal::cctz::seconds>::min();
  const std::string s = format(RFC3339_full, in, utc);
  time_point<abel::time_internal::cctz::seconds> out;
  EXPECT_TRUE(parse(RFC3339_full, s, utc, &out)) << s;
  EXPECT_EQ(in, out);
