...
using format_internal::extended_parsed_format;
using format_internal::parsed_format_base;
...
struct SummarizeConsumer {
    ...
    bool ConvertOne (const format_internal::unbound_conversion &conv,
                     string_view s) {
        *out += "{";
        *out += std::string(s);
        *out += ":";
...
using format_internal::format_conv;

TEST_F(ParsedFormatTest, UncheckedCorrect) {
    auto f = extended_parsed_format<format_conv::d>::New("ABC%dDEF");
    ASSERT_TRUE(f);
    EXPECT_EQ("[ABC]{d:1$d}[DEF]", SummarizeParsedFormat(*f));

    std::string format = "%sFFF%dZZZ%f";
    auto f2 =
        extended_parsed_format<format_conv::string, format_conv::d, format_conv::floating>::New(format);

    ASSERT_TRUE(f2);
    EXPECT_EQ("{s:1$s}[FFF]{d:2$d}[ZZZ]{f:3$f}", SummarizeParsedFormat(*f2));

    f2 = extended_parsed_format<format_conv::string, format_conv::d, format_conv::floating>::New(
        "%s %d %f");

    ASSERT_TRUE(f2);
    EXPECT_EQ("{s:1$s}[ ]{d:2$d}[ ]{f:3$f}", SummarizeParsedFormat(*f2));

    auto star = extended_parsed_format<format_conv::star, format_conv::d>::New("%*d");
    ASSERT_TRUE(star);
    EXPECT_EQ("{*d:2$1$*d}", SummarizeParsedFormat(*star));

    auto dollar = extended_parsed_format<format_conv::d, format_conv::s>::New("%2$s %1$d");
    ASSERT_TRUE(dollar);
    EXPECT_EQ("{2$s:2$s}[ ]{1$d:1$d}", SummarizeParsedFormat(*dollar));
    // with reuse
    dollar = extended_parsed_format<format_conv::d, format_conv::s>::New("%2$s %1$d %1$d");
    ASSERT_TRUE(dollar);
    EXPECT_EQ("{2$s:2$s}[ ]{1$d:1$d}[ ]{1$d:1$d}",
              SummarizeParsedFormat(*dollar));
}
...
TEST_F(ParsedFormatTest, UncheckedIgnoredArgs) {
    EXPECT_FALSE((extended_parsed_format<format_conv::d, format_conv::s>::New("ABC")));
    EXPECT_FALSE((extended_parsed_format<format_conv::d, format_conv::s>::New("%dABC")));
    EXPECT_FALSE((extended_parsed_format<format_conv::d, format_conv::s>::New("ABC%2$s")));
    auto f = extended_parsed_format<format_conv::d, format_conv::s>::NewAllowIgnored("ABC");
    ASSERT_TRUE(f);
    EXPECT_EQ("[ABC]", SummarizeParsedFormat(*f));
    f = extended_parsed_format<format_conv::d, format_conv::s>::NewAllowIgnored("%dABC");
    ASSERT_TRUE(f);
    EXPECT_EQ("{d:1$d}[ABC]", SummarizeParsedFormat(*f));
    f = extended_parsed_format<format_conv::d, format_conv::s>::NewAllowIgnored("ABC%2$s");
    ASSERT_TRUE(f);
    EXPECT_EQ("[ABC]{2$s:2$s}", SummarizeParsedFormat(*f));
}

TEST_F(ParsedFormatTest, UncheckedMultipleTypes) {
    auto dx = extended_parsed_format<format_conv::d | format_conv::x>::New("%1$d %1$x");
    EXPECT_TRUE(dx);
    EXPECT_EQ("{1$d:1$d}[ ]{1$x:1$x}", SummarizeParsedFormat(*dx));

    dx = extended_parsed_format<format_conv::d | format_conv::x>::New("%1$d");
    EXPECT_TRUE(dx);
    EXPECT_EQ("{1$d:1$d}", SummarizeParsedFormat(*dx));
}

TEST_F(ParsedFormatTest, UncheckedIncorrect) {
    EXPECT_FALSE(extended_parsed_format<format_conv::d>::New(""));

    EXPECT_FALSE(extended_parsed_format<format_conv::d>::New("ABC%dDEF%d"));

    std::string format = "%sFFF%dZZZ%f";
    EXPECT_FALSE((extended_parsed_format<format_conv::s, format_conv::d, format_conv::g>::New(format)));
}

TEST_F(ParsedFormatTest, RegressionMixPositional) {
    EXPECT_FALSE((extended_parsed_format<format_conv::d, format_conv::o>::New("%1$d %o")));
}
