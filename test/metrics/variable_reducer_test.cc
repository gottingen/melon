...
namespace {
    ...
    TEST_F(ReducerTest, atomicity) {
        ASSERT_EQ(sizeof(int32_t), sizeof(melon::metrics_detail::ElementContainer<int32_t>));
        ASSERT_EQ(sizeof(int64_t), sizeof(melon::metrics_detail::ElementContainer<int64_t>));
        ASSERT_EQ(sizeof(float), sizeof(melon::metrics_detail::ElementContainer<float>));
        ASSERT_EQ(sizeof(double), sizeof(melon::metrics_detail::ElementContainer<double>));
    }

    TEST_F(ReducerTest, adder) {
        melon::gauge<uint32_t> reducer1;
        ASSERT_TRUE(reducer1.valid());
        reducer1 << 2 << 4;
        ASSERT_EQ(6ul, reducer1.get_value());

        melon::gauge<double> reducer2;
        ASSERT_TRUE(reducer2.valid());
        reducer2 << 2.0 << 4.0;
        ASSERT_DOUBLE_EQ(6.0, reducer2.get_value());

        melon::gauge<int> reducer3;
    ...
    TEST_F(ReducerTest, Min) {
        melon::min_gauge<uint64_t> reducer;
    ...
    TEST_F(ReducerTest, max) {
        melon::max_gauge<uint64_t> reducer;
    ...
    void ReducerTest_window() {
        melon::gauge<int> c1;
        melon::max_gauge<int> c2;
        melon::min_gauge<int> c3;
        melon::window<melon::gauge<int> > w1(&c1, 1);
        melon::window<melon::gauge<int> > w2(&c1, 2);
        melon::window<melon::gauge<int> > w3(&c1, 3);
        melon::window<melon::max_gauge<int> > w4(&c2, 1);
        melon::window<melon::max_gauge<int> > w5(&c2, 2);
        melon::window<melon::max_gauge<int> > w6(&c2, 3);
        melon::window<melon::min_gauge<int> > w7(&c3, 1);
        melon::window<melon::min_gauge<int> > w8(&c3, 2);
        melon::window<melon::min_gauge<int> > w9(&c3, 3);
    ...
    TEST_F(ReducerTest, simple_window) {
        melon::gauge<int64_t> a;
        melon::window<melon::gauge<int64_t> > w(&a, 10);
        a << 100;
        sleep(3);
