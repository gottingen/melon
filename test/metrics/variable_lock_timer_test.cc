...
namespace {
    using melon::IntRecorder;
    using melon::LatencyRecorder;
    using melon::utils::MutexWithRecorder;
    using melon::utils::MutexWithLatencyRecorder;
    ...
    TEST_F(LockTimerTest, overhead) {
        ...
        timer.stop();
        ProfilerStop();
        MELON_LOG(INFO) << "The overhead of MutexWithLatencyRecorder is "
        ...
        timer.stop();
        ProfilerStop();
        MELON_LOG(INFO) << "The overhead of MutexWithRecorder is "
                        << timer.n_elapsed() / N << "ns";
        ...
        timer.stop();
        ProfilerStop();
        MELON_LOG(INFO) << "The overhead of timer is "
                        << timer.n_elapsed() / N << "ns";
    }
} // namespace
