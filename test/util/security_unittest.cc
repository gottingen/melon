...
// The wrapper function in process_util_linux.cc that is used when we
...
TEST(SecurityTest, TCMALLOC_TEST(MemoryAllocationRestrictionsMalloc)) {
  if (!IsTcMallocBypassed()) {
    scoped_ptr<char, mutil::FreeDeleter> ptr(static_cast<char*>(
...
bool CallocReturnsNull(size_t nmemb, size_t size) {
  scoped_ptr<char, mutil::FreeDeleter> array_pointer(
...
TEST(SecurityTest, CallocOverflow) {
  ...
  if (!CallocDiesOnOOM()) {
    ...
    // NOTE(gejun): process/memory.cc is not linked right now,
...
TEST(SecurityTest, TCMALLOC_TEST(RandomMemoryAllocations)) {
  ...
  scoped_ptr<char, mutil::FreeDeleter> ptr(
