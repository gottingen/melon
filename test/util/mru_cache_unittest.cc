...
#include "melon/utility/containers/mru_cache.h"
...
TEST(MRUCacheTest, Basic) {
  typedef mutil::MRUCache<int, CachedItem> Cache;
...
TEST(MRUCacheTest, Owning) {
  typedef mutil::OwningMRUCache<int, CachedItem*> Cache;
  Cache cache(Cache::NO_AUTO_EVICT);
...
TEST(MRUCacheTest, HashingMRUCache) {
  ...
  typedef mutil::HashingMRUCache<std::string, CachedItem> Cache;
  Cache cache(Cache::NO_AUTO_EVICT);
