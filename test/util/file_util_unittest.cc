...
#include "melon/utility/win/windows_version.h"
#endif
...
TEST_F(FileUtilTest, ReadFileToString) {
  ...
  // Delete test file.
  EXPECT_TRUE(mutil::DeleteFile(file_path, false));
...
TEST_F(VerifyPathControlledByUserTest, BadPaths) {
  ...
  FilePath does_not_exist = base_dir_.AppendASCII("does")
                                     ...
                                     .AppendASCII("exist");
  EXPECT_FALSE(
      mutil::VerifyPathControlledByUser(
...
TEST_F(FileUtilTest, CreateDirectoryParentsNotExist) {
  ASSERT_TRUE(mutil::CreateDirectory(temp_dir_.path()));
  mutil::FilePath creating_dir = temp_dir_.path().Append("sub1").Append("sub2");
  std::cout << "path=" << creating_dir.value() << std::endl;
  ASSERT_FALSE(mutil::CreateDirectory(creating_dir, false));
  ASSERT_TRUE(mutil::CreateDirectory(creating_dir, true));
}
...
TEST(ScopedFD, ScopedFDDoesClose) {
  ...
  mutil::ScopedFD read_end_closer(fds[0]);
  {
    mutil::ScopedFD write_end_closer(fds[1]);
  }
