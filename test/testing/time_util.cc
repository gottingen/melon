...
#include "melon/times/internal/zone_info_source.h"

namespace melon {

    namespace times_internal {

        melon::time_zone load_time_zone(const std::string &name) {
            melon::time_zone tz;
            MELON_DCHECK(load_time_zone(name, &tz))<< name;
            return tz;
        }
...
#include "melon/times/internal/zoneinfo.inc"
            ...
            class TestZoneInfoSource : public melon::times_internal::zone_info_source {
            public:
            ...
            std::unique_ptr<melon::times_internal::zone_info_source> TestFactory(
                    const std::string &name,
                    const std::function<std::unique_ptr<melon::times_internal::zone_info_source>(
                            const std::string &name)> & /*fallback_factory*/) {
                for (const ZoneInfo &zoneinfo : kZoneInfo) {
                    if (name == zoneinfo.name) {
                        if (zoneinfo.data == nullptr)
                            return nullptr;
                        return std::unique_ptr<melon::times_internal::zone_info_source>(
                                new TestZoneInfoSource(zoneinfo.data, zoneinfo.length));
                    }
                }
                MELON_LOG(FATAL)<<"Unexpected time zone"<<name<<" in test";
                return nullptr;
            }
