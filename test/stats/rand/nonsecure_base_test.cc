...
namespace {
    ...
    abel::random_internal::nonsecure_urbg_base<std::mt19937>;
...
TEST(nonsecure_urbg_base, DefaultConstructorIsValid) {
...
TEST(nonsecure_urbg_base, StandardInterface) {
    ...
    using E = abel::random_internal::nonsecure_urbg_base<std::minstd_rand>;

    using T = typename E::result_type;

    static_assert(!std::is_copy_constructible<E>::value,
                  "nonsecure_urbg_base should not be copy constructible");

    static_assert(!abel::is_copy_assignable<E>::value,
                  "nonsecure_urbg_base should not be copy assignable");

    static_assert(std::is_move_constructible<E>::value,
                  "nonsecure_urbg_base should be move constructible");

    static_assert(abel::is_move_assignable<E>::value,
                  "nonsecure_urbg_base should be move assignable");
...
TEST(nonsecure_urbg_base, SeedSeqConstructorIsValid) {
    std::seed_seq seq;
    ExampleNonsecureURBG rbg(seq);
}

TEST(nonsecure_urbg_base, CompatibleWithDistributionUtils) {
...
TEST(nonsecure_urbg_base, CompatibleWithStdDistributions) {
    ExampleNonsecureURBG rbg;
...
TEST(nonsecure_urbg_base, ConsecutiveDefaultInstancesYieldUniqueVariates) {
...
TEST(nonsecure_urbg_base, EqualSeedSequencesYieldEqualVariates) {
    std::seed_seq seq;
...
// that nonsecure_urbg_base::Seeder can correctly handle iterators
...
TEST(nonsecure_urbg_base, SeederWorksForU32) {
    using U32 =
    abel::random_internal::nonsecure_urbg_base<SeederTestEngine<uint32_t>>;
    U32 x;
    EXPECT_NE(0, x());
}

TEST(nonsecure_urbg_base, SeederWorksForU64) {
    using U64 =
    abel::random_internal::nonsecure_urbg_base<SeederTestEngine<uint64_t>>;
