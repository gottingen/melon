...
#include "melon/base/math/bit_cast.h"
#include "melon/log/logging.h"
#include "melon/debugging/internal/stack_consumption.h"


// MELON_ATTRIBUTE_SECTION_VARIABLE
//
// Tells the compiler/linker to put a given variable into a section and define
// `__start_ ## name` and `__stop_ ## name` symbols to bracket the section.
// This functionality is supported by GNU linker.
#ifdef MELON_COMPILER_GNUC
#ifndef MELON_ATTRIBUTE_SECTION_VARIABLE
#define MELON_ATTRIBUTE_SECTION_VARIABLE(name) __attribute__((section(#name)))
#endif
#else
#define MELON_ATTRIBUTE_SECTION_VARIABLE(name)
#endif
using testing::Contains;

#ifdef _WIN32
#define MELON_SYMBOLIZE_TEST_NOINLINE __declspec(noinline)
#else
#define MELON_SYMBOLIZE_TEST_NOINLINE MELON_NO_INLINE
#endif

// Functions to symbolize. Use C linkage to avoid mangled names.
extern "C" {
MELON_NO_INLINE void nonstatic_func() {
...
MELON_NO_INLINE static void static_func() {
    // The next line makes this a unique function to prevent the compiler from
    // folding identical functions together.
    volatile int x = __LINE__;
    static_cast<void>(x);
...
MELON_NO_INLINE void Foo::func(int) {
    // The next line makes this a unique function to prevent the compiler from
    // folding identical functions together.
    volatile int x = __LINE__;
    static_cast<void>(x);
...
int MELON_ATTRIBUTE_SECTION_VARIABLE(
.text.unlikely)
...
int /*MELON_ATTRIBUTE_SECTION_VARIABLE(.text)*/ regular_func() {
    return 0;
}

// Thread-local data may confuse the symbolizer, ensure that it does not.
// Variable sizes and order are important.
#ifdef MELON_PER_THREAD_TLS
static MELON_PER_THREAD_TLS_KEYWORD char symbolize_test_thread_small[1];
static MELON_PER_THREAD_TLS_KEYWORD char
...
MELON_ATTRIBUTE_SECTION_VARIABLE(.text) = "";
#endif  // !defined(__EMSCRIPTEN__)

static char try_symbolize_buffer[4096];

// A wrapper function for melon::debugging::symbolize() to make the unit test simple.  The
// limit must be < sizeof(try_symbolize_buffer).  Returns null if
// melon::debugging::symbolize() returns false, otherwise returns try_symbolize_buffer with
// the result of melon::debugging::symbolize().

static const char *TrySymbolizeWithLimit(void *pc, int limit) {
    MELON_CHECK(static_cast<size_t>(limit) <= sizeof(try_symbolize_buffer)) <<
    ...
    bool found = melon::debugging::symbolize(pc, heap_buffer.get(), limit);
    if (found) {
        MELON_CHECK(strnlen(heap_buffer.get(), static_cast<size_t>(limit)) < static_cast<size_t>(limit)) <<
                                                                                                   "melon::debugging::symbolize() did not properly terminate the string";
...
#ifdef MELON_INTERNAL_HAVE_ELF_SYMBOLIZE
...
static void SymbolizeSignalHandler(int signo) {
  if (melon::debugging::symbolize(g_pc_to_symbolize, g_symbolize_buffer,
...
static const char *SymbolizeStackConsumption(void *pc, int *stack_consumed) {
  ...
  *stack_consumed = melon::debugging::debugging_internal::GetSignalHandlerStackConsumption(
...
const char kPadding0[kPageSize * 4] MELON_ATTRIBUTE_SECTION_VARIABLE(.text) =
    "";
const char kPadding1[kPageSize * 4] MELON_ATTRIBUTE_SECTION_VARIABLE(.text) =
    "";
  ...
  for (int i = 0; i < info->dlpi_phnum; i++) {
    if (info->dlpi_phdr[i].p_type == PT_LOAD &&
      ...
      const void *const vaddr =
          melon::base::bit_cast<void *>(info->dlpi_addr + info->dlpi_phdr[i].p_vaddr);
      ...
      melon::debugging::debugging_internal::RegisterFileMappingHint(
...
TEST(symbolize, SymbolizeWithMultipleMaps) {
  ...
  if (volatile_bool) {
    MELON_LOG(INFO)<<kPadding0;
    MELON_LOG(INFO)<< kPadding1;
  }

  // Verify we can symbolize everything.
  char buf[512];
  memset(buf, 0, sizeof(buf));
  melon::debugging::symbolize(kPadding0, buf, sizeof(buf));
  EXPECT_STREQ("kPadding0", buf);

  memset(buf, 0, sizeof(buf));
  melon::debugging::symbolize(kPadding1, buf, sizeof(buf));
  ...
  melon::debugging::symbolize(nullptr, buf, sizeof(buf));
  ...
  for (int i = 0; i < 2; i++) {
    for (size_t offset : offsets) {
      memset(buf, 0, sizeof(buf));
      melon::debugging::symbolize(ptrs[i] + offset, buf, sizeof(buf));
...
static void DummySymbolDecorator(
    const melon::debugging::debugging_internal::SymbolDecoratorArgs *args) {
...
TEST(symbolize, InstallAndRemoveSymbolDecorators) {
  ...
  EXPECT_GE(ticket_a = melon::debugging::debugging_internal::InstallSymbolDecorator(
                DummySymbolDecorator, &a_message),
            0);

  int ticket_b;
  std::string b_message("b");
  EXPECT_GE(ticket_b = melon::debugging::debugging_internal::InstallSymbolDecorator(
                DummySymbolDecorator, &b_message),
            0);

  int ticket_c;
  std::string c_message("c");
  EXPECT_GE(ticket_c = melon::debugging::debugging_internal::InstallSymbolDecorator(
  ...
  EXPECT_TRUE(melon::debugging::debugging_internal::RemoveSymbolDecorator(ticket_b));

  EXPECT_STREQ("ac", TrySymbolize(address++));

  // Cleanup: remove all remaining decorators so other stack traces don't
  // get mystery "ac" decoration.
  EXPECT_TRUE(melon::debugging::debugging_internal::RemoveSymbolDecorator(ticket_a));
  EXPECT_TRUE(melon::debugging::debugging_internal::RemoveSymbolDecorator(ticket_c));
}
...
TEST(symbolize, ForEachSection) {
  ...
  ASSERT_TRUE(melon::debugging::debugging_internal::ForEachSection(
...
extern "C" {
    MELON_FORCE_INLINE void* inline_func() {
  void *pc = nullptr;
#if defined(__i386__)
...
MELON_NO_INLINE void * non_inline_func() {
  void *pc = nullptr;
#if defined(__i386__)
  __asm__ __volatile__("call 1f;\n 1: pop %[PC]" : [ PC ] "=r"(pc));
#elif defined(__x86_64__)
...
void MELON_NO_INLINE TestWithPCInsideNonInlineFunction() {
#if defined(MELON_HAVE_ATTRIBUTE_NOINLINE) && \
    (defined(__i386__) || defined(__x86_64__))
  void *pc = non_inline_func();
  const char *symbol = TrySymbolize(pc);
  MELON_CHECK(symbol != nullptr)"TestWithPCInsideNonInlineFunction failed";
  MELON_CHECK(strcmp(symbol, "non_inline_func") == 0)<<
                 "TestWithPCInsideNonInlineFunction failed";
  std::cout << "TestWithPCInsideNonInlineFunction passed" << std::endl;
#endif
}

void MELON_NO_INLINE TestWithPCInsideInlineFunction() {
#if (defined(MELON_COMPILER_CLANG) || defined(MELON_COMPILER_GNUC)) && \
    (defined(__i386__) || defined(__x86_64__))
  void *pc = inline_func();  // Must be inlined.
  const char *symbol = TrySymbolize(pc);
  MELON_CHECK(symbol != nullptr)<<"TestWithPCInsideInlineFunction failed";
  MELON_CHECK(strcmp(symbol, __FUNCTION__) == 0)<<
...
void MELON_NO_INLINE TestWithReturnAddress() {
#if defined(MELON_COMPILER_CLANG) || defined(MELON_COMPILER_GNUC)
  void *return_address = __builtin_return_address(0);
  const char *symbol = TrySymbolize(return_address);
  MELON_CHECK(symbol != nullptr)"TestWithReturnAddress failed";
  MELON_CHECK(strcmp(symbol, "main") == 0)<<"TestWithReturnAddress failed";
...
TEST(symbolize, Unimplemented) {
    char buf[64];
    EXPECT_FALSE(melon::debugging::symbolize((void *) (&nonstatic_func), buf, sizeof(buf)));
    EXPECT_FALSE(melon::debugging::symbolize((void *) (&static_func), buf, sizeof(buf)));
    EXPECT_FALSE(melon::debugging::symbolize((void *) (&Foo::func), buf, sizeof(buf)));
}
...
int main(int argc, char **argv) {
    ...
    if (volatile_bool) {
        MELON_LOG(INFO) << kHpageTextPadding;
    }
#endif  // !defined(__EMSCRIPTEN__)
