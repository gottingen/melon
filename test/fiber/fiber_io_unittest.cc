...
# endif
        ...
        cm[i]->times = REP;
#ifdef RUN_CLIENT_IN_FIBER
        mutil::make_non_blocking(cm[i]->fd);
...
#ifndef NDEBUG
    std::cout << "break_nums=" << fiber::break_nums << std::endl;
#endif
}
...
TEST(FDTest, interrupt_pthread) {
    ...
    fiber::interrupt_pthread(th);
    fiber_usleep(100000L);
    std::cout << "wake up " << th2 << std::endl;
    fiber::interrupt_pthread(th2);
...
void* wait_for_the_fd(void* arg) {
    timespec ts = mutil::milliseconds_from_now(50);
#if defined(OS_LINUX)
