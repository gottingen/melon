...
namespace {
    TEST(WaitableEventTest, wait_on_already_timedout_event) {
        uint32_t *event = melon::fiber_internal::waitable_event_create_checked<uint32_t>();
        ASSERT_TRUE(event);
        timespec now;
        ASSERT_EQ(0, clock_gettime(CLOCK_REALTIME, &now));
        *event = 1;
        ASSERT_EQ(-1, melon::fiber_internal::waitable_event_wait(event, 1, &now));
        ASSERT_EQ(ETIMEDOUT, errno);
    }

    void *sleeper(void *arg) {
        melon::fiber_sleep_for((uint64_t) arg);
        return nullptr;
    }
    ...
    TEST(WaitableEventTest, sanity) {
        ...
        pthread_t t1, t2;
        std::atomic<int> *b1 =
                melon::fiber_internal::waitable_event_create_checked<std::atomic<int> >();
        ASSERT_TRUE(b1);
        melon::fiber_internal::waitable_event_destroy(b1);

        b1 = melon::fiber_internal::waitable_event_create_checked<std::atomic<int> >();
        *b1 = 1;
        ASSERT_EQ(0, melon::fiber_internal::waitable_event_wake(b1));
        ...
        for (size_t i = 0; i < 2 * N; ++i) {
            ASSERT_EQ(1, melon::fiber_internal::waitable_event_wake(b1));
        }
        ASSERT_EQ(0, melon::fiber_internal::waitable_event_wake(b1));
        sleep(1);
        melon::fiber_internal::waitable_event_destroy(b1);
    ...
    TEST(WaitableEventTest, stop_after_running) {
        ...
        *event = 7;
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            tm.start();
            ASSERT_EQ(0, fiber_start_urgent(&th, &attr, wait_event, &arg));
            ASSERT_EQ(0, melon::fiber_sleep_for(SLEEP_MSEC * 1000L));
            ASSERT_EQ(0, fiber_stop(th));
            ASSERT_EQ(0, fiber_join(th, nullptr));
            tm.stop();

            ASSERT_LT(labs(tm.m_elapsed() - SLEEP_MSEC), 25);
            // ASSERT_TRUE(melon::fiber_internal::get_task_control()->
    ...
    TEST(WaitableEventTest, join_cant_be_wakeup) {
        ...
        *event = 7;
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            ASSERT_EQ(0, melon::fiber_sleep_for(WAIT_MSEC / 2 * 1000L));
            ASSERT_TRUE(melon::fiber_internal::fiber_worker::exists(th));
            ASSERT_TRUE(melon::fiber_internal::fiber_worker::exists(th2));
            ASSERT_EQ(0, melon::fiber_sleep_for(WAIT_MSEC / 2 * 1000L));
            ASSERT_EQ(0, fiber_stop(th));
    ...
    TEST(WaitableEventTest, stop_after_slept) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            ASSERT_EQ(0, melon::fiber_sleep_for(WAIT_MSEC * 1000L));
            ASSERT_EQ(0, fiber_stop(th));
            ASSERT_EQ(0, fiber_join(th, nullptr));
            tm.stop();
