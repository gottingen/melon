...
namespace {
    ...
    void *read_thread(void *arg) {
        ...
        while (!stop) {
            ...
            ++nthread;
            melon::fiber_internal::futex_wait_private(m/*lock1*/, 0/*consumed_njob*/, nullptr);
            --nthread;
        }
        return new int(njob);
    }
    ...
    TEST(FutexTest, rdlock_performance) {
        ...
        for (size_t i = 0; i < N; ++i) {
            if (nthread) {
                lock1.fetch_add(1);
                melon::fiber_internal::futex_wake_private(&lock1, 1);
            } else {
                lock1.fetch_add(1);
                if (nthread) {
                    melon::fiber_internal::futex_wake_private(&lock1, 1);
        ...
        stop = true;
        for (int i = 0; i < 10; ++i) {
            melon::fiber_internal::futex_wake_private(&lock1, INT_MAX);
            sched_yield();
        }
    ...
    TEST(FutexTest, futex_wake_before_wait) {
        ...
        ASSERT_EQ(0, melon::fiber_internal::futex_wake_private(&lock1, INT_MAX));
        ASSERT_EQ(-1, melon::fiber_internal::futex_wait_private(&lock1, 0, &timeout));
        ASSERT_EQ(ETIMEDOUT, errno);
    }

    void *dummy_waiter(void *lock) {
        melon::fiber_internal::futex_wait_private(lock, 0, nullptr);
        return nullptr;
    }
        ...
        for (size_t i = 0; i < REP; ++i) {
            if (nevent.fetch_add(1, std::memory_order_relaxed) == 0) {
                ...
                while (1) {
                    ...
                    nwakeup += melon::fiber_internal::futex_wake_private(lock, expected - last_expected);
                }
            }
        }
        tm.stop();
