...
void* futex_player(void* void_arg) {
    ...
    while (!stop) {
        int rc = fiber::futex_wait_private(arg->wait_addr, counter, NULL);
        ++counter;
        ++*arg->wake_addr;
        fiber::futex_wake_private(arg->wake_addr, 1);
        ++arg->counter;
        arg->wakeup += (rc == 0);
    }
    return NULL;
...
void* butex_player(void* void_arg) {
    ...
    int counter = INITIAL_FUTEX_VALUE;
    while (!stop) {
        int rc = fiber::butex_wait(arg->wait_addr, counter, NULL);
        ++counter;
        ++*arg->wake_addr;
        fiber::butex_wake(arg->wake_addr);
        ++arg->counter;
        arg->wakeup += (rc == 0);
    }
    return NULL;
...
TEST(PingPongTest, ping_pong) {
    ...
    for (int i = 0; i < FLAGS_thread_num; ++i) {
        ...
        if (!FLAGS_use_futex && !FLAGS_use_butex) {
            ...
            arg1->wait_addr = fiber::butex_create_checked<int>();
            *arg1->wait_addr = INITIAL_FUTEX_VALUE;
            arg1->wake_addr = fiber::butex_create_checked<int>();
        ...
        if (!FLAGS_use_futex && !FLAGS_use_butex) {
            ...
            ASSERT_EQ(0, fiber_start_background(&bth1, NULL, butex_player, arg1));
            ASSERT_EQ(0, fiber_start_background(&bth2, NULL, butex_player, arg2));
        } else {
            ASSERT_TRUE(false);
        }
        ...
        if (!FLAGS_use_futex && !FLAGS_use_butex) {
            ...
            ++*arg1->wait_addr;
            fiber::futex_wake_private(arg1->wait_addr, 1);
        } else if (FLAGS_use_butex) {
            ++*arg1->wait_addr;
            fiber::butex_wake(arg1->wait_addr);
        } else {
            ASSERT_TRUE(false);
        }
    }
