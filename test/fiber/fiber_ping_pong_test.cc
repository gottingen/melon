...
void* futex_player(void* void_arg) {
    ...
    while (!stop) {
        int rc = melon::fiber_internal::futex_wait_private(arg->wait_addr, counter, nullptr);
        ++counter;
        ++*arg->wake_addr;
        melon::fiber_internal::futex_wake_private(arg->wake_addr, 1);
        ++arg->counter;
        arg->wakeup += (rc == 0);
    }
    return nullptr;
...
void* butex_player(void* void_arg) {
    ...
    int counter = INITIAL_FUTEX_VALUE;
    while (!stop) {
        int rc = melon::fiber_internal::waitable_event_wait(arg->wait_addr, counter, nullptr);
        ++counter;
        ++*arg->wake_addr;
        melon::fiber_internal::waitable_event_wake(arg->wake_addr);
        ++arg->counter;
        arg->wakeup += (rc == 0);
    }
    return nullptr;
...
TEST(PingPongTest, ping_pong) {
    ...
    for (int i = 0; i < FLAGS_thread_num; ++i) {
        ...
        if (!FLAGS_use_futex && !FLAGS_use_butex) {
            ...
            arg1->wait_addr = melon::fiber_internal::waitable_event_create_checked<int>();
            *arg1->wait_addr = INITIAL_FUTEX_VALUE;
            arg1->wake_addr = melon::fiber_internal::waitable_event_create_checked<int>();
        ...
        if (!FLAGS_use_futex && !FLAGS_use_butex) {
            ...
            ++*arg1->wait_addr;
            melon::fiber_internal::futex_wake_private(arg1->wait_addr, 1);
        } else if (FLAGS_use_butex) {
            ++*arg1->wait_addr;
            melon::fiber_internal::waitable_event_wake(arg1->wait_addr);
        } else {
            ASSERT_TRUE(false);
        }
    }
