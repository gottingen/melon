...
// A simple class, could keep track of the time when it is invoked.
    ...
    public:
        ...
        void schedule(fiber::TimerThread *timer_thread) {
        ...
        void run() {
            ...
            if (saved_sleep_ms > 0) {
                timespec timeout = mutil::milliseconds_to_timespec(saved_sleep_ms);
                fiber::futex_wait_private(&_sleep_ms, saved_sleep_ms, &timeout);
            }
        }
        ...
        fiber::TimerThread::TaskId _task_id;
    ...
    TEST(TimerThreadTest, RunTasks) {
        fiber::TimerThread timer_thread;
        ASSERT_EQ(0, timer_thread.start(NULL));

        timespec _2s_later = mutil::seconds_from_now(2);
        ...
        timespec _1s_later = mutil::seconds_from_now(1);
        TimeKeeper keeper3(_1s_later, "keeper3");
        keeper3.schedule(&timer_thread);

        timespec _10s_later = mutil::seconds_from_now(10);
    ...
    TEST(TimerThreadTest, start_after_schedule) {
        ...
        ASSERT_EQ(fiber::TimerThread::INVALID_TASK_ID, keeper._task_id);
        ASSERT_EQ(0, timer_thread.start(NULL));
        keeper.schedule(&timer_thread);
        ASSERT_NE(fiber::TimerThread::INVALID_TASK_ID, keeper._task_id);
        timespec current_time = mutil::seconds_from_now(0);
    ...
    class TestTask {
    public:
        TestTask(fiber::TimerThread *timer_thread, TimeKeeper *keeper1,
    ...
    private:
        fiber::TimerThread *_timer_thread;  // not owned.
...
// Perform schedule and unschedule inside a running task
    TEST(TimerThreadTest, schedule_and_unschedule_in_task) {
        ...
        const timespec _500ms_after = mutil::milliseconds_from_now(500);
        ...
        timespec keeper3_addtime = mutil::seconds_from_now(0);
