...
namespace {
    ...
    TEST(CondTest, sanity) {
        ...
        melon::fiber_sleep_for(SIGNAL_INTERVAL_US * 200);
    ...
    void *cv_mutex_waiter(void *void_arg) {
        WrapperArg *a = (WrapperArg *) void_arg;
        std::unique_lock<melon::fiber_mutex> lck(a->mutex);
        while (!stop) {
            a->cond.wait(lck);
        }
        return nullptr;
...
#undef pthread_create
    ...
    protected:
        ...
        int wait(int old_signal) {
            std::unique_lock<melon::fiber_mutex> lck(_m);
    ...
    struct BroadcastArg {
        melon::fiber_cond wait_cond;
        melon::fiber_cond broadcast_cond;
        melon::fiber_mutex mutex;
        int nwaiter;
        int cur_waiter;
        int rounds;
    ...
    void *wait_thread(void *arg) {
        ...
        std::unique_lock<melon::fiber_mutex> lck(ba->mutex);
        while (ba->rounds > 0) {
    ...
    void *usleep_thread(void *) {
        while (!g_stop) {
            melon::fiber_sleep_for(1000L * 1000L);
        }
        return nullptr;
    }
