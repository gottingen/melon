...
namespace {
    ...
    TEST(FiberTest, setconcurrency) {
        ASSERT_EQ(8 + FIBER_EPOLL_THREAD_NUM, (size_t) melon::fiber_getconcurrency());
        ASSERT_EQ(EINVAL, melon::fiber_setconcurrency(FIBER_MIN_CONCURRENCY - 1));
        ASSERT_EQ(EINVAL, melon::fiber_setconcurrency(0));
        ASSERT_EQ(EINVAL, melon::fiber_setconcurrency(-1));
        ASSERT_EQ(EINVAL, melon::fiber_setconcurrency(FIBER_MAX_CONCURRENCY + 1));
        ASSERT_EQ(0, melon::fiber_setconcurrency(FIBER_MIN_CONCURRENCY));
        ASSERT_EQ(FIBER_MIN_CONCURRENCY, melon::fiber_getconcurrency());
        ASSERT_EQ(0, melon::fiber_setconcurrency(FIBER_MIN_CONCURRENCY + 1));
        ASSERT_EQ(FIBER_MIN_CONCURRENCY + 1, melon::fiber_getconcurrency());
        ASSERT_EQ(0, melon::fiber_setconcurrency(FIBER_MIN_CONCURRENCY));  // smaller value
        fiber_id_t th;
        ASSERT_EQ(0, fiber_start_urgent(&th, nullptr, dummy, nullptr));
        ASSERT_EQ(FIBER_MIN_CONCURRENCY + 1, melon::fiber_getconcurrency());
        ASSERT_EQ(0, melon::fiber_setconcurrency(FIBER_MIN_CONCURRENCY + 5));
        ASSERT_EQ(FIBER_MIN_CONCURRENCY + 5, melon::fiber_getconcurrency());
        ASSERT_EQ(EPERM, melon::fiber_setconcurrency(FIBER_MIN_CONCURRENCY + 1));
        ASSERT_EQ(FIBER_MIN_CONCURRENCY + 5, melon::fiber_getconcurrency());
    }
    ...
    static void *odd_thread(void *) {
        ...
        while (!stop) {
            ...
            melon::fiber_internal::waitable_event_wake_all(even);
            melon::fiber_internal::waitable_event_wait(odd, 0, nullptr);
        }
        return nullptr;
    }
    ...
    static void *even_thread(void *) {
        ...
        while (!stop) {
            ...
            melon::fiber_internal::waitable_event_wake_all(odd);
            melon::fiber_internal::waitable_event_wait(even, 0, nullptr);
        }
        return nullptr;
    }

    TEST(FiberTest, setconcurrency_with_running_fiber) {
        odd = melon::fiber_internal::waitable_event_create_checked<std::atomic<int> >();
        even = melon::fiber_internal::waitable_event_create_checked<std::atomic<int> >();
        ...
        *even = 0;
        ...
        for (int i = 100; i <= N; ++i) {
            ASSERT_EQ(0, melon::fiber_setconcurrency(i));
            ASSERT_EQ(i, melon::fiber_getconcurrency());
        }
        usleep(1000 * N);
        *odd = 1;
        *even = 1;
    ...
    TEST(FiberTest, min_concurrency) {
        ...
        int conn = melon::fiber_getconcurrency();
        int add_conn = 100;
        ...
        ASSERT_EQ(EINVAL, melon::fiber_setconcurrency(conn - 2)); // set max failed
        ASSERT_EQ(0, melon::fiber_setconcurrency(conn + add_conn + 1)); // set max success
        ASSERT_EQ(0, melon::fiber_setconcurrency(conn + add_conn)); // set max success
        ASSERT_EQ(conn + add_conn, melon::fiber_getconcurrency());
        ASSERT_EQ(conn, melon::fiber_internal::g_task_control->concurrency());

        ASSERT_EQ(1, set_min_concurrency(conn + 1)); // set min success
        ASSERT_EQ(conn + 1, get_min_concurrency());
        ASSERT_EQ(conn + 1, melon::fiber_internal::g_task_control->concurrency());
        ...
        ASSERT_EQ(conn + add_conn, melon::fiber_getconcurrency());
        ASSERT_EQ(conn + add_conn, melon::fiber_internal::g_task_control->concurrency());
    }
