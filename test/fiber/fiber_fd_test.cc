...
//#define CREATE_THREAD_TO_PROCESS 1
    ...
    void *process_thread(void *arg) {
        ...
        if (n != sizeof(count)) {
            MELON_LOG(FATAL) << "Should not happen in this test";
            return nullptr;
        }
        count += NCLIENT;
...
# ifndef USE_BLOCKING_EPOLL
            const int n = epoll_wait(epfd, e, MELON_ARRAY_SIZE(e), 0);
            if (stop) {
                break;
            }
            if (n == 0) {
...
# else
            const int n = epoll_wait(epfd, e, MELON_ARRAY_SIZE(e), -1);
            if (stop) {
                break;
            }
            if (n == 0) {
...
# endif
#elif defined(MELON_PLATFORM_OSX)
            const int n = kevent(epfd, nullptr, 0, e, MELON_ARRAY_SIZE(e), nullptr);
            if (stop) {
                break;
            }
            if (n == 0) {
...
#if defined(MELON_PLATFORM_LINUX)
                    MELON_PLOG(FATAL) << "Fail to epoll_wait";
#elif defined(MELON_PLATFORM_OSX)
                MELON_PLOG(FATAL) << "Fail to kevent";
#endif
                break;
            }
...
# endif
                EXPECT_EQ(0, wait_rc) << melon_error();
            ...
            if (rc != sizeof(m->count)) {
                MELON_PLOG(FATAL) << "Should not happen in this test, rc=" << rc;
                return nullptr;
            }
        }
        return nullptr;
...
#ifndef NDEBUG
        melon::fiber_internal::break_nums = 0;
#endif
...
# endif
            ...
            cm[i]->times = REP;
#ifdef RUN_CLIENT_IN_FIBER
            melon::base::make_non_blocking(cm[i]->fd);
        ...
        tm.stop();
        ProfilerStop();
        MELON_LOG(INFO) << "tid=" << REP * NCLIENT * 1000000L / tm.u_elapsed();
        stop = true;
        for (size_t i = 0; i < NEPOLL; ++i) {
#if defined(MELON_PLATFORM_LINUX)
        ...
        //melon::fiber_internal::stop_and_join_epoll_threads();
        melon::fiber_sleep_for(100000);

#ifndef NDEBUG
        std::cout << "break_nums=" << melon::fiber_internal::break_nums << std::endl;
#endif
    }
    ...
    TEST(FDTest, mod_closed_fd) {
        ...
        errno = 0;
        ASSERT_EQ(-1, epoll_ctl(epfd, EPOLL_CTL_MOD, fd[0], &e));
        ASSERT_EQ(EBADF, errno) << melon_error();
        ...
        errno = 0;
        ASSERT_EQ(-1, epoll_ctl(epfd, EPOLL_CTL_MOD, fd[0], &e));
        ASSERT_EQ(ENOENT, errno) << melon_error();
    ...
    TEST(FDTest, interrupt_pthread) {
        ...
        melon::fiber_internal::interrupt_pthread(th);
        melon::fiber_sleep_for(100000L);
        std::cout << "wake up " << th2 << std::endl;
        melon::fiber_internal::interrupt_pthread(th2);
        pthread_join(th, nullptr);
        pthread_join(th2, nullptr);
    }

    void *close_the_fd(void *arg) {
        melon::fiber_sleep_for(10000/*10ms*/);
    ...
    void *wait_for_the_fd(void *arg) {
        timespec ts = melon::time_point::future_unix_millis(50).to_timespec();
#if defined(MELON_PLATFORM_LINUX)
