...
namespace {
    ...
    void *signaller(void *void_arg) {
        ...
        melon::fiber_sleep_for(arg.sleep_us_before_fight);
        void *data = nullptr;
        ...
        if (rc == 0) {
            ...
            melon::fiber_sleep_for(arg.sleep_us_before_signal);
    ...
    TEST(FiberTokenTest, join_after_destroy) {
        ...
        ASSERT_EQ(get_version(id1) + 5, melon::fiber_internal::token_value(id1));
        ASSERT_EQ(get_version(id1) + 5, melon::fiber_internal::token_value(id2));
    }
    ...
    TEST(FiberTokenTest, many_error) {
        ...
        ASSERT_EQ(get_version(id1) + 1, melon::fiber_internal::token_value(id1));
        for (int i = 0; i < N; ++i) {
            ASSERT_EQ(0, fiber_token_error(id1, err++));
        }
        ASSERT_EQ((size_t) N, result.size());
    ...
    void *stopped_waiter(void *void_arg) {
        ...
        EXPECT_EQ(get_version(args->id) + 4, melon::fiber_internal::token_value(args->id));
        return nullptr;
    }
    ...
    void *waiter(void *arg) {
        ...
        EXPECT_EQ(get_version(id) + 4, melon::fiber_internal::token_value(id));
        return nullptr;
    }
    ...
    TEST(FiberTokenTest, status) {
        ...
        melon::fiber_internal::token_status(id, std::cout);
