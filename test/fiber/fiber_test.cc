...
namespace {
    ...
    static void f(intptr_t param) {
        ...
        p = (pair_t *) melon_fiber_jump_context(&fc, fcm, (intptr_t) (p->first + p->second));
        melon_fiber_jump_context(&fc, fcm, (intptr_t) (p->first + p->second));
    }
    ...
    TEST_F(FiberTest, context_sanity) {
        ...
        fc = melon_fiber_make_context((char *) sp + size, size, f);

        int res = (int) melon_fiber_jump_context(&fcm, fc, (intptr_t) &p);
        std::cout << p.first << " + " << p.second << " == " << res << std::endl;

        p = std::make_pair(5, 6);
        res = (int) melon_fiber_jump_context(&fcm, fc, (intptr_t) &p);
        std::cout << p.first << " + " << p.second << " == " << res << std::endl;
    }

    TEST_F(FiberTest, call_fiber_functions_before_tls_created) {
        ASSERT_EQ(0, melon::fiber_sleep_for(1000));
    ...
    void *sleep_for_awhile_with_sleep(void *arg) {
        melon::fiber_sleep_for((intptr_t) arg);
        return nullptr;
    }
    ...
    void *check_sleep(void *pthread_task) {
        ...
        if (pthread_task) {
            ...
            // melon::fiber_sleep_for = usleep for FIBER_ATTR_PTHREAD
