    ...
    EXPECT_STREQ(zone, al.abbr);                                  \
        ...
        TEST(Format, TimePointResolution) {
            ...
            EXPECT_EQ(
                    "03:04:05",
                    format(kFmt,
                           chrono::time_point_cast<melon::times_internal::seconds>(t0),
                           utc));
            EXPECT_EQ("03:04:00",
        ...
        TEST(Format, TimePointExtendedResolution) {
            const char kFmt[] = "%H:%M:%E*S";
            const time_zone utc = utc_time_zone();
            const time_point<melon::times_internal::seconds> tp =
                    chrono::time_point_cast<melon::times_internal::seconds>(
        ...
        TEST(Format, ExtendedOffset) {
            ...
            auto tz = fixed_time_zone(melon::times_internal::seconds::zero());
            TestFormatSpecifier(tp, tz, "%z", "+0000");
...
//
        ...
        TEST(Parse, TimePointExtendedResolution) {
            ...
            time_point<melon::times_internal::seconds> tp;
        ...
        TEST(Parse, MaxRange) {
            ...
            EXPECT_EQ(tp, time_point<melon::times_internal::seconds>::max());
            EXPECT_FALSE(
            ...
            EXPECT_EQ(tp, time_point<melon::times_internal::seconds>::min());
            EXPECT_FALSE(
            ...
            // appropriately. This can't be done until melon::times_internal::parse() properly detects
...
//
        ...
        TEST(FormatParse, RoundTripDistantFuture) {
            const time_zone utc = utc_time_zone();
            const time_point<melon::times_internal::seconds> in =
                    time_point<melon::times_internal::seconds>::max();
            const std::string s = format(RFC3339_full, in, utc);
            time_point<melon::times_internal::seconds> out;
        ...
        TEST(FormatParse, RoundTripDistantPast) {
            const time_zone utc = utc_time_zone();
            const time_point<melon::times_internal::seconds> in =
                    time_point<melon::times_internal::seconds>::min();
            const std::string s = format(RFC3339_full, in, utc);
            time_point<melon::times_internal::seconds> out;
            EXPECT_TRUE(parse(RFC3339_full, s, utc, &out)) << s;
            EXPECT_EQ(in, out);
