...
#include <abel/asl/format/posix.cc>
#include <memory>
#include <errno.h>
#include <fcntl.h>
#include <climits>
...
TEST(FileTest, CloseNoRetryInDtor) {
    ...
    unique_ptr<file> f(new file(std::move(read_end)));
...
TEST(BufferedFileTest, OpenRetry) {
    write_file("test", "there must be something here");
    unique_ptr<buffered_file> f{nullptr};
...
TEST(BufferedFileTest, CloseNoRetryInDtor) {
    file read_end, write_end;
    file::pipe(read_end, write_end);
    unique_ptr<buffered_file> f(new buffered_file(read_end.fdopen("r")));
...
struct LocaleMock {
    ...
    MOCK_METHOD3(newlocale, LocaleType(int
            category_mask,
            const char *locale,
            LocaleType base));
    MOCK_METHOD1(freelocale, void (LocaleType
            locale));

    MOCK_METHOD3(strtod_l, double(
            const char *nptr,
            char **endptr,
            LocaleType locale));
} *LocaleMock::instance;
