...
melon prefers static linkages of deps, so that they don't have to be installed on every machine running the app.

melon depends on following packages:
...
$ ./echo_client
```
Examples link melon statically, if you need to link the shared version, remove `CMakeCache.txt` and cmake with `-DLINK_SO=ON`
...
### Compile melon with cmake
Same with [here](#compile-melon-with-cmake)

## Linux with self-built deps

### Prepare deps

melon builds itself to both static and shared libs by default, so it needs static and shared libs of deps to be built as well.
...
git clone melon. cd into the repo and run
...
Here we pass multiple paths to `--headers` and `--libs` to make the script search for multiple places. You can also group all deps and melon into one directory, then pass the directory to --headers/--libs which actually search all subdirectories recursively and will find necessary files.
...
Examples link melon statically, if you need to link the shared version, `make clean` and `LINK_SO=1 make`
...
[tools/add_syntax_equal_proto2_to_all.sh](https://github.com/melon/melon/blob/master/tools/add_syntax_equal_proto2_to_all.sh)can add `syntax="proto2"` to all proto files without it.
...
melon detects valgrind automatically (and registers stacks of fiber). Older valgrind(say 3.2) is not supported.

## thrift: 0.9.3-0.11.0

## cmake 3.15+

melon build by cmake build system.

no known issues.

# Track instances

We provide a program to help you to track and monitor all melon instances. Just run [trackme_server](https://github.com/melon/melon/tree/master/tools/trackme_server/) somewhere and launch need-to-be-tracked instances with -trackme_server=SERVER. The trackme_server will receive pings from instances periodically and print logs when it does. You can aggregate instance addresses from the log and call builtin services of the instances for further information.
