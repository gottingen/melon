...
static void* sender(void* void_args) {
    ...
    for (int i = 0; i < FLAGS_batch; ++i) {
        kvs[i].first = turbo::string_printf(
            "%s_%04d", FLAGS_key.c_str(), args->base_index + i);
        kvs[i].second = turbo::string_printf(
            "%s_%04d", FLAGS_value.c_str(), args->base_index + i);
    }

    melon::rpc::RedisRequest request;
    for (int i = 0; i < FLAGS_batch; ++i) {
        TURBO_CHECK(request.AddCommand("GET %s", kvs[i].first.c_str()));
    }
    while (!melon::rpc::IsAskedToQuit()) {
        ...
        if (!cntl.Failed()) {
            ...
            TURBO_CHECK_EQ(response.reply_size(), FLAGS_batch);
            for (int i = 0; i < FLAGS_batch; ++i) {
                TURBO_CHECK_EQ(kvs[i].second.c_str(), response.reply(i).data())
...
int main(int argc, char* argv[]) {
    ...
    if (cntl.Failed()) {
        TURBO_LOG(ERROR) << "Fail to access redis, " << cntl.ErrorText();
        return -1;
    }
    if (FLAGS_batch * FLAGS_thread_num != response.reply_size()) {
        TURBO_LOG(ERROR) << "Fail to set";
        return -1;
    }
    for (int i = 0; i < FLAGS_batch * FLAGS_thread_num; ++i) {
        TURBO_CHECK_EQ("OK", response.reply(i).data());
    }
    TURBO_LOG(INFO) << "Set " << FLAGS_batch * FLAGS_thread_num << " values";
    ...
    while (!melon::rpc::IsAskedToQuit()) {
        ...
        TURBO_LOG(INFO) << "Accessing redis-server at qps=" << g_latency_recorder.qps(1)
                  << " latency=" << g_latency_recorder.latency(1);
    }

    TURBO_LOG(INFO) << "redis_client is going to quit";
    for (int i = 0; i < FLAGS_thread_num; ++i) {
        if (!FLAGS_use_fiber) {
