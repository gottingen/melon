...
DEFINE_string(protocol, "melon_std", "Protocol type.");
...
mutil::atomic<uint64_t> g_total_bytes;
mutil::atomic<uint64_t> g_total_cnt;
std::vector<std::string> g_servers;
int rr_index = 0;
volatile bool g_stop = false;

mutil::atomic<int64_t> g_token(10000);

static void* GenerateToken(void* arg) {
    int64_t start_time = mutil::monotonic_time_ns();
    int64_t accumulative_token = g_token.load(mutil::memory_order_relaxed);
    while (!g_stop) {
        fiber_usleep(100000);
        int64_t now = mutil::monotonic_time_ns();
        if (accumulative_token * 1000000000 / (now - start_time) < FLAGS_expected_qps) {
            ...
            g_token.fetch_add(delta, mutil::memory_order_relaxed);
...
public:
        ...
        , _stop(false)
    {
        if (attachment_size > 0) {
            ...
            mutil::fast_rand_bytes(_addr, attachment_size);
    ...
    void SendRequest() {
        if (FLAGS_expected_qps > 0) {
            while (g_token.load(mutil::memory_order_relaxed) <= 0) {
                fiber_usleep(10);
            }
            g_token.fetch_sub(1, mutil::memory_order_relaxed);
        }
    ...
    static void HandleResponse(RespClosure* closure) {
        ...
        g_total_bytes.fetch_add(closure->cntl->request_attachment().size(), mutil::memory_order_relaxed);
        g_total_cnt.fetch_add(1, mutil::memory_order_relaxed);
        ...
        uint64_t now = mutil::gettimeofday_us();
    ...
    static void* RunTest(void* arg) {
        ...
        test->_start_time = mutil::gettimeofday_us();
...
void Test(int thread_num, int attachment_size) {
    std::cout << "[Threads: " << thread_num
        ...
        << std::endl;
    g_total_bytes.store(0, mutil::memory_order_relaxed);
    g_total_cnt.store(0, mutil::memory_order_relaxed);
    ...
    uint64_t start_time = mutil::gettimeofday_us();
    fiber_t tid[thread_num];
    if (FLAGS_expected_qps > 0) {
        fiber_t tid;
        fiber_start_background(&tid, &FIBER_ATTR_NORMAL, GenerateToken, NULL);
    }
    for (int k = 0; k < thread_num; ++k) {
        fiber_start_background(&tid[k], &FIBER_ATTR_NORMAL,
    ...
    uint64_t end_time = mutil::gettimeofday_us();
    ...
    if (FLAGS_test_iterations == 0) {
        std::cout << "Avg-Latency: " << g_latency_recorder.latency(10)
            ...
            << ", QPS: " << (g_total_cnt.load(mutil::memory_order_relaxed) * 1000 / (end_time - start_time)) << "k"
    ...
    g_stop = true;
    for (int k = 0; k < thread_num; ++k) {
        fiber_start_background(&tid[k], &FIBER_ATTR_NORMAL, DeleteTest, tests[k]);
    }
}
