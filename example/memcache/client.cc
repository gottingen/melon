...
//#include <turbo/strings/strings.h>
...
turbo::static_atomic<int> g_sender_count = TURBO_STATIC_ATOMIC_INIT(0);
...
static void* sender(void* arg) {
    ...
    for (int i = 0; i < FLAGS_batch; ++i) {
        kvs[i].first = turbo::string_printf("%s%d", FLAGS_key.c_str(), base_index + i);
        kvs[i].second = turbo::string_printf("%s%d", FLAGS_value.c_str(), base_index + i);
    }
    melon::rpc::MemcacheRequest request;
    for (int i = 0; i < FLAGS_batch; ++i) {
        TURBO_CHECK(request.Get(kvs[i].first));
    }
        ...
        if (!cntl.Failed()) {
            ...
            for (int i = 0; i < FLAGS_batch; ++i) {
                ...
                if (!response.PopGet(&value, &flags, nullptr)) {
                    TURBO_LOG(INFO) << "Fail to GET the key, " << response.LastError();
                    melon::rpc::AskToQuit();
                    return nullptr;
                }
                TURBO_CHECK(flags == 0xdeadbeef + base_index + i)
                    << "flags=" << flags;
                TURBO_CHECK(kvs[i].second == value)
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < FLAGS_batch * FLAGS_thread_num; ++i) {
        if (!request.Set(turbo::string_printf("%s%d", FLAGS_key.c_str(), i),
                         turbo::string_printf("%s%d", FLAGS_value.c_str(), i),
                         0xdeadbeef + i, FLAGS_exptime, 0)) {
            TURBO_LOG(ERROR) << "Fail to SET " << i << "th request";
            return -1;
        }
    }
    channel.CallMethod(nullptr, &cntl, &request, &response, nullptr);
    if (cntl.Failed()) {
        TURBO_LOG(ERROR) << "Fail to access memcache, " << cntl.ErrorText();
        return -1;
    }
    for (int i = 0; i < FLAGS_batch * FLAGS_thread_num; ++i) {
        if (!response.PopSet(nullptr)) {
            TURBO_LOG(ERROR) << "Fail to SET memcache, i=" << i
                       << ", " << response.LastError();
            return -1;
        }
    }
    if (FLAGS_exptime > 0) {
        TURBO_LOG(INFO) << "Set " << FLAGS_batch * FLAGS_thread_num
    ...
    while (!melon::rpc::IsAskedToQuit()) {
        sleep(1);
        TURBO_LOG(INFO) << "Accessing memcache server at qps=" << g_latency_recorder.qps(1)
                  << " latency=" << g_latency_recorder.latency(1);
    }

    TURBO_LOG(INFO) << "memcache_client is going to quit";
    for (int i = 0; i < FLAGS_thread_num; ++i) {
        if (!FLAGS_use_fiber) {
