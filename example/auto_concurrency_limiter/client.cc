...
melon::status_gauge<uint32_t> g_timeout_var(cast_func, &g_timeout);
melon::status_gauge<uint32_t> g_error_var(cast_func, &g_error);
melon::status_gauge<uint32_t> g_succ_var(cast_func, &g_succ);
melon::LatencyRecorder g_latency_rec;
...
void RunCase(test::ControlService_Stub &cntl_stub,
    ...
    while (context.running.load(std::memory_order_acquire)) {
        ...
        melon::rpc::Controller *echo_cntl = new melon::rpc::Controller;
